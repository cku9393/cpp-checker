...
void apply_artifact_retention(const TestOptions& options) {
    ...
    const array<string, 5> leaves = {{"counterexamples", "traces", "logs", "reduced", "reduced_planner"}};
    ...
    for (const string& leaf : array<string, 3>{{"traces", "logs", "counterexamples"}}) {
...
vector<PlannerTraceEntry> collect_planner_trace_for_dump(const PlannerStateDump& dump) {
    ...
    try {
        execute_planner_capture(
            ...
            ctx,
            U,
            RawUpdaterRunOptions{dump.stepBudget},
...
bool replay_planner_state_dump(
    ...
    try {
        ...
        if (oracle) {
            const PlannerOracleComparison comparison = compare_planner_oracle(dump.engine, RE, ctx, runOptions, trace);
...
PlannerStateDump make_planner_reducer_smoke_state() {
    ...
    dump.traceLevel = TraceLevel::SUMMARY;
    for (int i = 0; i < 12; ++i) {
        dump.initialQueue.push_back(UpdJob{UpdJobKind::ENSURE_SOLE, occ31});
    }
    return dump;
}
