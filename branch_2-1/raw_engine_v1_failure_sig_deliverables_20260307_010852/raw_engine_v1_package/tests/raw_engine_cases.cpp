...
void record_fuzz_spec_stats(const FuzzSpec& spec) {
    if (!g_fuzzStats.has_value()) {
        return;
    }
    ++g_fuzzStats->totalIterations;
    g_fuzzStats->directABArtifactCount += (spec.directABCount > 0 ? 1U : 0U);
    g_fuzzStats->boundaryOnlyChildCount += static_cast<size_t>(max(spec.boundaryOnlyCount, 0));
    g_fuzzStats->multiEdgeCount += static_cast<size_t>(max(spec.multiEdgeCount, 0));
    if (SeedFuzzSummary* seedSummary = active_seed_summary(); seedSummary != nullptr) {
        ++seedSummary->iterations;
        seedSummary->directABArtifactCount += (spec.directABCount > 0 ? 1U : 0U);
        seedSummary->boundaryOnlyChildCount += static_cast<size_t>(max(spec.boundaryOnlyCount, 0));
        seedSummary->multiEdgeCount += static_cast<size_t>(max(spec.multiEdgeCount, 0));
    }
}

void record_planner_trace_stats(const vector<PlannerTraceEntry>& trace) {
    if (!g_fuzzStats.has_value()) {
        return;
    }
    ...
    for (size_t i = 0; i < trace.size(); ++i) {
        ...
        seq << planner_primitive_name(trace[i].primitive);
        const size_t idx = planner_primitive_index(trace[i].primitive);
        ++g_fuzzStats->primitiveHits[idx];
        if (SeedFuzzSummary* seedSummary = active_seed_summary(); seedSummary != nullptr) {
            ++seedSummary->primitiveHits[idx];
        }
        sawSplit = sawSplit || trace[i].primitive == PlannerPrimitiveKind::SPLIT;
    }

    ++g_fuzzStats->sequenceHistogram[trace.empty() ? string("empty") : seq.str()];
    if (sawSplit) {
...
void run_mixed_planner_iteration(const FuzzSpec& spec) {
    ...
    for (int step = 0; step < spec.opCount; ++step) {
        ...
        record_planner_trace_stats(last_planner_trace());
        assert_engine_bookkeeping_sane(RE);
...
void run_planner_oracle_fuzz_stats_case(const TestOptions& options) {
    ...
    if (!filesystem::exists(statsPath) ||
        statsText.find("\"primitive_hits\"") == string::npos ||
        statsText.find("\"sequence_histogram\"") == string::npos) {
        throw runtime_error("planner_oracle_fuzz_stats did not emit stats file");
    }
}
