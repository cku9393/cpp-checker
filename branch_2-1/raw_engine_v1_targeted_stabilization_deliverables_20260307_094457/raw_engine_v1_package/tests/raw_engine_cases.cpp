...
IterationOrdinal record_fuzz_spec_stats(const FuzzSpec& spec) {
...
void run_mixed_planner_iteration(const FuzzSpec& spec) {
    ...
    for (int step = 0; step < spec.opCount; ++step) {
        ...
        const PlannerExecutionResult result =
            run_planner_checked_capture(RE, ctx, U, runOptions, nullptr, "mixed_planner");
...
void run_planner_iteration(mt19937& rng, int iter, const TestOptions& options) {
    ...
    const RawUpdaterRunOptions runOptions{options.stepBudget};
    const PlannerExecutionResult result =
        run_planner_checked_capture(RE, ctx, U, runOptions, nullptr, "planner_iteration");
    record_planner_execution_stats(result, current_fuzz_iteration_ordinal());
...
void run_targeted_planner_case(
    ...
    for (u32 seed : seeds) {
        ...
        for (int iter = 0; iter < iters; ++iter) {
            ...
            const ScenarioFamily activeFamily =
                (family == ScenarioFamily::PLANNER_MIXED_TARGETED ? choose_targeted_family(seed, iter) : family);
...
void run_planner_oracle_fuzz_stats_case(const TestOptions& options) {
    ...
    if (fuzzOptions.scenarioFamily == ScenarioFamily::RANDOM) {
        fuzzOptions.scenarioFamily = ScenarioFamily::PLANNER_MIXED_TARGETED;
    }
...
void run_planner_targeted_split_smoke_case(const TestOptions& options) {
    ...
    set_active_test_options(&options);
    require_coverage_at_least(require_fuzz_stats_or_throw("planner_targeted_split_smoke"), "planner_targeted_split_smoke", 1, 0, 0, 0, 1, 0, 0);
}
...
void run_planner_targeted_join_smoke_case(const TestOptions& options) {
    ...
    set_active_test_options(&options);
    require_coverage_at_least(require_fuzz_stats_or_throw("planner_targeted_join_smoke"), "planner_targeted_join_smoke", 0, 0, 1, 0, 0, 1, 0);
}
...
void run_planner_targeted_integrate_smoke_case(const TestOptions& options) {
    ...
    set_active_test_options(&options);
    require_coverage_at_least(require_fuzz_stats_or_throw("planner_targeted_integrate_smoke"), "planner_targeted_integrate_smoke", 1, 1, 0, 1, 1, 0, 1);
}
...
void run_planner_targeted_mixed_smoke_case(const TestOptions& options) {
    ...
    set_active_test_options(&options);
    require_coverage_at_least(require_fuzz_stats_or_throw("planner_targeted_mixed_smoke"), "planner_targeted_mixed_smoke", 2, 2, 2, 2, 2, 2, 2);
}
...
void run_planner_coverage_smoke_case(const TestOptions& options) {
    ...
    require_coverage_at_least(stats, "planner_coverage_smoke", 2, 2, 2, 2, 2, 2, 2);
    require_stats_keys(
        stats_output_path(smokeOptions, "planner_coverage_smoke"),
        {
            "\"scenario_family\"",
            "\"split_ready_count\"",
