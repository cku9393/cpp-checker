...
filesystem::path write_candidate_dump(const LiveStateDump& dump, const string& stem) {
    const filesystem::path dir = filesystem::current_path() / "counterexamples";
...
bool failing_replay_child(const TestOptions& options, const LiveStateDump& dump, PrimitiveKind primitive, bool oracle) {
    if (options.executablePath.empty()) {
        throw runtime_error("missing executable path for state reducer");
    }

    const filesystem::path candidate = write_candidate_dump(dump, "reducer_candidate_state");
    ostringstream cmd;
...
bool accept_candidate(
    ...
    if (!failing_replay_child(options, candidate, primitive, oracle)) {
        return false;
    }
    best = candidate;
    return true;
...
PrimitiveCallDumpGuard::PrimitiveCallDumpGuard(
    ...
    const filesystem::path out =
        dir / ("live_" + label + "_" + to_string(++g_dumpCounter) + ".txt");
...
bool replay_state_dump(
    ...
    if (primitive == PrimitiveKind::ISOLATE) {
        ...
        if (oracle) {
            if (!check_prepare_isolate_oracle(RE, dump.invocation.sid, dump.invocation.occ, prep, failure)) {
                return false;
            }
        }
        assert_engine_bookkeeping_sane_local(RE);
    ...
    if (primitive == PrimitiveKind::SPLIT) {
        ...
        assert_engine_bookkeeping_sane_local(RE);
        if (oracle && !check_split_oracle(before, RE, dump.invocation, result, failure)) {
            return false;
        }
        return true;
    }
    ...
    if (primitive == PrimitiveKind::JOIN) {
        ...
        assert_engine_bookkeeping_sane_local(RE);
        if (oracle && !check_join_oracle(before, RE, dump.invocation, result, failure)) {
            return false;
        }
        return true;
    }
    ...
    assert_engine_bookkeeping_sane_local(RE);
    if (oracle && !check_integrate_oracle(before, RE, dump.invocation, result, failure)) {
        return false;
    }
    return true;
}
...
filesystem::path reduce_state_dump_file(
    ...
    if (!failing_replay_child(options, best, primitive, oracle)) {
    ...
    while (changed) {
        ...
        while (!best.invocation.sequence.empty()) {
            ...
            if (!accept_candidate(options, best, candidate, primitive, oracle)) {
                break;
            }
            changed = true;
        }
    ...
    filesystem::create_directories(dir);
    const filesystem::path out =
        dir / ("reduced_state_" + primitive_name_string(primitive) + ".txt");
