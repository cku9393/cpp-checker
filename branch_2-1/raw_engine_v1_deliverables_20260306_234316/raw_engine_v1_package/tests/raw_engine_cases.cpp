...
void run_replay_state_case(const TestOptions& options) {
    ...
    string failure;
    if (!replay_state_dump(options, dump, primitive, options.oracle, &failure)) {
        throw runtime_error(failure.empty() ? "replay_state failed" : failure);
    }
}
...
void run_reduce_state_case(const TestOptions& options) {
    ...
    if (options.stateFile.has_value()) {
        ...
        inputPath = filesystem::current_path() / "counterexamples" / "reducer_smoke_input.txt";
        filesystem::create_directories(inputPath.parent_path());
        save_state_dump(inputPath, make_reducer_smoke_state());
    }

    const LiveStateDump dump = load_state_dump(inputPath);
    const PrimitiveKind primitive = resolve_primitive_or_throw(options, dump.invocation.primitive);
    const filesystem::path reduced = reduce_state_dump_file(options, inputPath, primitive, options.oracle);
