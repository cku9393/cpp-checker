...
u64 hash_text(string_view text) {
...
optional<FailureClass> parse_failure_class_text(string_view text) {
    static const array<FailureClass, 7> classes = {{
        FailureClass::NONE,
        FailureClass::VALIDATOR_FAILURE,
        FailureClass::PRIMITIVE_ORACLE_MISMATCH,
        FailureClass::PLANNER_ORACLE_MISMATCH,
        FailureClass::STEP_BUDGET_EXCEEDED,
        FailureClass::SANITIZER_FAILURE,
        FailureClass::CRASH,
    }};
    for (FailureClass failureClass : classes) {
        if (text == failure_class_name(failureClass)) {
            return failureClass;
        }
    }
    return nullopt;
...
FailureSignature parse_logged_failure_signature(const string& text) {
    ...
    while (getline(iss, line)) {
        if (line.rfind("FAIL_CLASS=", 0) == 0) {
            const optional<FailureClass> failureClass = parse_failure_class_text(line.substr(11));
            if (failureClass.has_value()) {
                sig.failureClass = *failureClass;
            }
        } else if (line.rfind("FAIL_HASH=", 0) == 0) {
            sig.hash = line.substr(10);
        }
    }
    sig.detail = text;
    return sig;
...
FailureSignature classify_child_replay_result(int exitCode, const string& text) {
    ...
    if (contains_text(text, "AddressSanitizer") ||
        ...
        return make_failure_signature(FailureClass::SANITIZER_FAILURE, text);
    }
    return make_failure_signature(FailureClass::CRASH, text);
}
...
bool invocation_uses_skeleton(const PrimitiveInvocation& inv, RawSkelID sid) {
    return sid == inv.sid || sid == inv.leftSid || sid == inv.rightSid || sid == inv.parentSid || sid == inv.childSid;
}
...
FailureSignature replay_child_planner_failure(const TestOptions& options, const PlannerStateDump& dump, bool oracle) {
    ...
    if (dump.dumpTrace || options.dumpTrace) {
        cmd << " --dump-trace";
    }
    cmd << " > " << shell_quote(logPath.string()) << " 2>&1";
    const int exitCode = std::system(cmd.str().c_str());
...
size_t planner_state_metric(const PlannerStateDump& dump) {
    size_t total = live_entity_metric(planner_state_as_live_dump(dump));
    total += dump.dumpTrace ? 1U : 0U;
    return total;
}
...
optional<PrimitiveKind> parse_primitive_kind(const string& text) {
    static const array<pair<const char*, PrimitiveKind>, 4> primitives = {{
...
FailureSignature make_failure_signature(FailureClass failureClass, const string& detail) {
    ...
    sig.failureClass = failureClass;
    sig.detail = detail;
    if (failureClass != FailureClass::NONE && !detail.empty()) {
        sig.hash = hex_hash(hash_text(detail));
    }
    return sig;
}
...
PlannerStateDump load_planner_state_dump(const filesystem::path& path) {
    ...
    string magic;
    if (!(ifs >> magic) || magic != "planner_state_v1") {
...
FailureSignature classify_message(const string& message, bool plannerOracle) {
    if (contains_text(message, "step budget exceeded")) {
        return make_failure_signature(FailureClass::STEP_BUDGET_EXCEEDED, message);
    }
    ...
    if (contains_text(message, "state dump") ||
        ...
        return make_failure_signature(FailureClass::VALIDATOR_FAILURE, message);
    }
    if (contains_text(message, "AddressSanitizer") ||
        ...
        return make_failure_signature(FailureClass::SANITIZER_FAILURE, message);
    }
    return make_failure_signature(FailureClass::CRASH, message);
}
...
bool replay_state_dump(
    ...
    try {
        ...
        if (primitive == PrimitiveKind::ISOLATE) {
            ...
            if (oracle) {
                string detail;
                if (!check_prepare_isolate_oracle(RE, dump.invocation.sid, dump.invocation.occ, prep, &detail)) {
                    if (failure != nullptr) {
                        *failure = make_failure_signature(FailureClass::PRIMITIVE_ORACLE_MISMATCH, detail);
                    }
                    return false;
                }
            }
        ...
        if (primitive == PrimitiveKind::SPLIT) {
            ...
            if (oracle) {
                string detail;
                if (!check_split_oracle(before, RE, dump.invocation, result, &detail)) {
                    if (failure != nullptr) {
                        *failure = make_failure_signature(FailureClass::PRIMITIVE_ORACLE_MISMATCH, detail);
                    }
                    return false;
        ...
        if (primitive == PrimitiveKind::JOIN) {
            ...
            if (oracle) {
                string detail;
                if (!check_join_oracle(before, RE, dump.invocation, result, &detail)) {
                    if (failure != nullptr) {
                        *failure = make_failure_signature(FailureClass::PRIMITIVE_ORACLE_MISMATCH, detail);
                    }
                    return false;
        ...
        if (oracle) {
            string detail;
            if (!check_integrate_oracle(before, RE, dump.invocation, result, &detail)) {
                if (failure != nullptr) {
                    *failure = make_failure_signature(FailureClass::PRIMITIVE_ORACLE_MISMATCH, detail);
                }
                return false;
        ...
        if (failure != nullptr) {
            *failure = classify_message(ex.what(), false);
        }
        return false;
    }
}
...
bool replay_planner_state_dump(
    ...
    try {
        ...
        replayOptions.dumpOnFail = false;
        replayOptions.dumpTrace = dump.dumpTrace || options.dumpTrace;
        replayOptions.oracleMode = (oracle ? OracleMode::PLANNER : OracleMode::NONE);

        set_active_test_options(&replayOptions);
        run_planner_checked(RE, ctx, U, runOptions, "replay_planner");
        ...
        set_active_test_options(&options);
        if (failure != nullptr) {
            *failure = classify_message(ex.what(), true);
        }
        return false;
    }
}
