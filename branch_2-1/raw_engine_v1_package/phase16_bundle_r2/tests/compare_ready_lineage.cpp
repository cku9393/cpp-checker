...
CompareReadyLineageSummary audit_compare_ready_lineage(
    ...
    optional<string> referenceBaseStateHash;
    optional<string> referenceBaseTargetPrepareHash;
    optional<string> referenceCompareReadyStateHash;
    optional<string> referenceCompareReadyTargetPrepareHash;
    optional<string> referenceReasonCode;
    ...
    for (u32 scenarioSeed : scenarioSeeds) {
        ...
        if (compareReadyInfo.compareEligible) {
            ...
            if (auditResult.exactAuditAvailable) {
                const auto selectedIt = find_if(
                    auditResult.outcomes.begin(),
                    auditResult.outcomes.end(),
                    [&](const SplitChoiceCandidateOutcome& outcome) {
                        return outcome.pair == auditResult.exactShadowSelectedPair;
                    }
                );
                if (selectedIt != auditResult.outcomes.end()) {
                    ++summary.pairReplayCount;
                    entry.compareReadyTargetPrepareHash = selectedIt->targetPrepareHash;
                    if (!referenceBaseStateHash.has_value()) {
                        referenceBaseStateHash = entry.baseStateHash;
                        referenceBaseTargetPrepareHash = entry.baseTargetPrepareHash;
                        referenceCompareReadyStateHash = entry.compareReadyStateHash;
                        referenceCompareReadyTargetPrepareHash = entry.compareReadyTargetPrepareHash;
                        referenceReasonCode = entry.reasonCode;
                    }
                    entry.derivedFromBaseState =
                        entry.baseStateHash == *referenceBaseStateHash &&
                        entry.baseTargetPrepareHash == *referenceBaseTargetPrepareHash &&
                        entry.compareReadyStateHash == *referenceCompareReadyStateHash &&
                        entry.compareReadyTargetPrepareHash == *referenceCompareReadyTargetPrepareHash &&
                        entry.reasonCode == *referenceReasonCode;
