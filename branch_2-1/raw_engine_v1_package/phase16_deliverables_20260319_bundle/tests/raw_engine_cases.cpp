...
struct PolicyGraduationEvaluation {
    PolicyGraduationGateStatus status = PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE;
...
void record_compare_eligibility_probe(const CompareEligibilityInfo& info) {
    if (!g_fuzzStats.has_value()) {
        return;
    }
    auto apply = [&](auto& summary) {
        if (info.compareEligible) {
...
const char* policy_graduation_gate_status_name(PolicyGraduationGateStatus status) {
    switch (status) {
        case PolicyGraduationGateStatus::PASS:
            return "PASS";
        case PolicyGraduationGateStatus::SOFT_PASS:
            return "SOFT_PASS";
        case PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE:
...
optional<PolicyGraduationThreshold> policy_graduation_threshold_for_family(const string& family) {
    if (family == "split_tie_ready") {
        return PolicyGraduationThreshold{family, 16U, 16U, 16U};
    }
    if (family == "split_tie_organic_symmetric") {
        return PolicyGraduationThreshold{family, 32U, 32U, 32U};
    }
...
PolicyGraduationEvaluation evaluate_policy_graduation_gate(const TestOptions& options, const FuzzStats& stats) {
    ...
    if (stats.splitChoiceCompareStateCount < evaluation.effectiveComparedTarget ||
        ...
        evaluation.status = PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE;
        return evaluation;
    }

    evaluation.status = stats.representativeShiftCount == 0U
        ? PolicyGraduationGateStatus::PASS
        : PolicyGraduationGateStatus::SOFT_PASS;
    return evaluation;
}
...
PolicyGraduationGateStatus combine_policy_graduation_status(
    ...
    if (lhs == PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE ||
        rhs == PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE) {
        return PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE;
    }
    if (lhs == PolicyGraduationGateStatus::SOFT_PASS || rhs == PolicyGraduationGateStatus::SOFT_PASS) {
        return PolicyGraduationGateStatus::SOFT_PASS;
    }
...
void run_targeted_planner_case(
    ...
    auto maybe_run_split_choice_exact_audit = [&](ScenarioFamily activeFamily, u32 scenarioSeed, int iter, const PlannerTargetedScenario& scenario, const IterationOrdinal& ordinal) {
        ...
        record_compare_eligibility_probe(eligibility);
        ...
        if (!eligibility.compareEligible) {
...
void write_campaign_aggregate_outputs(
    ...
    PolicyGraduationGateStatus aggregateGateStatus = PolicyGraduationGateStatus::PASS;
    ...
    if (!haveGateStatus) {
        aggregateGateStatus = PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE;
    }
            ...
            for (size_t seedIndex = 0; seedIndex < entry.seeds.size(); ++seedIndex) {
                ...
                for (int iterStart = 0; iterStart < totalIters; iterStart += max(1, checkpointChunkSize)) {
                    ...
                    const bool gatePassed = [&]() {
                        ...
                        for (const auto& run : runStats) {
                            const PolicyGraduationEvaluation gate =
                                evaluate_policy_graduation_gate(effectiveOptions, run.second);
                            if (gate.threshold.family.empty()) {
                                return false;
                            }
                            sawThreshold = true;
                            if (gate.status != PolicyGraduationGateStatus::PASS &&
                                gate.status != PolicyGraduationGateStatus::SOFT_PASS) {
                                return false;
                            }
                        }
                        return sawThreshold;
                    }();
                    const bool partialRunLimitReached =
                        effectiveOptions.maxPartialRuns != 0U &&
                        existingChunks.size() >= effectiveOptions.maxPartialRuns;
                    if (effectiveOptions.stopWhenGatePasses && gatePassed) {
                        write_campaign_aggregate_outputs(
                            config,
                            effectiveOptions,
                            runStats,
                            CampaignStopReason::GATE_PASSED
                        );
                        return;
                    }
                    if (partialRunLimitReached) {
                        write_campaign_aggregate_outputs(
                            config,
                            effectiveOptions,
                            runStats,
                            CampaignStopReason::MAX_PARTIAL_RUNS
                        );
                        return;
                    }
                    if (effectiveOptions.stopAfterCheckpoint) {
...
void run_split_choice_policy_graduation_smoke_case(const TestOptions& options) {
    FuzzStats stats;
    stats.scenarioFamily = "planner_tie_mixed_organic_compare_ready";
    stats.splitChoiceCompareStateCount = 32U;
    stats.compareEligibleStateCount = 32U;
    stats.compareCompletedStateCount = 32U;
    PolicyGraduationEvaluation gate = evaluate_policy_graduation_gate(options, stats);
    require_test(gate.status == PolicyGraduationGateStatus::PASS, "policy graduation smoke expected PASS");

    stats.representativeShiftCount = 1U;
    gate = evaluate_policy_graduation_gate(options, stats);
    require_test(gate.status == PolicyGraduationGateStatus::SOFT_PASS, "policy graduation smoke expected SOFT_PASS");

    stats.semanticShiftCount = 1U;
    gate = evaluate_policy_graduation_gate(options, stats);
...
void run_compare_evidence_threshold_smoke_case(const TestOptions& options) {
    ...
    require_test(
        gate.status == PolicyGraduationGateStatus::INSUFFICIENT_EVIDENCE,
        "compare evidence threshold smoke expected insufficient evidence"
    );
