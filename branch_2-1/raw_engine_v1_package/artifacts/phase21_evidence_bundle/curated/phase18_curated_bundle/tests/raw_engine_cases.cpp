...
void write_campaign_aggregate_outputs(
    ...
    refresh_policy_gate_outputs(options);
    append_policy_gate_summary(summaryPath, options);
}
...
PolicyGateCompareEvidence make_compare_gate_smoke_evidence(
    const string& family,
    size_t comparedStateCount,
    size_t compareEligibleStateCount,
    size_t compareCompletedStateCount
) {
    PolicyGateCompareEvidence evidence;
    evidence.family = family;
    evidence.comparedStateCount = comparedStateCount;
    evidence.compareEligibleStateCount = compareEligibleStateCount;
    evidence.compareCompletedStateCount = compareCompletedStateCount;
...
PolicyGateManifest make_policy_gate_smoke_manifest() {
    ...
    manifest.families.push_back(
        evaluate_planner_tie_non_applicable_gate(
            applicability,
            PolicyGateCompareEvidence{}
        )
    );
