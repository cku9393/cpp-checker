...
filesystem::path save_fuzz_spec(const FuzzSpec& spec, const string& stem) {
    const filesystem::path outDir = filesystem::current_path() / "counterexamples";
...
void assert_target_prepare_equivalent(
    ...
    const NormalizedPrep got = normalize_prep(prepare_isolate_neighborhood(RE, sid, occ));
    if (!(got == expected)) {
        assert(false);
    }
}
...
void test_isolate_microcase() {
    ...
    const IsolatePrepared prep = prepare_isolate_neighborhood(RE, sid, occ17);
...
void test_isolate_multiedge_microcase() {
    ...
    const IsolatePrepared p = prepare_isolate_neighborhood(RE, sid, occ);
...
void test_split_microcase() {
    ...
    const SplitSeparationPairResult sp = split_separation_pair(fixture.RE, fixture.sid, 2, 8, fixture.U);
...
void test_join_microcase() {
    ...
    const NormalizedPrep expected = normalize_prep(
        prepare_isolate_neighborhood(fixture.RE, fixture.sid, fixture.occ31)
    );
    ...
    assert(join_separation_pair(fixture.RE, sid31, sid44, 2, 8, fixture.U).mergedSid == sid31);
...
void test_integrate_microcase() {
    ...
    assert(integrate_skeleton(RE, parentSid, childSid, identity_boundary_map(6, 8), U).mergedSid == parentSid);
...
void test_planner_microcase(const TestOptions& options) {
    ...
    run_raw_local_updater(RE, occ31, U, &hooks, runOptions);
...
void run_split_only_iteration(const FuzzSpec& spec) {
    ...
    const SplitSeparationPairResult sp = split_separation_pair(RE, gc.sid, 2, 8, U);
    if (sp.child.size() < 2U) {
        assert(false);
    }
    assert_engine_bookkeeping_sane(RE);
...
void run_join_only_iteration(const FuzzSpec& spec) {
    ...
    for (int i = 0; i < joinCount; ++i) {
        join_separation_pair(RE, layout.anchor, layout.otherOccChildren[static_cast<size_t>(i)], 2, 8, U);
        assert_engine_bookkeeping_sane(RE);
...
void run_integrate_only_iteration(const FuzzSpec& spec) {
    ...
    for (int i = 0; i < integrateCount; ++i) {
        integrate_skeleton(RE, layout.anchor, layout.boundaryOnly[static_cast<size_t>(i)], identity_boundary_map(), U);
        assert_engine_bookkeeping_sane(RE);
        const vector<OccID> hosted = RE.skel.get(layout.anchor).hostedOcc;
        assert_occurrences_match_initial(RE, hosted, gc.expected, sigs);
    }
...
void run_isolate_split_iteration(const FuzzSpec& spec) {
    ...
    const SplitSeparationPairResult sp = split_separation_pair(RE, ir.residualSkel, sep->first, sep->second, U);
    if (sp.child.size() < 2U) {
        assert(false);
    }
    assert_engine_bookkeeping_sane(RE);
...
void run_split_join_iteration(const FuzzSpec& spec) {
    ...
    for (RawSkelID sidAB : layout.boundaryOnly) {
        integrate_skeleton(RE, layout.anchor, sidAB, identity_boundary_map(), U);
    }
    for (RawSkelID sidX : layout.otherOccChildren) {
        join_separation_pair(RE, layout.anchor, sidX, 2, 8, U);
    }
...
void run_mixed_planner_iteration(const FuzzSpec& spec) {
    ...
    for (int step = 0; step < spec.opCount; ++step) {
        ...
        run_raw_local_updater(RE, target, U, &hooks, runOptions);
        assert_engine_bookkeeping_sane(RE);
...
void run_roundtrip_iteration(mt19937& rng) {
    ...
    for (RawSkelID sidAB : layout.boundaryOnly) {
        const IntegrateResult ir = integrate_skeleton(RE, layout.anchor, sidAB, identity_boundary_map(), U);
    ...
    for (RawSkelID sidX : layout.otherOccChildren) {
        const JoinSeparationPairResult jr = join_separation_pair(RE, layout.anchor, sidX, 2, 8, U);
...
filesystem::path write_temp_repro_spec(const FuzzSpec& spec) {
    const filesystem::path outDir = filesystem::current_path() / "counterexamples";
    filesystem::create_directories(outDir);
    const filesystem::path outPath = outDir / "repro_tmp.txt";
    ofstream ofs(outPath);
    if (!ofs) {
...
void run_regression_44001(const TestOptions& options) {
    ...
    for (RawSkelID sidAB : boundaryOnly) {
        const IntegrateResult ir = integrate_skeleton(RE, anchor, sidAB, identity_boundary_map(), U);
