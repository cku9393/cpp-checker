...
PlannerOracleComparison compare_planner_oracle(
    ...
    if (!(comparison.expected.trace == comparison.actual.trace)) {
        comparison.matches = false;
        comparison.mismatchKind = FailureMismatchKind::TRACE_PREFIX;
        return comparison;
    }
    return comparison;
}
...
bool check_planner_oracle(
    ...
    if (comparison.mismatchKind == FailureMismatchKind::TRACE_PREFIX) {
        if (failure != nullptr) {
            *failure =
                string("planner oracle trace mismatch\nreference_trace:\n") +
                describe_planner_trace(comparison.expected.trace) +
                "actual_trace:\n" +
                describe_planner_trace(actualTrace);
        }
