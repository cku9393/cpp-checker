...
RawSkeletonBuilder ref_build_isolated_occ_builder(const IsolatePrepared& in) {
    ...
    const u32 center = get_vid(in.orig, RawVertexKind::OCC_CENTER, in.occ);
    B.allocNbr[in.occ] = in.allocNbr;

    for (const IsoPort& port : in.ports) {
        const u32 other = get_vid(port.attachOrig, RawVertexKind::REAL, 0);
        const RawEdgeKind kind = (port.kind == IsoPort::REAL ? RawEdgeKind::REAL_PORT : RawEdgeKind::BRIDGE_PORT);
        B.E.push_back(make_builder_edge(center, other, kind, port.br, port.side));
    }

    vector<u32> coreLocal;
    for (const TinyEdge& edge : in.core.edges) {
        Vertex aOrig = in.core.orig[edge.a];
        Vertex bOrig = in.core.orig[edge.b];
        if (aOrig == bOrig) {
            continue;
        }
        const u32 a = get_vid(aOrig, RawVertexKind::REAL, 0);
        const u32 b = get_vid(bOrig, RawVertexKind::REAL, 0);
...
RawSkeletonBuilder ref_build_residual_after_isolate(const ReferenceModel& RE, RawSkelID sid, OccID removedOcc) {
    const RawSkeleton& S = RE.skel.get(sid);
    ...
    for (OccID occ : S.hostedOcc) {
        if (occ != removedOcc) {
            survivors.push_back(occ);
        }
    }

    for (OccID occ : survivors) {
        const RawOccRecord& O = RE.occ.get(occ);
...
IsolateVertexResult ref_isolate_vertex(ReferenceModel& RE, RawSkelID sid, OccID occ) {
    const IsolatePrepared prep = ref_prepare_isolate(RE, sid, occ);
    RawSkeletonBuilder occBuilder = ref_build_isolated_occ_builder(prep);
    RawSkeletonBuilder residualBuilder = ref_build_residual_after_isolate(RE, sid, occ);

    const RawSkelID sidOcc = RE.skel.alloc(RawSkeleton{});
    ref_commit_skeleton(RE, sidOcc, std::move(occBuilder));
    ref_commit_skeleton(RE, sid, std::move(residualBuilder));
...
SplitSeparationPairResult ref_split(ReferenceModel& RE, RawSkelID sid, Vertex saOrig, Vertex sbOrig) {
    ...
    ref_commit_skeleton(RE, sid, std::move(builders[0]));
    ...
    for (u32 i = 1; i < static_cast<u32>(builders.size()); ++i) {
        ...
        ref_commit_skeleton(RE, nsid, std::move(builders[i]));
    }
...
JoinSeparationPairResult ref_join(
    ...
    ref_commit_skeleton(RE, leftSid, std::move(B));
...
IntegrateResult ref_integrate(
    ...
    ref_commit_skeleton(RE, parentSid, std::move(B));
...
SplitResultSignature capture_split_result_signature(const RawEngine& RE, const SplitSeparationPairResult& result) {
    ...
    for (const SplitChildInfo& child : result.child) {
        ...
        sig.child.push_back(std::move(childSig));
    }
    return normalize_split_signature(std::move(sig));
}
...
MergeResultSignature capture_join_result_signature(const RawEngine& RE, const JoinSeparationPairResult& result) {
    ...
    return normalize_merge_signature(std::move(sig));
}
...
EngineStateSignature capture_engine_state_signature(const RawEngine& RE) {
    ...
    for (u32 sid = 0; sid < RE.skel.a.size(); ++sid) {
        ...
        sig.skeletons.push_back(std::move(skeletonSig));
    }
...
JoinSeparationPairResult reference_join_separation_pair(
    ...
    return ref_join(before, leftSid, rightSid, saOrig, sbOrig);
}
...
IntegrateResult reference_integrate_skeleton(
    ...
    return ref_integrate(before, parentSid, childSid, bm);
}
