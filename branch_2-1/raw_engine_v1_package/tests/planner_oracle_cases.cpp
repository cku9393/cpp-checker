...
UpdJob make_join_job(RawSkelID leftSid, RawSkelID rightSid, Vertex aOrig, Vertex bOrig) {
    UpdJob job;
...
RawUpdaterHooks make_fault_injected_hooks(const RawPlannerCtx& ctx, PlannerFaultKind fault) {
    RawUpdaterHooks hooks = make_basic_hooks_for_target(ctx);
    if (fault == PlannerFaultKind::NONE) {
        return hooks;
    }
    ...
    if (fault == PlannerFaultKind::OMIT_AFTER_SPLIT_JOIN) {
        hooks.afterSplit = [ctx](const SplitSeparationPairResult& res, deque<UpdJob>& q) {
            ...
            if (anchorIdx < 0) {
                throw runtime_error("planner fault injection missing split anchor child");
            }
            ...
            for (int i = 0; i < static_cast<int>(res.child.size()); ++i) {
                ...
                if (c.boundaryOnly) {
                    jobs.push_back(make_integrate_job(
                        anchorSid,
                        c.sid,
                        res.aOrig,
                        res.bOrig,
                        {BoundaryMapEntry{res.aOrig, res.aOrig}, BoundaryMapEntry{res.bOrig, res.bOrig}}
                    ));
                }
            }
    ...
    if (fault == PlannerFaultKind::OMIT_AFTER_SPLIT_INTEGRATE) {
        hooks.afterSplit = [ctx](const SplitSeparationPairResult& res, deque<UpdJob>& q) {
            if (res.child.empty()) {
                return;
            }
            ...
            for (int i = 0; i < static_cast<int>(res.child.size()); ++i) {
                ...
                if (!c.boundaryOnly && child_intersects_keep_occ(c, ctx)) {
                    jobs.push_back(make_join_job(anchorSid, c.sid, res.aOrig, res.bOrig));
                }
            }
            jobs.push_back(make_ensure_sole_job(ctx.targetOcc));
            prepend_jobs(q, std::move(jobs));
    ...
    if (fault == PlannerFaultKind::OMIT_ENSURE_SOLE_AFTER_JOIN) {
        hooks.afterJoin = [](const JoinSeparationPairResult&, deque<UpdJob>&) {};
        return hooks;
    }

    if (fault == PlannerFaultKind::OMIT_ENSURE_SOLE_AFTER_INTEGRATE) {
        hooks.afterIntegrate = [](const IntegrateResult&, deque<UpdJob>&) {};
        return hooks;
    }

    if (fault == PlannerFaultKind::WRONG_BRANCH_ROUTING) {
        hooks.afterSplit = [ctx](const SplitSeparationPairResult& res, deque<UpdJob>& q) {
            if (res.child.size() < 2U) {
                q.push_front(make_ensure_sole_job(ctx.targetOcc));
                return;
            }
            const int anchorIdx = choose_anchor_child(res, ctx);
            const int wrongIdx = (anchorIdx == 0 ? 1 : 0);
            const RawSkelID wrongSid = res.child[static_cast<size_t>(wrongIdx)].sid;
            vector<UpdJob> jobs;
            for (int i = 0; i < static_cast<int>(res.child.size()); ++i) {
                if (i == wrongIdx) {
                    continue;
                ...
                if (c.boundaryOnly) {
                    ...
                    jobs.push_back(make_join_job(wrongSid, c.sid, res.aOrig, res.bOrig));
                }
            }
            jobs.push_back(make_ensure_sole_job(ctx.targetOcc));
            prepend_jobs(q, std::move(jobs));
...
void run_actual_planner_impl(
    ...
    PlannerCoverageSummary* coverage,
    const RawUpdaterHooks* hooksOverride
) {
    ...
    if (initialQueue != nullptr && !initialQueue->empty()) {
        ...
        q.push_back(make_ensure_sole_job(ctx.targetOcc));
    }
    const RawUpdaterHooks defaultHooks = make_basic_hooks_for_target(ctx);
    const RawUpdaterHooks& hooks = (hooksOverride != nullptr ? *hooksOverride : defaultHooks);
    if (coverage != nullptr) {
    ...
    if (coverage != nullptr) {
        const SplitChoicePolicyStats& choiceStats = split_choice_policy_stats();
        coverage->splitChoiceCandidateCount += choiceStats.candidateCount;
        coverage->splitChoiceEvalCount += choiceStats.evalCount;
        coverage->splitChoiceTieCount += choiceStats.tieCount;
        coverage->splitChoiceMulticlassCount += choiceStats.multiclassCount;
        coverage->splitChoiceFallbackCount += choiceStats.fallbackCount;
        for (const auto& [classCount, seen] : choiceStats.equivClassCountHistogram) {
            coverage->splitChoiceEquivClassCountHistogram[classCount] += seen;
        }
    }
}
...
void execute_planner_capture(
    ...
    run_actual_planner_impl(RE, ctx, U, runOptions, initialQueue, trace, activePhase, nullptr, nullptr);
}
...
PlannerExecutionResult run_planner_checked_capture(
    ...
    run_actual_planner_impl(RE, ctx, U, runOptions, initialQueue, result.trace, nullptr, &result.coverage, nullptr);
...
PlannerExecutionResult run_planner_checked_capture_fault_injected(
    ...
    PlannerCallDumpGuard dumpGuard(active_test_options(), plannerDump, effective_label("planner_fault", label));
    PlannerExecutionResult result;
    const RawUpdaterHooks faultyHooks = make_fault_injected_hooks(ctx, fault);
    run_actual_planner_impl(RE, ctx, U, runOptions, initialQueue, result.trace, nullptr, &result.coverage, &faultyHooks);
    g_lastPlannerTrace = result.trace;
    assert_all_live_state_valid(RE);
    dumpGuard.mark_success();
    return result;
