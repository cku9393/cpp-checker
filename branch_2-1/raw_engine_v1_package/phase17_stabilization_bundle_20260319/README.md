...
- `--target-lineage-samples <N>` raises the lineage sample target used by proxy-family representativeness checks
...
- `--stop-when-gate-passes` exits after the next checkpoint flush once every gated compare family reaches `PASS`, `PROXY_PASS`, or `NON_APPLICABLE`
...
- `planner_tie_mixed_organic` stays the real post-split mixed follow-up family. `planner_tie_mixed_organic_compare_ready` is the tests-only split-tie precursor used to collect exact-shadow vs exact-full compare evidence without changing the production sole-hosted-occ split precondition.
...
- phase17 nightly compare targets are `32/32/32` compared/eligible/completed states for `split_tie_organic_symmetric`, `planner_tie_mixed_organic_compare_ready`, and `automorphism_probe_large`; `split_tie_ready` stays at `16/16/16`, and `planner_tie_mixed_organic` now graduates through applicability audit plus proxy/non-applicable classification instead of direct compare evidence
...
- stats include:
  ...
  - policy graduation gate: `gate_status`, `run_gate`, `target_compared_states`, `target_eligible_states`, `target_lineage_samples`, `target_applicability_states`, `target_applicability_confidence`, `stop_reason`
...
- `nightly` is opt-in: those tests are not even registered unless `-DRAW_ENGINE_REGISTER_NIGHTLY_TESTS=ON` is set at configure time, and it is the place where the exact compare graduation gate is expected to be evaluated.
...
- `nightly` passes when the opt-in heavy exhaustive/fuzz/tail coverage tests pass and the compare campaigns reach their required phase17 graduation outcome for every configured family: `PASS`, `PROXY_PASS`, or `NON_APPLICABLE`.
...
- `--stop-when-gate-passes` only stops after a checkpoint flush, and only when every gated compare family in the campaign is `PASS`, `PROXY_PASS`, or `NON_APPLICABLE`.
- `--max-partial-runs <N>` is the operational backstop for nightly time-slicing; hitting it writes the current merged aggregate with `stop_reason=max_partial_runs` and leaves `gate_status=FAIL` unless the configured gate already passed.
...
- For compare-heavy nightly runs, prefer `ctest -L compare` for focused local validation, use the phase17 compare configs to collect the `32/32/32` graduation evidence on the expensive families, and keep the direct mixed-family applicability audit separate because it is expected to graduate as either `NON_APPLICABLE` or `UNDER_GENERATED`, not direct compare `PASS`.

Policy graduation gate:
- Built-in family minima are `split_tie_ready >= 16/16/16` compared/eligible/completed states and `split_tie_organic_symmetric`, `planner_tie_mixed_organic_compare_ready`, `automorphism_probe_large >= 32/32/32`.
...
- `PROXY_PASS` means the direct family remains under-generated for direct compare, but the proxy compare family reached `PASS` and the lineage audit met its sample, semantic/final consistency, followup-preservation, and structure-only-enrichment targets.
...
- `FAIL` means a semantic disagreement, fallback, or semantic shift was observed, or the required direct/proxy/applicability evidence was not met.
- `planner_tie_mixed_organic` now graduates through applicability audit plus proxy-family interpretation rather than through direct compare `PASS`.
- Campaign aggregate summaries emit `gate_status`, per-family `run_gate` rows, `stop_reason`, and compare profiling ratios so nightly automation can decide whether `exact_shadow` stays graduated.
