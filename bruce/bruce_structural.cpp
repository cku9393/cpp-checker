...
vector<vi> graph;
vector<int> tmp_bucket_id, todo_seen;
int todo_token = 1;
queue<int> q;
...
void solve() {
    ...
    while (ord.size() < (size_t)N) {
        vector<int> todo;
        ++todo_token;
        if (todo_token == INT_MAX) {
            fill(todo_seen.begin(), todo_seen.end(), 0);
            todo_token = 1;
        }

        auto add_todo = [&](int g) {
            if (g < 0) return;
            if (g >= (int)todo_seen.size()) todo_seen.resize(g + 1, 0);
            if (todo_seen[g] == todo_token) return;
            todo_seen[g] = todo_token;
            todo.pb(g);
        };
