...
#define CHAINRELAX_SELECTED_GATE 0
...
inline std::string envs(const char* k) {
    const char* v = std::getenv(k);
    return v ? std::string(v) : std::string();
}
inline long long envll(const char* k, long long defv = -1) {
    const char* v = std::getenv(k);
    if (!v || !*v) return defv;
    try {
        return std::stoll(v);
...
inline bool forceRound45Shadowcheck() {
    return envll("DENSE_DECOMPOSESERIES_ROUND45_SHADOWCHECK", 0) != 0;
}
inline long long candidateRowLimit() {
    return envll("DENSE_ROUND45_CANDIDATE_ROW_LIMIT", 256);
}
...
inline void ensureHeader(const std::string& path,
...
inline bool isDenseMode() {
    static const std::unordered_set<std::string> denseModes = {
        "comb_dense",
        "comb_rect_dense",
    ...
    return denseModes.count(caseMode()) != 0;
}
...
inline void appendTsv(const std::string& path,
...
struct RowMeta {
    ...
    bool boundaryZero = false, ccOne = false, dense_guard = false, boundary_guard = false;
    bool E_guard = false, Q_guard = false;
    std::string badQueriesShape;
    long long fallback_total_ns = 0, fallback_local_id_adj_ns = 0, fallback_step3_ns = 0;
    long long fallback_keep_order_ns = 0, fallback_apply_ns = 0;
...
inline void beginAttempt(long long row_idx,
                         int currentV,
                         int currentE,
                         int currentQ,
                         double q_over_e,
                         ...
                         long long fallback_total_ns,
                         long long fallback_local_id_adj_ns,
                         long long fallback_step3_ns,
                         long long fallback_keep_order_ns,
                         long long fallback_apply_ns,
                         bool shadow_prefilter_hit) {
    RowMeta r;
    r.active = true;
    r.row_idx = row_idx;
    r.currentV = currentV;
    ...
    r.currentQ = currentQ;
    r.q_over_e = q_over_e;
    r.attachCuts = attachCuts;
    r.boundarySize = boundarySize;
    r.oldBoundarySize = oldBoundarySize;
    r.boundaryZero = boundaryZero;
    r.ccOne = ccOne;
    r.dense_guard = dense_guard;
    r.boundary_guard = boundary_guard;
    r.E_guard = E_guard;
    r.Q_guard = Q_guard;
    r.badQueriesShape = shape;
    r.fallback_total_ns = fallback_total_ns;
    r.fallback_local_id_adj_ns = fallback_local_id_adj_ns;
    r.fallback_step3_ns = fallback_step3_ns;
    r.fallback_keep_order_ns = fallback_keep_order_ns;
    r.fallback_apply_ns = fallback_apply_ns;
    r.shadow_prefilter_hit = shadow_prefilter_hit;
    R() = r;
}
...
inline void appendTsv(const std::string& path,
                      const std::vector<std::string>& cols) {
    std::ofstream ofs(outputPath(path), std::ios::app);
...
inline void appendRound45EventRow(long long candidate_idx,
    ...
    std::vector<std::string> cols = {
        ...
        std::to_string(same_side_probe_found_t ? 1 : 0),
        std::to_string(same_side_probe_visited_vertices),
...
inline void finishAttempt(bool shadowAttempted,
                          bool shadowMatch,
                          const std::string& mismatchReason,
                          bool validatorShadowFail,
                          long long spqr_total_ns) {
    auto r = R();
    if (!r.active) return;
    auto& g = G();
    ++g.rows;
    ...
    g.ds_build_subsetinfo_ns += r.ds_build_subsetinfo_ns;
    g.ds_candidate_loop_ns += r.ds_candidate_loop_ns;
    g.ds_comp_vector_init_ns += r.ds_comp_vector_init_ns;
    g.ds_seed_scan_ns += r.ds_seed_scan_ns;
    g.ds_bfs_queue_pushpop_ns += r.ds_bfs_queue_pushpop_ns;
    g.ds_bfs_neighbor_iter_ns += r.ds_bfs_neighbor_iter_ns;
    g.ds_comp_label_write_ns += r.ds_comp_label_write_ns;
    g.ds_partition_loop_ns += r.ds_partition_loop_ns;
    g.ds_connectivity_check_ns += r.ds_connectivity_check_ns;
    g.ds_normvec_ns += r.ds_normvec_ns;
    g.ds_other_ns += r.ds_other_ns;
    g.ds_candidate_count += r.ds_candidate_count;
    g.ds_bfs_run_count += r.ds_bfs_run_count;
    g.ds_seed_scan_hits += r.ds_seed_scan_hits;
    g.ds_queue_pushpop_hits += r.ds_queue_pushpop_hits;
    g.ds_neighbor_iter_hits += r.ds_neighbor_iter_hits;
    g.ds_label_write_hits += r.ds_label_write_hits;
    g.ds_partition_hits += r.ds_partition_hits;
    g.ds_connectivity_check_hits += r.ds_connectivity_check_hits;
    double accounted = 0.0;
    if (r.ds_total_ns > 0) {
        accounted = (double)(r.ds_build_subsetinfo_ns + r.ds_candidate_loop_ns +
                             r.ds_comp_vector_init_ns + r.ds_seed_scan_ns +
                             r.ds_bfs_queue_pushpop_ns + r.ds_bfs_neighbor_iter_ns +
                             r.ds_comp_label_write_ns + r.ds_partition_loop_ns +
                             r.ds_connectivity_check_ns + r.ds_normvec_ns) /
                    (double)r.ds_total_ns;
    }
    appendTsv("round38_dense_decomposeseries_rows.tsv", {
        caseMode(), std::to_string(caseN()), std::to_string(caseSeed()), std::to_string(r.row_idx),
        ...
        std::to_string(r.boundaryZero ? 1 : 0), std::to_string(r.ccOne ? 1 : 0),
        std::to_string(r.dense_guard ? 1 : 0), std::to_string(r.boundary_guard ? 1 : 0),
        std::to_string(r.E_guard ? 1 : 0), std::to_string(r.Q_guard ? 1 : 0), r.badQueriesShape,
        ...
        [&](){ std::ostringstream os; os << std::fixed << std::setprecision(6) << (r.ds_same_side_reject_count + r.ds_accepted_candidate_count + r.ds_other_reject_count > 0 ? (double)r.ds_st_separator_candidates / (double)(r.ds_same_side_reject_count + r.ds_accepted_candidate_count + r.ds_other_reject_count) : 0.0); return os.str(); }()
    });
    long long total_candidates = r.ds_same_side_reject_count + r.ds_accepted_candidate_count +
                                 r.ds_other_reject_count;
    double st_separator_ratio = total_candidates > 0
        ? (double)r.ds_st_separator_candidates / (double)total_candidates
        : 0.0;
...
inline void dump(std::ostream& os) {
    const auto& c = G();
    os << "[DENSE_DECOMPOSESERIES_R38 rows] rows=" << c.rows
       << " shadow_prefilter_hit=" << c.shadow_prefilter_hit
       << " shadow_attempted=" << c.shadow_attempted
       << " shadow_match=" << c.shadow_match
       << " shadow_mismatch=" << c.shadow_mismatch << "\n";
}
#else
struct Counters {};
inline Counters& G() {
...
inline void reset() {}
inline std::string caseMode() { return std::string(); }
inline long long caseN() { return -1; }
inline long long caseSeed() { return -1; }
inline bool forceRound45Shadowcheck() { return false; }
inline bool isDenseMode() { return false; }
inline long long candidateRowLimit() { return 0; }
inline long long nsSince(std::chrono::steady_clock::time_point) { return 0; }
inline double ms(long long) { return 0.0; }
...
public:
    ...
    struct CanonSparseSemantic {
        ...
        bool operator==(const CanonSparseSemantic& o) const {
            return pieceEdges == o.pieceEdges &&
...
Solver::SparsePatch Solver::splitBlockLocalRebuild(int oldCore, int x) const {
    ...
    if (B.badQueries.empty() && (int)B.attachCuts.size() >= 2) {
        ...
        if (nonIso_pre > 0 && survE_pre >= nonIso_pre) {
            ...
            if (minDegNZ_pre >= 2) {
                ...
                if ((int)oldBoundaryList_pre.size() == nonIso_pre - 2 &&
    ...
    ++__lidEpoch;
    if (__lidEpoch == INT_MAX) {
        std::fill(__lidStamp.begin(), __lidStamp.end(), 0);
        __lidEpoch = 1;
    }
    for (int v : B.allVertices) {
        if (v == x) continue;
    ...
    for (int e : B.realEdges) {
        ...
        int a = edges[e].u;
        int b = edges[e].v;
        chk(0 <= a && a < (int)__lidStamp.size() && 0 <= b && b < (int)__lidStamp.size() &&
            __lidStamp[a] == __lidEpoch && __lidStamp[b] == __lidEpoch,
            "splitBlockLocalRebuild: surviving edge endpoint missing in lid");
        int la = __lidVal[a];
        int lb = __lidVal[b];
        chk(la != lb,
            "splitBlockLocalRebuild: self-loop not expected");
        adj[la].push_back({lb, e});
        adj[lb].push_back({la, e});
        deg[la]++;
        deg[lb]++;
    }
        ...
        if (start != -1) {
            if (survE >= nonIso && minDegNZ >= 2) {
                ...
                auto branch5BoolCheck = [&]() -> bool {
                    ...
                    dfsArt(start, -1);
                    return seenNonIso == nonIso && !hasArt;
                };
    ...
    std::vector<int> tin(n, 0), low(n, 0), estk;
    std::vector<char> isArt(n, 0);
    std::vector<std::vector<int>> comps;
    std::vector<int> isolatedLocal;
    int timer = 0;

    std::function<void(int, int)> dfs = [&](int u, int peid) {
        tin[u] = low[u] = ++timer;
        int child = 0;
    ...
    for (int cutBC : B.attachCuts) {
        ...
        int v = bcNodes[cutBC].origVertex;
        if (v != x && 0 <= v && v < (int)orig.size() && orig[v].alive) {
            oldBoundary.insert(v);
        }
    }

    std::unordered_set<int> boundarySet = oldBoundary;
    for (int lidv = 0; lidv < n; ++lidv) {
        if (isArt[lidv]) boundarySet.insert(realOf[lidv]);
    }
    ...
    std::unordered_map<int, std::vector<int>> inc; // boundary vertex -> piece ids

    for (const auto& Eset : comps) {
        PieceTmp P;
        P.edges = Eset;

        std::unordered_set<int> seenV, seenH;
        for (int e : P.edges) {
            int a = edges[e].u;
            int b = edges[e].v;
            if (seenV.insert(a).second) P.vertices.push_back(a);
            if (seenV.insert(b).second) P.vertices.push_back(b);

            int h = edges[e].handleId;
            if (h != -1 && h < (int)handles.size() && handles[h].watched &&
                h < (int)handleOwnerCore.size() && handleOwnerCore[h] == oldCore) {
                if (seenH.insert(h).second) P.handles.push_back(h);
            }
        }
    ...
    for (auto& [v, vec] : inc) vec = normVec(std::move(vec));
    ...
    for (int i = 0; i < (int)pieces.size(); ++i) {
        ...
        if (pieces[i].mass > pieces[keep].mass) {
            keep = i;
        } else if (pieces[i].mass == pieces[keep].mass && pieces[i].edges < pieces[keep].edges) {
            keep = i;
        }
    }
    ...
    std::vector<int> boundaryList(boundarySet.begin(), boundarySet.end());
    std::sort(boundaryList.begin(), boundaryList.end());

    for (int v : boundaryList) {
        auto it = inc.find(v);
        std::vector<int> pieceInc = (it == inc.end() ? std::vector<int>{} : it->second);
        bool existedOldCut = oldBoundary.count(v);
    ...
    for (int lidv : isolatedLocal) {
        int v = realOf[lidv];
        if (!boundarySet.count(v)) S.isolatedExclusive.push_back(v);
    }
    S.isolatedExclusive = normVec(std::move(S.isolatedExclusive));
...
bool decomposeSeriesFixed(const Solver& s,
    ...
    auto __prof_finalize = [&](bool retv) {
        if (__prof_on) {
            ...
            long long explicit_ns =
                __prof_call.ds_build_subsetinfo_ns +
                __prof_call.ds_candidate_loop_ns +
                __prof_call.ds_comp_vector_init_ns +
                __prof_call.ds_seed_scan_ns +
                __prof_call.ds_bfs_queue_pushpop_ns +
                __prof_call.ds_bfs_neighbor_iter_ns +
                __prof_call.ds_comp_label_write_ns +
                __prof_call.ds_partition_loop_ns +
                __prof_call.ds_connectivity_check_ns +
                __prof_call.ds_normvec_ns;
            if (__prof_call.ds_total_ns > explicit_ns) {
                __prof_call.ds_other_ns += (__prof_call.ds_total_ns - explicit_ns);
            }
...
#if DENSE_DECOMPOSESERIES_ROUND38_PROFILE
    if (__prof_on) {
        __prof_call.ds_build_subsetinfo_ns += dense_decomposeseries_round38_prof::nsSince(__t_build);
    }
#endif
    auto its = G.lid.find(sTerm), itt = G.lid.find(tTerm);
#if DENSE_DECOMPOSESERIES_ROUND38_PROFILE
    ...
    for (int cand : G.verts) {
        ...
        if (comp[ls] == comp[lt]) {
            ...
            if (__prof_on) {
                ...
                for (int label : comp) {
                    if (label == comp[ls]) ++__cand_same_side_probe_visited_vertices;
                }
                ...
                long long cand_ns = dense_decomposeseries_round38_prof::nsSince(__cand_t0);
                if (cand_ns > __cand_parts_ns) {
                    __prof_call.ds_candidate_loop_ns += (cand_ns - __cand_parts_ns);
                }
            }
#endif
            continue;
...
bool Solver::shouldUseTrueSpqr(int oldCore) const {
    ...
    if (B.badQueries.empty()) return false;

    int E = (int)B.realEdges.size();
    int V = (int)B.allVertices.size();
    int Q = (int)B.badQueries.size();

    // TRUE_SPQR currently only helps on smaller, structured bad cores.
    // Large / dense bad cores are faster on the local rebuild fallback.
    if (E > 96) return false;
    if (V > 80) return false;
    if (Q > 24) return false;

    // Dense-ish cores are not worth the current raw builder cost.
    if (E > V + 24) return false;

    return true;
}

Solver::SparsePatch
Solver::splitBlockSPQRForced(int oldCore, int x) const {
    chk(0 <= oldCore && oldCore < (int)blocks.size(),
        "splitBlockSPQRForced: bad oldCore");
    chk(blocks[oldCore].alive,
        "splitBlockSPQRForced: dead oldCore");

    // ensure the SPQR cache for this core is fresh for the current state
    Solver* self = const_cast<Solver*>(this);
    self->rebuildBlockSpqrFull(oldCore);

    chk(0 <= oldCore && oldCore < (int)blockSpqr.size(),
        "splitBlockSPQRForced: missing BlockSpqr");
    chk(blockSpqr[oldCore].alive,
        "splitBlockSPQRForced: dead BlockSpqr");
    ...
    return materializeSparsePatch(B, T, A, C);
}
...
Solver::splitBlockSPQR(int oldCore, int x) const {
...
#if DENSE_DECOMPOSESERIES_ROUND38_PROFILE
    if (dense_decomposeseries_round38_prof::isDenseMode()) {
        const BlockCore& B = blocks[oldCore];
        int currentV = (int)B.allVertices.size();
        int currentE = (int)B.realEdges.size();
        int currentQ = (int)B.badQueries.size();
        double q_over_e = currentE > 0 ? (double)currentQ / (double)currentE : 0.0;

        auto local_t0 = dense_decomposeseries_round38_prof::Clock::now();
        SparsePatch localP = splitBlockLocalRebuild(oldCore, x);
        long long local_total_ns = dense_decomposeseries_round38_prof::nsSince(local_t0);
        long long local_id_adj_ns = 0;
        long long step3_ns = 0;
        long long keep_ns = 0;
        long long apply_ns = 0;

        int attachCutsSize = (int)B.attachCuts.size();
        int boundarySize = (int)localP.boundary.size();
        int oldBoundarySize = 0;
        for (const auto& bd : localP.boundary) {
            if (bd.existedOldCut) ++oldBoundarySize;
        }
        bool boundaryZero = (boundarySize == 0);
        bool ccOne = ((int)localP.small.size() <= 1);
        bool dense_guard = (currentE > currentV + 24);
        bool boundary_guard = false;
        bool e_guard = (currentE > 96);
        bool q_guard = (currentQ > 24);
        std::ostringstream shape_os;
        shape_os << "Q" << currentQ
                 << "_A" << attachCutsSize
                 << "_B" << boundarySize
                 << "_OB" << oldBoundarySize
                 << "_BZ" << (boundaryZero ? 1 : 0)
                 << "_CC" << (ccOne ? 1 : 0);
        std::string shape = shape_os.str();
        bool prefilter = true;
        bool shadow_attempted = false;
        bool shadow_match = false;
        std::string mismatch_reason;
        long long spqr_total_ns = 0;
        bool validator_shadow_fail = false;
        long long row_idx = dense_decomposeseries_round38_prof::G().rows + 1;

        if (prefilter) {
            static thread_local unsigned long long prefilter_counter = 0;
            ++prefilter_counter;
            bool doShadow = (prefilter_counter <= 8ULL) || ((prefilter_counter % 8ULL) == 1ULL);
            if (dense_decomposeseries_round38_prof::forceRound45Shadowcheck() &&
                dense_decomposeseries_round38_prof::G().rows == 0) {
                doShadow = true;
            }
            if (doShadow) {
                shadow_attempted = true;
                dense_decomposeseries_round38_prof::beginAttempt(
                    row_idx,
                    currentV,
                    currentE,
                    currentQ,
                    ...
                    apply_ns,
                    true);
                auto spqr_t0 = dense_decomposeseries_round38_prof::Clock::now();
                SparsePatch spqrP = splitBlockSPQRForced(oldCore, x);
                spqr_total_ns = dense_decomposeseries_round38_prof::nsSince(spqr_t0);
#ifdef DEBUG_SOLVER
                auto Asem = canonSparseSemantic(oldCore, localP);
                auto Bsem = canonSparseSemantic(oldCore, spqrP);
                shadow_match = (Asem == Bsem);
                if (!shadow_match) {
                    if (Asem.keepEdges != Bsem.keepEdges) mismatch_reason = "keep_choice_mismatch";
                    else if (Asem.pieceEdges.size() != Bsem.pieceEdges.size()) mismatch_reason = "piece_count_mismatch";
                    else if (Asem.pieceEdges != Bsem.pieceEdges) mismatch_reason = "piece_multiset_mismatch";
                    else mismatch_reason = "invariant_unsure";
                }
#else
                mismatch_reason = "debug_off";
#endif
                dense_decomposeseries_round38_prof::finishAttempt(
                    shadow_attempted,
                    shadow_match,
                    mismatch_reason,
                    validator_shadow_fail,
                    spqr_total_ns);
            } else {
            ...
            dense_decomposeseries_round38_prof::appendTsv("round38_dense_decomposeseries_census_rows.tsv", {
                dense_decomposeseries_round38_prof::caseMode(), std::to_string(dense_decomposeseries_round38_prof::caseN()), std::to_string(dense_decomposeseries_round38_prof::caseSeed()), std::to_string(row_idx),
                std::to_string(currentV), std::to_string(currentE), std::to_string(currentQ), [&](){ std::ostringstream os; os << std::fixed << std::setprecision(6) << q_over_e; return os.str(); }(),
                std::to_string(attachCutsSize), std::to_string(boundarySize), std::to_string(oldBoundarySize), std::to_string(boundaryZero ? 1 : 0), std::to_string(ccOne ? 1 : 0), std::to_string(dense_guard ? 1 : 0), std::to_string(boundary_guard ? 1 : 0), std::to_string(e_guard ? 1 : 0), std::to_string(q_guard ? 1 : 0), shape,
                [&](){ std::ostringstream os; os << std::fixed << std::setprecision(3) << dense_decomposeseries_round38_prof::ms(local_total_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os << std::fixed << std::setprecision(3) << dense_decomposeseries_round38_prof::ms(local_id_adj_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os << std::fixed << std::setprecision(3) << dense_decomposeseries_round38_prof::ms(step3_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os << std::fixed << std::setprecision(3) << dense_decomposeseries_round38_prof::ms(keep_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os << std::fixed << std::setprecision(3) << dense_decomposeseries_round38_prof::ms(apply_ns); return os.str(); }(),
                std::to_string(prefilter ? 1 : 0), std::to_string(shadow_attempted ? 1 : 0), std::to_string(shadow_match ? 1 : 0), mismatch_reason, std::to_string(validator_shadow_fail ? 1 : 0)
            });
    ...
    // Submission fast path: only use TRUE_SPQR on smaller structured bad cores.
    // Everything else stays on the cheaper local rebuild path.
    if (!shouldUseTrueSpqr(oldCore)) {
...
void Solver::buildFromQueries() {
    ...
    dense_decomposeseries_round38_prof::reset();
