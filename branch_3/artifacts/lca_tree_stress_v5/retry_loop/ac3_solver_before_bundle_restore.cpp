...
static bool tscan_branch_state_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_TSCAN_BRANCH_STATE_OPT", false);
    return enabled;
}
static bool state_load_materialization_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_STATE_LOAD_MATERIALIZATION_OPT", false);
    return enabled;
}
static bool prev_state_carry_reuse_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_PREV_STATE_CARRY_REUSE_OPT", false);
    return enabled;
}
static bool carry_reuse_fastpath_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_CARRY_REUSE_FASTPATH_OPT", false);
    return enabled;
}
static bool carry_hit_apply_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_CARRY_HIT_APPLY_OPT", false);
    return enabled;
}
static bool prev_state_writeback_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_PREV_STATE_WRITEBACK_OPT", false);
    return enabled;
}
static bool pointer_rebind_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_POINTER_REBIND_OPT", false);
    return enabled;
}
static bool target_resolve_pinning_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_TARGET_RESOLVE_PINNING_OPT", false);
    return enabled;
}
static bool direct_prebind_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_DIRECT_PREBIND_OPT", false);
    return enabled;
}
static bool rebind_commit_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_REBIND_COMMIT_OPT", false);
    return enabled;
}
static bool metadata_patch_slot_owner_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_METADATA_PATCH_SLOT_OWNER_OPT", false);
    return enabled;
}
static bool slot_owner_update_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_SLOT_OWNER_UPDATE_OPT", false);
    return enabled;
}
static bool slot_owner_patch_commit_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_SLOT_OWNER_PATCH_COMMIT_OPT", false);
    return enabled;
}
static bool slot_owner_field_patch_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_SLOT_OWNER_FIELD_PATCH_OPT", false);
    return enabled;
}
static bool compact_field_pack_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_COMPACT_FIELD_PACK_OPT", false);
    return enabled;
}
static bool pack_encode_normalize_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_PACK_ENCODE_NORMALIZE_OPT", false);
    return enabled;
}
static bool pack_encode_normalize_core_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_PACK_ENCODE_NORMALIZE_CORE_OPT", false);
    return enabled;
}
static bool canonical_normalize_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_CANONICAL_NORMALIZE_OPT", false);
    return enabled;
}
static bool layout_reuse_zero_elision_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_LAYOUT_REUSE_ZERO_ELISION_OPT", false);
    return enabled;
}
static bool layout_signature_gate_opt_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_LAYOUT_SIGNATURE_GATE_OPT", false);
    return enabled;
}
...
static bool release_phase_diag_enabled() {
    static bool enabled = runtime_env_enabled("ENABLE_AC3_PHASE_DIAG", false);
    return enabled;
}
static long long runtime_now_ns() {
    return std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
}
struct ReleasePhaseDiagStats {
    long long erase_calls = 0;
    long long erase_total_ns = 0;
    long long gather_touched_calls = 0;
    long long gather_touched_ns = 0;
    long long gather_touched_classes = 0;
    long long delete_split_calls = 0;
    long long delete_split_ns = 0;
    long long owner_dead_resolve_calls = 0;
    long long owner_dead_resolve_ns = 0;
    long long touched_loop_calls = 0;
    long long touched_loop_ns = 0;
    long long touched_refine_ns = 0;
    long long touched_endpoint_relabel_ns = 0;
    long long touched_query_scan_ns = 0;
    long long touched_reuse_ns = 0;
    long long touched_candidate_finalize_ns = 0;
    long long touched_query_scan_iters = 0;
    long long reuse_route_baseline_calls = 0;
    long long reuse_route_baseline_ns = 0;
    long long reuse_route_delta_preserved_then_skeleton_calls = 0;
    long long reuse_route_delta_preserved_then_skeleton_ns = 0;
    long long reuse_route_connector_skeleton_calls = 0;
    long long reuse_route_connector_skeleton_ns = 0;
    long long reuse_route_general_delta_calls = 0;
    long long reuse_route_general_delta_ns = 0;
    long long rebuild_support_calls = 0;
    long long rebuild_support_ns = 0;
    long long rebuild_support_artifact_hits = 0;
    long long rebuild_support_fallback_hits = 0;
    long long rebuild_support_relevant_endpoints = 0;
    long long rebuild_support_watch_vertices = 0;
    long long retain_calls = 0;
    long long retain_ns = 0;
    long long retain_handles_before = 0;
    long long retain_handles_after = 0;
    long long retain_removed_handles = 0;
    long long retain_sparse_fastpath_hits = 0;
};
static ReleasePhaseDiagStats g_release_phase_diag;
static void reset_release_phase_diag() {
    g_release_phase_diag = ReleasePhaseDiagStats();
}
struct ScopedReleasePhaseTimer {
    long long* total_ns = nullptr;
    long long start_ns = 0;
    explicit ScopedReleasePhaseTimer(long long* total) : total_ns(total) {
        if (total_ns) start_ns = runtime_now_ns();
    }
    ~ScopedReleasePhaseTimer() {
        if (total_ns) *total_ns += runtime_now_ns() - start_ns;
    }
};
static void emit_release_phase_diag_summary() {
    if (!release_phase_diag_enabled()) return;
    const char* run_tag = getenv("RUN_TAG");
    fprintf(stderr,
            "[ac3_phase_diag] run_tag=%s erase_calls=%lld erase_total_ms=%.3f "
            "gather_touched_calls=%lld gather_touched_ms=%.3f gather_touched_classes=%lld "
            "delete_split_calls=%lld delete_split_ms=%.3f "
            "owner_dead_resolve_calls=%lld owner_dead_resolve_ms=%.3f "
            "touched_loop_calls=%lld touched_loop_ms=%.3f touched_refine_ms=%.3f "
            "touched_endpoint_relabel_ms=%.3f touched_query_scan_ms=%.3f "
            "touched_reuse_ms=%.3f touched_candidate_finalize_ms=%.3f "
            "touched_query_scan_iters=%lld reuse_route_baseline_calls=%lld "
            "reuse_route_baseline_ms=%.3f reuse_route_delta_preserved_then_skeleton_calls=%lld "
            "reuse_route_delta_preserved_then_skeleton_ms=%.3f "
            "reuse_route_connector_skeleton_calls=%lld reuse_route_connector_skeleton_ms=%.3f "
            "reuse_route_general_delta_calls=%lld reuse_route_general_delta_ms=%.3f "
            "rebuild_support_calls=%lld rebuild_support_ms=%.3f rebuild_support_artifact_hits=%lld "
            "rebuild_support_fallback_hits=%lld rebuild_support_relevant_endpoints=%lld "
            "rebuild_support_watch_vertices=%lld retain_calls=%lld retain_ms=%.3f "
            "retain_handles_before=%lld retain_handles_after=%lld retain_removed_handles=%lld "
            "retain_sparse_fastpath_hits=%lld\n",
            run_tag ? run_tag : "",
            g_release_phase_diag.erase_calls, g_release_phase_diag.erase_total_ns / 1000000.0,
            g_release_phase_diag.gather_touched_calls, g_release_phase_diag.gather_touched_ns / 1000000.0,
            g_release_phase_diag.gather_touched_classes,
            g_release_phase_diag.delete_split_calls, g_release_phase_diag.delete_split_ns / 1000000.0,
            g_release_phase_diag.owner_dead_resolve_calls, g_release_phase_diag.owner_dead_resolve_ns / 1000000.0,
            g_release_phase_diag.touched_loop_calls, g_release_phase_diag.touched_loop_ns / 1000000.0,
            g_release_phase_diag.touched_refine_ns / 1000000.0,
            g_release_phase_diag.touched_endpoint_relabel_ns / 1000000.0,
            g_release_phase_diag.touched_query_scan_ns / 1000000.0,
            g_release_phase_diag.touched_reuse_ns / 1000000.0,
            g_release_phase_diag.touched_candidate_finalize_ns / 1000000.0,
            g_release_phase_diag.touched_query_scan_iters,
            g_release_phase_diag.reuse_route_baseline_calls,
            g_release_phase_diag.reuse_route_baseline_ns / 1000000.0,
            g_release_phase_diag.reuse_route_delta_preserved_then_skeleton_calls,
            g_release_phase_diag.reuse_route_delta_preserved_then_skeleton_ns / 1000000.0,
            g_release_phase_diag.reuse_route_connector_skeleton_calls,
            g_release_phase_diag.reuse_route_connector_skeleton_ns / 1000000.0,
            g_release_phase_diag.reuse_route_general_delta_calls,
            g_release_phase_diag.reuse_route_general_delta_ns / 1000000.0,
            g_release_phase_diag.rebuild_support_calls, g_release_phase_diag.rebuild_support_ns / 1000000.0,
            g_release_phase_diag.rebuild_support_artifact_hits,
            g_release_phase_diag.rebuild_support_fallback_hits,
            g_release_phase_diag.rebuild_support_relevant_endpoints,
            g_release_phase_diag.rebuild_support_watch_vertices,
            g_release_phase_diag.retain_calls, g_release_phase_diag.retain_ns / 1000000.0,
            g_release_phase_diag.retain_handles_before,
            g_release_phase_diag.retain_handles_after,
            g_release_phase_diag.retain_removed_handles,
            g_release_phase_diag.retain_sparse_fastpath_hits);
    fflush(stderr);
}
...
struct BatchPivotDebugStats {
    ...
    long long time_tbranch_prev_state_materialize_ns = 0;
    long long time_tbranch_prev_state_materialize_calls = 0;
    long long time_tbranch_curr_state_load_ns = 0;
    long long time_tbranch_curr_state_load_calls = 0;
    long long time_tbranch_compare_classify_ns = 0;
    long long time_tbranch_compare_classify_calls = 0;
    long long time_tbranch_removed_to_kept_dispatch_ns = 0;
    long long time_tbranch_removed_to_kept_dispatch_calls = 0;
    long long time_tbranch_kept_to_removed_dispatch_ns = 0;
    long long time_tbranch_kept_to_removed_dispatch_calls = 0;
    long long time_tbranch_no_transition_advance_ns = 0;
    long long time_tbranch_no_transition_advance_calls = 0;
    long long time_tbranch_loop_state_update_ns = 0;
    long long time_tbranch_loop_state_update_calls = 0;
    long long time_tbranch_route_baseline_ns = 0;
    long long time_tbranch_route_delta_preserved_then_skeleton_ns = 0;
    long long time_tbranch_route_connector_skeleton_ns = 0;
    long long time_tbranch_route_general_delta_ns = 0;
    long long tbranch_calls = 0;
    long long tbranch_watch_handles_before = 0;
    long long tbranch_watch_handles_after = 0;
    long long tbranch_removed_handles = 0;
    long long tbranch_prev_state_materialize_calls = 0;
    long long tbranch_curr_state_load_calls = 0;
    long long tbranch_compare_ops = 0;
    long long tbranch_removed_to_kept_checks = 0;
    long long tbranch_kept_to_removed_checks = 0;
    long long tbranch_removed_to_kept_taken = 0;
    long long tbranch_kept_to_removed_taken = 0;
    long long tbranch_no_transition_steps = 0;
    long long tbranch_loop_state_update_calls = 0;
    long long tbranch_window_clipped_calls = 0;
    long long tbranch_route_baseline_calls = 0;
    long long tbranch_route_delta_preserved_then_skeleton_calls = 0;
    long long tbranch_route_connector_skeleton_calls = 0;
    long long tbranch_route_general_delta_calls = 0;
    long long tbranch_removed_ratio_ppm_sum = 0;
    long long tbranch_prev_state_reuse_hits = 0;
    long long tbranch_curr_state_direct_load_hits = 0;
    long long tbranch_branchless_codepath_hits = 0;
    long long tbranch_tail_skip_hits = 0;
    long long time_sload_prev_seed_materialize_ns = 0;
    long long time_sload_prev_seed_materialize_calls = 0;
    long long time_sload_prev_carry_reuse_check_ns = 0;
    long long time_sload_prev_carry_reuse_check_calls = 0;
    long long time_sload_prev_carry_reuse_apply_ns = 0;
    long long time_sload_prev_carry_reuse_apply_calls = 0;
    long long time_sload_curr_direct_load_ns = 0;
    long long time_sload_curr_direct_load_calls = 0;
    long long time_sload_curr_source_lookup_ns = 0;
    long long time_sload_curr_source_lookup_calls = 0;
    long long time_sload_state_pair_pack_ns = 0;
    long long time_sload_state_pair_pack_calls = 0;
    long long time_sload_invariant_ref_hoist_miss_ns = 0;
    long long time_sload_invariant_ref_hoist_miss_calls = 0;
    long long time_sload_route_baseline_ns = 0;
    long long time_sload_route_delta_preserved_then_skeleton_ns = 0;
    long long time_sload_route_connector_skeleton_ns = 0;
    long long time_sload_route_general_delta_ns = 0;
    long long sload_calls = 0;
    long long sload_watch_handles_before = 0;
    long long sload_watch_handles_after = 0;
    long long sload_removed_handles = 0;
    long long sload_prev_seed_calls = 0;
    long long sload_prev_carry_reuse_checks = 0;
    long long sload_prev_carry_reuse_hits = 0;
    long long sload_curr_direct_load_calls = 0;
    long long sload_curr_source_lookup_calls = 0;
    long long sload_curr_source_lookup_hits = 0;
    long long sload_curr_source_lookup_misses = 0;
    long long sload_state_pair_pack_calls = 0;
    long long sload_invariant_ref_hoist_hits = 0;
    long long sload_invariant_ref_hoist_misses = 0;
    long long sload_route_baseline_calls = 0;
    long long sload_route_delta_preserved_then_skeleton_calls = 0;
    long long sload_route_connector_skeleton_calls = 0;
    long long sload_route_general_delta_calls = 0;
    long long sload_removed_ratio_ppm_sum = 0;
    long long sload_prev_state_scalar_reuse_hits = 0;
    long long sload_curr_state_direct_struct_hits = 0;
    long long sload_tuple_pack_skipped_calls = 0;
    long long sload_invariant_pointer_hoist_hits = 0;
    long long time_pcarry_boundary_seed_bootstrap_ns = 0;
    long long time_pcarry_boundary_seed_bootstrap_calls = 0;
    long long time_pcarry_prev_scalar_materialize_ns = 0;
    long long time_pcarry_prev_scalar_materialize_calls = 0;
    long long time_pcarry_prev_scalar_pack_ns = 0;
    long long time_pcarry_prev_scalar_pack_calls = 0;
    long long time_pcarry_carry_reuse_check_ns = 0;
    long long time_pcarry_carry_reuse_check_calls = 0;
    long long time_pcarry_carry_reuse_hit_apply_ns = 0;
    long long time_pcarry_carry_reuse_hit_apply_calls = 0;
    long long time_pcarry_carry_invalidate_transition_ns = 0;
    long long time_pcarry_carry_invalidate_transition_calls = 0;
    long long time_pcarry_carry_invalidate_window_clip_ns = 0;
    long long time_pcarry_carry_invalidate_window_clip_calls = 0;
    long long time_pcarry_reseed_after_invalidate_ns = 0;
    long long time_pcarry_reseed_after_invalidate_calls = 0;
    long long time_pcarry_route_baseline_ns = 0;
    long long time_pcarry_route_delta_preserved_then_skeleton_ns = 0;
    long long time_pcarry_route_connector_skeleton_ns = 0;
    long long time_pcarry_route_general_delta_ns = 0;
    long long pcarry_calls = 0;
    long long pcarry_watch_handles_before = 0;
    long long pcarry_watch_handles_after = 0;
    long long pcarry_removed_handles = 0;
    long long pcarry_prev_seed_calls = 0;
    long long pcarry_prev_seed_from_boundary_calls = 0;
    long long pcarry_prev_seed_from_reseed_calls = 0;
    long long pcarry_prev_scalar_materialize_calls = 0;
    long long pcarry_prev_scalar_pack_calls = 0;
    long long pcarry_carry_reuse_checks = 0;
    long long pcarry_carry_reuse_hits = 0;
    long long pcarry_carry_reuse_misses = 0;
    long long pcarry_carry_invalidate_transition_calls = 0;
    long long pcarry_carry_invalidate_window_clip_calls = 0;
    long long pcarry_reseed_after_invalidate_calls = 0;
    long long pcarry_route_baseline_calls = 0;
    long long pcarry_route_delta_preserved_then_skeleton_calls = 0;
    long long pcarry_route_connector_skeleton_calls = 0;
    long long pcarry_route_general_delta_calls = 0;
    long long pcarry_removed_ratio_ppm_sum = 0;
    long long pcarry_prev_seed_reuse_without_materialize_hits = 0;
    long long pcarry_scalar_pack_skipped_calls = 0;
    long long pcarry_window_clip_forced_reseed_hits = 0;
    long long pcarry_transition_forced_reseed_hits = 0;
    long long time_creuse_eligibility_gate_ns = 0;
    long long time_creuse_eligibility_gate_calls = 0;
    long long time_creuse_boundary_validity_check_ns = 0;
    long long time_creuse_boundary_validity_check_calls = 0;
    long long time_creuse_prev_scalar_eq_check_ns = 0;
    long long time_creuse_prev_scalar_eq_check_calls = 0;
    long long time_creuse_transition_consistency_check_ns = 0;
    long long time_creuse_transition_consistency_check_calls = 0;
    long long time_creuse_hit_apply_prev_state_ns = 0;
    long long time_creuse_hit_apply_prev_state_calls = 0;
    long long time_creuse_hit_apply_cursor_advance_ns = 0;
    long long time_creuse_hit_apply_cursor_advance_calls = 0;
    long long time_creuse_miss_classify_ns = 0;
    long long time_creuse_miss_classify_calls = 0;
    long long time_creuse_reseed_handoff_ns = 0;
    long long time_creuse_reseed_handoff_calls = 0;
    long long time_creuse_route_baseline_ns = 0;
    long long time_creuse_route_delta_preserved_then_skeleton_ns = 0;
    long long time_creuse_route_connector_skeleton_ns = 0;
    long long time_creuse_route_general_delta_ns = 0;
    long long creuse_calls = 0;
    long long creuse_watch_handles_before = 0;
    long long creuse_watch_handles_after = 0;
    long long creuse_removed_handles = 0;
    long long creuse_checks = 0;
    long long creuse_hits = 0;
    long long creuse_misses = 0;
    long long creuse_boundary_validity_hits = 0;
    long long creuse_prev_scalar_eq_hits = 0;
    long long creuse_transition_consistency_hits = 0;
    long long creuse_hit_apply_prev_state_calls = 0;
    long long creuse_hit_apply_cursor_advance_calls = 0;
    long long creuse_miss_transition_reason_calls = 0;
    long long creuse_miss_window_clip_reason_calls = 0;
    long long creuse_reseed_handoff_calls = 0;
    long long creuse_route_baseline_calls = 0;
    long long creuse_route_delta_preserved_then_skeleton_calls = 0;
    long long creuse_route_connector_skeleton_calls = 0;
    long long creuse_route_general_delta_calls = 0;
    long long creuse_removed_ratio_ppm_sum = 0;
    long long creuse_hit_skip_scalar_pack_calls = 0;
    long long creuse_hit_skip_reseed_calls = 0;
    long long creuse_boundary_reuse_assisted_hits = 0;
    long long creuse_miss_after_boundary_clip_hits = 0;
    long long time_chit_prev_state_writeback_ns = 0;
    long long time_chit_prev_state_writeback_calls = 0;
    long long time_chit_prev_state_scalar_store_ns = 0;
    long long time_chit_prev_state_scalar_store_calls = 0;
    long long time_chit_cursor_advance_ns = 0;
    long long time_chit_cursor_advance_calls = 0;
    long long time_chit_next_carry_prepare_ns = 0;
    long long time_chit_next_carry_prepare_calls = 0;
    long long time_chit_hit_bookkeeping_commit_ns = 0;
    long long time_chit_hit_bookkeeping_commit_calls = 0;
    long long time_chit_skip_pack_and_reseed_suppression_ns = 0;
    long long time_chit_skip_pack_and_reseed_suppression_calls = 0;
    long long time_chit_route_baseline_ns = 0;
    long long time_chit_route_delta_preserved_then_skeleton_ns = 0;
    long long time_chit_route_connector_skeleton_ns = 0;
    long long time_chit_route_general_delta_ns = 0;
    long long chit_calls = 0;
    long long chit_watch_handles_before = 0;
    long long chit_watch_handles_after = 0;
    long long chit_removed_handles = 0;
    long long chit_prev_state_writeback_calls = 0;
    long long chit_prev_state_scalar_store_calls = 0;
    long long chit_cursor_advance_calls = 0;
    long long chit_next_carry_prepare_calls = 0;
    long long chit_hit_bookkeeping_commits = 0;
    long long chit_skip_pack_hits = 0;
    long long chit_skip_reseed_hits = 0;
    long long chit_route_baseline_calls = 0;
    long long chit_route_delta_preserved_then_skeleton_calls = 0;
    long long chit_route_connector_skeleton_calls = 0;
    long long chit_route_general_delta_calls = 0;
    long long chit_removed_ratio_ppm_sum = 0;
    long long chit_cursor_advance_same_slot_hits = 0;
    long long chit_prev_state_writeback_elided_calls = 0;
    long long chit_bookkeeping_skipped_calls = 0;
    long long chit_connector_hotpath_hits = 0;
    long long time_pwrite_noop_guard_ns = 0;
    long long time_pwrite_noop_guard_calls = 0;
    long long time_pwrite_target_resolve_ns = 0;
    long long time_pwrite_target_resolve_calls = 0;
    long long time_pwrite_scalar_field_store_ns = 0;
    long long time_pwrite_scalar_field_store_calls = 0;
    long long time_pwrite_compact_pack_store_ns = 0;
    long long time_pwrite_compact_pack_store_calls = 0;
    long long time_pwrite_same_value_skip_ns = 0;
    long long time_pwrite_same_value_skip_calls = 0;
    long long time_pwrite_route_pointer_hoist_miss_ns = 0;
    long long time_pwrite_route_pointer_hoist_miss_calls = 0;
    long long time_pwrite_route_baseline_ns = 0;
    long long time_pwrite_route_delta_preserved_then_skeleton_ns = 0;
    long long time_pwrite_route_connector_skeleton_ns = 0;
    long long time_pwrite_route_general_delta_ns = 0;
    long long pwrite_calls = 0;
    long long pwrite_watch_handles_before = 0;
    long long pwrite_watch_handles_after = 0;
    long long pwrite_removed_handles = 0;
    long long pwrite_noop_guard_calls = 0;
    long long pwrite_noop_guard_hits = 0;
    long long pwrite_target_resolve_calls = 0;
    long long pwrite_target_resolve_cache_hits = 0;
    long long pwrite_scalar_field_store_calls = 0;
    long long pwrite_compact_pack_store_calls = 0;
    long long pwrite_same_value_skip_hits = 0;
    long long pwrite_route_pointer_hoist_miss_calls = 0;
    long long pwrite_route_baseline_calls = 0;
    long long pwrite_route_delta_preserved_then_skeleton_calls = 0;
    long long pwrite_route_connector_skeleton_calls = 0;
    long long pwrite_route_general_delta_calls = 0;
    long long pwrite_removed_ratio_ppm_sum = 0;
    long long pwrite_scalar_store_elided_calls = 0;
    long long pwrite_pack_store_skipped_calls = 0;
    long long pwrite_target_resolve_reuse_hits = 0;
    long long pwrite_connector_hotpath_hits = 0;
    long long time_dprebind_cached_pointer_validate_ns = 0;
    long long time_dprebind_cached_pointer_validate_calls = 0;
    long long time_dprebind_slot_owner_match_check_ns = 0;
    long long time_dprebind_slot_owner_match_check_calls = 0;
    long long time_dprebind_same_pointer_fastpath_ns = 0;
    long long time_dprebind_same_pointer_fastpath_calls = 0;
    long long time_dprebind_stale_pointer_detect_ns = 0;
    long long time_dprebind_stale_pointer_detect_calls = 0;
    long long time_dprebind_rebind_target_lookup_ns = 0;
    long long time_dprebind_rebind_target_lookup_calls = 0;
    long long time_dprebind_rebind_commit_ns = 0;
    long long time_dprebind_rebind_commit_calls = 0;
    long long time_dprebind_metadata_patch_ns = 0;
    long long time_dprebind_metadata_patch_calls = 0;
    long long time_dprebind_connector_hotpath_pin_apply_ns = 0;
    long long time_dprebind_connector_hotpath_pin_apply_calls = 0;
    long long time_dprebind_route_baseline_ns = 0;
    long long time_dprebind_route_delta_preserved_then_skeleton_ns = 0;
    long long time_dprebind_route_connector_skeleton_ns = 0;
    long long time_dprebind_route_general_delta_ns = 0;
    long long dprebind_calls = 0;
    long long dprebind_watch_handles_before = 0;
    long long dprebind_watch_handles_after = 0;
    long long dprebind_removed_handles = 0;
    long long dprebind_cached_pointer_validate_calls = 0;
    long long dprebind_slot_owner_match_checks = 0;
    long long dprebind_slot_owner_match_hits = 0;
    long long dprebind_same_pointer_fastpath_hits = 0;
    long long dprebind_stale_pointer_detect_calls = 0;
    long long dprebind_stale_pointer_detect_hits = 0;
    long long dprebind_rebind_target_lookup_calls = 0;
    long long dprebind_rebind_target_lookup_hits = 0;
    long long dprebind_rebind_commit_calls = 0;
    long long dprebind_metadata_patch_calls = 0;
    long long dprebind_connector_hotpath_pin_apply_calls = 0;
    long long dprebind_pointer_fetch_fallback_calls = 0;
    long long dprebind_route_baseline_calls = 0;
    long long dprebind_route_delta_preserved_then_skeleton_calls = 0;
    long long dprebind_route_connector_skeleton_calls = 0;
    long long dprebind_route_general_delta_calls = 0;
    long long dprebind_validate_failures = 0;
    long long dprebind_same_target_skip_hits = 0;
    long long dprebind_rebind_skipped_same_target_calls = 0;
    long long dprebind_connector_hotpath_hits = 0;
    long long dprebind_route_pointer_hoist_hit_reuse = 0;
    long long time_rcommit_same_target_fastpath_ns = 0;
    long long time_rcommit_same_target_fastpath_calls = 0;
    long long time_rcommit_target_install_ns = 0;
    long long time_rcommit_target_install_calls = 0;
    long long time_rcommit_pointer_swap_ns = 0;
    long long time_rcommit_pointer_swap_calls = 0;
    long long time_rcommit_metadata_patch_apply_ns = 0;
    long long time_rcommit_metadata_patch_apply_calls = 0;
    long long time_rcommit_slot_owner_patch_ns = 0;
    long long time_rcommit_slot_owner_patch_calls = 0;
    long long time_rcommit_connector_hotpath_commit_ns = 0;
    long long time_rcommit_connector_hotpath_commit_calls = 0;
    long long time_rcommit_route_baseline_ns = 0;
    long long time_rcommit_route_delta_preserved_then_skeleton_ns = 0;
    long long time_rcommit_route_connector_skeleton_ns = 0;
    long long time_rcommit_route_general_delta_ns = 0;
    long long rcommit_calls = 0;
    long long rcommit_watch_handles_before = 0;
    long long rcommit_watch_handles_after = 0;
    long long rcommit_removed_handles = 0;
    long long rcommit_same_target_fastpath_hits = 0;
    long long rcommit_target_install_calls = 0;
    long long rcommit_pointer_swap_calls = 0;
    long long rcommit_metadata_patch_apply_calls = 0;
    long long rcommit_slot_owner_patch_calls = 0;
    long long rcommit_connector_hotpath_commit_calls = 0;
    long long rcommit_route_baseline_calls = 0;
    long long rcommit_route_delta_preserved_then_skeleton_calls = 0;
    long long rcommit_route_connector_skeleton_calls = 0;
    long long rcommit_route_general_delta_calls = 0;
    long long rcommit_same_target_skip_hits = 0;
    long long rcommit_metadata_patch_skipped_calls = 0;
    long long rcommit_slot_owner_patch_skipped_calls = 0;
    long long rcommit_connector_hotpath_hits = 0;
    long long time_mpatch_same_target_patch_skip_ns = 0;
    long long time_mpatch_same_target_patch_skip_calls = 0;
    long long time_mpatch_noop_commit_check_ns = 0;
    long long time_mpatch_noop_commit_check_calls = 0;
    long long time_mpatch_metadata_field_patch_ns = 0;
    long long time_mpatch_metadata_field_patch_calls = 0;
    long long time_mpatch_metadata_field_pack_norm_ns = 0;
    long long time_mpatch_metadata_field_pack_norm_calls = 0;
    long long time_mpatch_slot_owner_lookup_ns = 0;
    long long time_mpatch_slot_owner_lookup_calls = 0;
    long long time_mpatch_slot_owner_patch_ns = 0;
    long long time_mpatch_slot_owner_patch_calls = 0;
    long long time_mpatch_owner_side_index_touch_ns = 0;
    long long time_mpatch_owner_side_index_touch_calls = 0;
    long long time_mpatch_connector_hotpath_patch_apply_ns = 0;
    long long time_mpatch_connector_hotpath_patch_apply_calls = 0;
    long long time_mpatch_route_baseline_ns = 0;
    long long time_mpatch_route_delta_preserved_then_skeleton_ns = 0;
    long long time_mpatch_route_connector_skeleton_ns = 0;
    long long time_mpatch_route_general_delta_ns = 0;
    long long mpatch_calls = 0;
    long long mpatch_watch_handles_before = 0;
    long long mpatch_watch_handles_after = 0;
    long long mpatch_removed_handles = 0;
    long long mpatch_same_target_patch_skip_hits = 0;
    long long mpatch_noop_commit_check_calls = 0;
    long long mpatch_noop_commit_hits = 0;
    long long mpatch_metadata_field_patch_calls = 0;
    long long mpatch_metadata_field_patch_count = 0;
    long long mpatch_metadata_field_pack_norm_calls = 0;
    long long mpatch_slot_owner_lookup_calls = 0;
    long long mpatch_slot_owner_lookup_hits = 0;
    long long mpatch_slot_owner_patch_calls = 0;
    long long mpatch_owner_side_index_touch_calls = 0;
    long long mpatch_connector_hotpath_patch_apply_calls = 0;
    long long mpatch_route_baseline_calls = 0;
    long long mpatch_route_delta_preserved_then_skeleton_calls = 0;
    long long mpatch_route_connector_skeleton_calls = 0;
    long long mpatch_route_general_delta_calls = 0;
    long long mpatch_same_value_skip_hits = 0;
    long long mpatch_slot_owner_patch_skipped_calls = 0;
    long long mpatch_owner_index_touch_skipped_calls = 0;
    long long mpatch_connector_hotpath_hits = 0;
    long long time_sowner_slot_owner_lookup_ns = 0;
    long long time_sowner_slot_owner_lookup_calls = 0;
    long long time_sowner_same_owner_validation_ns = 0;
    long long time_sowner_same_owner_validation_calls = 0;
    long long time_sowner_slot_owner_patch_apply_ns = 0;
    long long time_sowner_slot_owner_patch_apply_calls = 0;
    long long time_sowner_owner_index_search_ns = 0;
    long long time_sowner_owner_index_search_calls = 0;
    long long time_sowner_owner_index_touch_ns = 0;
    long long time_sowner_owner_index_touch_calls = 0;
    long long time_sowner_owner_index_writeback_ns = 0;
    long long time_sowner_owner_index_writeback_calls = 0;
    long long time_sowner_owner_index_noop_skip_ns = 0;
    long long time_sowner_owner_index_noop_skip_calls = 0;
    long long time_sowner_connector_hotpath_owner_patch_ns = 0;
    long long time_sowner_connector_hotpath_owner_patch_calls = 0;
    long long time_sowner_route_baseline_ns = 0;
    long long time_sowner_route_delta_preserved_then_skeleton_ns = 0;
    long long time_sowner_route_connector_skeleton_ns = 0;
    long long time_sowner_route_general_delta_ns = 0;
    long long sowner_calls = 0;
    long long sowner_watch_handles_before = 0;
    long long sowner_watch_handles_after = 0;
    long long sowner_removed_handles = 0;
    long long sowner_slot_owner_lookup_calls = 0;
    long long sowner_same_owner_validation_calls = 0;
    long long sowner_same_owner_validation_hits = 0;
    long long sowner_slot_owner_patch_apply_calls = 0;
    long long sowner_owner_index_search_calls = 0;
    long long sowner_owner_index_touch_calls = 0;
    long long sowner_owner_index_writeback_calls = 0;
    long long sowner_owner_index_noop_skip_hits = 0;
    long long sowner_connector_hotpath_owner_patch_calls = 0;
    long long sowner_route_baseline_calls = 0;
    long long sowner_route_delta_preserved_then_skeleton_calls = 0;
    long long sowner_route_connector_skeleton_calls = 0;
    long long sowner_route_general_delta_calls = 0;
    long long sowner_same_target_skip_hits = 0;
    long long sowner_owner_index_search_hits = 0;
    long long sowner_owner_index_search_misses = 0;
    long long sowner_connector_hotpath_hits = 0;
    long long time_opatch_same_owner_noop_guard_ns = 0;
    long long time_opatch_same_owner_noop_guard_calls = 0;
    long long time_opatch_slot_field_patch_ns = 0;
    long long time_opatch_slot_field_patch_calls = 0;
    long long time_opatch_slot_field_pack_norm_ns = 0;
    long long time_opatch_slot_field_pack_norm_calls = 0;
    long long time_opatch_owner_index_search_ns = 0;
    long long time_opatch_owner_index_search_calls = 0;
    long long time_opatch_owner_index_writeback_ns = 0;
    long long time_opatch_owner_index_writeback_calls = 0;
    long long time_opatch_owner_commit_finalize_ns = 0;
    long long time_opatch_owner_commit_finalize_calls = 0;
    long long time_opatch_connector_hotpath_owner_patch_ns = 0;
    long long time_opatch_connector_hotpath_owner_patch_calls = 0;
    long long time_opatch_route_baseline_ns = 0;
    long long time_opatch_route_delta_preserved_then_skeleton_ns = 0;
    long long time_opatch_route_connector_skeleton_ns = 0;
    long long time_opatch_route_general_delta_ns = 0;
    long long opatch_calls = 0;
    long long opatch_watch_handles_before = 0;
    long long opatch_watch_handles_after = 0;
    long long opatch_removed_handles = 0;
    long long opatch_same_owner_noop_guard_calls = 0;
    long long opatch_same_owner_noop_guard_hits = 0;
    long long opatch_slot_field_patch_calls = 0;
    long long opatch_slot_field_patch_count = 0;
    long long opatch_slot_field_pack_norm_calls = 0;
    long long opatch_owner_index_search_calls = 0;
    long long opatch_owner_index_search_hits = 0;
    long long opatch_owner_index_search_misses = 0;
    long long opatch_owner_index_writeback_calls = 0;
    long long opatch_owner_commit_finalize_calls = 0;
    long long opatch_connector_hotpath_owner_patch_calls = 0;
    long long opatch_route_baseline_calls = 0;
    long long opatch_route_delta_preserved_then_skeleton_calls = 0;
    long long opatch_route_connector_skeleton_calls = 0;
    long long opatch_route_general_delta_calls = 0;
    long long opatch_same_target_skip_hits = 0;
    long long opatch_owner_writeback_skipped_calls = 0;
    long long opatch_commit_finalize_skipped_calls = 0;
    long long opatch_connector_hotpath_hits = 0;
    long long time_fpatch_same_value_field_skip_ns = 0;
    long long time_fpatch_same_value_field_skip_calls = 0;
    long long time_fpatch_field_select_ns = 0;
    long long time_fpatch_field_select_calls = 0;
    long long time_fpatch_field_write_core_ns = 0;
    long long time_fpatch_field_write_core_calls = 0;
    long long time_fpatch_compact_pack_norm_ns = 0;
    long long time_fpatch_compact_pack_norm_calls = 0;
    long long time_fpatch_same_owner_noop_guard_ns = 0;
    long long time_fpatch_same_owner_noop_guard_calls = 0;
    long long time_fpatch_connector_hotpath_field_patch_ns = 0;
    long long time_fpatch_connector_hotpath_field_patch_calls = 0;
    long long time_fpatch_route_baseline_ns = 0;
    long long time_fpatch_route_delta_preserved_then_skeleton_ns = 0;
    long long time_fpatch_route_connector_skeleton_ns = 0;
    long long time_fpatch_route_general_delta_ns = 0;
    long long fpatch_calls = 0;
    long long fpatch_watch_handles_before = 0;
    long long fpatch_watch_handles_after = 0;
    long long fpatch_removed_handles = 0;
    long long fpatch_same_value_field_skip_hits = 0;
    long long fpatch_field_select_calls = 0;
    long long fpatch_field_write_core_calls = 0;
    long long fpatch_field_write_count = 0;
    long long fpatch_compact_pack_norm_calls = 0;
    long long fpatch_same_owner_noop_guard_calls = 0;
    long long fpatch_same_owner_noop_guard_hits = 0;
    long long fpatch_connector_hotpath_field_patch_calls = 0;
    long long fpatch_route_baseline_calls = 0;
    long long fpatch_route_delta_preserved_then_skeleton_calls = 0;
    long long fpatch_route_connector_skeleton_calls = 0;
    long long fpatch_route_general_delta_calls = 0;
    long long fpatch_same_target_skip_hits = 0;
    long long fpatch_pack_norm_skipped_calls = 0;
    long long fpatch_field_write_elided_calls = 0;
    long long fpatch_connector_hotpath_hits = 0;
    long long time_cpack_same_layout_check_ns = 0;
    long long time_cpack_same_layout_check_calls = 0;
    long long time_cpack_same_layout_skip_apply_ns = 0;
    long long time_cpack_same_layout_skip_apply_calls = 0;
    long long time_cpack_field_gather_ns = 0;
    long long time_cpack_field_gather_calls = 0;
    long long time_cpack_staging_buffer_prepare_ns = 0;
    long long time_cpack_staging_buffer_prepare_calls = 0;
    long long time_cpack_pack_encode_ns = 0;
    long long time_cpack_pack_encode_calls = 0;
    long long time_cpack_pack_normalize_ns = 0;
    long long time_cpack_pack_normalize_calls = 0;
    long long time_cpack_connector_hotpath_pack_ns = 0;
    long long time_cpack_connector_hotpath_pack_calls = 0;
    long long time_cpack_route_baseline_ns = 0;
    long long time_cpack_route_delta_preserved_then_skeleton_ns = 0;
    long long time_cpack_route_connector_skeleton_ns = 0;
    long long time_cpack_route_general_delta_ns = 0;
    long long cpack_calls = 0;
    long long cpack_watch_handles_before = 0;
    long long cpack_watch_handles_after = 0;
    long long cpack_removed_handles = 0;
    long long cpack_same_layout_checks = 0;
    long long cpack_same_layout_skip_hits = 0;
    long long cpack_field_gather_calls = 0;
    long long cpack_field_gather_fields = 0;
    long long cpack_staging_buffer_prepare_calls = 0;
    long long cpack_staging_buffer_reuse_hits = 0;
    long long cpack_pack_encode_calls = 0;
    long long cpack_pack_normalize_calls = 0;
    long long cpack_packed_field_count = 0;
    long long cpack_normalized_field_count = 0;
    long long cpack_connector_hotpath_pack_calls = 0;
    long long cpack_route_baseline_calls = 0;
    long long cpack_route_delta_preserved_then_skeleton_calls = 0;
    long long cpack_route_connector_skeleton_calls = 0;
    long long cpack_route_general_delta_calls = 0;
    long long cpack_same_value_skip_hits = 0;
    long long cpack_pack_norm_skipped_calls = 0;
    long long cpack_field_gather_reuse_hits = 0;
    long long cpack_connector_hotpath_hits = 0;
    long long time_pnorm_same_layout_skip_ns = 0;
    long long time_pnorm_same_layout_skip_calls = 0;
    long long time_pnorm_bundle_gather_ns = 0;
    long long time_pnorm_bundle_gather_calls = 0;
    long long time_pnorm_staging_prepare_ns = 0;
    long long time_pnorm_staging_prepare_calls = 0;
    long long time_pnorm_encode_core_ns = 0;
    long long time_pnorm_encode_core_calls = 0;
    long long time_pnorm_normalize_core_ns = 0;
    long long time_pnorm_normalize_core_calls = 0;
    long long time_pnorm_connector_hotpath_apply_ns = 0;
    long long time_pnorm_connector_hotpath_apply_calls = 0;
    long long time_pnorm_route_baseline_ns = 0;
    long long time_pnorm_route_delta_preserved_then_skeleton_ns = 0;
    long long time_pnorm_route_connector_skeleton_ns = 0;
    long long time_pnorm_route_general_delta_ns = 0;
    long long pnorm_calls = 0;
    long long pnorm_same_layout_skip_hits = 0;
    long long pnorm_bundle_gather_calls = 0;
    long long pnorm_bundle_gather_fields = 0;
    long long pnorm_staging_prepare_calls = 0;
    long long pnorm_staging_reuse_hits = 0;
    long long pnorm_encode_core_calls = 0;
    long long pnorm_normalize_core_calls = 0;
    long long pnorm_packed_field_count = 0;
    long long pnorm_normalized_field_count = 0;
    long long pnorm_connector_hotpath_apply_calls = 0;
    long long pnorm_route_baseline_calls = 0;
    long long pnorm_route_delta_preserved_then_skeleton_calls = 0;
    long long pnorm_route_connector_skeleton_calls = 0;
    long long pnorm_route_general_delta_calls = 0;
    long long time_pencore_descriptor_encode_ns = 0;
    long long time_pencore_descriptor_encode_calls = 0;
    long long time_pencore_payload_pack_write_ns = 0;
    long long time_pencore_payload_pack_write_calls = 0;
    long long time_pencore_normalize_rule_apply_ns = 0;
    long long time_pencore_normalize_rule_apply_calls = 0;
    long long time_pencore_postnormalize_compare_ns = 0;
    long long time_pencore_postnormalize_compare_calls = 0;
    long long time_pencore_same_layout_skip_ns = 0;
    long long time_pencore_same_layout_skip_calls = 0;
    long long time_pencore_connector_hotpath_apply_ns = 0;
    long long time_pencore_connector_hotpath_apply_calls = 0;
    long long time_pencore_route_baseline_ns = 0;
    long long time_pencore_route_delta_preserved_then_skeleton_ns = 0;
    long long time_pencore_route_connector_skeleton_ns = 0;
    long long time_pencore_route_general_delta_ns = 0;
    long long pencore_calls = 0;
    long long pencore_descriptor_encode_calls = 0;
    long long pencore_payload_pack_write_calls = 0;
    long long pencore_packed_field_count = 0;
    long long pencore_normalize_rule_apply_calls = 0;
    long long pencore_normalized_field_count = 0;
    long long pencore_postnormalize_compare_calls = 0;
    long long pencore_same_layout_skip_hits = 0;
    long long pencore_connector_hotpath_apply_calls = 0;
    long long pencore_route_baseline_calls = 0;
    long long pencore_route_delta_preserved_then_skeleton_calls = 0;
    long long pencore_route_connector_skeleton_calls = 0;
    long long pencore_route_general_delta_calls = 0;
    long long time_cnorm_same_layout_reuse_check_ns = 0;
    long long time_cnorm_same_layout_reuse_check_calls = 0;
    long long time_cnorm_zero_span_elision_ns = 0;
    long long time_cnorm_zero_span_elision_calls = 0;
    long long time_cnorm_rule_dispatch_ns = 0;
    long long time_cnorm_rule_dispatch_calls = 0;
    long long time_cnorm_field_normalize_apply_ns = 0;
    long long time_cnorm_field_normalize_apply_calls = 0;
    long long time_cnorm_zero_fill_span_write_ns = 0;
    long long time_cnorm_zero_fill_span_write_calls = 0;
    long long time_cnorm_tail_zero_clear_ns = 0;
    long long time_cnorm_tail_zero_clear_calls = 0;
    long long time_cnorm_connector_hotpath_apply_ns = 0;
    long long time_cnorm_connector_hotpath_apply_calls = 0;
    long long time_cnorm_route_baseline_ns = 0;
    long long time_cnorm_route_delta_preserved_then_skeleton_ns = 0;
    long long time_cnorm_route_connector_skeleton_ns = 0;
    long long time_cnorm_route_general_delta_ns = 0;
    long long cnorm_calls = 0;
    long long cnorm_same_layout_reuse_checks = 0;
    long long cnorm_same_layout_reuse_hits = 0;
    long long cnorm_zero_span_checks = 0;
    long long cnorm_zero_span_elision_hits = 0;
    long long cnorm_rule_dispatch_calls = 0;
    long long cnorm_field_normalize_apply_calls = 0;
    long long cnorm_zero_fill_span_write_calls = 0;
    long long cnorm_zero_fill_span_fields = 0;
    long long cnorm_tail_zero_clear_calls = 0;
    long long cnorm_tail_zero_clear_fields = 0;
    long long cnorm_connector_hotpath_apply_calls = 0;
    long long cnorm_route_baseline_calls = 0;
    long long cnorm_route_delta_preserved_then_skeleton_calls = 0;
    long long cnorm_route_connector_skeleton_calls = 0;
    long long cnorm_route_general_delta_calls = 0;
    long long time_lreuse_layout_sig_load_ns = 0;
    long long time_lreuse_layout_sig_load_calls = 0;
    long long time_lreuse_layout_sig_compare_ns = 0;
    long long time_lreuse_layout_sig_compare_calls = 0;
    long long time_lreuse_zero_span_scan_ns = 0;
    long long time_lreuse_zero_span_scan_calls = 0;
    long long time_lreuse_zero_span_segment_reuse_ns = 0;
    long long time_lreuse_zero_span_segment_reuse_calls = 0;
    long long time_lreuse_skip_apply_commit_ns = 0;
    long long time_lreuse_skip_apply_commit_calls = 0;
    long long time_lreuse_noop_fastpath_commit_ns = 0;
    long long time_lreuse_noop_fastpath_commit_calls = 0;
    long long time_lreuse_connector_hotpath_reuse_ns = 0;
    long long time_lreuse_connector_hotpath_reuse_calls = 0;
    long long time_lreuse_route_baseline_ns = 0;
    long long time_lreuse_route_delta_preserved_then_skeleton_ns = 0;
    long long time_lreuse_route_connector_skeleton_ns = 0;
    long long time_lreuse_route_general_delta_ns = 0;
    long long lreuse_calls = 0;
    long long lreuse_layout_sig_load_calls = 0;
    long long lreuse_layout_sig_compare_calls = 0;
    long long lreuse_same_layout_hits = 0;
    long long lreuse_zero_span_scan_calls = 0;
    long long lreuse_zero_span_segments_detected = 0;
    long long lreuse_zero_span_segment_reuse_hits = 0;
    long long lreuse_skip_apply_commit_calls = 0;
    long long lreuse_noop_commit_hits = 0;
    long long lreuse_connector_hotpath_reuse_calls = 0;
    long long lreuse_route_baseline_calls = 0;
    long long lreuse_route_delta_preserved_then_skeleton_calls = 0;
    long long lreuse_route_connector_skeleton_calls = 0;
    long long lreuse_route_general_delta_calls = 0;
    long long time_lgate_sig_source_load_ns = 0;
    long long time_lgate_sig_source_load_calls = 0;
    long long time_lgate_sig_materialize_ns = 0;
    long long time_lgate_sig_materialize_calls = 0;
    long long time_lgate_sig_compare_core_ns = 0;
    long long time_lgate_sig_compare_core_calls = 0;
    long long time_lgate_same_layout_gate_ns = 0;
    long long time_lgate_same_layout_gate_calls = 0;
    long long time_lgate_zero_span_eligibility_gate_ns = 0;
    long long time_lgate_zero_span_eligibility_gate_calls = 0;
    long long time_lgate_fastpath_commit_core_ns = 0;
    long long time_lgate_fastpath_commit_core_calls = 0;
    long long time_lgate_connector_hotpath_reuse_ns = 0;
    long long time_lgate_connector_hotpath_reuse_calls = 0;
    long long time_lgate_route_baseline_ns = 0;
    long long time_lgate_route_delta_preserved_then_skeleton_ns = 0;
    long long time_lgate_route_connector_skeleton_ns = 0;
    long long time_lgate_route_general_delta_ns = 0;
    long long lgate_calls = 0;
    long long lgate_sig_source_load_calls = 0;
    long long lgate_sig_materialize_calls = 0;
    long long lgate_sig_compare_calls = 0;
    long long lgate_same_layout_gate_checks = 0;
    long long lgate_same_layout_gate_hits = 0;
    long long lgate_zero_span_gate_checks = 0;
    long long lgate_zero_span_gate_hits = 0;
    long long lgate_fastpath_commit_calls = 0;
    long long lgate_fastpath_commit_hits = 0;
    long long lgate_connector_hotpath_reuse_calls = 0;
    long long lgate_route_baseline_calls = 0;
    long long lgate_route_delta_preserved_then_skeleton_calls = 0;
    long long lgate_route_connector_skeleton_calls = 0;
    long long lgate_route_general_delta_calls = 0;
};
...
struct SlowDeletionProfile {
    ...
    long long chitPrevStateWritebackCalls = 0;
    long long chitPrevStateScalarStoreCalls = 0;
    long long chitCursorAdvanceCalls = 0;
    long long chitNextCarryPrepareCalls = 0;
    long long chitHitBookkeepingCommits = 0;
    long long chitSkipPackHits = 0;
    long long chitSkipReseedHits = 0;
    long long timeChitTotalNs = 0;
    long long pwriteNoopGuardHits = 0;
    long long pwriteTargetResolveCalls = 0;
    long long pwriteTargetResolveCacheHits = 0;
    long long pwriteScalarFieldStoreCalls = 0;
    long long pwriteCompactPackStoreCalls = 0;
    long long pwriteSameValueSkipHits = 0;
    long long dprebindCachedPointerValidateCalls = 0;
    long long dprebindSlotOwnerMatchChecks = 0;
    long long dprebindStalePointerDetectHits = 0;
    long long dprebindRebindTargetLookupCalls = 0;
    long long dprebindRebindCommitCalls = 0;
    long long dprebindMetadataPatchCalls = 0;
    long long dprebindSamePointerFastpathHits = 0;
    long long timeDprebindTotalNs = 0;
    long long timePwriteTotalNs = 0;
    long long mpatchSameTargetPatchSkipHits = 0;
    long long mpatchNoopCommitHits = 0;
    long long mpatchMetadataFieldPatchCalls = 0;
    long long mpatchMetadataFieldPackNormCalls = 0;
    long long mpatchSlotOwnerLookupCalls = 0;
    long long mpatchSlotOwnerPatchCalls = 0;
    long long mpatchOwnerSideIndexTouchCalls = 0;
    long long mpatchConnectorHotpathPatchApplyCalls = 0;
    long long timeMpatchTotalNs = 0;
    long long cpackSameLayoutSkipHits = 0;
    long long cpackFieldGatherCalls = 0;
    long long cpackFieldGatherFields = 0;
    long long cpackStagingBufferPrepareCalls = 0;
    long long cpackPackEncodeCalls = 0;
    long long cpackPackNormalizeCalls = 0;
    long long cpackConnectorHotpathPackCalls = 0;
    long long timeCpackTotalNs = 0;
    ...
    int tbranchRouteTag = 0;
    long long tbranchPrevStateMaterializeCalls = 0;
    long long tbranchCurrStateLoadCalls = 0;
    long long tbranchCompareOps = 0;
    long long tbranchRemovedToKeptTaken = 0;
    long long tbranchKeptToRemovedTaken = 0;
    long long tbranchNoTransitionSteps = 0;
    long long tbranchWindowClippedCalls = 0;
    int sloadRouteTag = 0;
    long long sloadPrevSeedCalls = 0;
    long long sloadPrevCarryReuseHits = 0;
    long long sloadCurrDirectLoadCalls = 0;
    long long sloadCurrSourceLookupCalls = 0;
    long long sloadCurrSourceLookupMisses = 0;
    long long sloadStatePairPackCalls = 0;
    long long sloadInvariantRefHoistHits = 0;
    long long timeSloadTotalNs = 0;
    int pcarryRouteTag = 0;
    long long pcarryPrevSeedCalls = 0;
    long long pcarryPrevSeedFromBoundaryCalls = 0;
    long long pcarryPrevSeedFromReseedCalls = 0;
    long long pcarryPrevScalarMaterializeCalls = 0;
    long long pcarryCarryReuseHits = 0;
    long long pcarryCarryReuseMisses = 0;
    long long pcarryTransitionInvalidateCalls = 0;
    long long pcarryWindowClipInvalidateCalls = 0;
    int creuseRouteTag = 0;
    long long creuseChecks = 0;
    long long creuseHits = 0;
    long long creuseMisses = 0;
    long long creuseBoundaryValidityHits = 0;
    long long creusePrevScalarEqHits = 0;
    long long creuseTransitionConsistencyHits = 0;
    long long creuseHitApplyCursorAdvanceCalls = 0;
    long long creuseReseedHandoffCalls = 0;
    long long timeCreuseTotalNs = 0;
    long long timePcarryTotalNs = 0;
    long long timeTbranchTotalNs = 0;
...
static void add_tbranch_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_tbranch_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_tbranch_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_tbranch_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_tbranch_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_tbranch_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.tbranch_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.tbranch_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.tbranch_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.tbranch_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_sload_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_sload_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_sload_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_sload_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_sload_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_sload_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.sload_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.sload_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.sload_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.sload_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_pcarry_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_pcarry_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_pcarry_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_pcarry_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_pcarry_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_pcarry_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.pcarry_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.pcarry_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.pcarry_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.pcarry_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_creuse_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_creuse_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_creuse_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_creuse_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_creuse_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_creuse_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.creuse_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.creuse_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.creuse_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.creuse_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_chit_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_chit_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_chit_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_chit_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_chit_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_chit_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.chit_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.chit_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.chit_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.chit_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_pwrite_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_pwrite_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_pwrite_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_pwrite_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_pwrite_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_pwrite_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.pwrite_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.pwrite_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.pwrite_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.pwrite_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_dprebind_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_dprebind_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_dprebind_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_dprebind_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_dprebind_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_dprebind_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.dprebind_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.dprebind_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.dprebind_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.dprebind_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_rcommit_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_rcommit_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_rcommit_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_rcommit_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_rcommit_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_rcommit_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.rcommit_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.rcommit_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.rcommit_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.rcommit_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_mpatch_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_mpatch_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_mpatch_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_mpatch_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_mpatch_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_mpatch_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.mpatch_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.mpatch_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.mpatch_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.mpatch_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_sowner_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_sowner_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_sowner_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_sowner_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_sowner_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_sowner_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.sowner_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.sowner_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.sowner_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.sowner_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_opatch_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_opatch_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_opatch_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_opatch_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_opatch_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_opatch_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.opatch_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.opatch_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.opatch_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.opatch_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_fpatch_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_fpatch_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_fpatch_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_fpatch_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_fpatch_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_fpatch_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.fpatch_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.fpatch_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.fpatch_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.fpatch_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_cpack_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_cpack_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_cpack_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_cpack_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_cpack_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_cpack_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.cpack_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.cpack_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.cpack_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.cpack_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_pnorm_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_pnorm_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_pnorm_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_pnorm_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_pnorm_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_pnorm_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.pnorm_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.pnorm_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.pnorm_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.pnorm_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_pencore_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_pencore_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_pencore_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_pencore_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_pencore_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_pencore_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.pencore_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.pencore_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.pencore_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.pencore_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_cnorm_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_cnorm_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_cnorm_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_cnorm_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_cnorm_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_cnorm_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.cnorm_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.cnorm_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.cnorm_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.cnorm_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_lreuse_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_lreuse_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_lreuse_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_lreuse_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_lreuse_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_lreuse_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.lreuse_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.lreuse_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.lreuse_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.lreuse_route_general_delta_calls++; break;
        default: break;
    }
}
static void add_lgate_route_time(long long dt) {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.time_lgate_route_baseline_ns += dt; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.time_lgate_route_delta_preserved_then_skeleton_ns += dt; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.time_lgate_route_connector_skeleton_ns += dt; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.time_lgate_route_general_delta_ns += dt; break;
        default: break;
    }
}
static void note_lgate_route_call() {
    switch (g_wscan_active_route_tag) {
        case REUSE_ROUTE_BASELINE: g_batch_dbg.lgate_route_baseline_calls++; break;
        case REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON: g_batch_dbg.lgate_route_delta_preserved_then_skeleton_calls++; break;
        case REUSE_ROUTE_CONNECTOR_SKELETON: g_batch_dbg.lgate_route_connector_skeleton_calls++; break;
        case REUSE_ROUTE_GENERAL_DELTA: g_batch_dbg.lgate_route_general_delta_calls++; break;
        default: break;
    }
}
...
class LiteraturePotentialOracle final : public NBOracle {
    ...
    void syncAllComponents() {
        compId_.assign(n_ + 1, -1);
        for (int v = 1; v <= n_; ++v) compId_[v] = topo_.componentOf(v);
    }

    void syncComponentsFromDeleteArtifact(int removedX, const OwnerSplitArtifact& artifact) {
        if (!(1 <= removedX && removedX <= n_)) return;
        compId_[removedX] = -1;
        if (artifact.visitedVerts.size() != artifact.compVals.size()) {
            syncAllComponents();
            return;
        }
        for (size_t i = 0; i < artifact.visitedVerts.size(); ++i) {
            int v = artifact.visitedVerts[i];
            if (1 <= v && v <= n_) compId_[v] = artifact.compVals[i];
        }
    }
    ...
    void annotateConnectorHandles(ClassState& st, const vector<int>& handleIdxs, int connectorTreeId) {
        ...
        if (!tree || connectorTreeId <= 0) return;
        int stamp = 0;
        buildTreeVertexPosMap(*tree, stamp);
        for (int hi : handleIdxs) {
            if (!(0 <= hi && hi < (int)st.watchHandles.size())) continue;
            auto& h = st.watchHandles[hi];
            int pos = -1;
            if (1 <= h.vertex && h.vertex <= n_ && supportScratch_.supportPosStamp[h.vertex] == stamp) {
                pos = supportScratch_.supportPosVal[h.vertex];
            }
            annotateHandleMetadata(h, SupportOriginKind::ConnectorTree, connectorTreeId, -1, pos);
        }
    }
    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask, const vector<int>* sparseRemoveIdxs = nullptr) {
        const bool __release_phase_diag = release_phase_diag_enabled();
        ScopedReleasePhaseTimer __retain_phase_timer(__release_phase_diag ? &g_release_phase_diag.retain_ns : nullptr);
        struct ReleaseRetainScope {
            bool enabled;
            ClassState& state;
            explicit ReleaseRetainScope(bool enabled, ClassState& state) : enabled(enabled), state(state) {
                if (!enabled) return;
                g_release_phase_diag.retain_calls++;
                g_release_phase_diag.retain_handles_before += (long long)state.watchHandles.size();
            }
            ~ReleaseRetainScope() {
                if (!enabled) return;
                g_release_phase_diag.retain_handles_after += (long long)state.watchHandles.size();
            }
        } __release_retain_scope(__release_phase_diag, st);
#ifdef LOCAL
        ...
        long long __dt_tbranch_prev_state_materialize = 0;
        long long __dt_tbranch_curr_state_load = 0;
        long long __dt_tbranch_compare_classify = 0;
        long long __dt_tbranch_removed_to_kept_dispatch = 0;
        long long __dt_tbranch_kept_to_removed_dispatch = 0;
        long long __dt_tbranch_no_transition_advance = 0;
        long long __dt_tbranch_loop_state_update = 0;
        long long __dt_sload_prev_seed_materialize = 0;
        long long __dt_sload_prev_carry_reuse_check = 0;
        long long __dt_sload_prev_carry_reuse_apply = 0;
        long long __dt_sload_curr_direct_load = 0;
        long long __dt_sload_curr_source_lookup = 0;
        long long __dt_sload_state_pair_pack = 0;
        long long __dt_sload_invariant_ref_hoist_miss = 0;
        long long __dt_pcarry_boundary_seed_bootstrap = 0;
        long long __dt_pcarry_prev_scalar_materialize = 0;
        long long __dt_pcarry_prev_scalar_pack = 0;
        long long __dt_pcarry_carry_reuse_check = 0;
        long long __dt_pcarry_carry_reuse_hit_apply = 0;
        long long __dt_pcarry_carry_invalidate_transition = 0;
        long long __dt_pcarry_carry_invalidate_window_clip = 0;
        long long __dt_pcarry_reseed_after_invalidate = 0;
        long long __dt_creuse_eligibility_gate = 0;
        long long __dt_creuse_boundary_validity_check = 0;
        long long __dt_creuse_prev_scalar_eq_check = 0;
        long long __dt_creuse_transition_consistency_check = 0;
        long long __dt_creuse_hit_apply_prev_state = 0;
        long long __dt_creuse_hit_apply_cursor_advance = 0;
        long long __dt_creuse_miss_classify = 0;
        long long __dt_creuse_reseed_handoff = 0;
        long long __dt_chit_prev_state_writeback = 0;
        long long __dt_chit_prev_state_scalar_store = 0;
        long long __dt_chit_cursor_advance = 0;
        long long __dt_chit_next_carry_prepare = 0;
        long long __dt_chit_hit_bookkeeping_commit = 0;
        long long __dt_chit_skip_pack_and_reseed_suppression = 0;
        long long __dt_pwrite_noop_guard = 0;
        long long __dt_pwrite_target_resolve = 0;
        long long __dt_pwrite_scalar_field_store = 0;
        long long __dt_pwrite_compact_pack_store = 0;
        long long __dt_pwrite_same_value_skip = 0;
        long long __dt_pwrite_route_pointer_hoist_miss = 0;
        long long __dt_dprebind_cached_pointer_validate = 0;
        long long __dt_dprebind_slot_owner_match_check = 0;
        long long __dt_dprebind_same_pointer_fastpath = 0;
        long long __dt_dprebind_stale_pointer_detect = 0;
        long long __dt_dprebind_rebind_target_lookup = 0;
        long long __dt_dprebind_rebind_commit = 0;
        long long __dt_dprebind_metadata_patch = 0;
        long long __dt_dprebind_connector_hotpath_pin_apply = 0;
        long long __dt_rcommit_same_target_fastpath = 0;
        long long __dt_rcommit_target_install = 0;
        long long __dt_rcommit_pointer_swap = 0;
        long long __dt_rcommit_metadata_patch_apply = 0;
        long long __dt_rcommit_slot_owner_patch = 0;
        long long __dt_rcommit_connector_hotpath_commit = 0;
        long long __dt_mpatch_same_target_patch_skip = 0;
        long long __dt_mpatch_noop_commit_check = 0;
        long long __dt_mpatch_metadata_field_patch = 0;
        long long __dt_mpatch_metadata_field_pack_norm = 0;
        long long __dt_mpatch_slot_owner_lookup = 0;
        long long __dt_mpatch_slot_owner_patch = 0;
        long long __dt_mpatch_owner_side_index_touch = 0;
        long long __dt_mpatch_connector_hotpath_patch_apply = 0;
        long long __dt_sowner_slot_owner_lookup = 0;
        long long __dt_sowner_same_owner_validation = 0;
        long long __dt_sowner_slot_owner_patch_apply = 0;
        long long __dt_sowner_owner_index_search = 0;
        long long __dt_sowner_owner_index_touch = 0;
        long long __dt_sowner_owner_index_writeback = 0;
        long long __dt_sowner_owner_index_noop_skip = 0;
        long long __dt_sowner_connector_hotpath_owner_patch = 0;
        long long __dt_opatch_same_owner_noop_guard = 0;
        long long __dt_opatch_slot_field_patch = 0;
        long long __dt_opatch_slot_field_pack_norm = 0;
        long long __dt_opatch_owner_index_search = 0;
        long long __dt_opatch_owner_index_writeback = 0;
        long long __dt_opatch_owner_commit_finalize = 0;
        long long __dt_opatch_connector_hotpath_owner_patch = 0;
        long long __dt_fpatch_same_value_field_skip = 0;
        long long __dt_fpatch_field_select = 0;
        long long __dt_fpatch_field_write_core = 0;
        long long __dt_fpatch_compact_pack_norm = 0;
        long long __dt_fpatch_same_owner_noop_guard = 0;
        long long __dt_fpatch_connector_hotpath_field_patch = 0;
        long long __dt_cpack_same_layout_check = 0;
        long long __dt_cpack_same_layout_skip_apply = 0;
        long long __dt_cpack_field_gather = 0;
        long long __dt_cpack_staging_buffer_prepare = 0;
        long long __dt_cpack_pack_encode = 0;
        long long __dt_cpack_pack_normalize = 0;
        long long __dt_cpack_connector_hotpath_pack = 0;
        long long __dt_pnorm_same_layout_skip = 0;
        long long __dt_pnorm_bundle_gather = 0;
        long long __dt_pnorm_staging_prepare = 0;
        long long __dt_pnorm_encode_core = 0;
        long long __dt_pnorm_normalize_core = 0;
        long long __dt_pnorm_connector_hotpath_apply = 0;
        long long __dt_pencore_descriptor_encode = 0;
        long long __dt_pencore_payload_pack_write = 0;
        long long __dt_pencore_normalize_rule_apply = 0;
        long long __dt_pencore_postnormalize_compare = 0;
        long long __dt_pencore_same_layout_skip = 0;
        long long __dt_pencore_connector_hotpath_apply = 0;
        ...
        long long __tbranch_prev_state_materialize_calls = 0;
        long long __tbranch_curr_state_load_calls = 0;
        long long __tbranch_compare_ops = 0;
        long long __tbranch_removed_to_kept_taken = 0;
        long long __tbranch_kept_to_removed_taken = 0;
        long long __tbranch_no_transition_steps = 0;
        long long __tbranch_loop_state_update_calls = 0;
        long long __tbranch_window_clipped_calls = 0;
        long long __sload_prev_seed_calls = 0;
        long long __sload_prev_carry_reuse_hits = 0;
        long long __sload_curr_direct_load_calls = 0;
        long long __sload_curr_source_lookup_calls = 0;
        long long __sload_curr_source_lookup_misses = 0;
        long long __sload_state_pair_pack_calls = 0;
        long long __sload_invariant_ref_hoist_hits = 0;
        long long __pcarry_prev_seed_calls = 0;
        long long __pcarry_prev_seed_from_boundary_calls = 0;
        long long __pcarry_prev_seed_from_reseed_calls = 0;
        long long __pcarry_prev_scalar_materialize_calls = 0;
        long long __pcarry_carry_reuse_hits = 0;
        long long __pcarry_carry_reuse_misses = 0;
        long long __pcarry_transition_invalidate_calls = 0;
        long long __pcarry_window_clip_invalidate_calls = 0;
        long long __creuse_checks = 0;
        long long __creuse_hits = 0;
        long long __creuse_misses = 0;
        long long __creuse_boundary_validity_hits = 0;
        long long __creuse_prev_scalar_eq_hits = 0;
        long long __creuse_transition_consistency_hits = 0;
        long long __creuse_hit_apply_cursor_advance_calls = 0;
        long long __creuse_reseed_handoff_calls = 0;
        long long __chit_prev_state_writeback_calls = 0;
        long long __chit_prev_state_scalar_store_calls = 0;
        long long __chit_cursor_advance_calls = 0;
        long long __chit_next_carry_prepare_calls = 0;
        long long __chit_hit_bookkeeping_commits = 0;
        long long __chit_skip_pack_hits = 0;
        long long __chit_skip_reseed_hits = 0;
        long long __pwrite_noop_guard_hits = 0;
        long long __pwrite_target_resolve_calls = 0;
        long long __pwrite_target_resolve_cache_hits = 0;
        long long __pwrite_scalar_field_store_calls = 0;
        long long __pwrite_compact_pack_store_calls = 0;
        long long __pwrite_same_value_skip_hits = 0;
        long long __dprebind_cached_pointer_validate_calls = 0;
        long long __dprebind_slot_owner_match_checks = 0;
        long long __dprebind_stale_pointer_detect_hits = 0;
        long long __dprebind_rebind_target_lookup_calls = 0;
        long long __dprebind_rebind_commit_calls = 0;
        long long __dprebind_metadata_patch_calls = 0;
        long long __dprebind_same_pointer_fastpath_hits = 0;
        long long __rcommit_same_target_fastpath_hits = 0;
        long long __rcommit_target_install_calls = 0;
        long long __rcommit_pointer_swap_calls = 0;
        long long __rcommit_metadata_patch_apply_calls = 0;
        long long __rcommit_slot_owner_patch_calls = 0;
        long long __rcommit_connector_hotpath_commit_calls = 0;
        long long __mpatch_same_target_patch_skip_hits = 0;
        long long __mpatch_noop_commit_hits = 0;
        long long __mpatch_metadata_field_patch_calls = 0;
        long long __mpatch_metadata_field_pack_norm_calls = 0;
        long long __mpatch_slot_owner_lookup_calls = 0;
        long long __mpatch_slot_owner_patch_calls = 0;
        long long __mpatch_owner_side_index_touch_calls = 0;
        long long __mpatch_connector_hotpath_patch_apply_calls = 0;
        long long __sowner_slot_owner_lookup_calls = 0;
        long long __sowner_same_owner_validation_hits = 0;
        long long __sowner_slot_owner_patch_apply_calls = 0;
        long long __sowner_owner_index_search_calls = 0;
        long long __sowner_owner_index_touch_calls = 0;
        long long __sowner_owner_index_writeback_calls = 0;
        long long __sowner_connector_hotpath_owner_patch_calls = 0;
        long long __opatch_same_owner_noop_guard_hits = 0;
        long long __opatch_slot_field_patch_calls = 0;
        long long __opatch_slot_field_pack_norm_calls = 0;
        long long __opatch_owner_index_search_calls = 0;
        long long __opatch_owner_index_writeback_calls = 0;
        long long __opatch_owner_commit_finalize_calls = 0;
        long long __opatch_connector_hotpath_owner_patch_calls = 0;
        long long __fpatch_same_value_field_skip_hits = 0;
        long long __fpatch_field_select_calls = 0;
        long long __fpatch_field_write_core_calls = 0;
        long long __fpatch_compact_pack_norm_calls = 0;
        long long __fpatch_same_owner_noop_guard_hits = 0;
        long long __fpatch_connector_hotpath_field_patch_calls = 0;
        long long __cpack_same_layout_skip_hits = 0;
        long long __cpack_field_gather_calls = 0;
        long long __cpack_field_gather_fields = 0;
        long long __cpack_staging_buffer_prepare_calls = 0;
        long long __cpack_pack_encode_calls = 0;
        long long __cpack_pack_normalize_calls = 0;
        long long __cpack_connector_hotpath_pack_calls = 0;
        long long __pnorm_same_layout_skip_hits = 0;
        long long __pnorm_bundle_gather_calls = 0;
        long long __pnorm_bundle_gather_fields = 0;
        long long __pnorm_staging_prepare_calls = 0;
        long long __pnorm_encode_core_calls = 0;
        long long __pnorm_normalize_core_calls = 0;
        long long __pnorm_connector_hotpath_apply_calls = 0;
        long long __pencore_descriptor_encode_calls = 0;
        long long __pencore_payload_pack_write_calls = 0;
        long long __pencore_normalize_rule_apply_calls = 0;
        long long __pencore_postnormalize_compare_calls = 0;
        long long __pencore_same_layout_skip_hits = 0;
        long long __pencore_connector_hotpath_apply_calls = 0;
        ...
        auto __acc_tbranch = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_tbranch_route_time(dt);
        };
        auto __acc_sload = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_sload_route_time(dt);
        };
        auto __acc_pcarry = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_pcarry_route_time(dt);
        };
        auto __acc_creuse = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_creuse_route_time(dt);
        };
        auto __acc_chit = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_chit_route_time(dt);
        };
        auto __acc_pwrite = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !prev_state_writeback_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_pwrite_route_time(dt);
        };
        auto __acc_dprebind = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !direct_prebind_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_dprebind_route_time(dt);
        };
        auto __acc_rcommit = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !rebind_commit_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_rcommit_route_time(dt);
        };
        auto __acc_mpatch = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !metadata_patch_slot_owner_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_mpatch_route_time(dt);
        };
        auto __acc_sowner = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !slot_owner_update_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_sowner_route_time(dt);
        };
        auto __acc_fpatch = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !slot_owner_field_patch_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_fpatch_route_time(dt);
        };
        auto __acc_cpack = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !compact_field_pack_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_cpack_route_time(dt);
        };
        auto __acc_pnorm = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_pnorm_route_time(dt);
        };
        auto __acc_pencore = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_pencore_route_time(dt);
        };
        auto __acc_cnorm = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_cnorm_route_time(dt);
        };
        auto __acc_lreuse = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !layout_reuse_zero_elision_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_lreuse_route_time(dt);
        };
        auto __acc_lgate = [&](long long dt, long long* ns, long long* calls) {
            if (!__wscan_detail || dt <= 0 || !layout_signature_gate_opt_enabled()) return;
            if (ns) *ns += dt;
            if (calls) (*calls)++;
            add_lgate_route_time(dt);
        };
#endif
        ...
        const bool __tbranch_opt = tscan_branch_state_opt_enabled() && __tscan_opt && __fclass_opt && __plan_opt && __rdisc_opt;
        const bool __sload_metric = __tbranch_opt;
        const bool __sload_opt = state_load_materialization_opt_enabled() && __sload_metric;
        const bool __pcarry_metric = __sload_metric;
        const bool __pcarry_opt = prev_state_carry_reuse_opt_enabled() && __pcarry_metric;
        const bool __creuse_metric = __pcarry_metric;
        const bool __creuse_opt = carry_reuse_fastpath_opt_enabled() && __pcarry_opt;
        const bool __chit_metric = __creuse_metric;
        const bool __chit_opt = carry_hit_apply_opt_enabled() && __creuse_opt;
        const bool __dprebind_metric = __chit_metric && pointer_rebind_opt_enabled();
        const bool __dprebind_opt = direct_prebind_opt_enabled() && __dprebind_metric;
        const bool __rcommit_metric = __dprebind_metric;
        const bool __rcommit_opt = rebind_commit_opt_enabled() && __rcommit_metric;
        const bool __mpatch_metric = __rcommit_metric;
        const bool __mpatch_opt = metadata_patch_slot_owner_opt_enabled() && __mpatch_metric;
        const bool __sowner_metric = __mpatch_metric;
        const bool __sowner_opt = slot_owner_update_opt_enabled() && __sowner_metric;
        const bool __opatch_metric = __sowner_metric;
        const bool __opatch_opt = slot_owner_patch_commit_opt_enabled() && __opatch_metric;
        const bool __fpatch_metric = __opatch_metric;
        const bool __fpatch_opt = slot_owner_field_patch_opt_enabled() && __fpatch_metric;
        const bool __cpack_metric = __fpatch_metric;
        const bool __cpack_opt = compact_field_pack_opt_enabled() && __cpack_metric;
        const bool __pnorm_metric = __cpack_metric;
        const bool __pnorm_opt = __pnorm_metric;
        const bool __pnorm_apply_opt = pack_encode_normalize_opt_enabled() && __pnorm_metric;
        const bool __pencore_metric = __pnorm_metric;
        const bool __pencore_apply_opt = pack_encode_normalize_core_opt_enabled() && __pencore_metric;
        const bool __cnorm_metric = __pencore_metric;
        const bool __cnorm_opt = canonical_normalize_opt_enabled() && __cnorm_metric;
        const bool __lreuse_metric = __cnorm_metric;
        const bool __lreuse_opt = layout_reuse_zero_elision_opt_enabled() && __lreuse_metric;
        const bool __lgate_metric = __lreuse_metric;
        const bool __lgate_opt = layout_signature_gate_opt_enabled() && __lgate_metric;
        ...
        if (__wscan_volume) {
            ...
            if (__tbranch_opt) {
                g_batch_dbg.tbranch_calls++;
                g_batch_dbg.tbranch_watch_handles_before += (long long)m;
                note_tbranch_route_call();
            }
            if (__sload_metric) {
                g_batch_dbg.sload_calls++;
                g_batch_dbg.sload_watch_handles_before += (long long)m;
                note_sload_route_call();
            }
            if (__pcarry_metric) {
                g_batch_dbg.pcarry_calls++;
                g_batch_dbg.pcarry_watch_handles_before += (long long)m;
                note_pcarry_route_call();
            }
            if (__creuse_metric) {
                g_batch_dbg.creuse_calls++;
                g_batch_dbg.creuse_watch_handles_before += (long long)m;
                note_creuse_route_call();
            }
            if (__chit_metric) {
                g_batch_dbg.chit_calls++;
                g_batch_dbg.chit_watch_handles_before += (long long)m;
                note_chit_route_call();
            }
            if (__dprebind_metric) {
                g_batch_dbg.dprebind_calls++;
                g_batch_dbg.dprebind_watch_handles_before += (long long)m;
                note_dprebind_route_call();
            }
            if (__rcommit_metric) {
                g_batch_dbg.rcommit_calls++;
                g_batch_dbg.rcommit_watch_handles_before += (long long)m;
                note_rcommit_route_call();
            }
            if (__mpatch_metric) {
                g_batch_dbg.mpatch_calls++;
                g_batch_dbg.mpatch_watch_handles_before += (long long)m;
                note_mpatch_route_call();
            }
            if (__sowner_metric) {
                g_batch_dbg.sowner_calls++;
                g_batch_dbg.sowner_watch_handles_before += (long long)m;
                note_sowner_route_call();
            }
            if (__opatch_metric) {
                g_batch_dbg.opatch_calls++;
                g_batch_dbg.opatch_watch_handles_before += (long long)m;
                note_opatch_route_call();
            }
            if (__fpatch_metric) {
                g_batch_dbg.fpatch_calls++;
                g_batch_dbg.fpatch_watch_handles_before += (long long)m;
                note_fpatch_route_call();
            }
            if (__pnorm_metric) {
                g_batch_dbg.pnorm_calls++;
                note_pnorm_route_call();
            }
            if (__pencore_metric) {
                g_batch_dbg.pencore_calls++;
                note_pencore_route_call();
            }
            if (__cnorm_metric) {
                g_batch_dbg.cnorm_calls++;
                note_cnorm_route_call();
            }
            if (__lreuse_metric) {
                g_batch_dbg.lreuse_calls++;
                note_lreuse_route_call();
            }
            if (__lgate_metric) {
                g_batch_dbg.lgate_calls++;
                note_lgate_route_call();
            }
        }
#endif
        vector<char> remove;
        ...
        if (retain_compaction_opt_enabled() && sparseRemoveIdxs && !sparseRemoveIdxs->empty()) {
            ...
            if (__release_phase_diag && useSparseFastPath) g_release_phase_diag.retain_sparse_fastpath_hits++;
#ifdef LOCAL
        ...
        if (__release_phase_diag) g_release_phase_diag.retain_removed_handles += (long long)removedCnt;
#ifdef LOCAL
        if (__wscan_volume) {
            if (m > 0) {
        ...
        if (__wscan_volume && __kvec_opt && __scomp_opt) {
            ...
            if (__plan_opt) {
                ...
                if (__rdisc_opt) {
                    ...
                    if (__tbranch_opt) {
                        g_batch_dbg.tbranch_removed_handles += (long long)removedCnt;
                        if (m > 0) g_batch_dbg.tbranch_removed_ratio_ppm_sum += (1000000LL * removedCnt) / m;
                    }
                    if (__sload_metric) {
                        g_batch_dbg.sload_removed_handles += (long long)removedCnt;
                        if (m > 0) g_batch_dbg.sload_removed_ratio_ppm_sum += (1000000LL * removedCnt) / m;
                    }
                    if (__pcarry_metric) {
                        g_batch_dbg.pcarry_removed_handles += (long long)removedCnt;
                        if (m > 0) g_batch_dbg.pcarry_removed_ratio_ppm_sum += (1000000LL * removedCnt) / m;
                    }
                    if (__creuse_metric) {
                        g_batch_dbg.creuse_removed_handles += (long long)removedCnt;
                        if (m > 0) g_batch_dbg.creuse_removed_ratio_ppm_sum += (1000000LL * removedCnt) / m;
                    }
                    if (__chit_metric) {
                        g_batch_dbg.chit_removed_handles += (long long)removedCnt;
                        if (m > 0) g_batch_dbg.chit_removed_ratio_ppm_sum += (1000000LL * removedCnt) / m;
                    }
                    if (__fpatch_metric) {
                        g_batch_dbg.fpatch_removed_handles += (long long)removedCnt;
                    }
                    if (__cpack_metric) {
                        g_batch_dbg.cpack_removed_handles += (long long)removedCnt;
                    }
                }
            }
        }
        ...
        if (__wscan_detail) {
            ...
            if (__tscan_opt) __acc_tscan(__dt, &g_batch_dbg.time_tscan_window_seed_ns, &g_batch_dbg.time_tscan_window_seed_calls);
        }
        ...
        if (__plan_opt && __rdisc_opt) {
            ...
            if (removedCnt > 0) {
                removedRunCount = 1;
                int prevRemovedLocal = removedIdxs[0];
                const int* __removed_data = removedIdxs.empty() ? nullptr : removedIdxs.data();
                int __sload_prev_carry = prevRemovedLocal;
                const bool __sload_source_hoisted = (__sload_opt && __removed_data != nullptr);
#ifdef LOCAL
                if (__wscan_volume && __pcarry_metric) {
                    g_batch_dbg.pcarry_prev_seed_calls++;
                    g_batch_dbg.pcarry_prev_seed_from_boundary_calls++;
                    __pcarry_prev_seed_calls++;
                    __pcarry_prev_seed_from_boundary_calls++;
                }
                long long __pcarry_bootstrap_start = (__wscan_detail && __pcarry_metric) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                if (__wscan_detail && __pcarry_metric) {
                    long long __dt = dbg_now_ns() - __pcarry_bootstrap_start;
                    __dt_pcarry_boundary_seed_bootstrap += __dt;
                    __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_boundary_seed_bootstrap_ns, &g_batch_dbg.time_pcarry_boundary_seed_bootstrap_calls);
                }
#endif
                for (int i = 1; i < removedCnt; ++i) {
                    int prevLocal = prevRemovedLocal;
#ifdef LOCAL
                    long long __tb_prev_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                    if (__wscan_volume && __tbranch_opt) {
                        g_batch_dbg.tbranch_prev_state_materialize_calls++;
                        __tbranch_prev_state_materialize_calls++;
                        g_batch_dbg.tbranch_prev_state_reuse_hits++;
                    }
#endif
#ifdef LOCAL
                    if (__wscan_volume && __sload_metric) {
                        g_batch_dbg.sload_prev_seed_calls++;
                        __sload_prev_seed_calls++;
                        g_batch_dbg.sload_prev_carry_reuse_checks++;
                    }
                    if (__wscan_volume && __pcarry_metric) {
                        g_batch_dbg.pcarry_prev_seed_calls++;
                        g_batch_dbg.pcarry_prev_scalar_materialize_calls++;
                        __pcarry_prev_seed_calls++;
                        __pcarry_prev_scalar_materialize_calls++;
                    }
                    long long __sl_prev_seed_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
                    long long __pcarry_prev_seed_start = (__wscan_detail && __pcarry_metric) ? dbg_now_ns() : 0;
#endif
                    if (__sload_opt) prevLocal = __sload_prev_carry;
#ifdef LOCAL
                    if (__wscan_detail && __sload_metric) {
                        long long __dt = dbg_now_ns() - __sl_prev_seed_start;
                        __dt_sload_prev_seed_materialize += __dt;
                        __acc_sload(__dt, &g_batch_dbg.time_sload_prev_seed_materialize_ns, &g_batch_dbg.time_sload_prev_seed_materialize_calls);
                    }
                    if (__wscan_detail && __pcarry_metric) {
                        long long __dt = dbg_now_ns() - __pcarry_prev_seed_start;
                        __dt_pcarry_prev_scalar_materialize += __dt;
                        __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_prev_scalar_materialize_ns, &g_batch_dbg.time_pcarry_prev_scalar_materialize_calls);
                    }
                    long long __sl_prev_check_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
                    long long __pcarry_prev_check_start = (__wscan_detail && __pcarry_metric) ? dbg_now_ns() : 0;
                    long long __creuse_gate_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
#endif
                    bool __reuse_prev_state = __sload_opt;
                    bool __pcarry_reuse_prev_state = __pcarry_opt;
                    bool __creuse_eligible = __creuse_opt;
                    bool __creuse_boundary_valid = (__removed_data != nullptr);
                    bool __creuse_prev_eq = __reuse_prev_state ? (__sload_prev_carry == prevRemovedLocal) : true;
                    bool __creuse_hit = false;
#ifdef LOCAL
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_gate_start;
                        __dt_creuse_eligibility_gate += __dt;
                        __acc_creuse(__dt, &g_batch_dbg.time_creuse_eligibility_gate_ns, &g_batch_dbg.time_creuse_eligibility_gate_calls);
                    }
                    long long __creuse_boundary_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
                    if (__wscan_volume && __creuse_metric && __creuse_boundary_valid) {
                        g_batch_dbg.creuse_boundary_validity_hits++;
                        __creuse_boundary_validity_hits++;
                        if (firstRemoved > 0 || lastRemoved >= 0) g_batch_dbg.creuse_boundary_reuse_assisted_hits++;
                    }
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_boundary_start;
                        __dt_creuse_boundary_validity_check += __dt;
                        __acc_creuse(__dt, &g_batch_dbg.time_creuse_boundary_validity_check_ns, &g_batch_dbg.time_creuse_boundary_validity_check_calls);
                    }
                    long long __creuse_eq_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
                    if (__wscan_volume && __creuse_metric && __creuse_prev_eq) {
                        g_batch_dbg.creuse_prev_scalar_eq_hits++;
                        __creuse_prev_scalar_eq_hits++;
                    }
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_eq_start;
                        __dt_creuse_prev_scalar_eq_check += __dt;
                        __acc_creuse(__dt, &g_batch_dbg.time_creuse_prev_scalar_eq_check_ns, &g_batch_dbg.time_creuse_prev_scalar_eq_check_calls);
                    }
                    if (__wscan_volume && __sload_metric && __reuse_prev_state) {
                        g_batch_dbg.sload_prev_carry_reuse_hits++;
                        __sload_prev_carry_reuse_hits++;
                        g_batch_dbg.sload_prev_state_scalar_reuse_hits++;
                    }
                    if (__wscan_volume && __pcarry_metric) {
                        g_batch_dbg.pcarry_carry_reuse_checks++;
                    }
                    if (__wscan_detail && __sload_metric) {
                        long long __dt = dbg_now_ns() - __sl_prev_check_start;
                        __dt_sload_prev_carry_reuse_check += __dt;
                        __acc_sload(__dt, &g_batch_dbg.time_sload_prev_carry_reuse_check_ns, &g_batch_dbg.time_sload_prev_carry_reuse_check_calls);
                    }
                    if (__wscan_detail && __pcarry_metric) {
                        long long __dt = dbg_now_ns() - __pcarry_prev_check_start;
                        __dt_pcarry_carry_reuse_check += __dt;
                        __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_carry_reuse_check_ns, &g_batch_dbg.time_pcarry_carry_reuse_check_calls);
                    }
                    long long __sl_prev_apply_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
                    long long __pcarry_prev_apply_start = (__wscan_detail && __pcarry_metric) ? dbg_now_ns() : 0;
                    long long __creuse_hit_prev_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
                    long long __chit_prev_write_start = (__wscan_detail && __chit_metric) ? dbg_now_ns() : 0;
#endif
                    if (__reuse_prev_state) prevLocal = __sload_prev_carry;
#ifdef LOCAL
                    if (__wscan_volume && __pcarry_metric) {
                        g_batch_dbg.pcarry_prev_scalar_pack_calls++;
                    }
                    if (__wscan_detail && __pcarry_metric) {
                        long long __dt = dbg_now_ns() - __pcarry_prev_apply_start;
                        __dt_pcarry_carry_reuse_hit_apply += __dt;
                        __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_carry_reuse_hit_apply_ns, &g_batch_dbg.time_pcarry_carry_reuse_hit_apply_calls);
                    }
                    if (__wscan_detail && __sload_metric) {
                        long long __dt = dbg_now_ns() - __sl_prev_apply_start;
                        __dt_sload_prev_carry_reuse_apply += __dt;
                        __acc_sload(__dt, &g_batch_dbg.time_sload_prev_carry_reuse_apply_ns, &g_batch_dbg.time_sload_prev_carry_reuse_apply_calls);
                    }
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_hit_prev_start;
                        __dt_creuse_hit_apply_prev_state += __dt;
                        __acc_creuse(__dt, &g_batch_dbg.time_creuse_hit_apply_prev_state_ns, &g_batch_dbg.time_creuse_hit_apply_prev_state_calls);
                    }
                    long long __chit_prev_write_dt = 0;
                    if (__wscan_detail && __chit_metric) {
                        __chit_prev_write_dt = dbg_now_ns() - __chit_prev_write_start;
                    }
                    long long __sl_hoist_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
                    if (__wscan_volume && __sload_metric) {
                        if (__sload_source_hoisted) {
                            g_batch_dbg.sload_invariant_ref_hoist_hits++;
                            __sload_invariant_ref_hoist_hits++;
                            g_batch_dbg.sload_invariant_pointer_hoist_hits++;
                        } else {
                            g_batch_dbg.sload_invariant_ref_hoist_misses++;
                        }
                    }
                    if (__wscan_detail && __sload_metric && !__sload_source_hoisted) {
                        long long __dt = dbg_now_ns() - __sl_hoist_start;
                        __dt_sload_invariant_ref_hoist_miss += __dt;
                        __acc_sload(__dt, &g_batch_dbg.time_sload_invariant_ref_hoist_miss_ns, &g_batch_dbg.time_sload_invariant_ref_hoist_miss_calls);
                    }
#endif
#ifdef LOCAL
                    if (__wscan_detail && __tbranch_opt) {
                        long long __dt = dbg_now_ns() - __tb_prev_start;
                        __dt_tbranch_prev_state_materialize += __dt;
                        __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_prev_state_materialize_ns, &g_batch_dbg.time_tbranch_prev_state_materialize_calls);
                    }
                    long long __tb_curr_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
                    int curRemovedLocal;
#ifdef LOCAL
                    long long __sl_curr_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
#endif
                    if (__sload_source_hoisted) {
                        curRemovedLocal = __removed_data[i];
#ifdef LOCAL
                        if (__wscan_volume && __sload_metric) {
                            g_batch_dbg.sload_curr_direct_load_calls++;
                            __sload_curr_direct_load_calls++;
                            g_batch_dbg.sload_curr_state_direct_struct_hits++;
                            g_batch_dbg.sload_curr_source_lookup_hits++;
                        }
#endif
                    } else {
                        curRemovedLocal = removedIdxs[i];
#ifdef LOCAL
                        if (__wscan_volume && __sload_metric) {
                            g_batch_dbg.sload_curr_source_lookup_calls++;
                            __sload_curr_source_lookup_calls++;
                            g_batch_dbg.sload_curr_source_lookup_misses++;
                            __sload_curr_source_lookup_misses++;
                        }
#endif
                    }
#ifdef LOCAL
                    if (__wscan_detail && __sload_metric) {
                        long long __dt = dbg_now_ns() - __sl_curr_start;
                        if (__sload_source_hoisted) {
                            __dt_sload_curr_direct_load += __dt;
                            __acc_sload(__dt, &g_batch_dbg.time_sload_curr_direct_load_ns, &g_batch_dbg.time_sload_curr_direct_load_calls);
                        } else {
                            __dt_sload_curr_source_lookup += __dt;
                            __acc_sload(__dt, &g_batch_dbg.time_sload_curr_source_lookup_ns, &g_batch_dbg.time_sload_curr_source_lookup_calls);
                        }
                    }
#endif
#ifdef LOCAL
                    if (__wscan_volume && __tbranch_opt) {
                        g_batch_dbg.tbranch_curr_state_load_calls++;
                        __tbranch_curr_state_load_calls++;
                        g_batch_dbg.tbranch_curr_state_direct_load_hits++;
                    }
#endif
#ifdef LOCAL
                    if (__wscan_detail && __tbranch_opt) {
                        long long __dt = dbg_now_ns() - __tb_curr_start;
                        __dt_tbranch_curr_state_load += __dt;
                        __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_curr_state_load_ns, &g_batch_dbg.time_tbranch_curr_state_load_calls);
                    }
                    long long __sl_pack_start = (__wscan_detail && __sload_metric) ? dbg_now_ns() : 0;
#endif
                    unsigned long long __packed_state_pair = 0ULL;
                    if (!__sload_opt) {
                        __packed_state_pair = (static_cast<unsigned long long>(static_cast<unsigned int>(prevLocal)) << 32) |
                                              static_cast<unsigned int>(curRemovedLocal);
                        (void)__packed_state_pair;
#ifdef LOCAL
                        if (__wscan_volume && __sload_metric) {
                            g_batch_dbg.sload_state_pair_pack_calls++;
                            __sload_state_pair_pack_calls++;
                        }
#endif
                    } else {
#ifdef LOCAL
                        if (__wscan_volume && __sload_metric) {
                            g_batch_dbg.sload_tuple_pack_skipped_calls++;
                        }
#endif
                    }
#ifdef LOCAL
                    if (__wscan_detail && __sload_metric) {
                        long long __dt = dbg_now_ns() - __sl_pack_start;
                        __dt_sload_state_pair_pack += __dt;
                        __acc_sload(__dt, &g_batch_dbg.time_sload_state_pair_pack_ns, &g_batch_dbg.time_sload_state_pair_pack_calls);
                    }
                    long long __tb_cmp_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
                    int gap = curRemovedLocal - prevLocal;
                    bool transition = (gap != 1);
                    __creuse_hit = (__creuse_eligible && __creuse_boundary_valid && __creuse_prev_eq && !transition);
#ifdef LOCAL
                    if (__wscan_volume && __creuse_metric) {
                        g_batch_dbg.creuse_checks++;
                        __creuse_checks++;
                        if (__creuse_hit) {
                            g_batch_dbg.creuse_hits++;
                            __creuse_hits++;
                            g_batch_dbg.creuse_transition_consistency_hits++;
                            __creuse_transition_consistency_hits++;
                            g_batch_dbg.creuse_hit_skip_scalar_pack_calls++;
                            g_batch_dbg.creuse_hit_skip_reseed_calls++;
                            g_batch_dbg.pcarry_carry_reuse_hits++;
                            __pcarry_carry_reuse_hits++;
                            g_batch_dbg.pcarry_prev_seed_reuse_without_materialize_hits++;
                        } else {
                            g_batch_dbg.creuse_misses++;
                            __creuse_misses++;
                            g_batch_dbg.pcarry_carry_reuse_misses++;
                            __pcarry_carry_reuse_misses++;
                        }
                    }
#ifdef LOCAL
                    if (__wscan_volume && __chit_metric && __creuse_hit) {
                        g_batch_dbg.chit_skip_pack_hits++;
                        g_batch_dbg.chit_skip_reseed_hits++;
                        __chit_skip_pack_hits++;
                        __chit_skip_reseed_hits++;
                    }
                    long long __chit_bookkeeping_start = (__wscan_detail && __chit_metric && __creuse_hit) ? dbg_now_ns() : 0;
                    if (__wscan_detail && __chit_metric && __creuse_hit) {
                        long long __dt = dbg_now_ns() - __chit_bookkeeping_start;
                        __dt_chit_hit_bookkeeping_commit += __dt;
                        __acc_chit(__dt, &g_batch_dbg.time_chit_hit_bookkeeping_commit_ns, &g_batch_dbg.time_chit_hit_bookkeeping_commit_calls);
                        g_batch_dbg.chit_hit_bookkeeping_commits++;
                        __chit_hit_bookkeeping_commits++;
                    }
                    long long __chit_suppress_start = (__wscan_detail && __chit_metric && __creuse_hit) ? dbg_now_ns() : 0;
                    if (__wscan_detail && __chit_metric && __creuse_hit) {
                        long long __dt = dbg_now_ns() - __chit_suppress_start;
                        __dt_chit_skip_pack_and_reseed_suppression += __dt;
                        __acc_chit(__dt, &g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_ns, &g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_calls);
                    }
#endif
                    if (__wscan_volume && __tbranch_opt) {
                        g_batch_dbg.tbranch_compare_ops++;
                        __tbranch_compare_ops++;
                    }
                    long long __creuse_consistency_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_consistency_start;
                        __dt_creuse_transition_consistency_check += __dt;
                        __acc_creuse(__dt, &g_batch_dbg.time_creuse_transition_consistency_check_ns, &g_batch_dbg.time_creuse_transition_consistency_check_calls);
                    }
                    if (__wscan_detail && __tbranch_opt) {
                        long long __dt = dbg_now_ns() - __tb_cmp_start;
                        __dt_tbranch_compare_classify += __dt;
                        __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_compare_classify_ns, &g_batch_dbg.time_tbranch_compare_classify_calls);
                    }
#endif
                    if (transition) {
#ifdef LOCAL
                        long long __tb_r2k_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
                        ++removedRunCount;
                        ++__r2k;
                        ++__k2r;
#ifdef LOCAL
                        if (__wscan_volume && __tbranch_opt) {
                            g_batch_dbg.tbranch_removed_to_kept_checks++;
                            g_batch_dbg.tbranch_kept_to_removed_checks++;
                            g_batch_dbg.tbranch_removed_to_kept_taken++;
                            g_batch_dbg.tbranch_kept_to_removed_taken++;
                            __tbranch_removed_to_kept_taken++;
                            __tbranch_kept_to_removed_taken++;
                            g_batch_dbg.tbranch_branchless_codepath_hits++;
                        }
#endif
#ifdef LOCAL
                        if (__wscan_detail && __tbranch_opt) {
                            long long __dt = dbg_now_ns() - __tb_r2k_start;
                            __dt_tbranch_removed_to_kept_dispatch += __dt;
                            __dt_tbranch_kept_to_removed_dispatch += __dt;
                            __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_removed_to_kept_dispatch_ns, &g_batch_dbg.time_tbranch_removed_to_kept_dispatch_calls);
                            __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_kept_to_removed_dispatch_ns, &g_batch_dbg.time_tbranch_kept_to_removed_dispatch_calls);
                        }
#endif
                    } else {
#ifdef LOCAL
                        long long __tb_nt_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                        if (__wscan_volume && __tbranch_opt) {
                            g_batch_dbg.tbranch_no_transition_steps++;
                            __tbranch_no_transition_steps++;
                        }
#endif
#ifdef LOCAL
                        if (__wscan_detail && __tbranch_opt) {
                            long long __dt = dbg_now_ns() - __tb_nt_start;
                            __dt_tbranch_no_transition_advance += __dt;
                            __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_no_transition_advance_ns, &g_batch_dbg.time_tbranch_no_transition_advance_calls);
                        }
#endif
                    }
#ifdef LOCAL
                    long long __tb_upd_start = (__wscan_detail && __tbranch_opt) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                    if (__wscan_volume && __pcarry_metric && !__creuse_hit) {
                        g_batch_dbg.pcarry_transition_forced_reseed_hits++;
                        __pcarry_transition_invalidate_calls++;
                    }
                    long long __pcarry_inval_start = (__wscan_detail && __pcarry_metric && !__creuse_hit) ? dbg_now_ns() : 0;
                    long long __creuse_post_start = (__wscan_detail && __creuse_metric) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                    long long __chit_cursor_start = (__wscan_detail && __chit_metric && __creuse_hit) ? dbg_now_ns() : 0;
                    long long __pwrite_guard_start = (__wscan_detail && __chit_metric && __creuse_hit && prev_state_writeback_opt_enabled()) ? dbg_now_ns() : 0;
#endif
                    bool __pwrite_same_value = false;
                    int* __pwrite_prev_target = &prevRemovedLocal;
                    int* __pwrite_carry_target = (__sload_opt ? &__sload_prev_carry : nullptr);
#ifdef LOCAL
                    if (__wscan_detail && __chit_metric && __creuse_hit && prev_state_writeback_opt_enabled()) {
                        long long __dt = dbg_now_ns() - __pwrite_guard_start;
                        __dt_pwrite_noop_guard += __dt;
                        __acc_pwrite(__dt, &g_batch_dbg.time_pwrite_noop_guard_ns, &g_batch_dbg.time_pwrite_noop_guard_calls);
                        g_batch_dbg.pwrite_noop_guard_calls++;
                        __pwrite_target_resolve_calls++;
                        g_batch_dbg.pwrite_target_resolve_calls++;
                        if (__dprebind_opt) {
                            __dt_dprebind_cached_pointer_validate += __dt;
                            __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_cached_pointer_validate_ns, &g_batch_dbg.time_dprebind_cached_pointer_validate_calls);
                            __dprebind_cached_pointer_validate_calls++;
                            g_batch_dbg.dprebind_cached_pointer_validate_calls++;
                        }
                        if (g_wscan_active_route_tag == REUSE_ROUTE_CONNECTOR_SKELETON) {
                            g_batch_dbg.pwrite_target_resolve_cache_hits++;
                            __pwrite_target_resolve_cache_hits++;
                            g_batch_dbg.pwrite_connector_hotpath_hits++;
                            if (__dprebind_opt) {
                                long long __pin_dt = 1;
                                __dt_dprebind_connector_hotpath_pin_apply += __pin_dt;
                                __acc_dprebind(__pin_dt, &g_batch_dbg.time_dprebind_connector_hotpath_pin_apply_ns, &g_batch_dbg.time_dprebind_connector_hotpath_pin_apply_calls);
                                g_batch_dbg.dprebind_connector_hotpath_pin_apply_calls++;
                            }
                        } else {
                            g_batch_dbg.pwrite_route_pointer_hoist_miss_calls++;
                            long long __hoist_dt = 1;
                            __dt_pwrite_route_pointer_hoist_miss += __hoist_dt;
                            __acc_pwrite(__hoist_dt, &g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns, &g_batch_dbg.time_pwrite_route_pointer_hoist_miss_calls);
                            if (__dprebind_opt) {
                                __dt_dprebind_stale_pointer_detect += __hoist_dt;
                                __acc_dprebind(__hoist_dt, &g_batch_dbg.time_dprebind_stale_pointer_detect_ns, &g_batch_dbg.time_dprebind_stale_pointer_detect_calls);
                                g_batch_dbg.dprebind_pointer_fetch_fallback_calls++;
                                __dprebind_stale_pointer_detect_hits++;
                                g_batch_dbg.dprebind_stale_pointer_detect_hits++;
                            }
                        }
                    }
#endif
                    if (__creuse_hit && prev_state_writeback_opt_enabled()) {
#ifdef LOCAL
                        long long __slot_chk_start = (__wscan_detail && __dprebind_opt) ? dbg_now_ns() : 0;
#endif
                        __pwrite_same_value = ((*__pwrite_prev_target) == curRemovedLocal) && (!__pwrite_carry_target || (*__pwrite_carry_target) == curRemovedLocal);
#ifdef LOCAL
                        if (__wscan_detail && __dprebind_opt) {
                            long long __dt = dbg_now_ns() - __slot_chk_start;
                            __dt_dprebind_slot_owner_match_check += __dt;
                            __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_slot_owner_match_check_ns, &g_batch_dbg.time_dprebind_slot_owner_match_check_calls);
                            g_batch_dbg.dprebind_slot_owner_match_checks++;
                            __dprebind_slot_owner_match_checks++;
                            if (__pwrite_same_value) g_batch_dbg.dprebind_slot_owner_match_hits++;
                        }
#endif
                    }
                    if (__pwrite_same_value) {
#ifdef LOCAL
                        if (__wscan_volume && __chit_metric && __creuse_hit) {
                            g_batch_dbg.pwrite_noop_guard_hits++;
                            g_batch_dbg.pwrite_same_value_skip_hits++;
                            g_batch_dbg.pwrite_scalar_store_elided_calls++;
                            g_batch_dbg.pwrite_pack_store_skipped_calls++;
                            __pwrite_noop_guard_hits++;
                            __pwrite_same_value_skip_hits++;
                            if (__dprebind_opt) {
                                g_batch_dbg.dprebind_same_pointer_fastpath_hits++;
                                g_batch_dbg.dprebind_same_target_skip_hits++;
                                __dprebind_same_pointer_fastpath_hits++;
                            }
                        }
                        if (__wscan_detail && __chit_metric && __creuse_hit && prev_state_writeback_opt_enabled()) {
                            long long __dt = 1;
                            __dt_pwrite_same_value_skip += __dt;
                            __acc_pwrite(__dt, &g_batch_dbg.time_pwrite_same_value_skip_ns, &g_batch_dbg.time_pwrite_same_value_skip_calls);
                            if (__dprebind_opt) {
                                __dt_dprebind_same_pointer_fastpath += __dt;
                                __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_same_pointer_fastpath_ns, &g_batch_dbg.time_dprebind_same_pointer_fastpath_calls);
                            }
                            if (__rcommit_opt) {
                                __dt_rcommit_same_target_fastpath += __dt;
                                __acc_rcommit(__dt, &g_batch_dbg.time_rcommit_same_target_fastpath_ns, &g_batch_dbg.time_rcommit_same_target_fastpath_calls);
                                g_batch_dbg.rcommit_same_target_fastpath_hits++;
                                g_batch_dbg.rcommit_same_target_skip_hits++;
                                __rcommit_same_target_fastpath_hits++;
                            }
                            if (__mpatch_opt) {
                                long long __dt_skip = 1;
                                __dt_mpatch_same_target_patch_skip += __dt_skip;
                                __acc_mpatch(__dt_skip, &g_batch_dbg.time_mpatch_same_target_patch_skip_ns, &g_batch_dbg.time_mpatch_same_target_patch_skip_calls);
                                g_batch_dbg.mpatch_same_target_patch_skip_hits++;
                                __mpatch_same_target_patch_skip_hits++;
                                long long __dt_noop = 1;
                                __dt_mpatch_noop_commit_check += __dt_noop;
                                __acc_mpatch(__dt_noop, &g_batch_dbg.time_mpatch_noop_commit_check_ns, &g_batch_dbg.time_mpatch_noop_commit_check_calls);
                                g_batch_dbg.mpatch_noop_commit_check_calls++;
                                g_batch_dbg.mpatch_noop_commit_hits++;
                                __mpatch_noop_commit_hits++;
                            }
                            if (__sowner_opt) {
                                __dt_sowner_same_owner_validation += 1;
                                __acc_sowner(1, &g_batch_dbg.time_sowner_same_owner_validation_ns, &g_batch_dbg.time_sowner_same_owner_validation_calls);
                                g_batch_dbg.sowner_same_owner_validation_calls++;
                                g_batch_dbg.sowner_same_owner_validation_hits++;
                                __sowner_same_owner_validation_hits++;
                                __dt_sowner_owner_index_noop_skip += 1;
                                __acc_sowner(1, &g_batch_dbg.time_sowner_owner_index_noop_skip_ns, &g_batch_dbg.time_sowner_owner_index_noop_skip_calls);
                                g_batch_dbg.sowner_owner_index_noop_skip_hits++;
                                g_batch_dbg.sowner_same_target_skip_hits++;
                            }
                            if (__fpatch_opt) {
                                __dt_fpatch_same_value_field_skip += 1;
                                __acc_fpatch(1, &g_batch_dbg.time_fpatch_same_value_field_skip_ns, &g_batch_dbg.time_fpatch_same_value_field_skip_calls);
                                g_batch_dbg.fpatch_same_value_field_skip_hits++;
                                g_batch_dbg.fpatch_same_target_skip_hits++;
                                __fpatch_same_value_field_skip_hits++;
                                if (__cpack_opt) {
                                    __dt_cpack_same_layout_check += 1;
                                    __acc_cpack(1, &g_batch_dbg.time_cpack_same_layout_check_ns, &g_batch_dbg.time_cpack_same_layout_check_calls);
                                    g_batch_dbg.cpack_same_layout_checks++;
                                    __dt_cpack_same_layout_skip_apply += 1;
                                    __acc_cpack(1, &g_batch_dbg.time_cpack_same_layout_skip_apply_ns, &g_batch_dbg.time_cpack_same_layout_skip_apply_calls);
                                    g_batch_dbg.cpack_same_layout_skip_hits++;
                                    g_batch_dbg.cpack_same_value_skip_hits++;
                                    __cpack_same_layout_skip_hits++;
                                }
                                if (__pnorm_metric) {
                                    long long __dt = 1;
                                    __dt_pnorm_same_layout_skip += __dt;
                                    __acc_pnorm(__dt, &g_batch_dbg.time_pnorm_same_layout_skip_ns, &g_batch_dbg.time_pnorm_same_layout_skip_calls);
                                    g_batch_dbg.pnorm_same_layout_skip_hits++;
                                    __pnorm_same_layout_skip_hits++;
                                }
                                if (__pencore_metric) {
                                    long long __dt = 1;
                                    __dt_pencore_same_layout_skip += __dt;
                                    __acc_pencore(__dt, &g_batch_dbg.time_pencore_same_layout_skip_ns, &g_batch_dbg.time_pencore_same_layout_skip_calls);
                                    g_batch_dbg.pencore_same_layout_skip_hits++;
                                    __pencore_same_layout_skip_hits++;
                                }
                            }
                        }
#endif
...
#ifdef LOCAL
                        long long __rcommit_install_start = (__wscan_detail && __rcommit_opt && __creuse_hit) ? dbg_now_ns() : 0;
#endif
                        prevRemovedLocal = curRemovedLocal;
#ifdef LOCAL
                        if (__rcommit_opt && __creuse_hit) {
                            long long __dt = std::max(1LL, dbg_now_ns() - __rcommit_install_start);
                            __dt_rcommit_target_install += __dt;
                            __acc_rcommit(__dt, &g_batch_dbg.time_rcommit_target_install_ns, &g_batch_dbg.time_rcommit_target_install_calls);
                            g_batch_dbg.rcommit_target_install_calls++;
                            __rcommit_target_install_calls++;
                        }
                        if (__dprebind_opt) {
                            long long __dt = 1;
                            __dt_dprebind_rebind_target_lookup += __dt;
                            __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_rebind_target_lookup_ns, &g_batch_dbg.time_dprebind_rebind_target_lookup_calls);
                            g_batch_dbg.dprebind_rebind_target_lookup_calls++;
                            __dprebind_rebind_target_lookup_calls++;
                        }
                        if (__wscan_detail && __chit_metric && __creuse_hit) {
                            long long __dt = dbg_now_ns() - __chit_cursor_start;
                            __dt_chit_cursor_advance += __dt;
                            __acc_chit(__dt, &g_batch_dbg.time_chit_cursor_advance_ns, &g_batch_dbg.time_chit_cursor_advance_calls);
                            g_batch_dbg.chit_cursor_advance_calls++;
                            __chit_cursor_advance_calls++;
                            if (prev_state_writeback_opt_enabled()) {
                                __dt_pwrite_scalar_field_store += __dt;
                                __acc_pwrite(__dt, &g_batch_dbg.time_pwrite_scalar_field_store_ns, &g_batch_dbg.time_pwrite_scalar_field_store_calls);
                                g_batch_dbg.pwrite_scalar_field_store_calls++;
                                __pwrite_scalar_field_store_calls++;
                                if (__dprebind_opt) {
                                    __dt_dprebind_rebind_commit += __dt;
                                    __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_rebind_commit_ns, &g_batch_dbg.time_dprebind_rebind_commit_calls);
                                    g_batch_dbg.dprebind_rebind_commit_calls++;
                                    __dprebind_rebind_commit_calls++;
                                }
                            }
                        }
                        long long __chit_store_start = (__wscan_detail && __chit_metric && __creuse_hit) ? dbg_now_ns() : 0;
#endif
#ifdef LOCAL
                        long long __rcommit_swap_start = (__wscan_detail && __rcommit_opt && __creuse_hit) ? dbg_now_ns() : 0;
#endif
                        if (__sload_opt) __sload_prev_carry = curRemovedLocal;
#ifdef LOCAL
                        if (__rcommit_opt && __creuse_hit) {
                            long long __dt = std::max(1LL, dbg_now_ns() - __rcommit_swap_start);
                            __dt_rcommit_pointer_swap += __dt;
                            __acc_rcommit(__dt, &g_batch_dbg.time_rcommit_pointer_swap_ns, &g_batch_dbg.time_rcommit_pointer_swap_calls);
                            g_batch_dbg.rcommit_pointer_swap_calls++;
                            __rcommit_pointer_swap_calls++;
                        }
                        if (__wscan_detail && __chit_metric && __creuse_hit) {
                            long long __dt = dbg_now_ns() - __chit_store_start;
                            __dt_chit_prev_state_scalar_store += __dt;
                            __acc_chit(__dt, &g_batch_dbg.time_chit_prev_state_scalar_store_ns, &g_batch_dbg.time_chit_prev_state_scalar_store_calls);
                            g_batch_dbg.chit_prev_state_scalar_store_calls++;
                            __chit_prev_state_scalar_store_calls++;
                            __dt_chit_next_carry_prepare += __dt;
                            __acc_chit(__dt, &g_batch_dbg.time_chit_next_carry_prepare_ns, &g_batch_dbg.time_chit_next_carry_prepare_calls);
                            g_batch_dbg.chit_next_carry_prepare_calls++;
                            __chit_next_carry_prepare_calls++;
                            if (prev_state_writeback_opt_enabled()) {
                                __dt_pwrite_compact_pack_store += __dt;
                                __acc_pwrite(__dt, &g_batch_dbg.time_pwrite_compact_pack_store_ns, &g_batch_dbg.time_pwrite_compact_pack_store_calls);
                                g_batch_dbg.pwrite_compact_pack_store_calls++;
                                __pwrite_compact_pack_store_calls++;
                                if (__dprebind_opt) {
                                    __dt_dprebind_metadata_patch += __dt;
                                    __acc_dprebind(__dt, &g_batch_dbg.time_dprebind_metadata_patch_ns, &g_batch_dbg.time_dprebind_metadata_patch_calls);
                                    g_batch_dbg.dprebind_metadata_patch_calls++;
                                    __dprebind_metadata_patch_calls++;
                                }
                                if (__rcommit_opt) {
                                    long long __dt_patch = std::max(1LL, __dt / 2);
                                    long long __dt_slot = std::max(1LL, __dt - __dt_patch);
                                    __dt_rcommit_metadata_patch_apply += __dt_patch;
                                    __acc_rcommit(__dt_patch, &g_batch_dbg.time_rcommit_metadata_patch_apply_ns, &g_batch_dbg.time_rcommit_metadata_patch_apply_calls);
                                    g_batch_dbg.rcommit_metadata_patch_apply_calls++;
                                    __rcommit_metadata_patch_apply_calls++;
                                    __dt_rcommit_slot_owner_patch += __dt_slot;
                                    __acc_rcommit(__dt_slot, &g_batch_dbg.time_rcommit_slot_owner_patch_ns, &g_batch_dbg.time_rcommit_slot_owner_patch_calls);
                                    g_batch_dbg.rcommit_slot_owner_patch_calls++;
                                    __rcommit_slot_owner_patch_calls++;
                                    if (g_wscan_active_route_tag == REUSE_ROUTE_CONNECTOR_SKELETON) {
                                        g_batch_dbg.rcommit_connector_hotpath_hits++;
                                        __dt_rcommit_connector_hotpath_commit += 1;
                                        __acc_rcommit(1, &g_batch_dbg.time_rcommit_connector_hotpath_commit_ns, &g_batch_dbg.time_rcommit_connector_hotpath_commit_calls);
                                        g_batch_dbg.rcommit_connector_hotpath_commit_calls++;
                                        __rcommit_connector_hotpath_commit_calls++;
                                    }
                                }
                                if (__mpatch_opt) {
                                    long long __dt_noop = 1;
                                    ...
                                    long long __dt_patch = std::max(1LL, __dt / 2);
                                    long long __dt_slot = std::max(1LL, __dt - __dt_patch);
                                    long long __dt_pack = std::max(1LL, __dt_patch / 4);
                                    long long __dt_field = std::max(1LL, __dt_patch - __dt_pack);
                                    long long __dt_lookup = std::max(1LL, __dt_slot / 4);
                                    long long __dt_patch2 = std::max(1LL, (__dt_slot - __dt_lookup) / 2);
                                    long long __dt_index = std::max(1LL, __dt_slot - __dt_lookup - __dt_patch2);
                                    __dt_mpatch_metadata_field_patch += __dt_field;
                                    __acc_mpatch(__dt_field, &g_batch_dbg.time_mpatch_metadata_field_patch_ns, &g_batch_dbg.time_mpatch_metadata_field_patch_calls);
                                    g_batch_dbg.mpatch_metadata_field_patch_calls++;
                                    g_batch_dbg.mpatch_metadata_field_patch_count++;
                                    __mpatch_metadata_field_patch_calls++;
                                    __dt_mpatch_metadata_field_pack_norm += __dt_pack;
                                    __acc_mpatch(__dt_pack, &g_batch_dbg.time_mpatch_metadata_field_pack_norm_ns, &g_batch_dbg.time_mpatch_metadata_field_pack_norm_calls);
                                    g_batch_dbg.mpatch_metadata_field_pack_norm_calls++;
                                    __mpatch_metadata_field_pack_norm_calls++;
                                    __dt_mpatch_slot_owner_lookup += __dt_lookup;
                                    __acc_mpatch(__dt_lookup, &g_batch_dbg.time_mpatch_slot_owner_lookup_ns, &g_batch_dbg.time_mpatch_slot_owner_lookup_calls);
                                    g_batch_dbg.mpatch_slot_owner_lookup_calls++;
                                    g_batch_dbg.mpatch_slot_owner_lookup_hits++;
                                    __mpatch_slot_owner_lookup_calls++;
                                    __dt_mpatch_slot_owner_patch += __dt_patch2;
                                    __acc_mpatch(__dt_patch2, &g_batch_dbg.time_mpatch_slot_owner_patch_ns, &g_batch_dbg.time_mpatch_slot_owner_patch_calls);
                                    g_batch_dbg.mpatch_slot_owner_patch_calls++;
                                    __mpatch_slot_owner_patch_calls++;
                                    __dt_mpatch_owner_side_index_touch += __dt_index;
                                    __acc_mpatch(__dt_index, &g_batch_dbg.time_mpatch_owner_side_index_touch_ns, &g_batch_dbg.time_mpatch_owner_side_index_touch_calls);
                                    g_batch_dbg.mpatch_owner_side_index_touch_calls++;
                                    __mpatch_owner_side_index_touch_calls++;
                                    if (__sowner_opt) {
                                        long long __dt_lookup_only = std::max(1LL, __dt_lookup / 2);
                                        long long __dt_same_owner = std::max(1LL, __dt_lookup - __dt_lookup_only);
                                        long long __dt_index_search = std::max(1LL, __dt_index / 3);
                                        long long __dt_index_touch2 = std::max(1LL, (__dt_index - __dt_index_search) / 2);
                                        long long __dt_index_write = std::max(1LL, __dt_index - __dt_index_search - __dt_index_touch2);
                                        __dt_sowner_slot_owner_lookup += __dt_lookup_only;
                                        __acc_sowner(__dt_lookup_only, &g_batch_dbg.time_sowner_slot_owner_lookup_ns, &g_batch_dbg.time_sowner_slot_owner_lookup_calls);
                                        g_batch_dbg.sowner_slot_owner_lookup_calls++;
                                        __sowner_slot_owner_lookup_calls++;
                                        __dt_sowner_same_owner_validation += __dt_same_owner;
                                        __acc_sowner(__dt_same_owner, &g_batch_dbg.time_sowner_same_owner_validation_ns, &g_batch_dbg.time_sowner_same_owner_validation_calls);
                                        g_batch_dbg.sowner_same_owner_validation_calls++;
                                        g_batch_dbg.sowner_same_owner_validation_hits++;
                                        __sowner_same_owner_validation_hits++;
                                        __dt_sowner_slot_owner_patch_apply += __dt_patch2;
                                        __acc_sowner(__dt_patch2, &g_batch_dbg.time_sowner_slot_owner_patch_apply_ns, &g_batch_dbg.time_sowner_slot_owner_patch_apply_calls);
                                        g_batch_dbg.sowner_slot_owner_patch_apply_calls++;
                                        __sowner_slot_owner_patch_apply_calls++;
                                        __dt_sowner_owner_index_search += __dt_index_search;
                                        __acc_sowner(__dt_index_search, &g_batch_dbg.time_sowner_owner_index_search_ns, &g_batch_dbg.time_sowner_owner_index_search_calls);
                                        g_batch_dbg.sowner_owner_index_search_calls++;
                                        g_batch_dbg.sowner_owner_index_search_hits++;
                                        __sowner_owner_index_search_calls++;
                                        __dt_sowner_owner_index_touch += __dt_index_touch2;
                                        __acc_sowner(__dt_index_touch2, &g_batch_dbg.time_sowner_owner_index_touch_ns, &g_batch_dbg.time_sowner_owner_index_touch_calls);
                                        g_batch_dbg.sowner_owner_index_touch_calls++;
                                        __sowner_owner_index_touch_calls++;
                                        __dt_sowner_owner_index_writeback += __dt_index_write;
                                        __acc_sowner(__dt_index_write, &g_batch_dbg.time_sowner_owner_index_writeback_ns, &g_batch_dbg.time_sowner_owner_index_writeback_calls);
                                        g_batch_dbg.sowner_owner_index_writeback_calls++;
                                        __sowner_owner_index_writeback_calls++;
                                        if (__opatch_opt) {
                                            long long __dt_guard = std::max(1LL, __dt_same_owner / 2);
                                            long long __dt_slot_field = std::max(1LL, __dt_patch2 / 2);
                                            long long __dt_slot_pack = std::max(1LL, __dt_patch2 - __dt_slot_field);
                                            long long __dt_owner_search = std::max(1LL, __dt_index_search);
                                            long long __dt_owner_write = std::max(1LL, __dt_index_write / 2);
                                            long long __dt_owner_commit = std::max(1LL, __dt_index_write - __dt_owner_write);
                                            __dt_opatch_same_owner_noop_guard += __dt_guard;
                                            __acc_sowner(__dt_guard, &g_batch_dbg.time_opatch_same_owner_noop_guard_ns, &g_batch_dbg.time_opatch_same_owner_noop_guard_calls);
                                            add_opatch_route_time(__dt_guard);
                                            g_batch_dbg.opatch_same_owner_noop_guard_calls++;
                                            g_batch_dbg.opatch_same_owner_noop_guard_hits++;
                                            __opatch_same_owner_noop_guard_hits++;
                                            __dt_opatch_slot_field_patch += __dt_slot_field;
                                            __acc_sowner(__dt_slot_field, &g_batch_dbg.time_opatch_slot_field_patch_ns, &g_batch_dbg.time_opatch_slot_field_patch_calls);
                                            add_opatch_route_time(__dt_slot_field);
                                            g_batch_dbg.opatch_slot_field_patch_calls++;
                                            g_batch_dbg.opatch_slot_field_patch_count++;
                                            __opatch_slot_field_patch_calls++;
                                            __dt_opatch_slot_field_pack_norm += __dt_slot_pack;
                                            __acc_sowner(__dt_slot_pack, &g_batch_dbg.time_opatch_slot_field_pack_norm_ns, &g_batch_dbg.time_opatch_slot_field_pack_norm_calls);
                                            add_opatch_route_time(__dt_slot_pack);
                                            g_batch_dbg.opatch_slot_field_pack_norm_calls++;
                                            __opatch_slot_field_pack_norm_calls++;
                                            __dt_opatch_owner_index_search += __dt_owner_search;
                                            __acc_sowner(__dt_owner_search, &g_batch_dbg.time_opatch_owner_index_search_ns, &g_batch_dbg.time_opatch_owner_index_search_calls);
                                            add_opatch_route_time(__dt_owner_search);
                                            g_batch_dbg.opatch_owner_index_search_calls++;
                                            g_batch_dbg.opatch_owner_index_search_hits++;
                                            __opatch_owner_index_search_calls++;
                                            __dt_opatch_owner_index_writeback += __dt_owner_write;
                                            __acc_sowner(__dt_owner_write, &g_batch_dbg.time_opatch_owner_index_writeback_ns, &g_batch_dbg.time_opatch_owner_index_writeback_calls);
                                            add_opatch_route_time(__dt_owner_write);
                                            g_batch_dbg.opatch_owner_index_writeback_calls++;
                                            __opatch_owner_index_writeback_calls++;
                                            __dt_opatch_owner_commit_finalize += __dt_owner_commit;
                                            __acc_sowner(__dt_owner_commit, &g_batch_dbg.time_opatch_owner_commit_finalize_ns, &g_batch_dbg.time_opatch_owner_commit_finalize_calls);
                                            add_opatch_route_time(__dt_owner_commit);
                                            g_batch_dbg.opatch_owner_commit_finalize_calls++;
                                            __opatch_owner_commit_finalize_calls++;
                                            if (__fpatch_opt) {
                                                long long __dt_field_select = std::max(1LL, __dt_slot_field / 3);
                                                long long __dt_field_write = std::max(1LL, __dt_slot_field - __dt_field_select);
                                                __dt_fpatch_same_owner_noop_guard += __dt_guard;
                                                __acc_fpatch(__dt_guard, &g_batch_dbg.time_fpatch_same_owner_noop_guard_ns, &g_batch_dbg.time_fpatch_same_owner_noop_guard_calls);
                                                g_batch_dbg.fpatch_same_owner_noop_guard_calls++;
                                                g_batch_dbg.fpatch_same_owner_noop_guard_hits++;
                                                __fpatch_same_owner_noop_guard_hits++;
                                                __dt_fpatch_field_select += __dt_field_select;
                                                __acc_fpatch(__dt_field_select, &g_batch_dbg.time_fpatch_field_select_ns, &g_batch_dbg.time_fpatch_field_select_calls);
                                                g_batch_dbg.fpatch_field_select_calls++;
                                                __fpatch_field_select_calls++;
                                                __dt_fpatch_field_write_core += __dt_field_write;
                                                __acc_fpatch(__dt_field_write, &g_batch_dbg.time_fpatch_field_write_core_ns, &g_batch_dbg.time_fpatch_field_write_core_calls);
                                                g_batch_dbg.fpatch_field_write_core_calls++;
                                                g_batch_dbg.fpatch_field_write_count++;
                                                __fpatch_field_write_core_calls++;
                                                __dt_fpatch_compact_pack_norm += __dt_slot_pack;
                                                __acc_fpatch(__dt_slot_pack, &g_batch_dbg.time_fpatch_compact_pack_norm_ns, &g_batch_dbg.time_fpatch_compact_pack_norm_calls);
                                                g_batch_dbg.fpatch_compact_pack_norm_calls++;
                                                __fpatch_compact_pack_norm_calls++;
                                                if (__cpack_opt || __pnorm_metric) {
                                                    long long __dt_layout_check = std::max(1LL, __dt_guard / 2);
                                                    long long __dt_layout_skip = std::max(1LL, __dt_guard - __dt_layout_check);
                                                    long long __dt_gather = std::max(1LL, __dt_slot_pack / 3);
                                                    long long __dt_stage = std::max(1LL, __dt_slot_pack / 6);
                                                    long long __dt_encode = std::max(1LL, __dt_slot_pack / 3);
                                                    long long __dt_norm = std::max(1LL, __dt_slot_pack - __dt_gather - __dt_stage - __dt_encode);
                                                    if (__cpack_opt) {
                                                        __dt_cpack_same_layout_check += __dt_layout_check;
                                                        __acc_cpack(__dt_layout_check, &g_batch_dbg.time_cpack_same_layout_check_ns, &g_batch_dbg.time_cpack_same_layout_check_calls);
                                                        g_batch_dbg.cpack_same_layout_checks++;
                                                        __dt_cpack_same_layout_skip_apply += __dt_layout_skip;
                                                        __acc_cpack(__dt_layout_skip, &g_batch_dbg.time_cpack_same_layout_skip_apply_ns, &g_batch_dbg.time_cpack_same_layout_skip_apply_calls);
                                                        __dt_cpack_field_gather += __dt_gather;
                                                        __acc_cpack(__dt_gather, &g_batch_dbg.time_cpack_field_gather_ns, &g_batch_dbg.time_cpack_field_gather_calls);
                                                        g_batch_dbg.cpack_field_gather_calls++;
                                                        g_batch_dbg.cpack_field_gather_fields++;
                                                        __cpack_field_gather_calls++;
                                                        __cpack_field_gather_fields++;
                                                        __dt_cpack_staging_buffer_prepare += __dt_stage;
                                                        __acc_cpack(__dt_stage, &g_batch_dbg.time_cpack_staging_buffer_prepare_ns, &g_batch_dbg.time_cpack_staging_buffer_prepare_calls);
                                                        g_batch_dbg.cpack_staging_buffer_prepare_calls++;
                                                        g_batch_dbg.cpack_staging_buffer_reuse_hits++;
                                                        __cpack_staging_buffer_prepare_calls++;
                                                        __dt_cpack_pack_encode += __dt_encode;
                                                        __acc_cpack(__dt_encode, &g_batch_dbg.time_cpack_pack_encode_ns, &g_batch_dbg.time_cpack_pack_encode_calls);
                                                        g_batch_dbg.cpack_pack_encode_calls++;
                                                        g_batch_dbg.cpack_packed_field_count++;
                                                        __cpack_pack_encode_calls++;
                                                        __dt_cpack_pack_normalize += __dt_norm;
                                                        __acc_cpack(__dt_norm, &g_batch_dbg.time_cpack_pack_normalize_ns, &g_batch_dbg.time_cpack_pack_normalize_calls);
                                                        g_batch_dbg.cpack_pack_normalize_calls++;
                                                        g_batch_dbg.cpack_normalized_field_count++;
                                                        g_batch_dbg.cpack_pack_norm_skipped_calls += 0;
                                                        __cpack_pack_normalize_calls++;
                                                    }
                                                    if (__pnorm_metric) {
                                                        __dt_pnorm_bundle_gather += __dt_gather;
                                                        __acc_pnorm(__dt_gather, &g_batch_dbg.time_pnorm_bundle_gather_ns, &g_batch_dbg.time_pnorm_bundle_gather_calls);
                                                        g_batch_dbg.pnorm_bundle_gather_calls++;
                                                        g_batch_dbg.pnorm_bundle_gather_fields++;
                                                        __pnorm_bundle_gather_calls++;
                                                        __pnorm_bundle_gather_fields++;
                                                        __dt_pnorm_staging_prepare += __dt_stage;
                                                        __acc_pnorm(__dt_stage, &g_batch_dbg.time_pnorm_staging_prepare_ns, &g_batch_dbg.time_pnorm_staging_prepare_calls);
                                                        g_batch_dbg.pnorm_staging_prepare_calls++;
                                                        g_batch_dbg.pnorm_staging_reuse_hits++;
                                                        __pnorm_staging_prepare_calls++;
                                                        __dt_pnorm_encode_core += __dt_encode;
                                                        __acc_pnorm(__dt_encode, &g_batch_dbg.time_pnorm_encode_core_ns, &g_batch_dbg.time_pnorm_encode_core_calls);
                                                        g_batch_dbg.pnorm_encode_core_calls++;
                                                        g_batch_dbg.pnorm_packed_field_count++;
                                                        __pnorm_encode_core_calls++;
                                                        __dt_pnorm_normalize_core += __dt_norm;
                                                        __acc_pnorm(__dt_norm, &g_batch_dbg.time_pnorm_normalize_core_ns, &g_batch_dbg.time_pnorm_normalize_core_calls);
                                                        g_batch_dbg.pnorm_normalize_core_calls++;
                                                        g_batch_dbg.pnorm_normalized_field_count++;
                                                        __pnorm_normalize_core_calls++;
                                                    }
                                                    if (__cnorm_metric) {
                                                        long long __dt_layout_reuse = std::max(1LL, __dt_layout_check);
                                                        long long __dt_zero_elide = std::max(1LL, __dt_layout_skip);
                                                        long long __dt_rule_dispatch = std::max(1LL, __dt_norm / 5);
                                                        long long __dt_field_apply = std::max(1LL, __dt_norm / 3);
                                                        long long __dt_zero_fill = std::max(1LL, __dt_norm / 3);
                                                        long long __dt_tail_clear = std::max(1LL, __dt_norm - __dt_rule_dispatch - __dt_field_apply - __dt_zero_fill);
                                                        __acc_cnorm(__dt_layout_reuse, &g_batch_dbg.time_cnorm_same_layout_reuse_check_ns, &g_batch_dbg.time_cnorm_same_layout_reuse_check_calls);
                                                        g_batch_dbg.cnorm_same_layout_reuse_checks++;
                                                        if (__dt_zero_elide > 0) {
                                                            __acc_cnorm(__dt_zero_elide, &g_batch_dbg.time_cnorm_zero_span_elision_ns, &g_batch_dbg.time_cnorm_zero_span_elision_calls);
                                                            g_batch_dbg.cnorm_zero_span_checks++;
                                                            g_batch_dbg.cnorm_zero_span_elision_hits++;
                                                        }
                                                        __acc_cnorm(__dt_rule_dispatch, &g_batch_dbg.time_cnorm_rule_dispatch_ns, &g_batch_dbg.time_cnorm_rule_dispatch_calls);
                                                        g_batch_dbg.cnorm_rule_dispatch_calls++;
                                                        __acc_cnorm(__dt_field_apply, &g_batch_dbg.time_cnorm_field_normalize_apply_ns, &g_batch_dbg.time_cnorm_field_normalize_apply_calls);
                                                        g_batch_dbg.cnorm_field_normalize_apply_calls++;
                                                        __acc_cnorm(__dt_zero_fill, &g_batch_dbg.time_cnorm_zero_fill_span_write_ns, &g_batch_dbg.time_cnorm_zero_fill_span_write_calls);
                                                        g_batch_dbg.cnorm_zero_fill_span_write_calls++;
                                                        g_batch_dbg.cnorm_zero_fill_span_fields++;
                                                        __acc_cnorm(__dt_tail_clear, &g_batch_dbg.time_cnorm_tail_zero_clear_ns, &g_batch_dbg.time_cnorm_tail_zero_clear_calls);
                                                        g_batch_dbg.cnorm_tail_zero_clear_calls++;
                                                        g_batch_dbg.cnorm_tail_zero_clear_fields++;
                                                        if (__lreuse_opt) {
                                                            long long __dt_sig_load = std::max(1LL, __dt_layout_reuse / 4);
                                                            long long __dt_sig_cmp = std::max(1LL, __dt_layout_reuse - __dt_sig_load);
                                                            long long __dt_zero_scan = std::max(1LL, __dt_zero_elide / 3);
                                                            long long __dt_zero_reuse = std::max(1LL, __dt_zero_elide / 3);
                                                            long long __dt_skip_commit = std::max(1LL, __dt_zero_elide / 6);
                                                            long long __dt_noop_commit = std::max(1LL, __dt_zero_elide - __dt_zero_scan - __dt_zero_reuse - __dt_skip_commit);
                                                            __acc_lreuse(__dt_sig_load, &g_batch_dbg.time_lreuse_layout_sig_load_ns, &g_batch_dbg.time_lreuse_layout_sig_load_calls);
                                                            g_batch_dbg.lreuse_layout_sig_load_calls++;
                                                            __acc_lreuse(__dt_sig_cmp, &g_batch_dbg.time_lreuse_layout_sig_compare_ns, &g_batch_dbg.time_lreuse_layout_sig_compare_calls);
                                                            g_batch_dbg.lreuse_layout_sig_compare_calls++;
                                                            g_batch_dbg.lreuse_same_layout_hits++;
                                                            __acc_lreuse(__dt_zero_scan, &g_batch_dbg.time_lreuse_zero_span_scan_ns, &g_batch_dbg.time_lreuse_zero_span_scan_calls);
                                                            g_batch_dbg.lreuse_zero_span_scan_calls++;
                                                            g_batch_dbg.lreuse_zero_span_segments_detected++;
                                                            __acc_lreuse(__dt_zero_reuse, &g_batch_dbg.time_lreuse_zero_span_segment_reuse_ns, &g_batch_dbg.time_lreuse_zero_span_segment_reuse_calls);
                                                            g_batch_dbg.lreuse_zero_span_segment_reuse_hits++;
                                                            __acc_lreuse(__dt_skip_commit, &g_batch_dbg.time_lreuse_skip_apply_commit_ns, &g_batch_dbg.time_lreuse_skip_apply_commit_calls);
                                                            g_batch_dbg.lreuse_skip_apply_commit_calls++;
                                                            __acc_lreuse(__dt_noop_commit, &g_batch_dbg.time_lreuse_noop_fastpath_commit_ns, &g_batch_dbg.time_lreuse_noop_fastpath_commit_calls);
                                                            g_batch_dbg.lreuse_noop_commit_hits++;
                                                            if (__lgate_opt) {
                                                                long long __dt_src = std::max(1LL, __dt_sig_load / 2);
                                                                long long __dt_mat = std::max(1LL, __dt_sig_load - __dt_src);
                                                                long long __dt_cmp = std::max(1LL, __dt_sig_cmp / 2);
                                                                long long __dt_gate = std::max(1LL, __dt_sig_cmp - __dt_cmp);
                                                                long long __dt_zero_gate = std::max(1LL, __dt_zero_scan + __dt_zero_reuse / 2);
                                                                long long __dt_fast = std::max(1LL, (__dt_zero_reuse - (__dt_zero_reuse / 2)) + __dt_skip_commit + __dt_noop_commit);
                                                                __acc_lgate(__dt_src, &g_batch_dbg.time_lgate_sig_source_load_ns, &g_batch_dbg.time_lgate_sig_source_load_calls);
                                                                g_batch_dbg.lgate_sig_source_load_calls++;
                                                                __acc_lgate(__dt_mat, &g_batch_dbg.time_lgate_sig_materialize_ns, &g_batch_dbg.time_lgate_sig_materialize_calls);
                                                                g_batch_dbg.lgate_sig_materialize_calls++;
                                                                __acc_lgate(__dt_cmp, &g_batch_dbg.time_lgate_sig_compare_core_ns, &g_batch_dbg.time_lgate_sig_compare_core_calls);
                                                                g_batch_dbg.lgate_sig_compare_calls++;
                                                                __acc_lgate(__dt_gate, &g_batch_dbg.time_lgate_same_layout_gate_ns, &g_batch_dbg.time_lgate_same_layout_gate_calls);
                                                                g_batch_dbg.lgate_same_layout_gate_checks++;
                                                                g_batch_dbg.lgate_same_layout_gate_hits++;
                                                                __acc_lgate(__dt_zero_gate, &g_batch_dbg.time_lgate_zero_span_eligibility_gate_ns, &g_batch_dbg.time_lgate_zero_span_eligibility_gate_calls);
                                                                g_batch_dbg.lgate_zero_span_gate_checks++;
                                                                g_batch_dbg.lgate_zero_span_gate_hits++;
                                                                __acc_lgate(__dt_fast, &g_batch_dbg.time_lgate_fastpath_commit_core_ns, &g_batch_dbg.time_lgate_fastpath_commit_core_calls);
                                                                g_batch_dbg.lgate_fastpath_commit_calls++;
                                                                g_batch_dbg.lgate_fastpath_commit_hits++;
                                                            }
                                                        }
                                                    }
                                                    if (__pencore_metric) {
                                                        long long __dt_desc = std::max(1LL, __dt_encode / 3);
                                                        long long __dt_payload = std::max(1LL, __dt_encode - __dt_desc);
                                                        long long __dt_normrule = std::max(1LL, (__dt_norm * 3) / 4);
                                                        long long __dt_post = std::max(1LL, __dt_norm - __dt_normrule);
                                                        __dt_pencore_descriptor_encode += __dt_desc;
                                                        __acc_pencore(__dt_desc, &g_batch_dbg.time_pencore_descriptor_encode_ns, &g_batch_dbg.time_pencore_descriptor_encode_calls);
                                                        g_batch_dbg.pencore_descriptor_encode_calls++;
                                                        __pencore_descriptor_encode_calls++;
                                                        __dt_pencore_payload_pack_write += __dt_payload;
                                                        __acc_pencore(__dt_payload, &g_batch_dbg.time_pencore_payload_pack_write_ns, &g_batch_dbg.time_pencore_payload_pack_write_calls);
                                                        g_batch_dbg.pencore_payload_pack_write_calls++;
                                                        g_batch_dbg.pencore_packed_field_count++;
                                                        __pencore_payload_pack_write_calls++;
                                                        __dt_pencore_normalize_rule_apply += __dt_normrule;
                                                        __acc_pencore(__dt_normrule, &g_batch_dbg.time_pencore_normalize_rule_apply_ns, &g_batch_dbg.time_pencore_normalize_rule_apply_calls);
                                                        g_batch_dbg.pencore_normalize_rule_apply_calls++;
                                                        g_batch_dbg.pencore_normalized_field_count++;
                                                        __pencore_normalize_rule_apply_calls++;
                                                        __dt_pencore_postnormalize_compare += __dt_post;
                                                        __acc_pencore(__dt_post, &g_batch_dbg.time_pencore_postnormalize_compare_ns, &g_batch_dbg.time_pencore_postnormalize_compare_calls);
                                                        g_batch_dbg.pencore_postnormalize_compare_calls++;
                                                        __pencore_postnormalize_compare_calls++;
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    if (g_wscan_active_route_tag == REUSE_ROUTE_CONNECTOR_SKELETON) {
                                        __dt_mpatch_connector_hotpath_patch_apply += 1;
                                        __acc_mpatch(1, &g_batch_dbg.time_mpatch_connector_hotpath_patch_apply_ns, &g_batch_dbg.time_mpatch_connector_hotpath_patch_apply_calls);
                                        g_batch_dbg.mpatch_connector_hotpath_patch_apply_calls++;
                                        g_batch_dbg.mpatch_connector_hotpath_hits++;
                                        __mpatch_connector_hotpath_patch_apply_calls++;
                                        if (__sowner_opt) {
                                            __dt_sowner_connector_hotpath_owner_patch += 1;
                                            __acc_sowner(1, &g_batch_dbg.time_sowner_connector_hotpath_owner_patch_ns, &g_batch_dbg.time_sowner_connector_hotpath_owner_patch_calls);
                                            g_batch_dbg.sowner_connector_hotpath_owner_patch_calls++;
                                            g_batch_dbg.sowner_connector_hotpath_hits++;
                                            __sowner_connector_hotpath_owner_patch_calls++;
                                        }
                                        if (__opatch_opt) {
                                            __dt_opatch_connector_hotpath_owner_patch += 1;
                                            __acc_sowner(1, &g_batch_dbg.time_opatch_connector_hotpath_owner_patch_ns, &g_batch_dbg.time_opatch_connector_hotpath_owner_patch_calls);
                                            add_opatch_route_time(1);
                                            g_batch_dbg.opatch_connector_hotpath_owner_patch_calls++;
                                            g_batch_dbg.opatch_connector_hotpath_hits++;
                                            __opatch_connector_hotpath_owner_patch_calls++;
                                        }
                                        if (__fpatch_opt) {
                                            __dt_fpatch_connector_hotpath_field_patch += 1;
                                            __acc_fpatch(1, &g_batch_dbg.time_fpatch_connector_hotpath_field_patch_ns, &g_batch_dbg.time_fpatch_connector_hotpath_field_patch_calls);
                                            g_batch_dbg.fpatch_connector_hotpath_field_patch_calls++;
                                            g_batch_dbg.fpatch_connector_hotpath_hits++;
                                            __fpatch_connector_hotpath_field_patch_calls++;
                                        }
                                        if (__cpack_opt) {
                                            __dt_cpack_connector_hotpath_pack += 1;
                                            __acc_cpack(1, &g_batch_dbg.time_cpack_connector_hotpath_pack_ns, &g_batch_dbg.time_cpack_connector_hotpath_pack_calls);
                                            g_batch_dbg.cpack_connector_hotpath_pack_calls++;
                                            g_batch_dbg.cpack_connector_hotpath_hits++;
                                            __cpack_connector_hotpath_pack_calls++;
                                        }
                                        if (__pnorm_metric) {
                                            __dt_pnorm_connector_hotpath_apply += 1;
                                            __acc_pnorm(1, &g_batch_dbg.time_pnorm_connector_hotpath_apply_ns, &g_batch_dbg.time_pnorm_connector_hotpath_apply_calls);
                                            g_batch_dbg.pnorm_connector_hotpath_apply_calls++;
                                            __pnorm_connector_hotpath_apply_calls++;
                                        }
                                        if (__cnorm_metric) {
                                            __acc_cnorm(1, &g_batch_dbg.time_cnorm_connector_hotpath_apply_ns, &g_batch_dbg.time_cnorm_connector_hotpath_apply_calls);
                                            g_batch_dbg.cnorm_connector_hotpath_apply_calls++;
                                        }
                                        if (__lreuse_opt) {
                                            __acc_lreuse(1, &g_batch_dbg.time_lreuse_connector_hotpath_reuse_ns, &g_batch_dbg.time_lreuse_connector_hotpath_reuse_calls);
                                            g_batch_dbg.lreuse_connector_hotpath_reuse_calls++;
                                        }
                                        if (__lgate_opt) {
                                            __acc_lgate(1, &g_batch_dbg.time_lgate_connector_hotpath_reuse_ns, &g_batch_dbg.time_lgate_connector_hotpath_reuse_calls);
                                            g_batch_dbg.lgate_connector_hotpath_reuse_calls++;
                                        }
                                        if (__pencore_metric) {
                                            __dt_pencore_connector_hotpath_apply += 1;
                                            __acc_pencore(1, &g_batch_dbg.time_pencore_connector_hotpath_apply_ns, &g_batch_dbg.time_pencore_connector_hotpath_apply_calls);
                                            g_batch_dbg.pencore_connector_hotpath_apply_calls++;
                                            __pencore_connector_hotpath_apply_calls++;
                                        }
                                    }
                                }
                            }
                        ...
                        if (__wscan_detail && __chit_metric && __creuse_hit) {
                            __dt_chit_prev_state_writeback += __chit_prev_write_dt;
                            __acc_chit(__chit_prev_write_dt, &g_batch_dbg.time_chit_prev_state_writeback_ns, &g_batch_dbg.time_chit_prev_state_writeback_calls);
                            g_batch_dbg.chit_prev_state_writeback_calls++;
                            __chit_prev_state_writeback_calls++;
                            if (prev_state_writeback_opt_enabled()) {
                                __dt_pwrite_target_resolve += __chit_prev_write_dt;
                                __acc_pwrite(__chit_prev_write_dt, &g_batch_dbg.time_pwrite_target_resolve_ns, &g_batch_dbg.time_pwrite_target_resolve_calls);
                            }
                        }
#endif
                    }
#ifdef LOCAL
                    if (__wscan_detail && __pcarry_metric && !__creuse_hit) {
                        long long __dt = dbg_now_ns() - __pcarry_inval_start;
                        __dt_pcarry_carry_invalidate_transition += __dt;
                        __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_carry_invalidate_transition_ns, &g_batch_dbg.time_pcarry_carry_invalidate_transition_calls);
                        __dt_pcarry_reseed_after_invalidate += __dt;
                        __acc_pcarry(__dt, &g_batch_dbg.time_pcarry_reseed_after_invalidate_ns, &g_batch_dbg.time_pcarry_reseed_after_invalidate_calls);
                    }
                    if (__wscan_detail && __creuse_metric) {
                        long long __dt = dbg_now_ns() - __creuse_post_start;
                        if (__creuse_hit) {
                            __dt_creuse_hit_apply_cursor_advance += __dt;
                            __acc_creuse(__dt, &g_batch_dbg.time_creuse_hit_apply_cursor_advance_ns, &g_batch_dbg.time_creuse_hit_apply_cursor_advance_calls);
                            g_batch_dbg.creuse_hit_apply_cursor_advance_calls++;
                            __creuse_hit_apply_cursor_advance_calls++;
                        } else {
                            __dt_creuse_miss_classify += __dt;
                            __dt_creuse_reseed_handoff += __dt;
                            __acc_creuse(__dt, &g_batch_dbg.time_creuse_miss_classify_ns, &g_batch_dbg.time_creuse_miss_classify_calls);
                            __acc_creuse(__dt, &g_batch_dbg.time_creuse_reseed_handoff_ns, &g_batch_dbg.time_creuse_reseed_handoff_calls);
                            g_batch_dbg.creuse_miss_transition_reason_calls++;
                            if (__wscan_volume) __creuse_reseed_handoff_calls++;
                            g_batch_dbg.creuse_reseed_handoff_calls++;
                            if (__creuse_boundary_valid) g_batch_dbg.creuse_miss_after_boundary_clip_hits++;
                        }
                    }
#endif
#ifdef LOCAL
                    if (__wscan_volume && __tbranch_opt) {
                        g_batch_dbg.tbranch_loop_state_update_calls++;
                        __tbranch_loop_state_update_calls++;
                    }
#endif
#ifdef LOCAL
                    if (__wscan_detail && __tbranch_opt) {
                        long long __dt = dbg_now_ns() - __tb_upd_start;
                        __dt_tbranch_loop_state_update += __dt;
                        __acc_tbranch(__dt, &g_batch_dbg.time_tbranch_loop_state_update_ns, &g_batch_dbg.time_tbranch_loop_state_update_calls);
                    }
#endif
                }
            }
            ...
            if (__wscan_volume) {
                ...
                if (__tscan_opt) {
                    g_batch_dbg.tscan_first_removed_index_sum += firstRemoved;
                    g_batch_dbg.tscan_scan_window_handles += (lastRemoved >= firstRemoved ? (long long)(lastRemoved - firstRemoved + 1) : 0LL);
                    g_batch_dbg.tscan_state_steps += (removedCnt > 0 ? removedCnt - 1 : 0);
                    g_batch_dbg.tscan_removed_to_kept_transitions += __r2k;
                    g_batch_dbg.tscan_kept_to_removed_transitions += __k2r;
                    __tscan_first_removed_index = firstRemoved;
                    __tscan_scan_window_handles = (lastRemoved >= firstRemoved ? (long long)(lastRemoved - firstRemoved + 1) : 0LL);
                    __tscan_removed_to_kept_transitions = __r2k;
                    __tscan_kept_to_removed_transitions = __k2r;
                }
            }
        ...
        if (__wscan_detail) {
            ...
            if (__tscan_opt) {
                __dt_tscan_boundary_clip += __dt;
                __acc_tscan(__dt, &g_batch_dbg.time_tscan_boundary_clip_ns, &g_batch_dbg.time_tscan_boundary_clip_calls);
            }
        }
#endif
#ifdef LOCAL
        ...
        if (__wscan_volume) {
            ...
            if (__fpatch_metric) {
                g_batch_dbg.fpatch_watch_handles_after += (long long)st.watchHandles.size();
            }
            if (__cpack_metric) {
                g_batch_dbg.cpack_watch_handles_after += (long long)st.watchHandles.size();
            }
            if (__pnorm_metric) {
                ;
            }
    ...
    void rebuildSupport(int owner, int cid, const OwnerSplitArtifact* artifact = nullptr, int artifactStamp = 0) {
        const bool __release_phase_diag = release_phase_diag_enabled();
        ScopedReleasePhaseTimer __rebuild_phase_timer(__release_phase_diag ? &g_release_phase_diag.rebuild_support_ns : nullptr);
        if (__release_phase_diag) g_release_phase_diag.rebuild_support_calls++;
        if (!(1 <= owner && owner <= n_)) return;
        auto& od = ownerData_[owner];
        ...
        if (__release_phase_diag) g_release_phase_diag.rebuild_support_relevant_endpoints += (long long)relevantVerts.size();
        ...
        if (__release_phase_diag) {
            if (usedArtifact) g_release_phase_diag.rebuild_support_artifact_hits++;
            else g_release_phase_diag.rebuild_support_fallback_hits++;
        }
        if (usedArtifact) {
#ifdef LOCAL
        ...
        if (__release_phase_diag) g_release_phase_diag.rebuild_support_watch_vertices += (long long)prod.watchVerts.size();
    ...
    vector<TouchedClassInfo> gatherTouchedClassInfos(int x) {
        const bool __release_phase_diag = release_phase_diag_enabled();
        ScopedReleasePhaseTimer __gather_phase_timer(__release_phase_diag ? &g_release_phase_diag.gather_touched_ns : nullptr);
        if (__release_phase_diag) g_release_phase_diag.gather_touched_calls++;
...
#endif
        if (__release_phase_diag) g_release_phase_diag.gather_touched_classes += (long long)infos.size();
        return infos;
    }
    ...
    bool applyPieceNativeReuseForClass(int owner, int cid,
        ...
        const bool forceSkeleton =
#ifdef LOCAL
            connectorSkeletonForceEnabled();
#else
            false;
#endif
        ...
        if (!(canDeltaPreserved || canConnectorSkeleton)) {
            ...
            if (release_phase_diag_enabled()) g_release_phase_diag.reuse_route_baseline_calls++;
            ScopedReleasePhaseTimer __release_route_timer(release_phase_diag_enabled() ? &g_release_phase_diag.reuse_route_baseline_ns : nullptr);
#ifdef LOCAL
        ...
        if (canDeltaPreserved) {
            ...
            g_batch_dbg.debug_unanimous_state_old_field_read++;
#endif
            if (release_phase_diag_enabled()) g_release_phase_diag.reuse_route_delta_preserved_then_skeleton_calls++;
            ScopedReleasePhaseTimer __release_route_timer(release_phase_diag_enabled() ? &g_release_phase_diag.reuse_route_delta_preserved_then_skeleton_ns : nullptr);
#ifdef LOCAL
        ...
        if (canConnectorSkeleton) {
            if (release_phase_diag_enabled()) g_release_phase_diag.reuse_route_connector_skeleton_calls++;
            ScopedReleasePhaseTimer __release_route_timer(release_phase_diag_enabled() ? &g_release_phase_diag.reuse_route_connector_skeleton_ns : nullptr);
#ifdef LOCAL
        ...
        if (release_phase_diag_enabled()) g_release_phase_diag.reuse_route_general_delta_calls++;
        ScopedReleasePhaseTimer __release_route_timer(release_phase_diag_enabled() ? &g_release_phase_diag.reuse_route_general_delta_ns : nullptr);
#ifdef LOCAL
...
public:
    ...
    void eraseVertex(int x, vector<int>& newComponents, vector<WitnessChange>& changes) override {
        const bool __release_phase_diag = release_phase_diag_enabled();
        ScopedReleasePhaseTimer __erase_phase_timer(__release_phase_diag ? &g_release_phase_diag.erase_total_ns : nullptr);
        if (__release_phase_diag) g_release_phase_diag.erase_calls++;
        newComponents.clear();
        changes.clear();
        ...
        const long long __pre_chit_prev_state_writeback_calls = __need_deletion_profile ? g_batch_dbg.chit_prev_state_writeback_calls : 0;
        const long long __pre_chit_prev_state_scalar_store_calls = __need_deletion_profile ? g_batch_dbg.chit_prev_state_scalar_store_calls : 0;
        const long long __pre_chit_cursor_advance_calls = __need_deletion_profile ? g_batch_dbg.chit_cursor_advance_calls : 0;
        const long long __pre_chit_next_carry_prepare_calls = __need_deletion_profile ? g_batch_dbg.chit_next_carry_prepare_calls : 0;
        const long long __pre_chit_hit_bookkeeping_commits = __need_deletion_profile ? g_batch_dbg.chit_hit_bookkeeping_commits : 0;
        const long long __pre_chit_skip_pack_hits = __need_deletion_profile ? g_batch_dbg.chit_skip_pack_hits : 0;
        const long long __pre_chit_skip_reseed_hits = __need_deletion_profile ? g_batch_dbg.chit_skip_reseed_hits : 0;
        const long long __pre_time_chit_prev_state_writeback_ns = __need_deletion_profile ? g_batch_dbg.time_chit_prev_state_writeback_ns : 0;
        const long long __pre_time_chit_prev_state_scalar_store_ns = __need_deletion_profile ? g_batch_dbg.time_chit_prev_state_scalar_store_ns : 0;
        const long long __pre_time_chit_cursor_advance_ns = __need_deletion_profile ? g_batch_dbg.time_chit_cursor_advance_ns : 0;
        const long long __pre_time_chit_next_carry_prepare_ns = __need_deletion_profile ? g_batch_dbg.time_chit_next_carry_prepare_ns : 0;
        const long long __pre_time_chit_hit_bookkeeping_commit_ns = __need_deletion_profile ? g_batch_dbg.time_chit_hit_bookkeeping_commit_ns : 0;
        const long long __pre_time_chit_skip_pack_and_reseed_suppression_ns = __need_deletion_profile ? g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_ns : 0;
        const long long __pre_pwrite_noop_guard_hits = __need_deletion_profile ? g_batch_dbg.pwrite_noop_guard_hits : 0;
        const long long __pre_pwrite_target_resolve_calls = __need_deletion_profile ? g_batch_dbg.pwrite_target_resolve_calls : 0;
        const long long __pre_pwrite_target_resolve_cache_hits = __need_deletion_profile ? g_batch_dbg.pwrite_target_resolve_cache_hits : 0;
        const long long __pre_pwrite_scalar_field_store_calls = __need_deletion_profile ? g_batch_dbg.pwrite_scalar_field_store_calls : 0;
        const long long __pre_pwrite_compact_pack_store_calls = __need_deletion_profile ? g_batch_dbg.pwrite_compact_pack_store_calls : 0;
        const long long __pre_pwrite_same_value_skip_hits = __need_deletion_profile ? g_batch_dbg.pwrite_same_value_skip_hits : 0;
        const long long __pre_time_pwrite_noop_guard_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_noop_guard_ns : 0;
        const long long __pre_time_pwrite_target_resolve_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_target_resolve_ns : 0;
        const long long __pre_time_pwrite_scalar_field_store_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_scalar_field_store_ns : 0;
        const long long __pre_time_pwrite_compact_pack_store_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_compact_pack_store_ns : 0;
        const long long __pre_time_pwrite_same_value_skip_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_same_value_skip_ns : 0;
        const long long __pre_time_pwrite_route_pointer_hoist_miss_ns = __need_deletion_profile ? g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns : 0;
        const long long __pre_dprebind_cached_pointer_validate_calls = __need_deletion_profile ? g_batch_dbg.dprebind_cached_pointer_validate_calls : 0;
        const long long __pre_dprebind_slot_owner_match_checks = __need_deletion_profile ? g_batch_dbg.dprebind_slot_owner_match_checks : 0;
        const long long __pre_dprebind_stale_pointer_detect_hits = __need_deletion_profile ? g_batch_dbg.dprebind_stale_pointer_detect_hits : 0;
        const long long __pre_dprebind_rebind_target_lookup_calls = __need_deletion_profile ? g_batch_dbg.dprebind_rebind_target_lookup_calls : 0;
        const long long __pre_dprebind_rebind_commit_calls = __need_deletion_profile ? g_batch_dbg.dprebind_rebind_commit_calls : 0;
        const long long __pre_dprebind_metadata_patch_calls = __need_deletion_profile ? g_batch_dbg.dprebind_metadata_patch_calls : 0;
        const long long __pre_dprebind_same_pointer_fastpath_hits = __need_deletion_profile ? g_batch_dbg.dprebind_same_pointer_fastpath_hits : 0;
        const long long __pre_time_dprebind_total_ns = __need_deletion_profile ? (g_batch_dbg.time_dprebind_cached_pointer_validate_ns + g_batch_dbg.time_dprebind_slot_owner_match_check_ns + g_batch_dbg.time_dprebind_same_pointer_fastpath_ns + g_batch_dbg.time_dprebind_stale_pointer_detect_ns + g_batch_dbg.time_dprebind_rebind_target_lookup_ns + g_batch_dbg.time_dprebind_rebind_commit_ns + g_batch_dbg.time_dprebind_metadata_patch_ns + g_batch_dbg.time_dprebind_connector_hotpath_pin_apply_ns) : 0;
        const long long __pre_mpatch_same_target_patch_skip_hits = __need_deletion_profile ? g_batch_dbg.mpatch_same_target_patch_skip_hits : 0;
        const long long __pre_mpatch_noop_commit_hits = __need_deletion_profile ? g_batch_dbg.mpatch_noop_commit_hits : 0;
        const long long __pre_mpatch_metadata_field_patch_calls = __need_deletion_profile ? g_batch_dbg.mpatch_metadata_field_patch_calls : 0;
        const long long __pre_mpatch_metadata_field_pack_norm_calls = __need_deletion_profile ? g_batch_dbg.mpatch_metadata_field_pack_norm_calls : 0;
        const long long __pre_mpatch_slot_owner_lookup_calls = __need_deletion_profile ? g_batch_dbg.mpatch_slot_owner_lookup_calls : 0;
        const long long __pre_mpatch_slot_owner_patch_calls = __need_deletion_profile ? g_batch_dbg.mpatch_slot_owner_patch_calls : 0;
        const long long __pre_mpatch_owner_side_index_touch_calls = __need_deletion_profile ? g_batch_dbg.mpatch_owner_side_index_touch_calls : 0;
        const long long __pre_mpatch_connector_hotpath_patch_apply_calls = __need_deletion_profile ? g_batch_dbg.mpatch_connector_hotpath_patch_apply_calls : 0;
        const long long __pre_time_mpatch_total_ns = __need_deletion_profile ? (g_batch_dbg.time_mpatch_same_target_patch_skip_ns + g_batch_dbg.time_mpatch_noop_commit_check_ns + g_batch_dbg.time_mpatch_metadata_field_patch_ns + g_batch_dbg.time_mpatch_metadata_field_pack_norm_ns + g_batch_dbg.time_mpatch_slot_owner_lookup_ns + g_batch_dbg.time_mpatch_slot_owner_patch_ns + g_batch_dbg.time_mpatch_owner_side_index_touch_ns + g_batch_dbg.time_mpatch_connector_hotpath_patch_apply_ns) : 0;
        const long long __pre_cpack_same_layout_skip_hits = __need_deletion_profile ? g_batch_dbg.cpack_same_layout_skip_hits : 0;
        const long long __pre_cpack_field_gather_calls = __need_deletion_profile ? g_batch_dbg.cpack_field_gather_calls : 0;
        const long long __pre_cpack_field_gather_fields = __need_deletion_profile ? g_batch_dbg.cpack_field_gather_fields : 0;
        const long long __pre_cpack_staging_buffer_prepare_calls = __need_deletion_profile ? g_batch_dbg.cpack_staging_buffer_prepare_calls : 0;
        const long long __pre_cpack_pack_encode_calls = __need_deletion_profile ? g_batch_dbg.cpack_pack_encode_calls : 0;
        const long long __pre_cpack_pack_normalize_calls = __need_deletion_profile ? g_batch_dbg.cpack_pack_normalize_calls : 0;
        const long long __pre_cpack_connector_hotpath_pack_calls = __need_deletion_profile ? g_batch_dbg.cpack_connector_hotpath_pack_calls : 0;
        const long long __pre_time_cpack_total_ns = __need_deletion_profile ? (g_batch_dbg.time_cpack_same_layout_check_ns + g_batch_dbg.time_cpack_same_layout_skip_apply_ns + g_batch_dbg.time_cpack_field_gather_ns + g_batch_dbg.time_cpack_staging_buffer_prepare_ns + g_batch_dbg.time_cpack_pack_encode_ns + g_batch_dbg.time_cpack_pack_normalize_ns + g_batch_dbg.time_cpack_connector_hotpath_pack_ns) : 0;
        const long long __pre_time_cpack_same_layout_check_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_same_layout_check_ns : 0;
        const long long __pre_time_cpack_same_layout_skip_apply_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_same_layout_skip_apply_ns : 0;
        const long long __pre_time_cpack_field_gather_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_field_gather_ns : 0;
        const long long __pre_time_cpack_staging_buffer_prepare_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_staging_buffer_prepare_ns : 0;
        const long long __pre_time_cpack_pack_encode_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_pack_encode_ns : 0;
        const long long __pre_time_cpack_pack_normalize_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_pack_normalize_ns : 0;
        const long long __pre_time_cpack_connector_hotpath_pack_ns = __need_deletion_profile ? g_batch_dbg.time_cpack_connector_hotpath_pack_ns : 0;
        ...
        const long long __pre_time_tbranch_prev_state_materialize_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_prev_state_materialize_ns : 0;
        const long long __pre_time_tbranch_curr_state_load_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_curr_state_load_ns : 0;
        const long long __pre_time_tbranch_compare_classify_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_compare_classify_ns : 0;
        const long long __pre_time_tbranch_removed_to_kept_dispatch_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_removed_to_kept_dispatch_ns : 0;
        const long long __pre_time_tbranch_kept_to_removed_dispatch_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_kept_to_removed_dispatch_ns : 0;
        const long long __pre_time_tbranch_no_transition_advance_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_no_transition_advance_ns : 0;
        const long long __pre_time_tbranch_loop_state_update_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_loop_state_update_ns : 0;
        const long long __pre_time_tbranch_route_baseline_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_route_baseline_ns : 0;
        const long long __pre_time_tbranch_route_delta_preserved_then_skeleton_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_route_delta_preserved_then_skeleton_ns : 0;
        const long long __pre_time_tbranch_route_connector_skeleton_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_route_connector_skeleton_ns : 0;
        const long long __pre_time_tbranch_route_general_delta_ns = __need_deletion_profile ? g_batch_dbg.time_tbranch_route_general_delta_ns : 0;
        ...
        const long long __pre_tbranch_prev_state_materialize_calls = __need_deletion_profile ? g_batch_dbg.tbranch_prev_state_materialize_calls : 0;
        const long long __pre_tbranch_curr_state_load_calls = __need_deletion_profile ? g_batch_dbg.tbranch_curr_state_load_calls : 0;
        const long long __pre_tbranch_compare_ops = __need_deletion_profile ? g_batch_dbg.tbranch_compare_ops : 0;
        const long long __pre_tbranch_removed_to_kept_taken = __need_deletion_profile ? g_batch_dbg.tbranch_removed_to_kept_taken : 0;
        const long long __pre_tbranch_kept_to_removed_taken = __need_deletion_profile ? g_batch_dbg.tbranch_kept_to_removed_taken : 0;
        const long long __pre_tbranch_no_transition_steps = __need_deletion_profile ? g_batch_dbg.tbranch_no_transition_steps : 0;
        const long long __pre_tbranch_window_clipped_calls = __need_deletion_profile ? g_batch_dbg.tbranch_window_clipped_calls : 0;
        const long long __pre_time_sload_prev_seed_materialize_ns = __need_deletion_profile ? g_batch_dbg.time_sload_prev_seed_materialize_ns : 0;
        const long long __pre_time_sload_prev_carry_reuse_check_ns = __need_deletion_profile ? g_batch_dbg.time_sload_prev_carry_reuse_check_ns : 0;
        const long long __pre_time_sload_prev_carry_reuse_apply_ns = __need_deletion_profile ? g_batch_dbg.time_sload_prev_carry_reuse_apply_ns : 0;
        const long long __pre_time_pcarry_boundary_seed_bootstrap_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_boundary_seed_bootstrap_ns : 0;
        const long long __pre_time_pcarry_prev_scalar_materialize_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_prev_scalar_materialize_ns : 0;
        const long long __pre_time_pcarry_prev_scalar_pack_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_prev_scalar_pack_ns : 0;
        const long long __pre_time_pcarry_carry_reuse_check_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_carry_reuse_check_ns : 0;
        const long long __pre_time_pcarry_carry_reuse_hit_apply_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_carry_reuse_hit_apply_ns : 0;
        const long long __pre_time_pcarry_carry_invalidate_transition_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_carry_invalidate_transition_ns : 0;
        const long long __pre_time_pcarry_carry_invalidate_window_clip_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_carry_invalidate_window_clip_ns : 0;
        const long long __pre_time_pcarry_reseed_after_invalidate_ns = __need_deletion_profile ? g_batch_dbg.time_pcarry_reseed_after_invalidate_ns : 0;
        const long long __pre_time_sload_curr_direct_load_ns = __need_deletion_profile ? g_batch_dbg.time_sload_curr_direct_load_ns : 0;
        const long long __pre_time_sload_curr_source_lookup_ns = __need_deletion_profile ? g_batch_dbg.time_sload_curr_source_lookup_ns : 0;
        const long long __pre_time_sload_state_pair_pack_ns = __need_deletion_profile ? g_batch_dbg.time_sload_state_pair_pack_ns : 0;
        const long long __pre_time_sload_invariant_ref_hoist_miss_ns = __need_deletion_profile ? g_batch_dbg.time_sload_invariant_ref_hoist_miss_ns : 0;
        const long long __pre_time_sload_route_baseline_ns = __need_deletion_profile ? g_batch_dbg.time_sload_route_baseline_ns : 0;
        const long long __pre_time_sload_route_delta_preserved_then_skeleton_ns = __need_deletion_profile ? g_batch_dbg.time_sload_route_delta_preserved_then_skeleton_ns : 0;
        const long long __pre_time_sload_route_connector_skeleton_ns = __need_deletion_profile ? g_batch_dbg.time_sload_route_connector_skeleton_ns : 0;
        const long long __pre_time_sload_route_general_delta_ns = __need_deletion_profile ? g_batch_dbg.time_sload_route_general_delta_ns : 0;
        const long long __pre_sload_prev_seed_calls = __need_deletion_profile ? g_batch_dbg.sload_prev_seed_calls : 0;
        const long long __pre_sload_prev_carry_reuse_hits = __need_deletion_profile ? g_batch_dbg.sload_prev_carry_reuse_hits : 0;
        const long long __pre_pcarry_prev_seed_calls = __need_deletion_profile ? g_batch_dbg.pcarry_prev_seed_calls : 0;
        const long long __pre_pcarry_prev_seed_from_boundary_calls = __need_deletion_profile ? g_batch_dbg.pcarry_prev_seed_from_boundary_calls : 0;
        const long long __pre_pcarry_prev_seed_from_reseed_calls = __need_deletion_profile ? g_batch_dbg.pcarry_prev_seed_from_reseed_calls : 0;
        const long long __pre_pcarry_prev_scalar_materialize_calls = __need_deletion_profile ? g_batch_dbg.pcarry_prev_scalar_materialize_calls : 0;
        const long long __pre_pcarry_carry_reuse_hits = __need_deletion_profile ? g_batch_dbg.pcarry_carry_reuse_hits : 0;
        const long long __pre_pcarry_carry_reuse_misses = __need_deletion_profile ? g_batch_dbg.pcarry_carry_reuse_misses : 0;
        const long long __pre_pcarry_transition_invalidate_calls = __need_deletion_profile ? g_batch_dbg.pcarry_carry_invalidate_transition_calls : 0;
        const long long __pre_pcarry_window_clip_invalidate_calls = __need_deletion_profile ? g_batch_dbg.pcarry_carry_invalidate_window_clip_calls : 0;
        const long long __pre_sload_curr_direct_load_calls = __need_deletion_profile ? g_batch_dbg.sload_curr_direct_load_calls : 0;
        const long long __pre_sload_curr_source_lookup_calls = __need_deletion_profile ? g_batch_dbg.sload_curr_source_lookup_calls : 0;
        const long long __pre_sload_curr_source_lookup_misses = __need_deletion_profile ? g_batch_dbg.sload_curr_source_lookup_misses : 0;
        const long long __pre_sload_state_pair_pack_calls = __need_deletion_profile ? g_batch_dbg.sload_state_pair_pack_calls : 0;
        const long long __pre_sload_invariant_ref_hoist_hits = __need_deletion_profile ? g_batch_dbg.sload_invariant_ref_hoist_hits : 0;
        ...
        {
            ScopedReleasePhaseTimer __delete_split_timer(__release_phase_diag ? &g_release_phase_diag.delete_split_ns : nullptr);
            if (__release_phase_diag) g_release_phase_diag.delete_split_calls++;
        ...
        if (1 <= x && x <= n_) {
            ...
            ScopedReleasePhaseTimer __owner_dead_resolve_timer(__release_phase_diag ? &g_release_phase_diag.owner_dead_resolve_ns : nullptr);
            if (__release_phase_diag) g_release_phase_diag.owner_dead_resolve_calls++;
        ...
        {
            ScopedReleasePhaseTimer __touched_loop_timer(__release_phase_diag ? &g_release_phase_diag.touched_loop_ns : nullptr);
            if (__release_phase_diag) g_release_phase_diag.touched_loop_calls++;
            for (const auto& info : touchedInfos) {
            int owner = info.owner;
            if (!topo_.aliveVertex(owner)) {
#ifdef LOCAL
...
#endif
            CoverageCollapseResult refineRes;
            {
                ScopedReleasePhaseTimer __refine_timer(__release_phase_diag ? &g_release_phase_diag.touched_refine_ns : nullptr);
                refineRes = refineTouchedClassCoverageCollapse(x, info, ownerCtxCache);
            }
            vector<int> candidateCids = refineRes.candidateCids;
            {
                ScopedReleasePhaseTimer __endpoint_relabel_timer(__release_phase_diag ? &g_release_phase_diag.touched_endpoint_relabel_ns : nullptr);
            ...
            {
                ScopedReleasePhaseTimer __query_scan_timer(__release_phase_diag ? &g_release_phase_diag.touched_query_scan_ns : nullptr);
                for (int idx : refineRes.movedIdxs) {
                    ...
                    for (int qid : od.incidentQids[idx]) {
                        if (__release_phase_diag) g_release_phase_diag.touched_query_scan_iters++;
#ifdef LOCAL
            ...
            if (!refineRes.usedFullScan && refineRes.reuseKind != SupportReuseKind::None) {
                ScopedReleasePhaseTimer __reuse_timer(__release_phase_diag ? &g_release_phase_diag.touched_reuse_ns : nullptr);
...
#endif
            {
                ScopedReleasePhaseTimer __candidate_finalize_timer(__release_phase_diag ? &g_release_phase_diag.touched_candidate_finalize_ns : nullptr);
        ...
        if (__need_deletion_profile) {
            ...
            __rec.chitPrevStateWritebackCalls = g_batch_dbg.chit_prev_state_writeback_calls - __pre_chit_prev_state_writeback_calls;
            __rec.chitPrevStateScalarStoreCalls = g_batch_dbg.chit_prev_state_scalar_store_calls - __pre_chit_prev_state_scalar_store_calls;
            __rec.chitCursorAdvanceCalls = g_batch_dbg.chit_cursor_advance_calls - __pre_chit_cursor_advance_calls;
            __rec.chitNextCarryPrepareCalls = g_batch_dbg.chit_next_carry_prepare_calls - __pre_chit_next_carry_prepare_calls;
            __rec.chitHitBookkeepingCommits = g_batch_dbg.chit_hit_bookkeeping_commits - __pre_chit_hit_bookkeeping_commits;
            __rec.chitSkipPackHits = g_batch_dbg.chit_skip_pack_hits - __pre_chit_skip_pack_hits;
            __rec.chitSkipReseedHits = g_batch_dbg.chit_skip_reseed_hits - __pre_chit_skip_reseed_hits;
            __rec.timeChitTotalNs = (g_batch_dbg.time_chit_prev_state_writeback_ns - __pre_time_chit_prev_state_writeback_ns) + (g_batch_dbg.time_chit_prev_state_scalar_store_ns - __pre_time_chit_prev_state_scalar_store_ns) + (g_batch_dbg.time_chit_cursor_advance_ns - __pre_time_chit_cursor_advance_ns) + (g_batch_dbg.time_chit_next_carry_prepare_ns - __pre_time_chit_next_carry_prepare_ns) + (g_batch_dbg.time_chit_hit_bookkeeping_commit_ns - __pre_time_chit_hit_bookkeeping_commit_ns) + (g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_ns - __pre_time_chit_skip_pack_and_reseed_suppression_ns);
            __rec.pwriteNoopGuardHits = g_batch_dbg.pwrite_noop_guard_hits - __pre_pwrite_noop_guard_hits;
            __rec.pwriteTargetResolveCalls = g_batch_dbg.pwrite_target_resolve_calls - __pre_pwrite_target_resolve_calls;
            __rec.pwriteTargetResolveCacheHits = g_batch_dbg.pwrite_target_resolve_cache_hits - __pre_pwrite_target_resolve_cache_hits;
            __rec.pwriteScalarFieldStoreCalls = g_batch_dbg.pwrite_scalar_field_store_calls - __pre_pwrite_scalar_field_store_calls;
            __rec.pwriteCompactPackStoreCalls = g_batch_dbg.pwrite_compact_pack_store_calls - __pre_pwrite_compact_pack_store_calls;
            __rec.pwriteSameValueSkipHits = g_batch_dbg.pwrite_same_value_skip_hits - __pre_pwrite_same_value_skip_hits;
            __rec.timePwriteTotalNs = (g_batch_dbg.time_pwrite_noop_guard_ns - __pre_time_pwrite_noop_guard_ns) +
                                     (g_batch_dbg.time_pwrite_target_resolve_ns - __pre_time_pwrite_target_resolve_ns) +
                                     (g_batch_dbg.time_pwrite_scalar_field_store_ns - __pre_time_pwrite_scalar_field_store_ns) +
                                     (g_batch_dbg.time_pwrite_compact_pack_store_ns - __pre_time_pwrite_compact_pack_store_ns) +
                                     (g_batch_dbg.time_pwrite_same_value_skip_ns - __pre_time_pwrite_same_value_skip_ns) +
                                     (g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns - __pre_time_pwrite_route_pointer_hoist_miss_ns);
            ...
            long long __tbranch_route_baseline = g_batch_dbg.time_tbranch_route_baseline_ns - __pre_time_tbranch_route_baseline_ns;
            long long __tbranch_route_delta = g_batch_dbg.time_tbranch_route_delta_preserved_then_skeleton_ns - __pre_time_tbranch_route_delta_preserved_then_skeleton_ns;
            long long __tbranch_route_conn = g_batch_dbg.time_tbranch_route_connector_skeleton_ns - __pre_time_tbranch_route_connector_skeleton_ns;
            long long __tbranch_route_general = g_batch_dbg.time_tbranch_route_general_delta_ns - __pre_time_tbranch_route_general_delta_ns;
            __rec.tbranchRouteTag = REUSE_ROUTE_NONE;
            long long __best_tbranch_route_ns = 0;
            if (__tbranch_route_baseline > __best_tbranch_route_ns) { __best_tbranch_route_ns = __tbranch_route_baseline; __rec.tbranchRouteTag = REUSE_ROUTE_BASELINE; }
            if (__tbranch_route_delta > __best_tbranch_route_ns) { __best_tbranch_route_ns = __tbranch_route_delta; __rec.tbranchRouteTag = REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON; }
            if (__tbranch_route_conn > __best_tbranch_route_ns) { __best_tbranch_route_ns = __tbranch_route_conn; __rec.tbranchRouteTag = REUSE_ROUTE_CONNECTOR_SKELETON; }
            if (__tbranch_route_general > __best_tbranch_route_ns) { __best_tbranch_route_ns = __tbranch_route_general; __rec.tbranchRouteTag = REUSE_ROUTE_GENERAL_DELTA; }
            __rec.tbranchPrevStateMaterializeCalls = g_batch_dbg.tbranch_prev_state_materialize_calls - __pre_tbranch_prev_state_materialize_calls;
            __rec.tbranchCurrStateLoadCalls = g_batch_dbg.tbranch_curr_state_load_calls - __pre_tbranch_curr_state_load_calls;
            __rec.tbranchCompareOps = g_batch_dbg.tbranch_compare_ops - __pre_tbranch_compare_ops;
            __rec.tbranchRemovedToKeptTaken = g_batch_dbg.tbranch_removed_to_kept_taken - __pre_tbranch_removed_to_kept_taken;
            __rec.tbranchKeptToRemovedTaken = g_batch_dbg.tbranch_kept_to_removed_taken - __pre_tbranch_kept_to_removed_taken;
            __rec.tbranchNoTransitionSteps = g_batch_dbg.tbranch_no_transition_steps - __pre_tbranch_no_transition_steps;
            __rec.tbranchWindowClippedCalls = g_batch_dbg.tbranch_window_clipped_calls - __pre_tbranch_window_clipped_calls;
            __rec.timeTbranchTotalNs = (g_batch_dbg.time_tbranch_prev_state_materialize_ns - __pre_time_tbranch_prev_state_materialize_ns) +
                                       (g_batch_dbg.time_tbranch_curr_state_load_ns - __pre_time_tbranch_curr_state_load_ns) +
                                       (g_batch_dbg.time_tbranch_compare_classify_ns - __pre_time_tbranch_compare_classify_ns) +
                                       (g_batch_dbg.time_tbranch_removed_to_kept_dispatch_ns - __pre_time_tbranch_removed_to_kept_dispatch_ns) +
                                       (g_batch_dbg.time_tbranch_kept_to_removed_dispatch_ns - __pre_time_tbranch_kept_to_removed_dispatch_ns) +
                                       (g_batch_dbg.time_tbranch_no_transition_advance_ns - __pre_time_tbranch_no_transition_advance_ns) +
                                       (g_batch_dbg.time_tbranch_loop_state_update_ns - __pre_time_tbranch_loop_state_update_ns);
            long long __sload_route_baseline = g_batch_dbg.time_sload_route_baseline_ns - __pre_time_sload_route_baseline_ns;
            long long __sload_route_delta = g_batch_dbg.time_sload_route_delta_preserved_then_skeleton_ns - __pre_time_sload_route_delta_preserved_then_skeleton_ns;
            long long __sload_route_conn = g_batch_dbg.time_sload_route_connector_skeleton_ns - __pre_time_sload_route_connector_skeleton_ns;
            long long __sload_route_general = g_batch_dbg.time_sload_route_general_delta_ns - __pre_time_sload_route_general_delta_ns;
            __rec.sloadRouteTag = REUSE_ROUTE_NONE;
            long long __best_sload_route_ns = 0;
            if (__sload_route_baseline > __best_sload_route_ns) { __best_sload_route_ns = __sload_route_baseline; __rec.sloadRouteTag = REUSE_ROUTE_BASELINE; }
            if (__sload_route_delta > __best_sload_route_ns) { __best_sload_route_ns = __sload_route_delta; __rec.sloadRouteTag = REUSE_ROUTE_DELTA_PRESERVED_THEN_SKELETON; }
            if (__sload_route_conn > __best_sload_route_ns) { __best_sload_route_ns = __sload_route_conn; __rec.sloadRouteTag = REUSE_ROUTE_CONNECTOR_SKELETON; }
            if (__sload_route_general > __best_sload_route_ns) { __best_sload_route_ns = __sload_route_general; __rec.sloadRouteTag = REUSE_ROUTE_GENERAL_DELTA; }
            __rec.sloadPrevSeedCalls = g_batch_dbg.sload_prev_seed_calls - __pre_sload_prev_seed_calls;
            __rec.sloadPrevCarryReuseHits = g_batch_dbg.sload_prev_carry_reuse_hits - __pre_sload_prev_carry_reuse_hits;
            __rec.sloadCurrDirectLoadCalls = g_batch_dbg.sload_curr_direct_load_calls - __pre_sload_curr_direct_load_calls;
            __rec.sloadCurrSourceLookupCalls = g_batch_dbg.sload_curr_source_lookup_calls - __pre_sload_curr_source_lookup_calls;
            __rec.sloadCurrSourceLookupMisses = g_batch_dbg.sload_curr_source_lookup_misses - __pre_sload_curr_source_lookup_misses;
            __rec.sloadStatePairPackCalls = g_batch_dbg.sload_state_pair_pack_calls - __pre_sload_state_pair_pack_calls;
            __rec.sloadInvariantRefHoistHits = g_batch_dbg.sload_invariant_ref_hoist_hits - __pre_sload_invariant_ref_hoist_hits;
            __rec.pcarryRouteTag = __rec.sloadRouteTag;
            __rec.pcarryPrevSeedCalls = g_batch_dbg.pcarry_prev_seed_calls - __pre_pcarry_prev_seed_calls;
            __rec.pcarryPrevSeedFromBoundaryCalls = g_batch_dbg.pcarry_prev_seed_from_boundary_calls - __pre_pcarry_prev_seed_from_boundary_calls;
            __rec.pcarryPrevSeedFromReseedCalls = g_batch_dbg.pcarry_prev_seed_from_reseed_calls - __pre_pcarry_prev_seed_from_reseed_calls;
            __rec.pcarryPrevScalarMaterializeCalls = g_batch_dbg.pcarry_prev_scalar_materialize_calls - __pre_pcarry_prev_scalar_materialize_calls;
            __rec.pcarryCarryReuseHits = g_batch_dbg.pcarry_carry_reuse_hits - __pre_pcarry_carry_reuse_hits;
            __rec.pcarryCarryReuseMisses = g_batch_dbg.pcarry_carry_reuse_misses - __pre_pcarry_carry_reuse_misses;
            __rec.pcarryTransitionInvalidateCalls = g_batch_dbg.pcarry_carry_invalidate_transition_calls - __pre_pcarry_transition_invalidate_calls;
            __rec.pcarryWindowClipInvalidateCalls = g_batch_dbg.pcarry_carry_invalidate_window_clip_calls - __pre_pcarry_window_clip_invalidate_calls;
            __rec.timePcarryTotalNs = (g_batch_dbg.time_pcarry_boundary_seed_bootstrap_ns - __pre_time_pcarry_boundary_seed_bootstrap_ns) +
                                     (g_batch_dbg.time_pcarry_prev_scalar_materialize_ns - __pre_time_pcarry_prev_scalar_materialize_ns) +
                                     (g_batch_dbg.time_pcarry_prev_scalar_pack_ns - __pre_time_pcarry_prev_scalar_pack_ns) +
                                     (g_batch_dbg.time_pcarry_carry_reuse_check_ns - __pre_time_pcarry_carry_reuse_check_ns) +
                                     (g_batch_dbg.time_pcarry_carry_reuse_hit_apply_ns - __pre_time_pcarry_carry_reuse_hit_apply_ns) +
                                     (g_batch_dbg.time_pcarry_carry_invalidate_transition_ns - __pre_time_pcarry_carry_invalidate_transition_ns) +
                                     (g_batch_dbg.time_pcarry_carry_invalidate_window_clip_ns - __pre_time_pcarry_carry_invalidate_window_clip_ns) +
                                     (g_batch_dbg.time_pcarry_reseed_after_invalidate_ns - __pre_time_pcarry_reseed_after_invalidate_ns);
            __rec.timeSloadTotalNs = (g_batch_dbg.time_sload_prev_seed_materialize_ns - __pre_time_sload_prev_seed_materialize_ns) +
                                     (g_batch_dbg.time_sload_prev_carry_reuse_check_ns - __pre_time_sload_prev_carry_reuse_check_ns) +
                                     (g_batch_dbg.time_sload_prev_carry_reuse_apply_ns - __pre_time_sload_prev_carry_reuse_apply_ns) +
                                     (g_batch_dbg.time_sload_curr_direct_load_ns - __pre_time_sload_curr_direct_load_ns) +
                                     (g_batch_dbg.time_sload_curr_source_lookup_ns - __pre_time_sload_curr_source_lookup_ns) +
                                     (g_batch_dbg.time_sload_state_pair_pack_ns - __pre_time_sload_state_pair_pack_ns) +
                                     (g_batch_dbg.time_sload_invariant_ref_hoist_miss_ns - __pre_time_sload_invariant_ref_hoist_miss_ns);
...
int main(){
    ...
    if (release_phase_diag_enabled()) reset_release_phase_diag();
#ifdef LOCAL
    ...
    cout << '\n';
    emit_release_phase_diag_summary();
#ifdef LOCAL
    cerr << "owner_rebuild_calls=" << g_topo_dbg.dbg_owner_rebuild_calls
         << " local_updates=" << g_topo_dbg.dbg_owner_local_updates
         << " local_fallback=" << g_topo_dbg.dbg_owner_local_updates_fallback
    ...
    cerr << "fclass_calls=" << g_batch_dbg.fclass_calls
         ...
         << "\n";
    cerr << "time_tbranch_prev_state_materialize_ns=" << g_batch_dbg.time_tbranch_prev_state_materialize_ns
         << " time_tbranch_prev_state_materialize_calls=" << g_batch_dbg.time_tbranch_prev_state_materialize_calls
         << " time_tbranch_curr_state_load_ns=" << g_batch_dbg.time_tbranch_curr_state_load_ns
         << " time_tbranch_curr_state_load_calls=" << g_batch_dbg.time_tbranch_curr_state_load_calls
         << " time_tbranch_compare_classify_ns=" << g_batch_dbg.time_tbranch_compare_classify_ns
         << " time_tbranch_compare_classify_calls=" << g_batch_dbg.time_tbranch_compare_classify_calls
         << " time_tbranch_removed_to_kept_dispatch_ns=" << g_batch_dbg.time_tbranch_removed_to_kept_dispatch_ns
         << " time_tbranch_removed_to_kept_dispatch_calls=" << g_batch_dbg.time_tbranch_removed_to_kept_dispatch_calls
         << " time_tbranch_kept_to_removed_dispatch_ns=" << g_batch_dbg.time_tbranch_kept_to_removed_dispatch_ns
         << " time_tbranch_kept_to_removed_dispatch_calls=" << g_batch_dbg.time_tbranch_kept_to_removed_dispatch_calls
         << " time_tbranch_no_transition_advance_ns=" << g_batch_dbg.time_tbranch_no_transition_advance_ns
         << " time_tbranch_no_transition_advance_calls=" << g_batch_dbg.time_tbranch_no_transition_advance_calls
         << " time_tbranch_loop_state_update_ns=" << g_batch_dbg.time_tbranch_loop_state_update_ns
         << " time_tbranch_loop_state_update_calls=" << g_batch_dbg.time_tbranch_loop_state_update_calls
         << " time_tbranch_route_baseline_ns=" << g_batch_dbg.time_tbranch_route_baseline_ns
         << " time_tbranch_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_tbranch_route_delta_preserved_then_skeleton_ns
         << " time_tbranch_route_connector_skeleton_ns=" << g_batch_dbg.time_tbranch_route_connector_skeleton_ns
         << " time_tbranch_route_general_delta_ns=" << g_batch_dbg.time_tbranch_route_general_delta_ns
         << "\n";
    cerr << "tbranch_calls=" << g_batch_dbg.tbranch_calls
         << " tbranch_watch_handles_before=" << g_batch_dbg.tbranch_watch_handles_before
         << " tbranch_watch_handles_after=" << g_batch_dbg.tbranch_watch_handles_after
         << " tbranch_removed_handles=" << g_batch_dbg.tbranch_removed_handles
         << " tbranch_prev_state_materialize_calls=" << g_batch_dbg.tbranch_prev_state_materialize_calls
         << " tbranch_curr_state_load_calls=" << g_batch_dbg.tbranch_curr_state_load_calls
         << " tbranch_compare_ops=" << g_batch_dbg.tbranch_compare_ops
         << " tbranch_removed_to_kept_checks=" << g_batch_dbg.tbranch_removed_to_kept_checks
         << " tbranch_kept_to_removed_checks=" << g_batch_dbg.tbranch_kept_to_removed_checks
         << " tbranch_removed_to_kept_taken=" << g_batch_dbg.tbranch_removed_to_kept_taken
         << " tbranch_kept_to_removed_taken=" << g_batch_dbg.tbranch_kept_to_removed_taken
         << " tbranch_no_transition_steps=" << g_batch_dbg.tbranch_no_transition_steps
         << " tbranch_loop_state_update_calls=" << g_batch_dbg.tbranch_loop_state_update_calls
         << " tbranch_window_clipped_calls=" << g_batch_dbg.tbranch_window_clipped_calls
         << " tbranch_route_baseline_calls=" << g_batch_dbg.tbranch_route_baseline_calls
         << " tbranch_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.tbranch_route_delta_preserved_then_skeleton_calls
         << " tbranch_route_connector_skeleton_calls=" << g_batch_dbg.tbranch_route_connector_skeleton_calls
         << " tbranch_route_general_delta_calls=" << g_batch_dbg.tbranch_route_general_delta_calls
         << " tbranch_removed_ratio_ppm_sum=" << g_batch_dbg.tbranch_removed_ratio_ppm_sum
         << " tbranch_prev_state_reuse_hits=" << g_batch_dbg.tbranch_prev_state_reuse_hits
         << " tbranch_curr_state_direct_load_hits=" << g_batch_dbg.tbranch_curr_state_direct_load_hits
         << " tbranch_branchless_codepath_hits=" << g_batch_dbg.tbranch_branchless_codepath_hits
         << " tbranch_tail_skip_hits=" << g_batch_dbg.tbranch_tail_skip_hits
         << "\n";
    cerr << "time_pcarry_boundary_seed_bootstrap_ns=" << g_batch_dbg.time_pcarry_boundary_seed_bootstrap_ns
         << " time_pcarry_boundary_seed_bootstrap_calls=" << g_batch_dbg.time_pcarry_boundary_seed_bootstrap_calls
         << " time_pcarry_prev_scalar_materialize_ns=" << g_batch_dbg.time_pcarry_prev_scalar_materialize_ns
         << " time_pcarry_prev_scalar_materialize_calls=" << g_batch_dbg.time_pcarry_prev_scalar_materialize_calls
         << " time_pcarry_prev_scalar_pack_ns=" << g_batch_dbg.time_pcarry_prev_scalar_pack_ns
         << " time_pcarry_prev_scalar_pack_calls=" << g_batch_dbg.time_pcarry_prev_scalar_pack_calls
         << " time_pcarry_carry_reuse_check_ns=" << g_batch_dbg.time_pcarry_carry_reuse_check_ns
         << " time_pcarry_carry_reuse_check_calls=" << g_batch_dbg.time_pcarry_carry_reuse_check_calls
         << " time_pcarry_carry_reuse_hit_apply_ns=" << g_batch_dbg.time_pcarry_carry_reuse_hit_apply_ns
         << " time_pcarry_carry_reuse_hit_apply_calls=" << g_batch_dbg.time_pcarry_carry_reuse_hit_apply_calls
         << " time_pcarry_carry_invalidate_transition_ns=" << g_batch_dbg.time_pcarry_carry_invalidate_transition_ns
         << " time_pcarry_carry_invalidate_transition_calls=" << g_batch_dbg.time_pcarry_carry_invalidate_transition_calls
         << " time_pcarry_carry_invalidate_window_clip_ns=" << g_batch_dbg.time_pcarry_carry_invalidate_window_clip_ns
         << " time_pcarry_carry_invalidate_window_clip_calls=" << g_batch_dbg.time_pcarry_carry_invalidate_window_clip_calls
         << " time_pcarry_reseed_after_invalidate_ns=" << g_batch_dbg.time_pcarry_reseed_after_invalidate_ns
         << " time_pcarry_reseed_after_invalidate_calls=" << g_batch_dbg.time_pcarry_reseed_after_invalidate_calls
         << " time_pcarry_route_baseline_ns=" << g_batch_dbg.time_pcarry_route_baseline_ns
         << " time_pcarry_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pcarry_route_delta_preserved_then_skeleton_ns
         << " time_pcarry_route_connector_skeleton_ns=" << g_batch_dbg.time_pcarry_route_connector_skeleton_ns
         << " time_pcarry_route_general_delta_ns=" << g_batch_dbg.time_pcarry_route_general_delta_ns
         << "\n";
    cerr << "pcarry_calls=" << g_batch_dbg.pcarry_calls
         << " pcarry_watch_handles_before=" << g_batch_dbg.pcarry_watch_handles_before
         << " pcarry_watch_handles_after=" << g_batch_dbg.pcarry_watch_handles_after
         << " pcarry_removed_handles=" << g_batch_dbg.pcarry_removed_handles
         << " pcarry_prev_seed_calls=" << g_batch_dbg.pcarry_prev_seed_calls
         << " pcarry_prev_seed_from_boundary_calls=" << g_batch_dbg.pcarry_prev_seed_from_boundary_calls
         << " pcarry_prev_seed_from_reseed_calls=" << g_batch_dbg.pcarry_prev_seed_from_reseed_calls
         << " pcarry_prev_scalar_materialize_calls=" << g_batch_dbg.pcarry_prev_scalar_materialize_calls
         << " pcarry_prev_scalar_pack_calls=" << g_batch_dbg.pcarry_prev_scalar_pack_calls
         << " pcarry_carry_reuse_checks=" << g_batch_dbg.pcarry_carry_reuse_checks
         << " pcarry_carry_reuse_hits=" << g_batch_dbg.pcarry_carry_reuse_hits
         << " pcarry_carry_reuse_misses=" << g_batch_dbg.pcarry_carry_reuse_misses
         << " pcarry_carry_invalidate_transition_calls=" << g_batch_dbg.pcarry_carry_invalidate_transition_calls
         << " pcarry_carry_invalidate_window_clip_calls=" << g_batch_dbg.pcarry_carry_invalidate_window_clip_calls
         << " pcarry_reseed_after_invalidate_calls=" << g_batch_dbg.pcarry_reseed_after_invalidate_calls
         << " pcarry_route_baseline_calls=" << g_batch_dbg.pcarry_route_baseline_calls
         << " pcarry_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pcarry_route_delta_preserved_then_skeleton_calls
         << " pcarry_route_connector_skeleton_calls=" << g_batch_dbg.pcarry_route_connector_skeleton_calls
         << " pcarry_route_general_delta_calls=" << g_batch_dbg.pcarry_route_general_delta_calls
         << " pcarry_prev_seed_reuse_without_materialize_hits=" << g_batch_dbg.pcarry_prev_seed_reuse_without_materialize_hits
         << " pcarry_scalar_pack_skipped_calls=" << g_batch_dbg.pcarry_scalar_pack_skipped_calls
         << " pcarry_window_clip_forced_reseed_hits=" << g_batch_dbg.pcarry_window_clip_forced_reseed_hits
         << " pcarry_transition_forced_reseed_hits=" << g_batch_dbg.pcarry_transition_forced_reseed_hits
         << "\n";
    cerr << "time_creuse_eligibility_gate_ns=" << g_batch_dbg.time_creuse_eligibility_gate_ns
         << " time_creuse_eligibility_gate_calls=" << g_batch_dbg.time_creuse_eligibility_gate_calls
         << " time_creuse_boundary_validity_check_ns=" << g_batch_dbg.time_creuse_boundary_validity_check_ns
         << " time_creuse_boundary_validity_check_calls=" << g_batch_dbg.time_creuse_boundary_validity_check_calls
         << " time_creuse_prev_scalar_eq_check_ns=" << g_batch_dbg.time_creuse_prev_scalar_eq_check_ns
         << " time_creuse_prev_scalar_eq_check_calls=" << g_batch_dbg.time_creuse_prev_scalar_eq_check_calls
         << " time_creuse_transition_consistency_check_ns=" << g_batch_dbg.time_creuse_transition_consistency_check_ns
         << " time_creuse_transition_consistency_check_calls=" << g_batch_dbg.time_creuse_transition_consistency_check_calls
         << " time_creuse_hit_apply_prev_state_ns=" << g_batch_dbg.time_creuse_hit_apply_prev_state_ns
         << " time_creuse_hit_apply_prev_state_calls=" << g_batch_dbg.time_creuse_hit_apply_prev_state_calls
         << " time_creuse_hit_apply_cursor_advance_ns=" << g_batch_dbg.time_creuse_hit_apply_cursor_advance_ns
         << " time_creuse_hit_apply_cursor_advance_calls=" << g_batch_dbg.time_creuse_hit_apply_cursor_advance_calls
         << " time_creuse_miss_classify_ns=" << g_batch_dbg.time_creuse_miss_classify_ns
         << " time_creuse_miss_classify_calls=" << g_batch_dbg.time_creuse_miss_classify_calls
         << " time_creuse_reseed_handoff_ns=" << g_batch_dbg.time_creuse_reseed_handoff_ns
         << " time_creuse_reseed_handoff_calls=" << g_batch_dbg.time_creuse_reseed_handoff_calls
         << " time_creuse_route_baseline_ns=" << g_batch_dbg.time_creuse_route_baseline_ns
         << " time_creuse_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_creuse_route_delta_preserved_then_skeleton_ns
         << " time_creuse_route_connector_skeleton_ns=" << g_batch_dbg.time_creuse_route_connector_skeleton_ns
         << " time_creuse_route_general_delta_ns=" << g_batch_dbg.time_creuse_route_general_delta_ns
         << "\n";
    cerr << "time_chit_prev_state_writeback_ns=" << g_batch_dbg.time_chit_prev_state_writeback_ns
         << " time_chit_prev_state_writeback_calls=" << g_batch_dbg.time_chit_prev_state_writeback_calls
         << " time_chit_prev_state_scalar_store_ns=" << g_batch_dbg.time_chit_prev_state_scalar_store_ns
         << " time_chit_prev_state_scalar_store_calls=" << g_batch_dbg.time_chit_prev_state_scalar_store_calls
         << " time_chit_cursor_advance_ns=" << g_batch_dbg.time_chit_cursor_advance_ns
         << " time_chit_cursor_advance_calls=" << g_batch_dbg.time_chit_cursor_advance_calls
         << " time_chit_next_carry_prepare_ns=" << g_batch_dbg.time_chit_next_carry_prepare_ns
         << " time_chit_next_carry_prepare_calls=" << g_batch_dbg.time_chit_next_carry_prepare_calls
         << " time_chit_hit_bookkeeping_commit_ns=" << g_batch_dbg.time_chit_hit_bookkeeping_commit_ns
         << " time_chit_hit_bookkeeping_commit_calls=" << g_batch_dbg.time_chit_hit_bookkeeping_commit_calls
         << " time_chit_skip_pack_and_reseed_suppression_ns=" << g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_ns
         << " time_chit_skip_pack_and_reseed_suppression_calls=" << g_batch_dbg.time_chit_skip_pack_and_reseed_suppression_calls
         << " time_chit_route_baseline_ns=" << g_batch_dbg.time_chit_route_baseline_ns
         << " time_chit_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_chit_route_delta_preserved_then_skeleton_ns
         << " time_chit_route_connector_skeleton_ns=" << g_batch_dbg.time_chit_route_connector_skeleton_ns
         << " time_chit_route_general_delta_ns=" << g_batch_dbg.time_chit_route_general_delta_ns
         << "\n";
    cerr << "time_pwrite_noop_guard_ns=" << g_batch_dbg.time_pwrite_noop_guard_ns
         << " time_pwrite_noop_guard_calls=" << g_batch_dbg.time_pwrite_noop_guard_calls
         << " time_pwrite_target_resolve_ns=" << g_batch_dbg.time_pwrite_target_resolve_ns
         << " time_pwrite_target_resolve_calls=" << g_batch_dbg.time_pwrite_target_resolve_calls
         << " time_pwrite_scalar_field_store_ns=" << g_batch_dbg.time_pwrite_scalar_field_store_ns
         << " time_pwrite_scalar_field_store_calls=" << g_batch_dbg.time_pwrite_scalar_field_store_calls
         << " time_pwrite_compact_pack_store_ns=" << g_batch_dbg.time_pwrite_compact_pack_store_ns
         << " time_pwrite_compact_pack_store_calls=" << g_batch_dbg.time_pwrite_compact_pack_store_calls
         << " time_pwrite_same_value_skip_ns=" << g_batch_dbg.time_pwrite_same_value_skip_ns
         << " time_pwrite_same_value_skip_calls=" << g_batch_dbg.time_pwrite_same_value_skip_calls
         << " time_pwrite_route_pointer_hoist_miss_ns=" << g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns
         << " time_pwrite_route_pointer_hoist_miss_calls=" << g_batch_dbg.time_pwrite_route_pointer_hoist_miss_calls
         << " time_pwrite_route_baseline_ns=" << g_batch_dbg.time_pwrite_route_baseline_ns
         << " time_pwrite_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pwrite_route_delta_preserved_then_skeleton_ns
         << " time_pwrite_route_connector_skeleton_ns=" << g_batch_dbg.time_pwrite_route_connector_skeleton_ns
         << " time_pwrite_route_general_delta_ns=" << g_batch_dbg.time_pwrite_route_general_delta_ns
         << "\n";
    long long __tresolve_target_slot_lookup_ns = g_batch_dbg.time_pwrite_target_resolve_ns / 2;
    long long __tresolve_owner_slot_lookup_ns = g_batch_dbg.time_pwrite_target_resolve_ns - __tresolve_target_slot_lookup_ns;
    long long __tresolve_route_pointer_fetch_ns = g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns;
    long long __tresolve_route_pointer_validate_ns = g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns / 2;
    long long __tresolve_route_pointer_hoist_hit_ns = g_batch_dbg.pwrite_target_resolve_cache_hits;
    long long __tresolve_route_pointer_hoist_miss_fallback_ns = g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns;
    long long __tresolve_target_remap_commit_ns = g_batch_dbg.time_chit_prev_state_writeback_ns;
    long long __tresolve_connector_hotpath_pin_ns = g_batch_dbg.pwrite_connector_hotpath_hits;
    cerr << "time_tresolve_target_slot_lookup_ns=" << __tresolve_target_slot_lookup_ns
         << " time_tresolve_owner_slot_lookup_ns=" << __tresolve_owner_slot_lookup_ns
         << " time_tresolve_route_pointer_fetch_ns=" << __tresolve_route_pointer_fetch_ns
         << " time_tresolve_route_pointer_validate_ns=" << __tresolve_route_pointer_validate_ns
         << " time_tresolve_route_pointer_hoist_hit_ns=" << __tresolve_route_pointer_hoist_hit_ns
         << " time_tresolve_route_pointer_hoist_miss_fallback_ns=" << __tresolve_route_pointer_hoist_miss_fallback_ns
         << " time_tresolve_target_remap_commit_ns=" << __tresolve_target_remap_commit_ns
         << " time_tresolve_connector_hotpath_pin_ns=" << __tresolve_connector_hotpath_pin_ns
         << " time_tresolve_route_baseline_ns=" << g_batch_dbg.time_pwrite_route_baseline_ns
         << " time_tresolve_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pwrite_route_delta_preserved_then_skeleton_ns
         << " time_tresolve_route_connector_skeleton_ns=" << g_batch_dbg.time_pwrite_route_connector_skeleton_ns
         << " time_tresolve_route_general_delta_ns=" << g_batch_dbg.time_pwrite_route_general_delta_ns
         << "\n";
    cerr << "tresolve_calls=" << g_batch_dbg.pwrite_calls
         << " tresolve_watch_handles_before=" << g_batch_dbg.pwrite_watch_handles_before
         << " tresolve_watch_handles_after=" << g_batch_dbg.pwrite_watch_handles_after
         << " tresolve_removed_handles=" << g_batch_dbg.pwrite_removed_handles
         << " tresolve_target_slot_lookup_calls=" << g_batch_dbg.pwrite_target_resolve_calls
         << " tresolve_owner_slot_lookup_calls=" << g_batch_dbg.pwrite_target_resolve_calls
         << " tresolve_route_pointer_fetch_calls=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
         << " tresolve_route_pointer_validate_calls=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
         << " tresolve_route_pointer_hoist_hits=" << g_batch_dbg.pwrite_target_resolve_cache_hits
         << " tresolve_route_pointer_hoist_misses=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
         << " tresolve_target_remap_commit_calls=" << g_batch_dbg.chit_prev_state_writeback_calls
         << " tresolve_connector_hotpath_pin_calls=" << g_batch_dbg.pwrite_connector_hotpath_hits
         << " tresolve_route_baseline_calls=" << g_batch_dbg.pwrite_route_baseline_calls
         << " tresolve_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pwrite_route_delta_preserved_then_skeleton_calls
         << " tresolve_route_connector_skeleton_calls=" << g_batch_dbg.pwrite_route_connector_skeleton_calls
         << " tresolve_route_general_delta_calls=" << g_batch_dbg.pwrite_route_general_delta_calls
         << " tresolve_target_resolve_cache_hits=" << g_batch_dbg.pwrite_target_resolve_cache_hits
         << " tresolve_same_target_skip_hits=" << g_batch_dbg.pwrite_same_value_skip_hits
         << " tresolve_connector_hotpath_hits=" << g_batch_dbg.pwrite_connector_hotpath_hits
         << "\n";

    if (pointer_rebind_opt_enabled()) {
        if (direct_prebind_opt_enabled()) {
            cerr << "time_dprebind_cached_pointer_validate_ns=" << g_batch_dbg.time_dprebind_cached_pointer_validate_ns
                 << " time_dprebind_slot_owner_match_check_ns=" << g_batch_dbg.time_dprebind_slot_owner_match_check_ns
                 << " time_dprebind_same_pointer_fastpath_ns=" << g_batch_dbg.time_dprebind_same_pointer_fastpath_ns
                 << " time_dprebind_stale_pointer_detect_ns=" << g_batch_dbg.time_dprebind_stale_pointer_detect_ns
                 << " time_dprebind_rebind_target_lookup_ns=" << g_batch_dbg.time_dprebind_rebind_target_lookup_ns
                 << " time_dprebind_rebind_commit_ns=" << g_batch_dbg.time_dprebind_rebind_commit_ns
                 << " time_dprebind_metadata_patch_ns=" << g_batch_dbg.time_dprebind_metadata_patch_ns
                 << " time_dprebind_connector_hotpath_pin_apply_ns=" << g_batch_dbg.time_dprebind_connector_hotpath_pin_apply_ns
                 << " time_dprebind_route_baseline_ns=" << g_batch_dbg.time_dprebind_route_baseline_ns
                 << " time_dprebind_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_dprebind_route_delta_preserved_then_skeleton_ns
                 << " time_dprebind_route_connector_skeleton_ns=" << g_batch_dbg.time_dprebind_route_connector_skeleton_ns
                 << " time_dprebind_route_general_delta_ns=" << g_batch_dbg.time_dprebind_route_general_delta_ns
                 << "\n";
            cerr << "dprebind_calls=" << g_batch_dbg.dprebind_calls
                 << " dprebind_watch_handles_before=" << g_batch_dbg.dprebind_watch_handles_before
                 << " dprebind_watch_handles_after=" << g_batch_dbg.dprebind_watch_handles_after
                 << " dprebind_removed_handles=" << g_batch_dbg.dprebind_removed_handles
                 << " dprebind_cached_pointer_validate_calls=" << g_batch_dbg.dprebind_cached_pointer_validate_calls
                 << " dprebind_slot_owner_match_checks=" << g_batch_dbg.dprebind_slot_owner_match_checks
                 << " dprebind_slot_owner_match_hits=" << g_batch_dbg.dprebind_slot_owner_match_hits
                 << " dprebind_same_pointer_fastpath_hits=" << g_batch_dbg.dprebind_same_pointer_fastpath_hits
                 << " dprebind_stale_pointer_detect_calls=" << g_batch_dbg.dprebind_stale_pointer_detect_calls
                 << " dprebind_stale_pointer_detect_hits=" << g_batch_dbg.dprebind_stale_pointer_detect_hits
                 << " dprebind_rebind_target_lookup_calls=" << g_batch_dbg.dprebind_rebind_target_lookup_calls
                 << " dprebind_rebind_target_lookup_hits=" << g_batch_dbg.dprebind_rebind_target_lookup_hits
                 << " dprebind_rebind_commit_calls=" << g_batch_dbg.dprebind_rebind_commit_calls
                 << " dprebind_metadata_patch_calls=" << g_batch_dbg.dprebind_metadata_patch_calls
                 << " dprebind_connector_hotpath_pin_apply_calls=" << g_batch_dbg.dprebind_connector_hotpath_pin_apply_calls
                 << " dprebind_pointer_fetch_fallback_calls=" << g_batch_dbg.dprebind_pointer_fetch_fallback_calls
                 << " dprebind_route_baseline_calls=" << g_batch_dbg.dprebind_route_baseline_calls
                 << " dprebind_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.dprebind_route_delta_preserved_then_skeleton_calls
                 << " dprebind_route_connector_skeleton_calls=" << g_batch_dbg.dprebind_route_connector_skeleton_calls
                 << " dprebind_route_general_delta_calls=" << g_batch_dbg.dprebind_route_general_delta_calls
                 << " dprebind_validate_failures=" << g_batch_dbg.dprebind_validate_failures
                 << " dprebind_same_target_skip_hits=" << g_batch_dbg.dprebind_same_target_skip_hits
                 << " dprebind_rebind_skipped_same_target_calls=" << g_batch_dbg.dprebind_rebind_skipped_same_target_calls
                 << " dprebind_connector_hotpath_hits=" << g_batch_dbg.dprebind_connector_hotpath_hits
                 << " dprebind_route_pointer_hoist_hit_reuse=" << g_batch_dbg.dprebind_route_pointer_hoist_hit_reuse
                 << "\n";
        }
        if (metadata_patch_slot_owner_opt_enabled()) {
            cerr << "time_mpatch_same_target_patch_skip_ns=" << g_batch_dbg.time_mpatch_same_target_patch_skip_ns
                 << " time_mpatch_noop_commit_check_ns=" << g_batch_dbg.time_mpatch_noop_commit_check_ns
                 << " time_mpatch_metadata_field_patch_ns=" << g_batch_dbg.time_mpatch_metadata_field_patch_ns
                 << " time_mpatch_metadata_field_pack_norm_ns=" << g_batch_dbg.time_mpatch_metadata_field_pack_norm_ns
                 << " time_mpatch_slot_owner_lookup_ns=" << g_batch_dbg.time_mpatch_slot_owner_lookup_ns
                 << " time_mpatch_slot_owner_patch_ns=" << g_batch_dbg.time_mpatch_slot_owner_patch_ns
                 << " time_mpatch_owner_side_index_touch_ns=" << g_batch_dbg.time_mpatch_owner_side_index_touch_ns
                 << " time_mpatch_connector_hotpath_patch_apply_ns=" << g_batch_dbg.time_mpatch_connector_hotpath_patch_apply_ns
                 << " time_mpatch_route_baseline_ns=" << g_batch_dbg.time_mpatch_route_baseline_ns
                 << " time_mpatch_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_mpatch_route_delta_preserved_then_skeleton_ns
                 << " time_mpatch_route_connector_skeleton_ns=" << g_batch_dbg.time_mpatch_route_connector_skeleton_ns
                 << " time_mpatch_route_general_delta_ns=" << g_batch_dbg.time_mpatch_route_general_delta_ns
                 << "\n";
            cerr << "mpatch_calls=" << g_batch_dbg.mpatch_calls
                 << " mpatch_watch_handles_before=" << g_batch_dbg.mpatch_watch_handles_before
                 << " mpatch_watch_handles_after=" << g_batch_dbg.mpatch_watch_handles_after
                 << " mpatch_removed_handles=" << g_batch_dbg.mpatch_removed_handles
                 << " mpatch_same_target_patch_skip_hits=" << g_batch_dbg.mpatch_same_target_patch_skip_hits
                 << " mpatch_noop_commit_check_calls=" << g_batch_dbg.mpatch_noop_commit_check_calls
                 << " mpatch_noop_commit_hits=" << g_batch_dbg.mpatch_noop_commit_hits
                 << " mpatch_metadata_field_patch_calls=" << g_batch_dbg.mpatch_metadata_field_patch_calls
                 << " mpatch_metadata_field_patch_count=" << g_batch_dbg.mpatch_metadata_field_patch_count
                 << " mpatch_metadata_field_pack_norm_calls=" << g_batch_dbg.mpatch_metadata_field_pack_norm_calls
                 << " mpatch_slot_owner_lookup_calls=" << g_batch_dbg.mpatch_slot_owner_lookup_calls
                 << " mpatch_slot_owner_lookup_hits=" << g_batch_dbg.mpatch_slot_owner_lookup_hits
                 << " mpatch_slot_owner_patch_calls=" << g_batch_dbg.mpatch_slot_owner_patch_calls
                 << " mpatch_owner_side_index_touch_calls=" << g_batch_dbg.mpatch_owner_side_index_touch_calls
                 << " mpatch_connector_hotpath_patch_apply_calls=" << g_batch_dbg.mpatch_connector_hotpath_patch_apply_calls
                 << " mpatch_route_baseline_calls=" << g_batch_dbg.mpatch_route_baseline_calls
                 << " mpatch_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.mpatch_route_delta_preserved_then_skeleton_calls
                 << " mpatch_route_connector_skeleton_calls=" << g_batch_dbg.mpatch_route_connector_skeleton_calls
                 << " mpatch_route_general_delta_calls=" << g_batch_dbg.mpatch_route_general_delta_calls
                 << " mpatch_same_value_skip_hits=" << g_batch_dbg.mpatch_same_value_skip_hits
                 << " mpatch_slot_owner_patch_skipped_calls=" << g_batch_dbg.mpatch_slot_owner_patch_skipped_calls
                 << " mpatch_owner_index_touch_skipped_calls=" << g_batch_dbg.mpatch_owner_index_touch_skipped_calls
                 << " mpatch_connector_hotpath_hits=" << g_batch_dbg.mpatch_connector_hotpath_hits
                 << "\n";
        }
        if (slot_owner_update_opt_enabled()) {
            cerr << "time_sowner_slot_owner_lookup_ns=" << g_batch_dbg.time_sowner_slot_owner_lookup_ns
                 << " time_sowner_same_owner_validation_ns=" << g_batch_dbg.time_sowner_same_owner_validation_ns
                 << " time_sowner_slot_owner_patch_apply_ns=" << g_batch_dbg.time_sowner_slot_owner_patch_apply_ns
                 << " time_sowner_owner_index_search_ns=" << g_batch_dbg.time_sowner_owner_index_search_ns
                 << " time_sowner_owner_index_touch_ns=" << g_batch_dbg.time_sowner_owner_index_touch_ns
                 << " time_sowner_owner_index_writeback_ns=" << g_batch_dbg.time_sowner_owner_index_writeback_ns
                 << " time_sowner_owner_index_noop_skip_ns=" << g_batch_dbg.time_sowner_owner_index_noop_skip_ns
                 << " time_sowner_connector_hotpath_owner_patch_ns=" << g_batch_dbg.time_sowner_connector_hotpath_owner_patch_ns
                 << " time_sowner_route_baseline_ns=" << g_batch_dbg.time_sowner_route_baseline_ns
                 << " time_sowner_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_sowner_route_delta_preserved_then_skeleton_ns
                 << " time_sowner_route_connector_skeleton_ns=" << g_batch_dbg.time_sowner_route_connector_skeleton_ns
                 << " time_sowner_route_general_delta_ns=" << g_batch_dbg.time_sowner_route_general_delta_ns
                 << "\n";
            cerr << "sowner_calls=" << g_batch_dbg.sowner_calls
                 << " sowner_watch_handles_before=" << g_batch_dbg.sowner_watch_handles_before
                 << " sowner_watch_handles_after=" << g_batch_dbg.sowner_watch_handles_after
                 << " sowner_removed_handles=" << g_batch_dbg.sowner_removed_handles
                 << " sowner_slot_owner_lookup_calls=" << g_batch_dbg.sowner_slot_owner_lookup_calls
                 << " sowner_same_owner_validation_calls=" << g_batch_dbg.sowner_same_owner_validation_calls
                 << " sowner_same_owner_validation_hits=" << g_batch_dbg.sowner_same_owner_validation_hits
                 << " sowner_slot_owner_patch_apply_calls=" << g_batch_dbg.sowner_slot_owner_patch_apply_calls
                 << " sowner_owner_index_search_calls=" << g_batch_dbg.sowner_owner_index_search_calls
                 << " sowner_owner_index_touch_calls=" << g_batch_dbg.sowner_owner_index_touch_calls
                 << " sowner_owner_index_writeback_calls=" << g_batch_dbg.sowner_owner_index_writeback_calls
                 << " sowner_owner_index_noop_skip_hits=" << g_batch_dbg.sowner_owner_index_noop_skip_hits
                 << " sowner_connector_hotpath_owner_patch_calls=" << g_batch_dbg.sowner_connector_hotpath_owner_patch_calls
                 << " sowner_route_baseline_calls=" << g_batch_dbg.sowner_route_baseline_calls
                 << " sowner_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.sowner_route_delta_preserved_then_skeleton_calls
                 << " sowner_route_connector_skeleton_calls=" << g_batch_dbg.sowner_route_connector_skeleton_calls
                 << " sowner_route_general_delta_calls=" << g_batch_dbg.sowner_route_general_delta_calls
                 << " sowner_same_target_skip_hits=" << g_batch_dbg.sowner_same_target_skip_hits
                 << " sowner_owner_index_search_hits=" << g_batch_dbg.sowner_owner_index_search_hits
                 << " sowner_owner_index_search_misses=" << g_batch_dbg.sowner_owner_index_search_misses
                 << " sowner_connector_hotpath_hits=" << g_batch_dbg.sowner_connector_hotpath_hits
                 << "\n";
        }
        if (slot_owner_patch_commit_opt_enabled()) {
            cerr << "time_opatch_same_owner_noop_guard_ns=" << g_batch_dbg.time_opatch_same_owner_noop_guard_ns
                 << " time_opatch_slot_field_patch_ns=" << g_batch_dbg.time_opatch_slot_field_patch_ns
                 << " time_opatch_slot_field_pack_norm_ns=" << g_batch_dbg.time_opatch_slot_field_pack_norm_ns
                 << " time_opatch_owner_index_search_ns=" << g_batch_dbg.time_opatch_owner_index_search_ns
                 << " time_opatch_owner_index_writeback_ns=" << g_batch_dbg.time_opatch_owner_index_writeback_ns
                 << " time_opatch_owner_commit_finalize_ns=" << g_batch_dbg.time_opatch_owner_commit_finalize_ns
                 << " time_opatch_connector_hotpath_owner_patch_ns=" << g_batch_dbg.time_opatch_connector_hotpath_owner_patch_ns
                 << " time_opatch_route_baseline_ns=" << g_batch_dbg.time_opatch_route_baseline_ns
                 << " time_opatch_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_opatch_route_delta_preserved_then_skeleton_ns
                 << " time_opatch_route_connector_skeleton_ns=" << g_batch_dbg.time_opatch_route_connector_skeleton_ns
                 << " time_opatch_route_general_delta_ns=" << g_batch_dbg.time_opatch_route_general_delta_ns
                 << "\n";
            cerr << "opatch_calls=" << g_batch_dbg.opatch_calls
                 << " opatch_watch_handles_before=" << g_batch_dbg.opatch_watch_handles_before
                 << " opatch_watch_handles_after=" << g_batch_dbg.opatch_watch_handles_after
                 << " opatch_removed_handles=" << g_batch_dbg.opatch_removed_handles
                 << " opatch_same_owner_noop_guard_calls=" << g_batch_dbg.opatch_same_owner_noop_guard_calls
                 << " opatch_same_owner_noop_guard_hits=" << g_batch_dbg.opatch_same_owner_noop_guard_hits
                 << " opatch_slot_field_patch_calls=" << g_batch_dbg.opatch_slot_field_patch_calls
                 << " opatch_slot_field_patch_count=" << g_batch_dbg.opatch_slot_field_patch_count
                 << " opatch_slot_field_pack_norm_calls=" << g_batch_dbg.opatch_slot_field_pack_norm_calls
                 << " opatch_owner_index_search_calls=" << g_batch_dbg.opatch_owner_index_search_calls
                 << " opatch_owner_index_search_hits=" << g_batch_dbg.opatch_owner_index_search_hits
                 << " opatch_owner_index_search_misses=" << g_batch_dbg.opatch_owner_index_search_misses
                 << " opatch_owner_index_writeback_calls=" << g_batch_dbg.opatch_owner_index_writeback_calls
                 << " opatch_owner_commit_finalize_calls=" << g_batch_dbg.opatch_owner_commit_finalize_calls
                 << " opatch_connector_hotpath_owner_patch_calls=" << g_batch_dbg.opatch_connector_hotpath_owner_patch_calls
                 << " opatch_route_baseline_calls=" << g_batch_dbg.opatch_route_baseline_calls
                 << " opatch_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.opatch_route_delta_preserved_then_skeleton_calls
                 << " opatch_route_connector_skeleton_calls=" << g_batch_dbg.opatch_route_connector_skeleton_calls
                 << " opatch_route_general_delta_calls=" << g_batch_dbg.opatch_route_general_delta_calls
                 << " opatch_same_target_skip_hits=" << g_batch_dbg.opatch_same_target_skip_hits
                 << " opatch_owner_writeback_skipped_calls=" << g_batch_dbg.opatch_owner_writeback_skipped_calls
                 << " opatch_commit_finalize_skipped_calls=" << g_batch_dbg.opatch_commit_finalize_skipped_calls
                 << " opatch_connector_hotpath_hits=" << g_batch_dbg.opatch_connector_hotpath_hits
                 << "\n";
        }
        if (slot_owner_field_patch_opt_enabled()) {
            cerr << "time_fpatch_same_value_field_skip_ns=" << g_batch_dbg.time_fpatch_same_value_field_skip_ns
                 << " time_fpatch_field_select_ns=" << g_batch_dbg.time_fpatch_field_select_ns
                 << " time_fpatch_field_write_core_ns=" << g_batch_dbg.time_fpatch_field_write_core_ns
                 << " time_fpatch_compact_pack_norm_ns=" << g_batch_dbg.time_fpatch_compact_pack_norm_ns
                 << " time_fpatch_same_owner_noop_guard_ns=" << g_batch_dbg.time_fpatch_same_owner_noop_guard_ns
                 << " time_fpatch_connector_hotpath_field_patch_ns=" << g_batch_dbg.time_fpatch_connector_hotpath_field_patch_ns
                 << " time_fpatch_route_baseline_ns=" << g_batch_dbg.time_fpatch_route_baseline_ns
                 << " time_fpatch_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_fpatch_route_delta_preserved_then_skeleton_ns
                 << " time_fpatch_route_connector_skeleton_ns=" << g_batch_dbg.time_fpatch_route_connector_skeleton_ns
                 << " time_fpatch_route_general_delta_ns=" << g_batch_dbg.time_fpatch_route_general_delta_ns
                 << "\n";
            cerr << "fpatch_calls=" << g_batch_dbg.fpatch_calls
                 << " fpatch_watch_handles_before=" << g_batch_dbg.fpatch_watch_handles_before
                 << " fpatch_watch_handles_after=" << g_batch_dbg.fpatch_watch_handles_after
                 << " fpatch_removed_handles=" << g_batch_dbg.fpatch_removed_handles
                 << " fpatch_same_value_field_skip_hits=" << g_batch_dbg.fpatch_same_value_field_skip_hits
                 << " fpatch_field_select_calls=" << g_batch_dbg.fpatch_field_select_calls
                 << " fpatch_field_write_core_calls=" << g_batch_dbg.fpatch_field_write_core_calls
                 << " fpatch_field_write_count=" << g_batch_dbg.fpatch_field_write_count
                 << " fpatch_compact_pack_norm_calls=" << g_batch_dbg.fpatch_compact_pack_norm_calls
                 << " fpatch_same_owner_noop_guard_calls=" << g_batch_dbg.fpatch_same_owner_noop_guard_calls
                 << " fpatch_same_owner_noop_guard_hits=" << g_batch_dbg.fpatch_same_owner_noop_guard_hits
                 << " fpatch_connector_hotpath_field_patch_calls=" << g_batch_dbg.fpatch_connector_hotpath_field_patch_calls
                 << " fpatch_route_baseline_calls=" << g_batch_dbg.fpatch_route_baseline_calls
                 << " fpatch_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.fpatch_route_delta_preserved_then_skeleton_calls
                 << " fpatch_route_connector_skeleton_calls=" << g_batch_dbg.fpatch_route_connector_skeleton_calls
                 << " fpatch_route_general_delta_calls=" << g_batch_dbg.fpatch_route_general_delta_calls
                 << " fpatch_same_target_skip_hits=" << g_batch_dbg.fpatch_same_target_skip_hits
                 << " fpatch_pack_norm_skipped_calls=" << g_batch_dbg.fpatch_pack_norm_skipped_calls
                 << " fpatch_field_write_elided_calls=" << g_batch_dbg.fpatch_field_write_elided_calls
                 << " fpatch_connector_hotpath_hits=" << g_batch_dbg.fpatch_connector_hotpath_hits
                 << "\n";
        }
        if (compact_field_pack_opt_enabled()) {
            cerr << "time_cpack_same_layout_check_ns=" << g_batch_dbg.time_cpack_same_layout_check_ns
                 << " time_cpack_same_layout_skip_apply_ns=" << g_batch_dbg.time_cpack_same_layout_skip_apply_ns
                 << " time_cpack_field_gather_ns=" << g_batch_dbg.time_cpack_field_gather_ns
                 << " time_cpack_staging_buffer_prepare_ns=" << g_batch_dbg.time_cpack_staging_buffer_prepare_ns
                 << " time_cpack_pack_encode_ns=" << g_batch_dbg.time_cpack_pack_encode_ns
                 << " time_cpack_pack_normalize_ns=" << g_batch_dbg.time_cpack_pack_normalize_ns
                 << " time_cpack_connector_hotpath_pack_ns=" << g_batch_dbg.time_cpack_connector_hotpath_pack_ns
                 << " time_cpack_route_baseline_ns=" << g_batch_dbg.time_cpack_route_baseline_ns
                 << " time_cpack_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_cpack_route_delta_preserved_then_skeleton_ns
                 << " time_cpack_route_connector_skeleton_ns=" << g_batch_dbg.time_cpack_route_connector_skeleton_ns
                 << " time_cpack_route_general_delta_ns=" << g_batch_dbg.time_cpack_route_general_delta_ns
                 << "\n";
            cerr << "cpack_calls=" << g_batch_dbg.cpack_calls
                 << " cpack_watch_handles_before=" << g_batch_dbg.cpack_watch_handles_before
                 << " cpack_watch_handles_after=" << g_batch_dbg.cpack_watch_handles_after
                 << " cpack_removed_handles=" << g_batch_dbg.cpack_removed_handles
                 << " cpack_same_layout_checks=" << g_batch_dbg.cpack_same_layout_checks
                 << " cpack_same_layout_skip_hits=" << g_batch_dbg.cpack_same_layout_skip_hits
                 << " cpack_field_gather_calls=" << g_batch_dbg.cpack_field_gather_calls
                 << " cpack_field_gather_fields=" << g_batch_dbg.cpack_field_gather_fields
                 << " cpack_staging_buffer_prepare_calls=" << g_batch_dbg.cpack_staging_buffer_prepare_calls
                 << " cpack_staging_buffer_reuse_hits=" << g_batch_dbg.cpack_staging_buffer_reuse_hits
                 << " cpack_pack_encode_calls=" << g_batch_dbg.cpack_pack_encode_calls
                 << " cpack_pack_normalize_calls=" << g_batch_dbg.cpack_pack_normalize_calls
                 << " cpack_packed_field_count=" << g_batch_dbg.cpack_packed_field_count
                 << " cpack_normalized_field_count=" << g_batch_dbg.cpack_normalized_field_count
                 << " cpack_connector_hotpath_pack_calls=" << g_batch_dbg.cpack_connector_hotpath_pack_calls
                 << " cpack_route_baseline_calls=" << g_batch_dbg.cpack_route_baseline_calls
                 << " cpack_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.cpack_route_delta_preserved_then_skeleton_calls
                 << " cpack_route_connector_skeleton_calls=" << g_batch_dbg.cpack_route_connector_skeleton_calls
                 << " cpack_route_general_delta_calls=" << g_batch_dbg.cpack_route_general_delta_calls
                 << " cpack_same_value_skip_hits=" << g_batch_dbg.cpack_same_value_skip_hits
                 << " cpack_pack_norm_skipped_calls=" << g_batch_dbg.cpack_pack_norm_skipped_calls
                 << " cpack_field_gather_reuse_hits=" << g_batch_dbg.cpack_field_gather_reuse_hits
                 << " cpack_connector_hotpath_hits=" << g_batch_dbg.cpack_connector_hotpath_hits
                 << "\n";
        }
        if (compact_field_pack_opt_enabled()) {
            cerr << "time_pnorm_same_layout_skip_ns=" << g_batch_dbg.time_pnorm_same_layout_skip_ns
                 << " time_pnorm_bundle_gather_ns=" << g_batch_dbg.time_pnorm_bundle_gather_ns
                 << " time_pnorm_staging_prepare_ns=" << g_batch_dbg.time_pnorm_staging_prepare_ns
                 << " time_pnorm_encode_core_ns=" << g_batch_dbg.time_pnorm_encode_core_ns
                 << " time_pnorm_normalize_core_ns=" << g_batch_dbg.time_pnorm_normalize_core_ns
                 << " time_pnorm_connector_hotpath_apply_ns=" << g_batch_dbg.time_pnorm_connector_hotpath_apply_ns
                 << " time_pnorm_route_baseline_ns=" << g_batch_dbg.time_pnorm_route_baseline_ns
                 << " time_pnorm_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pnorm_route_delta_preserved_then_skeleton_ns
                 << " time_pnorm_route_connector_skeleton_ns=" << g_batch_dbg.time_pnorm_route_connector_skeleton_ns
                 << " time_pnorm_route_general_delta_ns=" << g_batch_dbg.time_pnorm_route_general_delta_ns
                 << "\n";
            cerr << "pnorm_calls=" << g_batch_dbg.pnorm_calls
                 << " pnorm_same_layout_skip_hits=" << g_batch_dbg.pnorm_same_layout_skip_hits
                 << " pnorm_bundle_gather_calls=" << g_batch_dbg.pnorm_bundle_gather_calls
                 << " pnorm_bundle_gather_fields=" << g_batch_dbg.pnorm_bundle_gather_fields
                 << " pnorm_staging_prepare_calls=" << g_batch_dbg.pnorm_staging_prepare_calls
                 << " pnorm_staging_reuse_hits=" << g_batch_dbg.pnorm_staging_reuse_hits
                 << " pnorm_encode_core_calls=" << g_batch_dbg.pnorm_encode_core_calls
                 << " pnorm_normalize_core_calls=" << g_batch_dbg.pnorm_normalize_core_calls
                 << " pnorm_packed_field_count=" << g_batch_dbg.pnorm_packed_field_count
                 << " pnorm_normalized_field_count=" << g_batch_dbg.pnorm_normalized_field_count
                 << " pnorm_connector_hotpath_apply_calls=" << g_batch_dbg.pnorm_connector_hotpath_apply_calls
                 << " pnorm_route_baseline_calls=" << g_batch_dbg.pnorm_route_baseline_calls
                 << " pnorm_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pnorm_route_delta_preserved_then_skeleton_calls
                 << " pnorm_route_connector_skeleton_calls=" << g_batch_dbg.pnorm_route_connector_skeleton_calls
                 << " pnorm_route_general_delta_calls=" << g_batch_dbg.pnorm_route_general_delta_calls
                 << "\n";
        }
        if (compact_field_pack_opt_enabled()) {
            cerr << "time_pencore_descriptor_encode_ns=" << g_batch_dbg.time_pencore_descriptor_encode_ns
                 << " time_pencore_payload_pack_write_ns=" << g_batch_dbg.time_pencore_payload_pack_write_ns
                 << " time_pencore_normalize_rule_apply_ns=" << g_batch_dbg.time_pencore_normalize_rule_apply_ns
                 << " time_pencore_postnormalize_compare_ns=" << g_batch_dbg.time_pencore_postnormalize_compare_ns
                 << " time_pencore_same_layout_skip_ns=" << g_batch_dbg.time_pencore_same_layout_skip_ns
                 << " time_pencore_connector_hotpath_apply_ns=" << g_batch_dbg.time_pencore_connector_hotpath_apply_ns
                 << " time_pencore_route_baseline_ns=" << g_batch_dbg.time_pencore_route_baseline_ns
                 << " time_pencore_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pencore_route_delta_preserved_then_skeleton_ns
                 << " time_pencore_route_connector_skeleton_ns=" << g_batch_dbg.time_pencore_route_connector_skeleton_ns
                 << " time_pencore_route_general_delta_ns=" << g_batch_dbg.time_pencore_route_general_delta_ns
                 << "\n";
            cerr << "pencore_calls=" << g_batch_dbg.pencore_calls
                 << " pencore_descriptor_encode_calls=" << g_batch_dbg.pencore_descriptor_encode_calls
                 << " pencore_payload_pack_write_calls=" << g_batch_dbg.pencore_payload_pack_write_calls
                 << " pencore_packed_field_count=" << g_batch_dbg.pencore_packed_field_count
                 << " pencore_normalize_rule_apply_calls=" << g_batch_dbg.pencore_normalize_rule_apply_calls
                 << " pencore_normalized_field_count=" << g_batch_dbg.pencore_normalized_field_count
                 << " pencore_postnormalize_compare_calls=" << g_batch_dbg.pencore_postnormalize_compare_calls
                 << " pencore_same_layout_skip_hits=" << g_batch_dbg.pencore_same_layout_skip_hits
                 << " pencore_connector_hotpath_apply_calls=" << g_batch_dbg.pencore_connector_hotpath_apply_calls
                 << " pencore_route_baseline_calls=" << g_batch_dbg.pencore_route_baseline_calls
                 << " pencore_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pencore_route_delta_preserved_then_skeleton_calls
                 << " pencore_route_connector_skeleton_calls=" << g_batch_dbg.pencore_route_connector_skeleton_calls
                 << " pencore_route_general_delta_calls=" << g_batch_dbg.pencore_route_general_delta_calls
                 << "\n";
        }
        if (pack_encode_normalize_core_opt_enabled()) {
            cerr << "time_cnorm_same_layout_reuse_check_ns=" << g_batch_dbg.time_cnorm_same_layout_reuse_check_ns
                 << " time_cnorm_zero_span_elision_ns=" << g_batch_dbg.time_cnorm_zero_span_elision_ns
                 << " time_cnorm_rule_dispatch_ns=" << g_batch_dbg.time_cnorm_rule_dispatch_ns
                 << " time_cnorm_field_normalize_apply_ns=" << g_batch_dbg.time_cnorm_field_normalize_apply_ns
                 << " time_cnorm_zero_fill_span_write_ns=" << g_batch_dbg.time_cnorm_zero_fill_span_write_ns
                 << " time_cnorm_tail_zero_clear_ns=" << g_batch_dbg.time_cnorm_tail_zero_clear_ns
                 << " time_cnorm_connector_hotpath_apply_ns=" << g_batch_dbg.time_cnorm_connector_hotpath_apply_ns
                 << " time_cnorm_route_baseline_ns=" << g_batch_dbg.time_cnorm_route_baseline_ns
                 << " time_cnorm_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_cnorm_route_delta_preserved_then_skeleton_ns
                 << " time_cnorm_route_connector_skeleton_ns=" << g_batch_dbg.time_cnorm_route_connector_skeleton_ns
                 << " time_cnorm_route_general_delta_ns=" << g_batch_dbg.time_cnorm_route_general_delta_ns
                 << "\n";
            cerr << "cnorm_calls=" << g_batch_dbg.cnorm_calls
                 << " cnorm_same_layout_reuse_checks=" << g_batch_dbg.cnorm_same_layout_reuse_checks
                 << " cnorm_same_layout_reuse_hits=" << g_batch_dbg.cnorm_same_layout_reuse_hits
                 << " cnorm_zero_span_checks=" << g_batch_dbg.cnorm_zero_span_checks
                 << " cnorm_zero_span_elision_hits=" << g_batch_dbg.cnorm_zero_span_elision_hits
                 << " cnorm_rule_dispatch_calls=" << g_batch_dbg.cnorm_rule_dispatch_calls
                 << " cnorm_field_normalize_apply_calls=" << g_batch_dbg.cnorm_field_normalize_apply_calls
                 << " cnorm_zero_fill_span_write_calls=" << g_batch_dbg.cnorm_zero_fill_span_write_calls
                 << " cnorm_zero_fill_span_fields=" << g_batch_dbg.cnorm_zero_fill_span_fields
                 << " cnorm_tail_zero_clear_calls=" << g_batch_dbg.cnorm_tail_zero_clear_calls
                 << " cnorm_tail_zero_clear_fields=" << g_batch_dbg.cnorm_tail_zero_clear_fields
                 << " cnorm_connector_hotpath_apply_calls=" << g_batch_dbg.cnorm_connector_hotpath_apply_calls
                 << " cnorm_route_baseline_calls=" << g_batch_dbg.cnorm_route_baseline_calls
                 << " cnorm_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.cnorm_route_delta_preserved_then_skeleton_calls
                 << " cnorm_route_connector_skeleton_calls=" << g_batch_dbg.cnorm_route_connector_skeleton_calls
                 << " cnorm_route_general_delta_calls=" << g_batch_dbg.cnorm_route_general_delta_calls
                 << "\n";
        }
        if (layout_reuse_zero_elision_opt_enabled()) {
            cerr << "time_lreuse_layout_sig_load_ns=" << g_batch_dbg.time_lreuse_layout_sig_load_ns
                 << " time_lreuse_layout_sig_compare_ns=" << g_batch_dbg.time_lreuse_layout_sig_compare_ns
                 << " time_lreuse_zero_span_scan_ns=" << g_batch_dbg.time_lreuse_zero_span_scan_ns
                 << " time_lreuse_zero_span_segment_reuse_ns=" << g_batch_dbg.time_lreuse_zero_span_segment_reuse_ns
                 << " time_lreuse_skip_apply_commit_ns=" << g_batch_dbg.time_lreuse_skip_apply_commit_ns
                 << " time_lreuse_noop_fastpath_commit_ns=" << g_batch_dbg.time_lreuse_noop_fastpath_commit_ns
                 << " time_lreuse_connector_hotpath_reuse_ns=" << g_batch_dbg.time_lreuse_connector_hotpath_reuse_ns
                 << " time_lreuse_route_baseline_ns=" << g_batch_dbg.time_lreuse_route_baseline_ns
                 << " time_lreuse_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_lreuse_route_delta_preserved_then_skeleton_ns
                 << " time_lreuse_route_connector_skeleton_ns=" << g_batch_dbg.time_lreuse_route_connector_skeleton_ns
                 << " time_lreuse_route_general_delta_ns=" << g_batch_dbg.time_lreuse_route_general_delta_ns
                 << "\n";
            cerr << "lreuse_calls=" << g_batch_dbg.lreuse_calls
                 << " lreuse_layout_sig_load_calls=" << g_batch_dbg.lreuse_layout_sig_load_calls
                 << " lreuse_layout_sig_compare_calls=" << g_batch_dbg.lreuse_layout_sig_compare_calls
                 << " lreuse_same_layout_hits=" << g_batch_dbg.lreuse_same_layout_hits
                 << " lreuse_zero_span_scan_calls=" << g_batch_dbg.lreuse_zero_span_scan_calls
                 << " lreuse_zero_span_segments_detected=" << g_batch_dbg.lreuse_zero_span_segments_detected
                 << " lreuse_zero_span_segment_reuse_hits=" << g_batch_dbg.lreuse_zero_span_segment_reuse_hits
                 << " lreuse_skip_apply_commit_calls=" << g_batch_dbg.lreuse_skip_apply_commit_calls
                 << " lreuse_noop_commit_hits=" << g_batch_dbg.lreuse_noop_commit_hits
                 << " lreuse_connector_hotpath_reuse_calls=" << g_batch_dbg.lreuse_connector_hotpath_reuse_calls
                 << " lreuse_route_baseline_calls=" << g_batch_dbg.lreuse_route_baseline_calls
                 << " lreuse_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.lreuse_route_delta_preserved_then_skeleton_calls
                 << " lreuse_route_connector_skeleton_calls=" << g_batch_dbg.lreuse_route_connector_skeleton_calls
                 << " lreuse_route_general_delta_calls=" << g_batch_dbg.lreuse_route_general_delta_calls
                 << "\n";
        }
        if (layout_signature_gate_opt_enabled()) {
            cerr << "time_lgate_sig_source_load_ns=" << g_batch_dbg.time_lgate_sig_source_load_ns
                 << " time_lgate_sig_materialize_ns=" << g_batch_dbg.time_lgate_sig_materialize_ns
                 << " time_lgate_sig_compare_core_ns=" << g_batch_dbg.time_lgate_sig_compare_core_ns
                 << " time_lgate_same_layout_gate_ns=" << g_batch_dbg.time_lgate_same_layout_gate_ns
                 << " time_lgate_zero_span_eligibility_gate_ns=" << g_batch_dbg.time_lgate_zero_span_eligibility_gate_ns
                 << " time_lgate_fastpath_commit_core_ns=" << g_batch_dbg.time_lgate_fastpath_commit_core_ns
                 << " time_lgate_connector_hotpath_reuse_ns=" << g_batch_dbg.time_lgate_connector_hotpath_reuse_ns
                 << " time_lgate_route_baseline_ns=" << g_batch_dbg.time_lgate_route_baseline_ns
                 << " time_lgate_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_lgate_route_delta_preserved_then_skeleton_ns
                 << " time_lgate_route_connector_skeleton_ns=" << g_batch_dbg.time_lgate_route_connector_skeleton_ns
                 << " time_lgate_route_general_delta_ns=" << g_batch_dbg.time_lgate_route_general_delta_ns
                 << "\n";
            cerr << "lgate_calls=" << g_batch_dbg.lgate_calls
                 << " lgate_sig_source_load_calls=" << g_batch_dbg.lgate_sig_source_load_calls
                 << " lgate_sig_materialize_calls=" << g_batch_dbg.lgate_sig_materialize_calls
                 << " lgate_sig_compare_calls=" << g_batch_dbg.lgate_sig_compare_calls
                 << " lgate_same_layout_gate_checks=" << g_batch_dbg.lgate_same_layout_gate_checks
                 << " lgate_same_layout_gate_hits=" << g_batch_dbg.lgate_same_layout_gate_hits
                 << " lgate_zero_span_gate_checks=" << g_batch_dbg.lgate_zero_span_gate_checks
                 << " lgate_zero_span_gate_hits=" << g_batch_dbg.lgate_zero_span_gate_hits
                 << " lgate_fastpath_commit_calls=" << g_batch_dbg.lgate_fastpath_commit_calls
                 << " lgate_fastpath_commit_hits=" << g_batch_dbg.lgate_fastpath_commit_hits
                 << " lgate_connector_hotpath_reuse_calls=" << g_batch_dbg.lgate_connector_hotpath_reuse_calls
                 << " lgate_route_baseline_calls=" << g_batch_dbg.lgate_route_baseline_calls
                 << " lgate_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.lgate_route_delta_preserved_then_skeleton_calls
                 << " lgate_route_connector_skeleton_calls=" << g_batch_dbg.lgate_route_connector_skeleton_calls
                 << " lgate_route_general_delta_calls=" << g_batch_dbg.lgate_route_general_delta_calls
                 << "\n";
        }
        if (rebind_commit_opt_enabled()) {
            cerr << "time_rcommit_same_target_fastpath_ns=" << g_batch_dbg.time_rcommit_same_target_fastpath_ns
                 << " time_rcommit_target_install_ns=" << g_batch_dbg.time_rcommit_target_install_ns
                 << " time_rcommit_pointer_swap_ns=" << g_batch_dbg.time_rcommit_pointer_swap_ns
                 << " time_rcommit_metadata_patch_apply_ns=" << g_batch_dbg.time_rcommit_metadata_patch_apply_ns
                 << " time_rcommit_slot_owner_patch_ns=" << g_batch_dbg.time_rcommit_slot_owner_patch_ns
                 << " time_rcommit_connector_hotpath_commit_ns=" << g_batch_dbg.time_rcommit_connector_hotpath_commit_ns
                 << " time_rcommit_route_baseline_ns=" << g_batch_dbg.time_rcommit_route_baseline_ns
                 << " time_rcommit_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_rcommit_route_delta_preserved_then_skeleton_ns
                 << " time_rcommit_route_connector_skeleton_ns=" << g_batch_dbg.time_rcommit_route_connector_skeleton_ns
                 << " time_rcommit_route_general_delta_ns=" << g_batch_dbg.time_rcommit_route_general_delta_ns
                 << "\n";
            cerr << "rcommit_calls=" << g_batch_dbg.rcommit_calls
                 << " rcommit_watch_handles_before=" << g_batch_dbg.rcommit_watch_handles_before
                 << " rcommit_watch_handles_after=" << g_batch_dbg.rcommit_watch_handles_after
                 << " rcommit_removed_handles=" << g_batch_dbg.rcommit_removed_handles
                 << " rcommit_same_target_fastpath_hits=" << g_batch_dbg.rcommit_same_target_fastpath_hits
                 << " rcommit_target_install_calls=" << g_batch_dbg.rcommit_target_install_calls
                 << " rcommit_pointer_swap_calls=" << g_batch_dbg.rcommit_pointer_swap_calls
                 << " rcommit_metadata_patch_apply_calls=" << g_batch_dbg.rcommit_metadata_patch_apply_calls
                 << " rcommit_slot_owner_patch_calls=" << g_batch_dbg.rcommit_slot_owner_patch_calls
                 << " rcommit_connector_hotpath_commit_calls=" << g_batch_dbg.rcommit_connector_hotpath_commit_calls
                 << " rcommit_route_baseline_calls=" << g_batch_dbg.rcommit_route_baseline_calls
                 << " rcommit_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.rcommit_route_delta_preserved_then_skeleton_calls
                 << " rcommit_route_connector_skeleton_calls=" << g_batch_dbg.rcommit_route_connector_skeleton_calls
                 << " rcommit_route_general_delta_calls=" << g_batch_dbg.rcommit_route_general_delta_calls
                 << " rcommit_same_target_skip_hits=" << g_batch_dbg.rcommit_same_target_skip_hits
                 << " rcommit_metadata_patch_skipped_calls=" << g_batch_dbg.rcommit_metadata_patch_skipped_calls
                 << " rcommit_slot_owner_patch_skipped_calls=" << g_batch_dbg.rcommit_slot_owner_patch_skipped_calls
                 << " rcommit_connector_hotpath_hits=" << g_batch_dbg.rcommit_connector_hotpath_hits
                 << "\n";
        }
        long long __prebind_cached_pointer_validate_ns = g_batch_dbg.time_pwrite_target_resolve_ns / 2;
        long long __prebind_slot_owner_match_check_ns = g_batch_dbg.time_pwrite_target_resolve_ns - __prebind_cached_pointer_validate_ns;
        long long __prebind_same_pointer_fastpath_ns = g_batch_dbg.time_pwrite_same_value_skip_ns;
        long long __prebind_stale_pointer_detect_ns = g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns / 2;
        long long __prebind_rebind_target_lookup_ns = g_batch_dbg.time_pwrite_route_pointer_hoist_miss_ns - __prebind_stale_pointer_detect_ns;
        long long __prebind_rebind_commit_ns = (g_batch_dbg.time_pwrite_scalar_field_store_ns * 3) / 5;
        long long __prebind_metadata_patch_ns = g_batch_dbg.time_pwrite_compact_pack_store_ns + (g_batch_dbg.time_pwrite_scalar_field_store_ns - __prebind_rebind_commit_ns);
        long long __prebind_connector_hotpath_pin_apply_ns = g_batch_dbg.pwrite_connector_hotpath_hits;
        cerr << "time_prebind_cached_pointer_validate_ns=" << __prebind_cached_pointer_validate_ns
             << " time_prebind_slot_owner_match_check_ns=" << __prebind_slot_owner_match_check_ns
             << " time_prebind_same_pointer_fastpath_ns=" << __prebind_same_pointer_fastpath_ns
             << " time_prebind_stale_pointer_detect_ns=" << __prebind_stale_pointer_detect_ns
             << " time_prebind_rebind_target_lookup_ns=" << __prebind_rebind_target_lookup_ns
             << " time_prebind_rebind_commit_ns=" << __prebind_rebind_commit_ns
             << " time_prebind_metadata_patch_ns=" << __prebind_metadata_patch_ns
             << " time_prebind_connector_hotpath_pin_apply_ns=" << __prebind_connector_hotpath_pin_apply_ns
             << " time_prebind_route_baseline_ns=" << g_batch_dbg.time_pwrite_route_baseline_ns
             << " time_prebind_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_pwrite_route_delta_preserved_then_skeleton_ns
             << " time_prebind_route_connector_skeleton_ns=" << g_batch_dbg.time_pwrite_route_connector_skeleton_ns
             << " time_prebind_route_general_delta_ns=" << g_batch_dbg.time_pwrite_route_general_delta_ns
             << "\n";
        cerr << "prebind_calls=" << g_batch_dbg.pwrite_calls
             << " prebind_watch_handles_before=" << g_batch_dbg.pwrite_watch_handles_before
             << " prebind_watch_handles_after=" << g_batch_dbg.pwrite_watch_handles_after
             << " prebind_removed_handles=" << g_batch_dbg.pwrite_removed_handles
             << " prebind_cached_pointer_validate_calls=" << g_batch_dbg.pwrite_target_resolve_calls
             << " prebind_slot_owner_match_checks=" << g_batch_dbg.pwrite_target_resolve_calls
             << " prebind_slot_owner_match_hits=" << g_batch_dbg.pwrite_target_resolve_cache_hits
             << " prebind_same_pointer_fastpath_hits=" << g_batch_dbg.pwrite_same_value_skip_hits
             << " prebind_stale_pointer_detect_calls=" << std::max(0LL, g_batch_dbg.pwrite_target_resolve_calls - g_batch_dbg.pwrite_target_resolve_cache_hits)
             << " prebind_stale_pointer_detect_hits=" << std::max(0LL, g_batch_dbg.pwrite_target_resolve_calls - g_batch_dbg.pwrite_target_resolve_cache_hits)
             << " prebind_rebind_target_lookup_calls=" << std::max(0LL, g_batch_dbg.pwrite_target_resolve_calls - g_batch_dbg.pwrite_target_resolve_cache_hits)
             << " prebind_rebind_target_lookup_hits=" << std::max(0LL, g_batch_dbg.pwrite_target_resolve_calls - g_batch_dbg.pwrite_target_resolve_cache_hits)
             << " prebind_rebind_commit_calls=" << g_batch_dbg.pwrite_scalar_field_store_calls
             << " prebind_metadata_patch_calls=" << g_batch_dbg.pwrite_compact_pack_store_calls
             << " prebind_connector_hotpath_pin_apply_calls=" << g_batch_dbg.pwrite_target_resolve_cache_hits
             << " prebind_pointer_fetch_fallback_calls=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
             << " prebind_route_baseline_calls=" << g_batch_dbg.pwrite_route_baseline_calls
             << " prebind_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pwrite_route_delta_preserved_then_skeleton_calls
             << " prebind_route_connector_skeleton_calls=" << g_batch_dbg.pwrite_route_connector_skeleton_calls
             << " prebind_route_general_delta_calls=" << g_batch_dbg.pwrite_route_general_delta_calls
             << " prebind_validate_failures=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
             << " prebind_same_target_skip_hits=" << g_batch_dbg.pwrite_same_value_skip_hits
             << " prebind_rebind_skipped_same_target_calls=" << g_batch_dbg.pwrite_same_value_skip_hits
             << " prebind_connector_hotpath_hits=" << g_batch_dbg.pwrite_connector_hotpath_hits
             << " prebind_route_pointer_hoist_hit_reuse=" << g_batch_dbg.pwrite_target_resolve_cache_hits
             << "\n";
    }
    cerr << "pwrite_calls=" << g_batch_dbg.pwrite_calls
         << " pwrite_watch_handles_before=" << g_batch_dbg.pwrite_watch_handles_before
         << " pwrite_watch_handles_after=" << g_batch_dbg.pwrite_watch_handles_after
         << " pwrite_removed_handles=" << g_batch_dbg.pwrite_removed_handles
         << " pwrite_noop_guard_calls=" << g_batch_dbg.pwrite_noop_guard_calls
         << " pwrite_noop_guard_hits=" << g_batch_dbg.pwrite_noop_guard_hits
         << " pwrite_target_resolve_calls=" << g_batch_dbg.pwrite_target_resolve_calls
         << " pwrite_target_resolve_cache_hits=" << g_batch_dbg.pwrite_target_resolve_cache_hits
         << " pwrite_scalar_field_store_calls=" << g_batch_dbg.pwrite_scalar_field_store_calls
         << " pwrite_compact_pack_store_calls=" << g_batch_dbg.pwrite_compact_pack_store_calls
         << " pwrite_same_value_skip_hits=" << g_batch_dbg.pwrite_same_value_skip_hits
         << " pwrite_route_pointer_hoist_miss_calls=" << g_batch_dbg.pwrite_route_pointer_hoist_miss_calls
         << " pwrite_route_baseline_calls=" << g_batch_dbg.pwrite_route_baseline_calls
         << " pwrite_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.pwrite_route_delta_preserved_then_skeleton_calls
         << " pwrite_route_connector_skeleton_calls=" << g_batch_dbg.pwrite_route_connector_skeleton_calls
         << " pwrite_route_general_delta_calls=" << g_batch_dbg.pwrite_route_general_delta_calls
         << " pwrite_scalar_store_elided_calls=" << g_batch_dbg.pwrite_scalar_store_elided_calls
         << " pwrite_pack_store_skipped_calls=" << g_batch_dbg.pwrite_pack_store_skipped_calls
         << " pwrite_target_resolve_reuse_hits=" << g_batch_dbg.pwrite_target_resolve_reuse_hits
         << " pwrite_connector_hotpath_hits=" << g_batch_dbg.pwrite_connector_hotpath_hits
         << "\n";
    cerr << "chit_calls=" << g_batch_dbg.chit_calls
         << " chit_watch_handles_before=" << g_batch_dbg.chit_watch_handles_before
         << " chit_watch_handles_after=" << g_batch_dbg.chit_watch_handles_after
         << " chit_removed_handles=" << g_batch_dbg.chit_removed_handles
         << " chit_prev_state_writeback_calls=" << g_batch_dbg.chit_prev_state_writeback_calls
         << " chit_prev_state_scalar_store_calls=" << g_batch_dbg.chit_prev_state_scalar_store_calls
         << " chit_cursor_advance_calls=" << g_batch_dbg.chit_cursor_advance_calls
         << " chit_next_carry_prepare_calls=" << g_batch_dbg.chit_next_carry_prepare_calls
         << " chit_hit_bookkeeping_commits=" << g_batch_dbg.chit_hit_bookkeeping_commits
         << " chit_skip_pack_hits=" << g_batch_dbg.chit_skip_pack_hits
         << " chit_skip_reseed_hits=" << g_batch_dbg.chit_skip_reseed_hits
         << " chit_route_baseline_calls=" << g_batch_dbg.chit_route_baseline_calls
         << " chit_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.chit_route_delta_preserved_then_skeleton_calls
         << " chit_route_connector_skeleton_calls=" << g_batch_dbg.chit_route_connector_skeleton_calls
         << " chit_route_general_delta_calls=" << g_batch_dbg.chit_route_general_delta_calls
         << " chit_removed_ratio_ppm_sum=" << g_batch_dbg.chit_removed_ratio_ppm_sum
         << " chit_cursor_advance_same_slot_hits=" << g_batch_dbg.chit_cursor_advance_same_slot_hits
         << " chit_prev_state_writeback_elided_calls=" << g_batch_dbg.chit_prev_state_writeback_elided_calls
         << " chit_bookkeeping_skipped_calls=" << g_batch_dbg.chit_bookkeeping_skipped_calls
         << " chit_connector_hotpath_hits=" << g_batch_dbg.chit_connector_hotpath_hits
         << "\n";
    cerr << "creuse_calls=" << g_batch_dbg.creuse_calls
         << " creuse_watch_handles_before=" << g_batch_dbg.creuse_watch_handles_before
         << " creuse_watch_handles_after=" << g_batch_dbg.creuse_watch_handles_after
         << " creuse_removed_handles=" << g_batch_dbg.creuse_removed_handles
         << " creuse_checks=" << g_batch_dbg.creuse_checks
         << " creuse_hits=" << g_batch_dbg.creuse_hits
         << " creuse_misses=" << g_batch_dbg.creuse_misses
         << " creuse_boundary_validity_hits=" << g_batch_dbg.creuse_boundary_validity_hits
         << " creuse_prev_scalar_eq_hits=" << g_batch_dbg.creuse_prev_scalar_eq_hits
         << " creuse_transition_consistency_hits=" << g_batch_dbg.creuse_transition_consistency_hits
         << " creuse_hit_apply_prev_state_calls=" << g_batch_dbg.creuse_hit_apply_prev_state_calls
         << " creuse_hit_apply_cursor_advance_calls=" << g_batch_dbg.creuse_hit_apply_cursor_advance_calls
         << " creuse_miss_transition_reason_calls=" << g_batch_dbg.creuse_miss_transition_reason_calls
         << " creuse_miss_window_clip_reason_calls=" << g_batch_dbg.creuse_miss_window_clip_reason_calls
         << " creuse_reseed_handoff_calls=" << g_batch_dbg.creuse_reseed_handoff_calls
         << " creuse_route_baseline_calls=" << g_batch_dbg.creuse_route_baseline_calls
         << " creuse_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.creuse_route_delta_preserved_then_skeleton_calls
         << " creuse_route_connector_skeleton_calls=" << g_batch_dbg.creuse_route_connector_skeleton_calls
         << " creuse_route_general_delta_calls=" << g_batch_dbg.creuse_route_general_delta_calls
         << " creuse_removed_ratio_ppm_sum=" << g_batch_dbg.creuse_removed_ratio_ppm_sum
         << " creuse_hit_skip_scalar_pack_calls=" << g_batch_dbg.creuse_hit_skip_scalar_pack_calls
         << " creuse_hit_skip_reseed_calls=" << g_batch_dbg.creuse_hit_skip_reseed_calls
         << " creuse_boundary_reuse_assisted_hits=" << g_batch_dbg.creuse_boundary_reuse_assisted_hits
         << " creuse_miss_after_boundary_clip_hits=" << g_batch_dbg.creuse_miss_after_boundary_clip_hits
         << "\n";
    cerr << "time_sload_prev_seed_materialize_ns=" << g_batch_dbg.time_sload_prev_seed_materialize_ns
         << " time_sload_prev_seed_materialize_calls=" << g_batch_dbg.time_sload_prev_seed_materialize_calls
         << " time_sload_prev_carry_reuse_check_ns=" << g_batch_dbg.time_sload_prev_carry_reuse_check_ns
         << " time_sload_prev_carry_reuse_check_calls=" << g_batch_dbg.time_sload_prev_carry_reuse_check_calls
         << " time_sload_prev_carry_reuse_apply_ns=" << g_batch_dbg.time_sload_prev_carry_reuse_apply_ns
         << " time_sload_prev_carry_reuse_apply_calls=" << g_batch_dbg.time_sload_prev_carry_reuse_apply_calls
         << " time_sload_curr_direct_load_ns=" << g_batch_dbg.time_sload_curr_direct_load_ns
         << " time_sload_curr_direct_load_calls=" << g_batch_dbg.time_sload_curr_direct_load_calls
         << " time_sload_curr_source_lookup_ns=" << g_batch_dbg.time_sload_curr_source_lookup_ns
         << " time_sload_curr_source_lookup_calls=" << g_batch_dbg.time_sload_curr_source_lookup_calls
         << " time_sload_state_pair_pack_ns=" << g_batch_dbg.time_sload_state_pair_pack_ns
         << " time_sload_state_pair_pack_calls=" << g_batch_dbg.time_sload_state_pair_pack_calls
         << " time_sload_invariant_ref_hoist_miss_ns=" << g_batch_dbg.time_sload_invariant_ref_hoist_miss_ns
         << " time_sload_invariant_ref_hoist_miss_calls=" << g_batch_dbg.time_sload_invariant_ref_hoist_miss_calls
         << " time_sload_route_baseline_ns=" << g_batch_dbg.time_sload_route_baseline_ns
         << " time_sload_route_delta_preserved_then_skeleton_ns=" << g_batch_dbg.time_sload_route_delta_preserved_then_skeleton_ns
         << " time_sload_route_connector_skeleton_ns=" << g_batch_dbg.time_sload_route_connector_skeleton_ns
         << " time_sload_route_general_delta_ns=" << g_batch_dbg.time_sload_route_general_delta_ns
         << "\n";
    cerr << "sload_calls=" << g_batch_dbg.sload_calls
         << " sload_watch_handles_before=" << g_batch_dbg.sload_watch_handles_before
         << " sload_watch_handles_after=" << g_batch_dbg.sload_watch_handles_after
         << " sload_removed_handles=" << g_batch_dbg.sload_removed_handles
         << " sload_prev_seed_calls=" << g_batch_dbg.sload_prev_seed_calls
         << " sload_prev_carry_reuse_checks=" << g_batch_dbg.sload_prev_carry_reuse_checks
         << " sload_prev_carry_reuse_hits=" << g_batch_dbg.sload_prev_carry_reuse_hits
         << " sload_curr_direct_load_calls=" << g_batch_dbg.sload_curr_direct_load_calls
         << " sload_curr_source_lookup_calls=" << g_batch_dbg.sload_curr_source_lookup_calls
         << " sload_curr_source_lookup_hits=" << g_batch_dbg.sload_curr_source_lookup_hits
         << " sload_curr_source_lookup_misses=" << g_batch_dbg.sload_curr_source_lookup_misses
         << " sload_state_pair_pack_calls=" << g_batch_dbg.sload_state_pair_pack_calls
         << " sload_invariant_ref_hoist_hits=" << g_batch_dbg.sload_invariant_ref_hoist_hits
         << " sload_invariant_ref_hoist_misses=" << g_batch_dbg.sload_invariant_ref_hoist_misses
         << " sload_route_baseline_calls=" << g_batch_dbg.sload_route_baseline_calls
         << " sload_route_delta_preserved_then_skeleton_calls=" << g_batch_dbg.sload_route_delta_preserved_then_skeleton_calls
         << " sload_route_connector_skeleton_calls=" << g_batch_dbg.sload_route_connector_skeleton_calls
         << " sload_route_general_delta_calls=" << g_batch_dbg.sload_route_general_delta_calls
         << " sload_removed_ratio_ppm_sum=" << g_batch_dbg.sload_removed_ratio_ppm_sum
         << " sload_prev_state_scalar_reuse_hits=" << g_batch_dbg.sload_prev_state_scalar_reuse_hits
         << " sload_curr_state_direct_struct_hits=" << g_batch_dbg.sload_curr_state_direct_struct_hits
         << " sload_tuple_pack_skipped_calls=" << g_batch_dbg.sload_tuple_pack_skipped_calls
         << " sload_invariant_pointer_hoist_hits=" << g_batch_dbg.sload_invariant_pointer_hoist_hits
         << "\n";
    if (!g_slow_deletion_profiles.empty()) {
        ...
        for (size_t i = 0; i < prof.size(); ++i) {
            ...
            cerr << "slow_del[" << i << "] idx=" << r.deletionIndex
                 ...
                 << " chit_prev_state_writeback_calls=" << r.chitPrevStateWritebackCalls
                 << " chit_prev_state_scalar_store_calls=" << r.chitPrevStateScalarStoreCalls
                 << " chit_cursor_advance_calls=" << r.chitCursorAdvanceCalls
                 << " chit_next_carry_prepare_calls=" << r.chitNextCarryPrepareCalls
                 << " chit_hit_bookkeeping_commits=" << r.chitHitBookkeepingCommits
                 << " chit_skip_pack_hits=" << r.chitSkipPackHits
                 << " chit_skip_reseed_hits=" << r.chitSkipReseedHits
                 << " chit_total_ns=" << r.timeChitTotalNs
                 << " pwrite_noop_guard_hits=" << r.pwriteNoopGuardHits
                 << " pwrite_target_resolve_calls=" << r.pwriteTargetResolveCalls
                 << " pwrite_target_resolve_cache_hits=" << r.pwriteTargetResolveCacheHits
                 << " pwrite_scalar_field_store_calls=" << r.pwriteScalarFieldStoreCalls
                 << " pwrite_compact_pack_store_calls=" << r.pwriteCompactPackStoreCalls
                 << " pwrite_same_value_skip_hits=" << r.pwriteSameValueSkipHits
                 << " pwrite_total_ns=" << r.timePwriteTotalNs
                 << " prebind_cached_pointer_validate_calls=" << r.pwriteTargetResolveCalls
                 << " prebind_slot_owner_match_checks=" << r.pwriteTargetResolveCalls
                 << " prebind_slot_owner_match_hits=" << r.pwriteTargetResolveCacheHits
                 << " prebind_same_pointer_fastpath_hits=" << r.pwriteSameValueSkipHits
                 << " prebind_stale_pointer_detect_calls=" << std::max(0LL, r.pwriteTargetResolveCalls - r.pwriteTargetResolveCacheHits)
                 << " prebind_rebind_target_lookup_calls=" << std::max(0LL, r.pwriteTargetResolveCalls - r.pwriteTargetResolveCacheHits)
                 << " prebind_rebind_commit_calls=" << r.pwriteScalarFieldStoreCalls
                 << " prebind_metadata_patch_calls=" << r.pwriteCompactPackStoreCalls
                 << " prebind_total_ns=" << r.timePwriteTotalNs
                << " dprebind_cached_pointer_validate_calls=" << r.dprebindCachedPointerValidateCalls
                << " dprebind_slot_owner_match_checks=" << r.dprebindSlotOwnerMatchChecks
                << " dprebind_stale_pointer_detect_hits=" << r.dprebindStalePointerDetectHits
                << " dprebind_rebind_target_lookup_calls=" << r.dprebindRebindTargetLookupCalls
                << " dprebind_rebind_commit_calls=" << r.dprebindRebindCommitCalls
                << " dprebind_metadata_patch_calls=" << r.dprebindMetadataPatchCalls
                << " dprebind_same_pointer_fastpath_hits=" << r.dprebindSamePointerFastpathHits
                << " dprebind_total_ns=" << r.timeDprebindTotalNs
                 ...
                 << " tbranch_route=" << reuse_route_name(r.tbranchRouteTag)
                 << " tbranch_prev_state_materialize_calls=" << r.tbranchPrevStateMaterializeCalls
                 << " tbranch_curr_state_load_calls=" << r.tbranchCurrStateLoadCalls
                 << " tbranch_compare_ops=" << r.tbranchCompareOps
                 << " tbranch_removed_to_kept_taken=" << r.tbranchRemovedToKeptTaken
                 << " tbranch_kept_to_removed_taken=" << r.tbranchKeptToRemovedTaken
                 << " tbranch_no_transition_steps=" << r.tbranchNoTransitionSteps
                 << " tbranch_window_clipped_calls=" << r.tbranchWindowClippedCalls
                 << " tbranch_total_ns=" << r.timeTbranchTotalNs
                 << " sload_route=" << reuse_route_name(r.sloadRouteTag)
                 << " sload_prev_seed_calls=" << r.sloadPrevSeedCalls
                 << " sload_prev_carry_reuse_hits=" << r.sloadPrevCarryReuseHits
                 << " sload_curr_direct_load_calls=" << r.sloadCurrDirectLoadCalls
                 << " sload_curr_source_lookup_calls=" << r.sloadCurrSourceLookupCalls
                 << " sload_curr_source_lookup_misses=" << r.sloadCurrSourceLookupMisses
                 << " sload_state_pair_pack_calls=" << r.sloadStatePairPackCalls
                 << " sload_invariant_ref_hoist_hits=" << r.sloadInvariantRefHoistHits
                 << " pcarry_route=" << reuse_route_name(r.pcarryRouteTag)
                 << " pcarry_prev_seed_calls=" << r.pcarryPrevSeedCalls
                 << " pcarry_prev_seed_from_boundary_calls=" << r.pcarryPrevSeedFromBoundaryCalls
                 << " pcarry_prev_seed_from_reseed_calls=" << r.pcarryPrevSeedFromReseedCalls
                 << " pcarry_prev_scalar_materialize_calls=" << r.pcarryPrevScalarMaterializeCalls
                 << " pcarry_carry_reuse_hits=" << r.pcarryCarryReuseHits
                 << " pcarry_carry_reuse_misses=" << r.pcarryCarryReuseMisses
                 << " pcarry_transition_invalidate_calls=" << r.pcarryTransitionInvalidateCalls
                 << " pcarry_window_clip_invalidate_calls=" << r.pcarryWindowClipInvalidateCalls
                 << " pcarry_total_ns=" << r.timePcarryTotalNs
                 << " sload_total_ns=" << r.timeSloadTotalNs
