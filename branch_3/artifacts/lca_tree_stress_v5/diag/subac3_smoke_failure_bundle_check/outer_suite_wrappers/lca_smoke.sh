...
print(retryable)
print(message)
print("" if solver_exit is None else solver_exit)
print("" if solver_signal is None else solver_signal)
PY
  ); then
    return 1
  fi

  if (( ${#parsed[@]} > 0 )) && [[ "${parsed[0]}" == ERROR:* ]]; then
    return 1
  fi
  if (( ${#parsed[@]} < 7 )); then
    return 1
  ...
  CURRENT_FAILURE_KIND="${parsed[0]}"
  CURRENT_FAILURE_ORIGIN="${parsed[1]}"
  raw_exit_code="${parsed[2]}"
  CURRENT_FAILURE_RETRYABLE="${parsed[3]}"
  CURRENT_FAILURE_SUMMARY="${parsed[4]}"
  CURRENT_FAILURE_SOLVER_EXIT="${parsed[5]}"
  CURRENT_FAILURE_SIGNAL="${parsed[6]}"
...
print("" if validator_ok is None else int(bool(validator_ok)))
print(checker_result_kind)
print(message)
PY
    ); then
      if (( ${#checker_fields[@]} >= 6 )); then
        recorded_status="${checker_fields[0]}"
        recorded_category="${checker_fields[1]}"
        recorded_exit_code="${checker_fields[2]}"
        recorded_validator_ok="${checker_fields[3]}"
        checker_result_kind="${checker_fields[4]}"
        recorded_message="${checker_fields[5]}"
      fi
    fi
  fi
