        ...
        if (__kvec_opt) {
            if (removedIsSuffix) {
                ...
                if (__wscan_volume) {
                    ...
                    if (__kvec_opt && __scomp_opt && __bcopy_opt) { g_batch_dbg.plan_watch_handles_after += (long long)st.watchHandles.size(); if (__plan_opt) g_batch_dbg.rdisc_watch_handles_after += (long long)st.watchHandles.size(); }
