...
class LiteraturePotentialOracle final : public NBOracle {
    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask, const vector<int>* sparseRemoveIdxs = nullptr) {
        ...
        if (singleMiddleRun) {
            ...
            if (__wscan_volume) g_batch_dbg.scomp_single_middle_run_calls++;
        }
#endif
...
#ifdef LOCAL
                if (__wscan_detail) __dt_scomp_kept_run_partition_build += dbg_now_ns() - __run_build_start_ns;
#endif
                int write = firstRemoved;
                if (singleMiddleRun && !__scratch_kept_runs.empty()) {
                    int srcL = __scratch_kept_runs[0].first;
                    int srcR = __scratch_kept_runs[0].second;
                    int len = srcR - srcL + 1;
                    if (len > 0) {
#ifdef LOCAL
