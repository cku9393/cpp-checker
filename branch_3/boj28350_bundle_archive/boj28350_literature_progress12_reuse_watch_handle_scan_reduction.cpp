...
class LiteraturePotentialOracle final : public NBOracle {
    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask) {
...
#ifdef LOCAL
        if (__wscan_volume && removedCnt == 0) g_batch_dbg.wscan_retain_noop_calls++;
#endif
        if (removedCnt == 0) return;
#ifdef LOCAL
        if (__wscan_volume) g_batch_dbg.wscan_retain_removed_handles += removedCnt;
#endif
        for (int i = 0; i < m; ++i) if (remove[i]) {
            ...
            WatchEntry moved = vec.back();
#ifdef LOCAL
            long long __remove_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
            if (movedNeeded) vec[h.slotPos] = moved;
            vec.pop_back();
#ifdef LOCAL
            if (__wscan_detail) {
                acc_wscan_retain_ns(dbg_now_ns() - __remove_start_ns,
            ...
            if (movedNeeded) {
                ...
                if (1 <= moved.owner && moved.owner <= n_) {
#ifdef LOCAL
                    if (__wscan_volume) g_batch_dbg.wscan_retain_owner_state_lookups++;
#endif
                    auto itOwner = ownerData_[moved.owner].classStates.find(moved.cid);
                    if (itOwner != ownerData_[moved.owner].classStates.end() &&
                        0 <= moved.handleIdx && moved.handleIdx < (int)itOwner->second.watchHandles.size()) {
...
#ifdef LOCAL
                if (__wscan_detail) {
                    acc_wscan_retain_ns(dbg_now_ns() - __owner_lookup_start_ns,
                ...
                if (movedState) {
#ifdef LOCAL
                    long long __slot_fix_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
                    movedState->watchHandles[moved.handleIdx].slotPos = h.slotPos;
#ifdef LOCAL
                    if (__wscan_volume) g_batch_dbg.wscan_retain_slotpos_fixups++;
                    if (__wscan_detail) {
                        acc_wscan_retain_ns(dbg_now_ns() - __slot_fix_start_ns,
...
#ifdef LOCAL
            long long __compact_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
            kept.push_back(st.watchHandles[i]);
            auto& h = kept.back();
#ifdef LOCAL
            if (__wscan_detail) {
                acc_wscan_retain_ns(dbg_now_ns() - __compact_start_ns,
...
#ifdef LOCAL
                    long long __handle_fix_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
                    vec[h.slotPos].handleIdx = newIdx;
                    vec[h.slotPos].owner = owner;
                    vec[h.slotPos].cid = cid;
#ifdef LOCAL
                    if (__wscan_volume) g_batch_dbg.wscan_retain_handleidx_fixups++;
                    if (__wscan_detail) {
                        acc_wscan_retain_ns(dbg_now_ns() - __handle_fix_start_ns,
