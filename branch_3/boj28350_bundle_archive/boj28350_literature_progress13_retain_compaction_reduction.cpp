...
class LiteraturePotentialOracle final : public NBOracle {
    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask, const vector<int>* sparseRemoveIdxs = nullptr) {
...
#ifdef LOCAL
        if (__wscan_volume) {
            if (m > 0) g_batch_dbg.retain_remove_ratio_ppm_sum += (1000000LL * removedCnt) / m;
            if (removedCnt == 0) {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            if (__wscan_volume) g_batch_dbg.retain_kept_handles_copied++;
#endif
            auto& h = kept.back();
...
#ifdef LOCAL
                        if (__wscan_volume) g_batch_dbg.retain_skip_handleidx_patch_calls++;
#endif
                    }
                }
            }
            ...
            if (__wscan_detail) {
                ...
                __dt_kept_vector_build += __overhead_dt;
                __dt_kept_handle_copy += __copy_dt;
                __dt_kept_handleidx_patch += __patch_dt;
            }
#endif
        }
#ifdef LOCAL
        ...
        st.watchActive = !st.watchHandles.empty();
#ifdef LOCAL
        if (__wscan_detail) __dt_final_swap_state_update += dbg_now_ns() - __final_swap_start_ns;
        if (__wscan_volume) {
            g_batch_dbg.retain_final_swap_calls++;
