...
#endif
#ifdef LOCAL
                    if (__wscan_volume && __pcarry_metric && !__pcarry_reuse_prev_state) {
                        g_batch_dbg.pcarry_transition_forced_reseed_hits++;
                        __pcarry_transition_invalidate_calls++;
                    }
                    long long __pcarry_inval_start = (__wscan_detail && __pcarry_metric && !__pcarry_reuse_prev_state) ? dbg_now_ns() : 0;
#endif
                    prevRemovedLocal = curRemovedLocal;
                    if (__sload_opt) __sload_prev_carry = curRemovedLocal;
#ifdef LOCAL
                    if (__wscan_detail && __pcarry_metric && !__pcarry_reuse_prev_state) {
