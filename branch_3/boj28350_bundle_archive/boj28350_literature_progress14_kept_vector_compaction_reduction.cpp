    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask, const vector<int>* sparseRemoveIdxs = nullptr) {
...
#endif
        if (firstRemoved > 0) {
            // unchanged prefix already accounted for volume only
        }
#ifdef LOCAL
        if (__wscan_detail) __dt_kvec_prefix_fastpath_check += dbg_now_ns() - __prefix_check_start_ns;
        long long __suffix_check_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
        if (firstRemoved == newSize) {
            removedIsSuffix = true;
            for (int k = 0; k < removedCnt; ++k) {
                if (removedIdxs[k] != firstRemoved + k) {
                    removedIsSuffix = false;
                    break;
                }
            }
        }
#ifdef LOCAL
        if (__wscan_detail) __dt_kvec_suffix_fastpath_check += dbg_now_ns() - __suffix_check_start_ns;
#endif
        auto __patch_handleidx = [&](int newIdx) {
            ...
            if (needPatch) {
                ...
                if (__wscan_detail) {
                    long long __skip_start_ns = dbg_now_ns();
                    __dt_kvec_handleidx_patch_skip_same_index += dbg_now_ns() - __skip_start_ns;
                }
#endif
            }
        };
        if (__kvec_opt) {
            if (removedIsSuffix) {
                ...
                st.watchActive = !st.watchHandles.empty();
#ifdef LOCAL
                if (__wscan_detail) __dt_kvec_final_resize_or_swap += dbg_now_ns() - __final_resize_start_ns;
                if (__wscan_volume) g_batch_dbg.kvec_watch_handles_after += (long long)st.watchHandles.size();
#endif
            } else {
#ifdef LOCAL
...
#ifdef LOCAL
                if (__wscan_detail) __dt_kvec_kept_count_scan += dbg_now_ns() - __count_scan_start_ns;
                long long __scratch_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
                vector<int> patchNewIdxs;
                patchNewIdxs.reserve(max(0, keptAfterPrefix));
#ifdef LOCAL
                if (__wscan_detail) __dt_kvec_scratch_prepare += dbg_now_ns() - __scratch_start_ns;
#endif
                int write = firstRemoved;
                int removedPtr = 1;
                for (int i = firstRemoved + 1; i < m; ++i) {
                    while (removedPtr < removedCnt && removedIdxs[removedPtr] < i) ++removedPtr;
                    if (removedPtr < removedCnt && removedIdxs[removedPtr] == i) continue;
#ifdef LOCAL
...
#ifdef LOCAL
                        if (__wscan_volume) {
                            g_batch_dbg.kvec_inplace_write_same_slot_hits++;
...
#endif
                        st.watchHandles[write] = st.watchHandles[i];
#ifdef LOCAL
...
#endif
                        patchNewIdxs.push_back(write);
                    }
                    ++write;
#ifdef LOCAL
                    if (__wscan_detail) {
                        long long __emit_dt = dbg_now_ns() - __emit_start_ns;
                        long long __ov = __emit_dt - __copy_dt;
                        if (__ov < 0) __ov = 0;
                        __dt_kvec_stable_emit_moved_suffix += __ov;
...
#ifdef LOCAL
                if (__wscan_volume) {
                    g_batch_dbg.kvec_changed_patchlist_entries += (long long)patchNewIdxs.size();
                    __kvec_changed_patchlist_entries += (long long)patchNewIdxs.size();
                }
                long long __patchlist_build_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
                // patch list already built inline above
#ifdef LOCAL
                if (__wscan_detail) __dt_kvec_patchlist_build += dbg_now_ns() - __patchlist_build_start_ns;
#endif
                for (int newIdx : patchNewIdxs) __patch_handleidx(newIdx);
#ifdef LOCAL
                long long __final_resize_start_ns = __wscan_detail ? dbg_now_ns() : 0;
#endif
                st.watchHandles.resize(newSize);
            ...
            for (int i = 0; i < m; ++i) {
                ...
                if (__wscan_detail) {
                    ...
                    __dt_kvec_stable_emit_moved_suffix += __overhead_dt;
                    __dt_kept_handle_copy += __copy_dt;
            ...
            st.watchActive = !st.watchHandles.empty();
#ifdef LOCAL
            if (__wscan_detail) __dt_kvec_final_resize_or_swap += dbg_now_ns() - __final_swap_start_ns;
            if (__wscan_volume) g_batch_dbg.kvec_watch_handles_after += (long long)st.watchHandles.size();
#endif
        }
#ifdef LOCAL
