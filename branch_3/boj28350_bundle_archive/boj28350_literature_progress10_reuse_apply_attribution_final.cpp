...
class LiteraturePotentialOracle final : public NBOracle {
    ...
    void noteReuseWatchFullScan(const ClassState& st) {
        ...
        g_batch_dbg.reuse_watch_handle_full_scan_handles += (long long)st.watchHandles.size();
    }
#endif
    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask) {
#ifdef LOCAL
    ...
    bool applyPieceNativeReuseForClassBaseline(int owner, int cid,
        ...
        ScopedNsAcc __keepmask_timer(ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_keepmask_scan_ns),
                                     ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_keepmask_scan_calls));
#endif
        for (int i = 0; i < (int)st.watchHandles.size(); ++i) {
            const auto& h = st.watchHandles[i];
            if (h.originKind == SupportOriginKind::ConnectorTree) {
...
#ifdef LOCAL
        {
            ScopedNsAcc __retain_timer(ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_watch_retain_ns),
                                       ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_watch_retain_calls));
    ...
    bool applyConnectorSkeletonRebuildForClass(int owner, int cid,
...
#endif
#ifdef LOCAL
        ScopedNsAcc __old_attachment_map_timer(ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_old_attachment_map_build_ns),
        ...
        auto findXLocalPosForPiece = [&](const SupportPieceRef& piece) -> int {
            for (const auto& ph : info.pieceHits) {
                if (ph.pieceId == piece.pieceId && ph.treeId == piece.treeId) return ph.localPos;
            }
            const auto* tree = getSupportTreeObject(piece.treeId);
            if (!tree || !(1 <= currentDeleteX_ && currentDeleteX_ <= n_)) return -1;
            int stamp = 0;
            buildTreeVertexPosMap(*tree, stamp);
            if (supportScratch_.supportPosStamp[currentDeleteX_] != stamp) return -1;
            return supportScratch_.supportPosVal[currentDeleteX_];
        };
        for (const auto& p : st.preservedPieces) {
            if (!p.pieceAlive || p.pieceEndpointCount <= 0) continue;
            int oldAtt = oldAttachmentByPiece.count(p.pieceId) ? oldAttachmentByPiece[p.pieceId] : -1;
            bool containsX = (1 <= currentDeleteX_ && currentDeleteX_ <= n_ && pieceContainsVertex(p, currentDeleteX_));
            if (!containsX) {
                newPreserved.push_back(p);
            ...
            if (xLocalPos < 0 || !splitPieceAtLocalPos(owner, p, xLocalPos, replacements, removedPieceVertices, boundaryOps, &deadIdx)) {
#ifdef LOCAL
                g_batch_dbg.piece_materialize_fallback_calls++;
            ...
            for (auto& rp : replacements) {
                ...
                int att = oldAtt;
                const auto* tree = getSupportTreeObject(rp.treeId);
                bool ok = false;
                if (tree && 1 <= att && att <= n_ && topo_.aliveVertex(att)) {
                    int pstamp = 0;
                    buildTreeVertexPosMap(*tree, pstamp);
...
#endif
                    att = retargetAttachmentIfDead(*tree, rp);
                }
        ...
        for (size_t i = 0; i < newPreserved.size(); ++i) {
            ...
            const auto* tree = getSupportTreeObject(newPreserved[i].treeId);
            bool ok = false;
            if (tree && 1 <= att && att <= n_ && topo_.aliveVertex(att)) {
                int pstamp = 0;
                buildTreeVertexPosMap(*tree, pstamp);
            ...
            if (!ok && tree) {
                ...
                if (1 <= newAtt && newAtt <= n_ && newAtt != att) ++retargets;
        ...
        if (!(1 <= currentDeleteX_ && currentDeleteX_ <= n_)) {
            ...
            for (const auto& p : newPreserved) {
                ...
                forEachPiecePos(*tree, p, [&](int pos){
                    int v = tree->vertexByPos[pos];
                    if (1 <= v && v <= n_) supportScratch_.collectStamp[v] = keepStamp;
                });
            }
        ...
        vector<int> terminals = normalizedAliveAttachmentVertices(st.attachmentVerticesByPiece);
        int newConnectorTreeId = -1;
...
#ifdef LOCAL
        {
            ScopedIntInc __ctx(&g_connector_skeleton_unregister_ctx);
            ScopedNsAcc __retain_timer(ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_watch_retain_ns),
                                       ptr_if(local_profile_detailed_enabled(), &g_batch_dbg.time_reuse_watch_retain_calls));
            retainClassWatchByKeepMask(owner, cid, st, keepConnectorMask);
        }
#else
        retainClassWatchByKeepMask(owner, cid, st, keepConnectorMask);
#endif
