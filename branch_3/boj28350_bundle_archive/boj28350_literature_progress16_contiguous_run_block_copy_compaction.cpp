    ...
    void retainClassWatchByKeepMask(int owner, int cid, ClassState& st, const vector<char>& keepMask, const vector<int>* sparseRemoveIdxs = nullptr) {
...
#ifdef LOCAL
        if (__wscan_detail) __dt_scomp_first_removed_seek += dbg_now_ns() - __first_seek_start_ns;
...
#ifdef LOCAL
        if (__wscan_detail) {
            long long __dt = dbg_now_ns() - __suffix_check_start_ns;
...
#ifdef LOCAL
        if (__wscan_detail) __dt_bcopy_single_middle_run_detect += dbg_now_ns() - __single_detect_start_ns;
        if (singleMiddleRun) {
        ...
        if (__kvec_opt) {
            if (removedIsSuffix) {
                ...
                __scratch_kept_runs.reserve((size_t)max(1, keptRunCount));
#ifdef LOCAL
                if (__wscan_detail) {
                ...
                if (!singleMiddleRun) {
                    ...
                    for (int i = 1; i < removedCnt; ++i) {
                        if (prevRemoved + 1 <= removedIdxs[i] - 1) __scratch_kept_runs.emplace_back(prevRemoved + 1, removedIdxs[i] - 1);
                        prevRemoved = removedIdxs[i];
                    }
                    if (lastRemoved + 1 <= m - 1) __scratch_kept_runs.emplace_back(lastRemoved + 1, m - 1);
                    if (__bcopy_opt && __scratch_kept_runs.size() >= 2) {
                        size_t wr = 0;
                ...
                if (__wscan_detail) {
                    ...
                    __dt_bcopy_run_coalesce_build += __dt;
                }
                __bcopy_copy_plan_entries += (long long)__scratch_kept_runs.size();
                if (__wscan_volume && __kvec_opt && __scomp_opt) g_batch_dbg.bcopy_copy_plan_entries += (long long)__scratch_kept_runs.size();
#endif
                ...
                if (singleMiddleRun) {
                    ...
                    for (auto [srcL, srcR] : __scratch_kept_runs) {
                        int len = srcR - srcL + 1;
                        if (len <= 0) continue;
                        bool sameSlot = (write == srcL);
