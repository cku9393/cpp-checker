...
GEN = ROOT / "branch_gen_case_local.py"
VAL = ROOT / "branch_validator_local.py"
...
def mode_list_from_generator(*, cwd: Path) -> List[str]:
    import subprocess

    res = subprocess.run(
        [sys.executable, str(GEN), "--list-modes"],
        capture_output=True,
        text=True,
        cwd=ensure_case_outdir(cwd),
    )
    if res.returncode != 0:
...
def build_case_solver_env(case_dir: Path, mode: str, n: int, seed: int) -> Dict[str, str]:
    case_dir = ensure_case_outdir(case_dir)
    env = os.environ.copy()
...
def resolve_certify_out_dir(path_like: str | Path | None) -> Path:
    return resolve_output_path(path_like, default_key="branch_outer_certify")
...
def run_one_case(
    ...
    out_dir = ensure_case_outdir(out_dir)
    case_dir = ensure_under_artifacts(
        (out_dir / "runs" / stage_name / mode / f"n{n}" / f"seed{seed}_L{shuffle_labels}_Q{shuffle_queries}").resolve()
    )
    ...
    rc_gen, to_gen, _ = run_cmd(
        gen_cmd,
        stdout_path=in_path,
        stderr_path=gen_stderr,
        timeout=None,
        cwd=case_dir,
    )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=float,
        default=1.0,
        help="Backward-compatible scale for time-based limits; applied together with --limit-scale.",
    )
    ...
    out = resolve_certify_out_dir(args.out)
    ...
    for stage in preset["stages"]:
        ...
        modes = stage.get("modes") or mode_list_from_generator(cwd=out)
    ...
    slow_rows = [
        [r.stage, r.mode, str(r.n), str(r.seed), f"{r.sec:.3f}", "-" if r.rss_kb is None else str(r.rss_kb), r.case_dir]
        for r in slowest
    ]
