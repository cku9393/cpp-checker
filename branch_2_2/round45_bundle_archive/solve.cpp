...
inline void appendTsv(const std::string& path, const std::vector<std::string>& cols){ std::ofstream ofs(path, std::ios::app); for(size_t i=0;i<cols.size();++i){ if(i) ofs << '\t'; ofs << cols[i]; } ofs << '\n'; }
...
bool decomposeSeriesFixed(const Solver& s,
    ...
    for (int cand : G.verts) {
        ...
        if (__prof_on) ++__prof_call.ds_candidate_count;
#endif
