...
원래 번들 루트 이름은 `lca_tree_stress_v5`였고, 현재 아카이브 이름은 `round45_bundle_archive`다.
루트의 build/run/gate 동선은 그대로 유지하고, historical artifacts만 `reports/`, `history/`, `runs/`, `build/`, `fixtures/` 아래로 정리했다.
예전 이름이 들어간 보고서와 hold 문서는 provenance 보존을 위해 그대로 둔다.
shell wrapper와 일부 계측 실행은 번들 당시 Linux/GNU userland 기준으로 작성돼 있다.
...
```bash
python3 gen_case.py --describe-modes
```
...
```bash
cd branch_2_2/round45_bundle_archive
./build.sh
```
...
```bash
python3 find_breakpoint.py --solver ./solve --mode multi_comb_cap \
...
```bash
python3 gen_case.py --mode comb_rect_dense --n 99999 --seed 1 \
