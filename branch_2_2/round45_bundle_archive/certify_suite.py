        ...
        for mode in modes:
            for n in sizes:
                for seed in seeds:
                    for sl in shuf_l:
                        for sq in shuf_q:
                            row = run_one_case(solver, out, stage_name, mode, n, seed, sl, sq, timeout)
                            rows.append(row)
                            sec_txt = '-' if row.sec is None else f'{row.sec:.3f}'
                            rss_txt = '-' if row.rss_kb is None else str(row.rss_kb)
                            print(f'[certify] stage={stage_name} mode={mode} n={n} seed={seed} L={sl} Q={sq} gen={row.gen_ok} rc={row.solver_rc} to={row.timed_out} val={row.val_ok} sec={sec_txt} rss={rss_txt}', flush=True)
