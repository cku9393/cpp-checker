...
#endif
#ifndef DENSE_DECOMPOSESERIES_ROUND37_PROFILE
#define DENSE_DECOMPOSESERIES_ROUND37_PROFILE 0
#endif
...
#define DENSE_LOCALIDADJ_ROUND9_PROFILE 1
#endif
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
#undef DENSE_RECT_ROUND1_PROFILE
#define DENSE_RECT_ROUND1_PROFILE 1
#undef DENSE_RECT_KEEP_ORDER_ROUND4_PROFILE
#define DENSE_RECT_KEEP_ORDER_ROUND4_PROFILE 1
...
namespace dense_decomposeseries_round37_prof {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
using Clock = std::chrono::steady_clock;
inline long long nsSince(Clock::time_point t0){ return std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now()-t0).count(); }
inline double ms(long long ns){ return (double)ns / 1e6; }
...
bool decomposeSeriesFixed(const Solver& s,
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
    dense_decomposeseries_round37_prof::RowMeta __prof_call;
    bool __prof_on = dense_decomposeseries_round37_prof::R().active;
    auto __prof_t0 = dense_decomposeseries_round37_prof::Clock::now();
    auto __prof_finalize = [&](bool retv) {
        if (__prof_on) {
            __prof_call.ds_total_ns = dense_decomposeseries_round37_prof::nsSince(__prof_t0);
            ...
            dense_decomposeseries_round37_prof::noteDecomposeSeriesCall(__prof_call);
        }
        return retv;
    };
#endif
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
    auto __t_build = dense_decomposeseries_round37_prof::Clock::now();
#endif
    EdgeSubsetInfo G = buildEdgeSubsetInfo(s, edgeIds);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
    if (__prof_on) __prof_call.ds_build_subsetinfo_ns += dense_decomposeseries_round37_prof::nsSince(__t_build);
#endif
    auto its = G.lid.find(sTerm), itt = G.lid.find(tTerm);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
    ...
    for (int cand : G.verts) {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        auto __cand_t0 = dense_decomposeseries_round37_prof::Clock::now();
        long long __cand_parts_ns = 0;
        if (__prof_on) ++__prof_call.ds_candidate_count;
#endif
        if (cand == sTerm || cand == tTerm) {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            if (__prof_on) __prof_call.ds_candidate_loop_ns += dense_decomposeseries_round37_prof::nsSince(__cand_t0);
#endif
            continue;
        }
        int lm = G.lid[cand];
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        auto __t_compinit = dense_decomposeseries_round37_prof::Clock::now();
#endif
        std::vector<int> comp(G.verts.size(), -1);
        comp[lm] = -2; // removed
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        if (__prof_on) {
            long long d = dense_decomposeseries_round37_prof::nsSince(__t_compinit);
        ...
        for (int src = 0; src < (int)G.verts.size(); ++src) {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            auto __t_seed = dense_decomposeseries_round37_prof::Clock::now();
#endif
            bool skipSrc = (src == lm || comp[src] != -1);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            if (__prof_on) {
                long long d = dense_decomposeseries_round37_prof::nsSince(__t_seed);
...
#endif
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            auto __t_q0 = dense_decomposeseries_round37_prof::Clock::now();
#endif
            q.push(src);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            if (__prof_on) {
                long long d = dense_decomposeseries_round37_prof::nsSince(__t_q0);
...
#endif
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            auto __t_lw0 = dense_decomposeseries_round37_prof::Clock::now();
#endif
            comp[src] = cid;
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            if (__prof_on) {
                long long d = dense_decomposeseries_round37_prof::nsSince(__t_lw0);
            ...
            while (!q.empty()) {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                auto __t_qpop = dense_decomposeseries_round37_prof::Clock::now();
#endif
                int u = q.front(); q.pop();
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                if (__prof_on) {
                    long long d = dense_decomposeseries_round37_prof::nsSince(__t_qpop);
                    __prof_call.ds_bfs_queue_pushpop_ns += d;
                    __cand_parts_ns += d;
                    ++__prof_call.ds_queue_pushpop_hits;
                }
                ...
                for (auto [v, e] : G.adj[u]) {
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    auto __t_nei = dense_decomposeseries_round37_prof::Clock::now();
#endif
                    (void)e;
                    bool skipNei = (v == lm || comp[v] != -1);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    if (__prof_on) {
                        long long d = dense_decomposeseries_round37_prof::nsSince(__t_nei);
                    ...
                    if (skipNei) continue;
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    auto __t_lw = dense_decomposeseries_round37_prof::Clock::now();
#endif
                    comp[v] = cid;
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    if (__prof_on) {
                        long long d = dense_decomposeseries_round37_prof::nsSince(__t_lw);
                        __prof_call.ds_comp_label_write_ns += d;
                        __cand_parts_ns += d;
                        ++__prof_call.ds_label_write_hits;
                    }
#endif
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    auto __t_qpush = dense_decomposeseries_round37_prof::Clock::now();
#endif
                    q.push(v);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
                    if (__prof_on) {
                        long long d = dense_decomposeseries_round37_prof::nsSince(__t_qpush);
                        __prof_call.ds_bfs_queue_pushpop_ns += d;
                        __cand_parts_ns += d;
                        ++__prof_call.ds_queue_pushpop_hits;
                    }
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
            if (__prof_on) {
                long long cand = dense_decomposeseries_round37_prof::nsSince(__cand_t0);
        ...
        std::vector<int> A, B;
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        auto __t_part = dense_decomposeseries_round37_prof::Clock::now();
#endif
        for (int e : edgeIds) {
            int a = s.edges[e].u, b = s.edges[e].v;
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        if (__prof_on) {
            long long d = dense_decomposeseries_round37_prof::nsSince(__t_part);
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        auto __t_norm = dense_decomposeseries_round37_prof::Clock::now();
#endif
        A = normVec(std::move(A));
        B = normVec(std::move(B));
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        if (__prof_on) {
            long long d = dense_decomposeseries_round37_prof::nsSince(__t_norm);
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        auto __t_conn = dense_decomposeseries_round37_prof::Clock::now();
#endif
        bool connA = isConnectedEdgeSubset(s, A);
        bool connB = isConnectedEdgeSubset(s, B);
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
        if (__prof_on) {
            long long d = dense_decomposeseries_round37_prof::nsSince(__t_conn);
...
#if DENSE_DECOMPOSESERIES_ROUND37_PROFILE
    if (!useTrue && dense_decomposeseries_round37_prof::isDenseMode()) {
        ...
        auto __local_t0 = dense_decomposeseries_round37_prof::Clock::now();
        SparsePatch __localP = splitBlockLocalRebuild(oldCore, x);
        long long __local_total_ns = dense_decomposeseries_round37_prof::nsSince(__local_t0);
        ...
        long long __row_idx = dense_decomposeseries_round37_prof::G().rows + 1;
        if (__prefilter) {
...
#endif
                dense_decomposeseries_round37_prof::finishAttempt(__shadow_attempted, __shadow_match, __mismatch_reason, __validator_shadow_fail, __spqr_total_ns);
            } else {
                ++dense_decomposeseries_round37_prof::G().rows;
            }
            dense_decomposeseries_round37_prof::appendTsv("round37_dense_decomposeseries_census_rows.tsv", {
                dense_decomposeseries_round37_prof::caseMode(), std::to_string(dense_decomposeseries_round37_prof::caseN()), std::to_string(dense_decomposeseries_round37_prof::caseSeed()), std::to_string(__row_idx),
                ...
                [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<dense_decomposeseries_round37_prof::ms(__local_total_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<dense_decomposeseries_round37_prof::ms(__local_id_adj_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<dense_decomposeseries_round37_prof::ms(__step3_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<dense_decomposeseries_round37_prof::ms(__keep_ns); return os.str(); }(),
                [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<dense_decomposeseries_round37_prof::ms(__apply_ns); return os.str(); }(),
...
int main() {
    ...
    dense_decomposeseries_round37_prof::dump(cerr);
