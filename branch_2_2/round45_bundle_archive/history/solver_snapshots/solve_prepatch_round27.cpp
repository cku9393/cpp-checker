...
#ifndef DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
#define DENSE_SPQR_RAWBUILD_ROUND27_PROFILE 0
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
#undef DENSE_RECT_ROUND1_PROFILE
#define DENSE_RECT_ROUND1_PROFILE 1
#undef DENSE_RECT_KEEP_ORDER_ROUND4_PROFILE
#define DENSE_RECT_KEEP_ORDER_ROUND4_PROFILE 1
...
namespace dense_spqr_rawbuild_round27_prof {
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
using Clock = std::chrono::steady_clock;
inline long long nsSince(Clock::time_point t0){ return std::chrono::duration_cast<std::chrono::nanoseconds>(Clock::now()-t0).count(); }
inline double ms(long long ns){ return (double)ns / 1e6; }
struct Counters {
    ...
    long long spqr_prefilter_query_collect_total_ns = 0;
    long long spqr_raw_total_ns = 0;
    long long spqr_raw_norm_edge_ids_total_ns = 0;
    long long spqr_raw_choose_parallel_pair_total_ns = 0;
    long long spqr_raw_recursive_total_ns = 0;
    long long spqr_raw_recursive_parallel_split_total_ns = 0;
    long long spqr_raw_recursive_series_split_total_ns = 0;
    long long spqr_raw_append_shift_total_ns = 0;
    long long spqr_raw_stitch_virtual_total_ns = 0;
    long long spqr_rebuild_total_ns = 0;
    long long spqr_rebuild_node_materialize_total_ns = 0;
    long long spqr_rebuild_virtual_pair_total_ns = 0;
    long long spqr_rebuild_owned_edge_cover_total_ns = 0;
    long long spqr_rebuild_occnode_total_ns = 0;
    long long spqr_rebuild_sidebag_total_ns = 0;
};
inline Counters& G(){ static Counters c; return c; }
inline void reset(){ G() = Counters{}; }
inline std::string envs(const char* k){ const char* v = std::getenv(k); return v ? std::string(v) : std::string(); }
...
inline long long caseSeed(){ return envll("DENSE_SHADOW_CASE_SEED", -1); }
inline bool isDenseMode(){ static const std::unordered_set<std::string> denseModes={"comb_dense","comb_rect_dense","caterpillar_rect_dense","multi_comb_rect","multi_comb_cap","caterpillar_mixed"}; return denseModes.count(caseMode())!=0; }
inline std::string suffix(){ return envs("DENSE_SPQR_RAWBUILD_SUFFIX"); }
inline std::string baseName(){ return std::string("round27_dense_spqr_rawbuild") + suffix(); }
inline std::string censusPath(){ return baseName() + std::string("_census_rows.tsv"); }
inline std::string rawRowsPath(){ return baseName() + std::string("_rows.tsv"); }
inline std::string profileSummaryPath(){ return baseName() + std::string("_profile_summary.tsv"); }
inline std::string hotspotPath(){ return baseName() + std::string("_hotspots.tsv"); }
struct ScopedNs { long long* dst; Clock::time_point t0; explicit ScopedNs(long long* p): dst(p), t0(Clock::now()) {} ~ScopedNs(){ if(dst) *dst += nsSince(t0); } };
inline std::string fmtms(long long ns){ std::ostringstream os; os<<std::fixed<<std::setprecision(3)<<ms(ns); return os.str(); }
inline void appendRowWithHeader(const std::string& path, const std::vector<std::string>& header, const std::vector<std::string>& cols){ bool needHeader=false; { std::ifstream ifs(path); needHeader=!ifs.good() || ifs.peek()==std::ifstream::traits_type::eof(); } std::ofstream ofs(path, std::ios::app); if(needHeader){ for(size_t i=0;i<header.size();++i){ if(i) ofs<<'\t'; ofs<<header[i]; } ofs<<'\n'; } for(size_t i=0;i<cols.size();++i){ if(i) ofs<<'\t'; ofs<<cols[i]; } ofs<<'\n'; }
inline std::string bucketShape(int q,int attachCuts,int boundarySize,int xNbrCount,bool boundaryZero,bool ccOne){ auto bq=[&](int x){ if(x<=8) return 8; if(x<=16) return 16; if(x<=32) return 32; if(x<=64) return 64; if(x<=128) return 128; if(x<=256) return 256; if(x<=512) return 512; if(x<=1024) return 1024; if(x<=4096) return 4096; if(x<=16384) return 16384; return 65535; }; std::ostringstream os; os<<"Q"<<bq(q)<<"_A"<<bq(attachCuts)<<"_B"<<bq(boundarySize)<<"_X"<<bq(xNbrCount)<<"_BZ"<<(boundaryZero?1:0)<<"_CC"<<(ccOne?1:0); return os.str(); }
inline uint64_t fnv1a_u64(uint64_t h, uint64_t x){ h ^= x; h *= 1099511628211ULL; return h; }
inline std::string hashVec(const std::vector<int>& v){ uint64_t h=1469598103934665603ULL; for(int x:v) h=fnv1a_u64(h,(uint64_t)(uint32_t)x+0x9e3779b97f4a7c15ULL); std::ostringstream os; os<<std::hex<<h; return os.str(); }
inline std::string hashVV(const std::vector<std::vector<int>>& vv){ uint64_t h=1469598103934665603ULL; for(const auto& v:vv){ h=fnv1a_u64(h,0xabcddcbaULL); for(int x:v) h=fnv1a_u64(h,(uint64_t)(uint32_t)x+0x517cc1b727220a95ULL);} std::ostringstream os; os<<std::hex<<h; return os.str(); }
...
struct Snap { long long localIdAdjNs=0, step3Ns=0, keepNs=0, applyNs=0; long long rawPrefetchNs=0, rawTotalNs=0, rawNormNs=0, rawChooseNs=0, rawRecNs=0, rawParNs=0, rawSerNs=0, rawAppendNs=0, rawStitchNs=0, rebuildNs=0, rebuildNodeNs=0, rebuildVirtNs=0, rebuildOwnedNs=0, rebuildOccNs=0, rebuildSidebagNs=0; };
inline Snap snap(){ Snap s; s.localIdAdjNs=totalLocalIdAdjNs(); s.step3Ns=totalStep3Ns(); s.keepNs=totalKeepOrderNs(); s.applyNs=totalApplyNs(); auto &g=G(); s.rawPrefetchNs=g.spqr_prefilter_query_collect_total_ns; s.rawTotalNs=g.spqr_raw_total_ns; s.rawNormNs=g.spqr_raw_norm_edge_ids_total_ns; s.rawChooseNs=g.spqr_raw_choose_parallel_pair_total_ns; s.rawRecNs=g.spqr_raw_recursive_total_ns; s.rawParNs=g.spqr_raw_recursive_parallel_split_total_ns; s.rawSerNs=g.spqr_raw_recursive_series_split_total_ns; s.rawAppendNs=g.spqr_raw_append_shift_total_ns; s.rawStitchNs=g.spqr_raw_stitch_virtual_total_ns; s.rebuildNs=g.spqr_rebuild_total_ns; s.rebuildNodeNs=g.spqr_rebuild_node_materialize_total_ns; s.rebuildVirtNs=g.spqr_rebuild_virtual_pair_total_ns; s.rebuildOwnedNs=g.spqr_rebuild_owned_edge_cover_total_ns; s.rebuildOccNs=g.spqr_rebuild_occnode_total_ns; s.rebuildSidebagNs=g.spqr_rebuild_sidebag_total_ns; return s; }
inline void appendCensusRow(const std::vector<std::string>& cols){ appendRowWithHeader(censusPath(), {"case_mode","case_n","case_seed","call_idx","currentV","currentE","currentQ","q_over_e","attachCuts","boundarySize","oldBoundarySize","boundaryZero","ccOne","dense_guard","boundary_guard","E_guard","Q_guard","badQueriesShape","fallback_total_ms","fallback_local_id_adj_ms","fallback_step3_ms","fallback_keep_order_ms","fallback_applyPatch_ms","shadow_prefilter_hit","shadow_attempted","shadow_match","shadow_mismatch_reason","validator_shadow_fail"}, cols); }
inline void appendRawRow(const std::vector<std::string>& cols){ appendRowWithHeader(rawRowsPath(), {"case_mode","case_n","case_seed","call_idx","currentV","currentE","currentQ","q_over_e","attachCuts","boundarySize","oldBoundarySize","boundaryZero","ccOne","dense_guard","boundary_guard","E_guard","Q_guard","badQueriesShape","fallback_total_ms","fallback_local_id_adj_ms","fallback_step3_ms","fallback_keep_order_ms","fallback_applyPatch_ms","shadow_prefilter_hit","shadow_attempted","shadow_match","shadow_mismatch_reason","validator_shadow_fail","spqr_total_ms","spqr_delta_ms","spqr_prefilter_query_collect_ms","spqr_raw_total_ms","spqr_raw_norm_edge_ids_ms","spqr_raw_choose_parallel_pair_ms","spqr_raw_recursive_total_ms","spqr_raw_recursive_parallel_split_ms","spqr_raw_recursive_series_split_ms","spqr_raw_append_shift_ms","spqr_raw_stitch_virtual_ms","spqr_rebuild_total_ms","spqr_rebuild_node_materialize_ms","spqr_rebuild_virtual_pair_ms","spqr_rebuild_owned_edge_cover_ms","spqr_rebuild_occnode_ms","spqr_rebuild_sidebag_ms","spqr_keep_selection_ms","spqr_piece_emit_ms","spqr_output_adapter_ms"}, cols); }
inline void dump(std::ostream& os){ const auto& c=G(); os<<"[DENSE_SPQR_RAWBUILD_R27] shadow_attempted="<<c.shadow_attempted<<" shadow_match="<<c.shadow_match<<" shadow_mismatch="<<c.shadow_mismatch<<" raw_total="<<ms(c.spqr_raw_total_ns)<<" rebuild_total="<<ms(c.spqr_rebuild_total_ns)<<"\n"; }
#else
struct Counters{}; inline Counters& G(){ static Counters c; return c; } inline void reset(){} inline bool isDenseMode(){ return false; } inline void dump(std::ostream&){}
#endif
} // namespace dense_spqr_rawbuild_round27_prof
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof(&dense_spqr_rawbuild_round27_prof::G().spqr_raw_append_shift_total_ns);
#endif
...
void stitchVirtual(Solver::RawSpqrBuild& raw,
                   ...
                   int v, int slotV) {
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof(&dense_spqr_rawbuild_round27_prof::G().spqr_raw_stitch_virtual_total_ns);
#endif
...
RawSubtree buildTwoTerminalRecursive(const Solver& s,
                                     ...
                                     int depth) {
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof_total(&dense_spqr_rawbuild_round27_prof::G().spqr_raw_recursive_total_ns);
#endif
    chk(!edgeIds.empty(),
    ...
    std::vector<int> normE;
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    { auto __t0 = dense_spqr_rawbuild_round27_prof::Clock::now(); normE = normVec(edgeIds); dense_spqr_rawbuild_round27_prof::G().spqr_raw_norm_edge_ids_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__t0); }
#else
    normE = normVec(edgeIds);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    bool __hasParallel = false;
    { auto __t0 = dense_spqr_rawbuild_round27_prof::Clock::now(); __hasParallel = decomposeParallelFixed(s, normE, sTerm, tTerm, pBranches); dense_spqr_rawbuild_round27_prof::G().spqr_raw_recursive_parallel_split_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__t0); }
    if (__hasParallel) {
#else
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    bool __hasSeries = false;
    { auto __t0 = dense_spqr_rawbuild_round27_prof::Clock::now(); __hasSeries = decomposeSeriesFixed(s, normE, sTerm, tTerm, mid, leftE, rightE); dense_spqr_rawbuild_round27_prof::G().spqr_raw_recursive_series_split_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__t0); }
    if (__hasSeries) {
#else
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof(&dense_spqr_rawbuild_round27_prof::G().spqr_raw_choose_parallel_pair_total_ns);
#endif
    bestS = bestT = -1;
    bestBranches.clear();
    EdgeSubsetInfo G = buildEdgeSubsetInfo(s, edgeIds);
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof_total(&dense_spqr_rawbuild_round27_prof::G().spqr_raw_total_ns);
#endif
    chk(0 <= core && core < (int)blocks.size(),
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    { auto __t0 = dense_spqr_rawbuild_round27_prof::Clock::now(); edgeIds = normVec(B.realEdges); dense_spqr_rawbuild_round27_prof::G().spqr_raw_norm_edge_ids_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__t0); }
#else
    edgeIds = normVec(B.realEdges);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::ScopedNs __prof_total(&dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_total_ns);
#endif
    chk(0 <= core && core < (int)blocks.size(),
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    auto __r27_node_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_node_materialize_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__r27_node_t0);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    auto __r27_virt_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
#endif
    for (int u = 0; u < (int)raw.node.size(); ++u) {
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_virtual_pair_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__r27_virt_t0);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    auto __r27_owned_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_owned_edge_cover_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__r27_owned_t0);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    auto __r27_occ_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_occnode_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__r27_occ_t0);
    auto __r27_sidebag_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
#endif

    T.rootNode = 0;
    rebuildSideBagSummaries(core, T);
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    dense_spqr_rawbuild_round27_prof::G().spqr_rebuild_sidebag_total_ns += dense_spqr_rawbuild_round27_prof::nsSince(__r27_sidebag_t0);
#endif
...
#if DENSE_SPQR_RAWBUILD_ROUND27_PROFILE
    if (!useTrue && dense_spqr_rawbuild_round27_prof::isDenseMode()) {
        ...
        std::string __shape = dense_spqr_rawbuild_round27_prof::bucketShape(__info.badQueriesSize, __info.attachCutsSize, __info.boundarySize, __info.xNbrCount, __info.boundaryZero, __info.ccOne);
        bool __prefilter = (__dense_guard && __E_guard && __Q_guard && !__boundary_guard);
        if (__prefilter) ++dense_spqr_rawbuild_round27_prof::G().shadow_prefilter_hit;
        auto __snap_local_before = dense_spqr_rawbuild_round27_prof::snap();
        auto __local_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
        SparsePatch __localP = splitBlockLocalRebuild(oldCore, x);
        long long __local_total_ns = dense_spqr_rawbuild_round27_prof::nsSince(__local_t0);
        auto __snap_local_after = dense_spqr_rawbuild_round27_prof::snap();
        ...
        long long __keep_ns = __snap_local_after.keepNs - __snap_local_before.keepNs;
        ...
        long long __spqr_total_ns=0, __spqr_prefetch_ns=0, __spqr_raw_total_ns=0, __spqr_raw_norm_ns=0, __spqr_raw_choose_ns=0, __spqr_raw_rec_ns=0, __spqr_raw_par_ns=0, __spqr_raw_ser_ns=0, __spqr_raw_append_ns=0, __spqr_raw_stitch_ns=0, __spqr_rebuild_ns=0, __spqr_rebuild_node_ns=0, __spqr_rebuild_virt_ns=0, __spqr_rebuild_owned_ns=0, __spqr_rebuild_occ_ns=0, __spqr_rebuild_sidebag_ns=0;
        long long __spqr_keep_ns=0, __spqr_piece_ns=0, __spqr_out_ns=0;
        if (__prefilter) {
            static thread_local unsigned long long __ctr = 0;
            ++__ctr;
            bool __doShadow = (__ctr <= 8ULL) || ((__ctr % 16ULL) == 1ULL);
            if (__doShadow) {
                __shadow_attempted = true;
                ++dense_spqr_rawbuild_round27_prof::G().shadow_attempted;
                auto __snap_spqr_before = dense_spqr_rawbuild_round27_prof::snap();
                auto __spqr_t0 = dense_spqr_rawbuild_round27_prof::Clock::now();
                SparsePatch __spqrP = splitBlockSPQRForced(oldCore, x);
                __spqr_total_ns = dense_spqr_rawbuild_round27_prof::nsSince(__spqr_t0);
                auto __snap_spqr_after = dense_spqr_rawbuild_round27_prof::snap();
                __spqr_prefetch_ns = __snap_spqr_after.rawPrefetchNs - __snap_spqr_before.rawPrefetchNs;
                __spqr_raw_total_ns = __snap_spqr_after.rawTotalNs - __snap_spqr_before.rawTotalNs;
                __spqr_raw_norm_ns = __snap_spqr_after.rawNormNs - __snap_spqr_before.rawNormNs;
                __spqr_raw_choose_ns = __snap_spqr_after.rawChooseNs - __snap_spqr_before.rawChooseNs;
                __spqr_raw_rec_ns = __snap_spqr_after.rawRecNs - __snap_spqr_before.rawRecNs;
                __spqr_raw_par_ns = __snap_spqr_after.rawParNs - __snap_spqr_before.rawParNs;
                __spqr_raw_ser_ns = __snap_spqr_after.rawSerNs - __snap_spqr_before.rawSerNs;
                __spqr_raw_append_ns = __snap_spqr_after.rawAppendNs - __snap_spqr_before.rawAppendNs;
                __spqr_raw_stitch_ns = __snap_spqr_after.rawStitchNs - __snap_spqr_before.rawStitchNs;
                __spqr_rebuild_ns = __snap_spqr_after.rebuildNs - __snap_spqr_before.rebuildNs;
                __spqr_rebuild_node_ns = __snap_spqr_after.rebuildNodeNs - __snap_spqr_before.rebuildNodeNs;
                __spqr_rebuild_virt_ns = __snap_spqr_after.rebuildVirtNs - __snap_spqr_before.rebuildVirtNs;
                __spqr_rebuild_owned_ns = __snap_spqr_after.rebuildOwnedNs - __snap_spqr_before.rebuildOwnedNs;
                __spqr_rebuild_occ_ns = __snap_spqr_after.rebuildOccNs - __snap_spqr_before.rebuildOccNs;
                __spqr_rebuild_sidebag_ns = __snap_spqr_after.rebuildSidebagNs - __snap_spqr_before.rebuildSidebagNs;
                __spqr_keep_ns = dense_spqr_rawbuild_round27_prof::totalKeepOrderNs();
                __spqr_piece_ns = 0;
                __spqr_out_ns = 0;
#ifdef DEBUG_SOLVER
                ...
                if (__shadow_match) ++dense_spqr_rawbuild_round27_prof::G().shadow_match;
                else {
                    ++dense_spqr_rawbuild_round27_prof::G().shadow_mismatch;
                    __mismatch_reason = "semantic_mismatch";
                }
#else
        ...
        ++dense_spqr_rawbuild_round27_prof::G().rows;
        dense_spqr_rawbuild_round27_prof::appendCensusRow({
            dense_spqr_rawbuild_round27_prof::caseMode(), std::to_string(dense_spqr_rawbuild_round27_prof::caseN()), std::to_string(dense_spqr_rawbuild_round27_prof::caseSeed()), std::to_string(dense_spqr_rawbuild_round27_prof::G().rows),
            ...
            std::to_string(__info.attachCutsSize), std::to_string(__info.boundarySize), std::to_string(__info.oldBoundarySize), std::to_string(__info.boundaryZero?1:0), std::to_string(__info.ccOne?1:0),
            std::to_string(__dense_guard?1:0), std::to_string(__boundary_guard?1:0), std::to_string(__E_guard?1:0), std::to_string(__Q_guard?1:0), __shape,
            dense_spqr_rawbuild_round27_prof::fmtms(__local_total_ns), dense_spqr_rawbuild_round27_prof::fmtms(__local_id_adj_ns), dense_spqr_rawbuild_round27_prof::fmtms(__step3_ns), dense_spqr_rawbuild_round27_prof::fmtms(__keep_ns), dense_spqr_rawbuild_round27_prof::fmtms(__apply_ns),
            std::to_string(__prefilter?1:0), std::to_string(__shadow_attempted?1:0), std::to_string(__shadow_match?1:0), __mismatch_reason, std::to_string(0)
        });
        if (__shadow_attempted && __shadow_match) {
            ++dense_spqr_rawbuild_round27_prof::G().raw_rows;
            dense_spqr_rawbuild_round27_prof::appendRawRow({
                dense_spqr_rawbuild_round27_prof::caseMode(), std::to_string(dense_spqr_rawbuild_round27_prof::caseN()), std::to_string(dense_spqr_rawbuild_round27_prof::caseSeed()), std::to_string(dense_spqr_rawbuild_round27_prof::G().raw_rows),
                std::to_string(__info.currentV), std::to_string(__info.currentE), std::to_string(__info.badQueriesSize), [&](){ std::ostringstream os; os<<std::fixed<<std::setprecision(6)<<__q_over_e; return os.str(); }(),
                std::to_string(__info.attachCutsSize), std::to_string(__info.boundarySize), std::to_string(__info.oldBoundarySize), std::to_string(__info.boundaryZero?1:0), std::to_string(__info.ccOne?1:0),
                std::to_string(__dense_guard?1:0), std::to_string(__boundary_guard?1:0), std::to_string(__E_guard?1:0), std::to_string(__Q_guard?1:0), __shape,
                dense_spqr_rawbuild_round27_prof::fmtms(__local_total_ns), dense_spqr_rawbuild_round27_prof::fmtms(__local_id_adj_ns), dense_spqr_rawbuild_round27_prof::fmtms(__step3_ns), dense_spqr_rawbuild_round27_prof::fmtms(__keep_ns), dense_spqr_rawbuild_round27_prof::fmtms(__apply_ns),
                std::to_string(__prefilter?1:0), std::to_string(__shadow_attempted?1:0), std::to_string(__shadow_match?1:0), __mismatch_reason, std::to_string(0),
                dense_spqr_rawbuild_round27_prof::fmtms(__spqr_total_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_total_ns - __local_total_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_prefetch_ns),
                dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_total_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_norm_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_choose_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_rec_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_par_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_ser_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_append_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_raw_stitch_ns),
                dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_node_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_virt_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_owned_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_occ_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_rebuild_sidebag_ns),
                dense_spqr_rawbuild_round27_prof::fmtms(__spqr_keep_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_piece_ns), dense_spqr_rawbuild_round27_prof::fmtms(__spqr_out_ns)
            });
        }
        return __localP;
    }
...
int main() {
    ...
    dense_spqr_rawbuild_round27_prof::dump(cerr);
    for (int i = 1; i <= N; ++i) {
        if (i > 1) cout << ' ';
        cout << ans[i];
    }
