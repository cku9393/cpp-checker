...
Solver::SparsePatch Solver::splitBlockLocalRebuild(int oldCore, int x) const {
    ...
    std::vector<int> survEdges;
    std::vector<int> survEdgeU;
    std::vector<int> survEdgeV;
    survEdges.reserve(B.realEdges.size());
    survEdgeU.reserve(B.realEdges.size());
    survEdgeV.reserve(B.realEdges.size());
    ...
    for (int e : B.realEdges) {
        ...
        int le = (int)survEdges.size();
        survEdges.push_back(e);
        survEdgeU.push_back(la);
        survEdgeV.push_back(lb);
        adj[la].push_back({lb, le});
        adj[lb].push_back({la, le});
        deg[la]++;
        deg[lb]++;
    }
#if DENSE_RECT_ROUND1_PROFILE
