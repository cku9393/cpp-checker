...
bool graftMiniCoreIntoPlace(harness::ReducedSPQRCore &core,
    ...
    auto captureReplayPhase = [&](harness::ReplaySnapshotPhase phase) {
        if (!trace) return;
        harness::captureReplaySnapshotsForPhase(core,
                                                oldNode,
                                                oldNode,
                                                actualNodes,
    ...
    noteSnapshotPhase();
    captureReplayPhase(harness::ReplaySnapshotPhase::BEFORE_CLEAR);
    ...
    captureReplayPhase(harness::ReplaySnapshotPhase::AFTER_CLEAR_PRESERVE);
    ...
    captureReplayPhase(harness::ReplaySnapshotPhase::AFTER_MATERIALIZE);
    ...
    captureReplayPhase(harness::ReplaySnapshotPhase::AFTER_INTERNAL_ARC_CONNECT);
    ...
    auto failPreserveDuringGraft = [&](int inputEdgeId,
        ...
        const auto otherSubtype = inferGraftOtherSubtype(msg);
        stampGraftOtherTrace(otherSubtype,
                             inputEdgeId,
                             oldArc,
                             failingOldSlot,
                             failingNewSlot,
                             msg);
        if (trace && otherSubtype == harness::GraftOtherSubtype::GOS_POSTCHECK_ADJ_MISMATCH) {
            std::string whyDetailed;
            trace->postcheckSubtype =
                harness::classifyPostcheckFailureDetailed(core, whyDetailed);
    ...
    captureReplayPhase(harness::ReplaySnapshotPhase::AFTER_PROXY_REWIRE);
    ...
    if (useResolvedProxySnapshot) {
        ...
        captureReplayPhase(harness::ReplaySnapshotPhase::AFTER_ADJ_REPAIR);
