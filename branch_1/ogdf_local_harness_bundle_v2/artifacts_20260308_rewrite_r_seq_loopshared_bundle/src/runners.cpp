...
static SequenceChooseStatus chooseDeterministicSequenceRewriteTarget(const ReducedSPQRCore &core,
                                                                    NodeId &chosenR,
                                                                    VertexId &chosenX,
                                                                    std::string &why) {
    chosenR = -1;
    ...
    for (NodeId nodeId = 0; nodeId < static_cast<NodeId>(core.nodes.size()); ++nodeId) {
        ...
        for (const auto &slot : node.slots) {
            ...
            if (slot.poleA < 0 || slot.poleB < 0) {
                ...
                return SequenceChooseStatus::ERROR;
            }
        ...
        if (bestX >= 0) {
            chosenR = nodeId;
            chosenX = bestX;
            return SequenceChooseStatus::FOUND;
        }
    }

    return SequenceChooseStatus::NONE;
}
...
HarnessResult runRewriteRSequenceCaseDumpAware(const ExplicitBlockGraph &G,
    ...
    std::string why;
    if (!buildWholeCoreForTesting(G, backend, current, why)) {
        noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
    ...
    for (int step = 0; step < kMaxSequenceSteps; ++step) {
        ...
        if (chooseStatus == SequenceChooseStatus::ERROR) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            setFailure(B, HarnessStage::SEQ_CHOOSE_RX_FAIL, "chooseDeterministicSequenceRewriteTarget", why);
            return failAndDump(B, dumpDir);
        }
        ...
        if (!rewriteOk) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            captureRewriteAfter(B, ops, after);
            setFailure(B, HarnessStage::SEQ_REWRITE_R_FAIL, "rewriteR_fallback", why);
            return failAndDump(B, dumpDir);
        }
        ...
        if (!ops.normalizeTouchedRegion(after, why)) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            captureRewriteAfter(B, ops, after);
            setFailure(B, HarnessStage::SEQ_NORMALIZE_FAIL, "normalizeTouchedRegion", why);
            return failAndDump(B, dumpDir);
        }
        captureRewriteAfter(B, ops, after);
        ...
        if (!ops.checkActualReducedInvariant(after, nullptr, why)) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            setFailure(B, HarnessStage::SEQ_ACTUAL_INVARIANT_FAIL, "checkActualReducedInvariant", why);
            return failAndDump(B, dumpDir);
        }
        ...
        if (nextChooseStatus == SequenceChooseStatus::ERROR) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            setFailure(B, HarnessStage::SEQ_CHOOSE_RX_FAIL, "chooseDeterministicSequenceRewriteTarget(after)", why);
            return failAndDump(B, dumpDir);
        }
        if (B.explicitAfter && B.explicitBefore &&
            B.explicitAfter->edges.size() >= B.explicitBefore->edges.size() &&
            nextChooseStatus == SequenceChooseStatus::FOUND) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            setFailure(B, HarnessStage::SEQ_PROGRESS_STUCK, "sequence progress check",
        ...
        if (!buildWholeCoreForTesting(*B.explicitAfter, backend, oracle, why)) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            B.explicitGot = B.explicitAfter;
            setFailure(B, HarnessStage::SEQ_ORACLE_FAIL, "buildWholeCoreForTesting(oracle)", why);
            return failAndDump(B, dumpDir);
        }
        B.explicitExpected = ops.materializeWholeCoreExplicit(oracle);
        B.explicitGot = B.explicitAfter;
        if (!ops.checkEquivalentExplicitGraphs(*B.explicitGot, *B.explicitExpected, why)) {
            noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, false);
            setFailure(B, HarnessStage::SEQ_ORACLE_FAIL, "checkEquivalentExplicitGraphs", why);
            return failAndDump(B, dumpDir);
        }
    ...
    B.actualBeforeRewrite = current;
    B.explicitBefore = ops.materializeWholeCoreExplicit(current);
    noteRewriteSeqCaseFinish(false, completedSteps, hadSequenceFallback, true);
    setFailure(B, HarnessStage::SEQ_MAX_STEPS_REACHED, "rewrite sequence loop",
