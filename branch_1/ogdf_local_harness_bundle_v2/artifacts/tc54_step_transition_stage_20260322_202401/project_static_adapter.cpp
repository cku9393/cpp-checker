...
bool stepTransitionTargetStateEqual(const StepTransitionSnapshot &lhs,
    ...
    const auto &lt = lhs.nextStepTargetSnapshot;
    const auto &rt = rhs.nextStepTargetSnapshot;
    return lhs.aliveRNodesForNextStep == rhs.aliveRNodesForNextStep &&
           lhs.nextStepCandidates == rhs.nextStepCandidates &&
           lt.hasNextTarget == rt.hasNextTarget &&
           lt.chosenR == rt.chosenR &&
           lt.chosenX == rt.chosenX &&
           lt.noTargetReason == rt.noTargetReason;
}
...
bool replaySolverSingleTransitionFromExplicit(const ExplicitBlockGraph &inputExplicit,
    ...
    out.nextInputSourceTag =
        localStats.solverShadowResyncAppliedCount > 0 ? "RESYNC" : "NORMALIZE";
    why.clear();
    return true;
}
...
bool runExplicitCoreBuilderReplayCaseDumpAware(const ExplicitBlockGraph &input,
    ...
    if (!extractSourceStepCoreForBuilderReplay(
        ...
        bundle.builderPipelineSeamWhy =
            "explicit-core-builder-replay: source extraction failed: " + bundle.sourceWhy;
    ...
    bundle.builderPipelineSeamKind = classifyBuilderPipelineSeam(bundle.sourceLiveCore,
                                                                 bundle.rawSnapshot,
                                                                 bundle.miniBeforeSnapshot,
                                                                 bundle.miniAfterSnapshot,
                                                                 bundle.coreAfterMaterializeSnapshot,
