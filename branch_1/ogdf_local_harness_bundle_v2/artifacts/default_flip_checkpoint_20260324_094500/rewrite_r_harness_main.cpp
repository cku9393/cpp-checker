...
RunConfig parseArgs(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--seed") cfg.seed = std::stoull(next("--seed"));
            ...
            std::cout << "--backend ogdf --mode {static|dummy|rewrite-r|rewrite-r-seq|rewrite-seq|rewrite-r-seq-replay|rewrite-r-seq-regression|rewrite-r-seq-bench|solver-compare|solver-baseline-replay|solver-semantic-replay|solver-semantic-target-replay|solver-semantic-transition-replay|solver-handoff-replay|solver-step-transition-replay|solver-handoff-policy-replay|solver-compare-replay|solver-finalcore-replay|solver-shape-replay|explicit-core-builder-replay|handoff-raw-replay|materialize-core-replay|ogdf-raw-crash-replay} "
            ...
            std::cout << "Defaults: solver adapter routes to rewrite-seq when built with USE_REWRITE_SEQ_ENGINE=ON.\n";
            std::cout << "Diagnostic-only legacy path: use --mode rewrite-r explicitly.\n";
            std::cout << "Recommended compare baseline: --baseline oracle --oracle-handoff normalize.\n";
            std::exit(0);
        } else {
...
struct JsonValue {
    ...
    std::variant<std::nullptr_t, bool, int64_t, double, std::string, Object, Array> data;
...
private:
    ...
    JsonValue parseNumber() {
        ...
        if (peek() == '.') {
            ...
            ++pos_;
            if (std::isdigit(static_cast<unsigned char>(peek())) == 0) {
                throw std::runtime_error("expected fractional digits in JSON number");
            }
            while (std::isdigit(static_cast<unsigned char>(peek())) != 0) ++pos_;
        }
        if (peek() == 'e' || peek() == 'E') {
            ...
            if (peek() == '+' || peek() == '-') ++pos_;
            if (std::isdigit(static_cast<unsigned char>(peek())) == 0) {
                throw std::runtime_error("expected exponent digits in JSON number");
            }
            while (std::isdigit(static_cast<unsigned char>(peek())) != 0) ++pos_;
        }
        const std::string number = std::string(text_.substr(start, pos_ - start));
        if (isDouble) {
...
bool writeTextFileAtomic(const std::string &path, const std::string &text, std::string &why) {
    ...
    if (fd < 0) {
        why = "open(" + tmp.string() + "): " + std::strerror(errno);
        return false;
    }
    ...
    while (remaining != 0) {
        ...
        if (written < 0) {
            why = "write(" + tmp.string() + "): " + std::strerror(errno);
            ::close(fd);
            std::error_code ec;
            fs::remove(tmp, ec);
            return false;
    ...
    if (::fsync(fd) != 0) {
        why = "fsync(" + tmp.string() + "): " + std::strerror(errno);
        ::close(fd);
        std::error_code ec;
        fs::remove(tmp, ec);
        return false;
    ...
    if (::close(fd) != 0) {
        why = "close(" + tmp.string() + "): " + std::strerror(errno);
        std::error_code ec;
        fs::remove(tmp, ec);
        return false;
    }
    if (::rename(tmp.c_str(), target.c_str()) != 0) {
        why = "rename(" + tmp.string() + " -> " + target.string() + "): " + std::strerror(errno);
        std::error_code ec;
        fs::remove(tmp, ec);
        return false;
    }
...
public:
    ...
    bool validate(std::string &why) {
        try {
            parseValue();
            skipWs();
            if (pos_ != text_.size()) {
                throw std::runtime_error("unexpected trailing characters in summary JSON");
            }
            return true;
...
private:
    ...
    char take() {
        if (pos_ >= text_.size()) {
            throw std::runtime_error("unexpected end of summary JSON");
        }
        return text_[pos_++];
    }
    ...
    void expect(char c) {
        skipWs();
        if (take() != c) {
            throw std::runtime_error(std::string("expected '") + c + "' in summary JSON");
        }
    }
    ...
    void parseValue() {
        ...
        default:
            ...
            throw std::runtime_error("unexpected token in summary JSON");
        }
    }
    ...
    void parseString() {
        ...
        while (true) {
            ...
            if (static_cast<unsigned char>(c) < 0x20) {
                throw std::runtime_error("control character in summary JSON string");
            }
            if (c != '\\') continue;
            const char esc = take();
            switch (esc) {
            ...
            case 'u':
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (std::isxdigit(static_cast<unsigned char>(hex)) == 0) {
                        throw std::runtime_error("invalid unicode escape in summary JSON");
                    }
                }
                break;
            default:
                throw std::runtime_error("unsupported escape in summary JSON");
            }
        }
    }
    ...
    void parseNumber() {
        ...
        if (peek() == '0') {
            ...
            if (std::isdigit(static_cast<unsigned char>(peek())) == 0) {
                throw std::runtime_error("invalid number in summary JSON");
            }
            while (std::isdigit(static_cast<unsigned char>(peek())) != 0) ++pos_;
        }
        if (peek() == '.') {
            ++pos_;
            if (std::isdigit(static_cast<unsigned char>(peek())) == 0) {
                throw std::runtime_error("invalid fractional number in summary JSON");
            }
            while (std::isdigit(static_cast<unsigned char>(peek())) != 0) ++pos_;
        }
        if (peek() == 'e' || peek() == 'E') {
            ...
            if (peek() == '+' || peek() == '-') ++pos_;
            if (std::isdigit(static_cast<unsigned char>(peek())) == 0) {
                throw std::runtime_error("invalid exponent in summary JSON");
            }
            while (std::isdigit(static_cast<unsigned char>(peek())) != 0) ++pos_;
        }
    }
    ...
    void parseLiteral(std::string_view literal) {
        if (text_.substr(pos_, literal.size()) != literal) {
            throw std::runtime_error("invalid literal in summary JSON");
        }
        pos_ += literal.size();
    }
...
bool validateSummaryJsonRoundTrip(const std::string &json, std::string &why) {
    JsonSyntaxValidator validator(json);
    return validator.validate(why);
}

std::string injectSummaryMetadata(const std::string &json,
                                  bool summaryValidated,
                                  const std::string &summaryValidationWhy) {
    const size_t brace = json.find('{');
    if (brace == std::string::npos) return json;

    size_t bodyStart = brace + 1;
    while (bodyStart < json.size() && (json[bodyStart] == '\n' || json[bodyStart] == '\r')) {
        ++bodyStart;
    }

    std::ostringstream oss;
    oss << "{\n";
    oss << "  \"summaryWriteMode\": \"atomic\",\n";
    oss << "  \"summaryValidated\": " << (summaryValidated ? "true" : "false") << ",\n";
    oss << "  \"summaryValidationWhy\": \"" << jsonEscape(summaryValidationWhy) << "\"";
    if (bodyStart < json.size() && json[bodyStart] != '}') {
        oss << ",\n" << json.substr(bodyStart);
    } else if (bodyStart < json.size()) {
        oss << "\n" << json.substr(bodyStart);
    } else {
        oss << "\n}\n";
    }
...
bool writeJsonSummaryAtomic(const std::string &path, const std::string &json, std::string &why) {
    const std::string finalJson = injectSummaryMetadata(json, true, "");
    std::string validationWhy;
    if (!validateSummaryJsonRoundTrip(finalJson, validationWhy)) {
        const std::string invalidPath = path + ".invalid.json";
        std::string dumpWhy;
        if (!writeTextFileAtomic(invalidPath, json, dumpWhy)) {
            why = "summary validation failed: " + validationWhy +
                  "; failed to write invalid dump: " + dumpWhy;
        } else {
            why = "summary validation failed: " + validationWhy +
                  "; invalid dump=" + invalidPath;
        }
        return false;
    }
    return writeTextFileAtomic(path, finalJson, why);
}

class AtomicSummaryFile : public std::ostringstream {
public:
    explicit AtomicSummaryFile(std::string path) : path_(std::move(path)) {}

    ~AtomicSummaryFile() override {
        finalize();
    }

    AtomicSummaryFile(const AtomicSummaryFile &) = delete;
    AtomicSummaryFile &operator=(const AtomicSummaryFile &) = delete;
...
private:
    void finalize() {
        ...
        finalized_ = true;
        std::string why;
        if (!writeJsonSummaryAtomic(path_, str(), why)) {
            std::cerr << "[SUMMARY_WRITE_FAIL] path=" << path_ << " why=" << why << "\n";
        }
    }
...
int main(int argc, char **argv) {
    try {
        ...
        if (cfg.mode == "rewrite-r-seq-regression") {
            ...
            AtomicSummaryFile ofs(summaryPath);
            ofs << "{\n";
            ...
            ofs << "  \"solverHandoffResyncApplied\": "
                << (bundle.solverStep1Snapshot.solverHandoffResyncApplied ? "true" : "false")
                << ",\n";
            ofs << "  \"solverHandoffResyncWhy\": \""
                << jsonEscape(bundle.solverStep1Snapshot.solverHandoffResyncWhy) << "\",\n";
            ...
            printRewriteStats();
            return ok ? 0 : 1;
        } else if (cfg.mode == "handoff-raw-replay") {
            if (cfg.manualOnly) {
                throw std::runtime_error("handoff-raw-replay does not support --manual-only");
            }
            if (cfg.manifestPath.empty()) {
                throw std::runtime_error("handoff-raw-replay requires --manifest");
            }
            if (cfg.caseName.empty()) {
                throw std::runtime_error("handoff-raw-replay requires --case-name");
            }
            if (cfg.sourceStep <= 0) {
                throw std::runtime_error("handoff-raw-replay requires --source-step > 0");
            }
            ...
            if (it == cases.end()) {
                throw std::runtime_error("handoff-raw-replay case not found in manifest: " +
                                         cfg.caseName);
            }
            if (!it->inputExplicit.has_value()) {
                throw std::runtime_error(
                    "handoff-raw-replay case is missing inputExplicit snapshot: " +
                    cfg.caseName);
            }
            ...
            std::string why;
            setRewriteRCaseContext(it->seed, it->tcIndex);
            const bool ok = runHandoffRawReplayCaseDumpAware(
                *it->inputExplicit,
                cfg.manifestPath,
                it->name,
                it->seed,
            ...
            std::ostringstream bundlePathStream;
            bundlePathStream << cfg.dumpDir << "/"
                             << rawReplaySeamKindName(bundle.rawReplaySeamKind)
                             << "_seed" << it->seed
                             << "_tc" << it->tcIndex
                             << "_step" << cfg.sourceStep
                             << "_" << sanitizePathComponent(it->name) << ".txt";
            const std::string bundlePath = bundlePathStream.str();
            dumpHandoffRawReplayBundle(bundle, bundlePath);

            const std::string summaryPath = cfg.dumpDir + "/summary.json";
            AtomicSummaryFile ofs(summaryPath);
            ofs << "{\n";
            ofs << "  \"mode\": \"handoff-raw-replay\",\n";
            ofs << "  \"manifestPath\": \"" << jsonEscape(cfg.manifestPath) << "\",\n";
            ofs << "  \"caseName\": \"" << jsonEscape(it->name) << "\",\n";
            ofs << "  \"outputDumpDir\": \"" << jsonEscape(cfg.dumpDir) << "\",\n";
            ofs << "  \"bundlePath\": \"" << jsonEscape(bundlePath) << "\",\n";
            ofs << "  \"topLevelOk\": " << (bundle.topLevelOk ? "true" : "false") << ",\n";
            ofs << "  \"sourceStep\": " << cfg.sourceStep << ",\n";
            ofs << "  \"rawReplaySeamKind\": \""
                << rawReplaySeamKindName(bundle.rawReplaySeamKind) << "\",\n";
            ofs << "  \"rawReplaySeamWhy\": \""
                << jsonEscape(bundle.rawReplaySeamWhy) << "\",\n";
            ofs << "  \"rawNodeCount\": " << bundle.rawReplaySnapshot.rawNodeCount << ",\n";
            ofs << "  \"rawTreeEdgeCount\": " << bundle.rawReplaySnapshot.rawTreeEdgeCount
                << ",\n";
            ofs << "  \"rawRNodeCount\": " << bundle.rawReplaySnapshot.rawRNodeCount << ",\n";
            ofs << "  \"rawSNodeCount\": " << bundle.rawReplaySnapshot.rawSNodeCount << ",\n";
            ofs << "  \"rawPNodeCount\": " << bundle.rawReplaySnapshot.rawPNodeCount << ",\n";
            ofs << "  \"rawSingleRSplitCanonAttemptCount\": "
                << stats.rawSingleRSplitCanonAttemptCount << ",\n";
            ofs << "  \"rawSingleRSplitCanonAppliedCount\": "
                << stats.rawSingleRSplitCanonAppliedCount << ",\n";
            ofs << "  \"rawSingleRSplitCanonRejectedCount\": "
                << stats.rawSingleRSplitCanonRejectedCount << ",\n";
            ofs << "  \"rawSingleRSplitCanonRollbackCount\": "
                << stats.rawSingleRSplitCanonRollbackCount << ",\n";
            ofs << "  \"firstRawSingleRSplitCanonDumpPath\": \""
                << jsonEscape(stats.firstRawSingleRSplitCanonDumpPath) << "\",\n";
            ofs << "  \"sourceLiveNodeCount\": "
                << bundle.sourceLiveCoreSnapshot.aliveNodeCount << ",\n";
            ofs << "  \"sourceLiveArcCount\": "
                << bundle.sourceLiveCoreSnapshot.aliveArcCount << ",\n";
            ofs << "  \"sourceWhy\": \"" << jsonEscape(bundle.sourceWhy) << "\",\n";
            ofs << "  \"rawWhy\": \"" << jsonEscape(bundle.rawWhy) << "\"\n";
            ofs << "}\n";

            std::cout << "[HANDOFF_RAW_REPLAY] case=" << it->name
                      << " seamKind=" << rawReplaySeamKindName(bundle.rawReplaySeamKind)
                      << " sourceStep=" << cfg.sourceStep
                      << " bundle=" << bundlePath
                      << " summary=" << summaryPath << "\n";
            if (!why.empty()) {
                std::cout << "[HANDOFF_RAW_REPLAY_WHY] " << why << "\n";
            }
            printRewriteStats();
            return ok ? 0 : 1;
