...
RunConfig parseArgs(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--seed") cfg.seed = std::stoull(next("--seed"));
            ...
            std::cout << "--backend ogdf --mode {static|dummy|rewrite-r|rewrite-r-seq|rewrite-seq|rewrite-r-seq-replay|rewrite-r-seq-regression|rewrite-r-seq-bench|solver-compare|solver-baseline-replay|solver-semantic-replay|solver-compare-replay|solver-finalcore-replay|solver-shape-replay|explicit-core-builder-replay} "
                         "--seed N --rounds N --tc-index N --target-step N --manifest PATH --case-name NAME --baseline {legacy|oracle|both} --semantic-stop {raw|canonical|end} --source-step N --source-side {replay|solver|shadow} --manual-only --dump-dir DIR\n";
            std::exit(0);
        } else {
